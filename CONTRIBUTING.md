...
Before filing a new issue, please search our [open issues](https://github.com/sila-chain/sila-nethermind/issues) to check if it already exists.
...
- **DO** especially follow our rules in the [Contributing](https://github.com/sila-chain/sila-nethermind/blob/master/CODE_OF_CONDUCT.md#contributing) section of our code of conduct.
...
Branch names must follow the `kebab-case` or `snake_case` pattern and be all lowsrcase. When possible, Follow the pattern `project-if-any/type-of-change/issue-title` and add an issue reference if applicable. For example:
