...
"""Shared utilities for SAVM opcode benchmark comparison.

Used by the detect-noisy and compare steps in savm-opcode-benchmark-diff.yml.
"""
...
def collect_logs(base_pattern="savm-opcodes-base*.log", pr_pattern="savm-opcodes-pr*.log"):
    ...
    if not base_logs:
        base_logs = ["savm-opcodes-base.log"]
    if not pr_logs:
        pr_logs = ["savm-opcodes-pr.log"]
