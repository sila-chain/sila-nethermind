...
This guide helps to get started with the Nethermind Sila execution client repository, which targets `net10.0` and uses C# language version `14.0`. It covers the project structure, how to build and test the code, and follow the PR workflow.
...
- [SilaTests.slnx](./src/Nethermind/SilaTests.slnx): The Sila Foundation test suite
...
- **Consensus algorithms**
  ...
  - [Nethermind.Consensus.Silash](./src/Nethermind/Nethermind.Consensus.Silash/): Proof of Work (PoW)
...
- **Core blockchain**
  ...
  - [Nethermind.Savm](./src/Nethermind/Nethermind.Savm/): SAVM implementation
  - [Nethermind.Savm.Precompiles](./src/Nethermind/Nethermind.Savm.Precompiles/): SAVM precompiled contracts
...
- **Networking:**
  ...
  - [Nethermind.Network.Enr](./src/Nethermind/Nethermind.Network.Enr/): Sila Node Records (ENR) handling
...
- Reference run used for structure validation:
  - Run: `https://github.com/sila-chain/sila-nethermind/actions/runs/22185801008`
  - Job: `https://github.com/sila-chain/sila-nethermind/actions/runs/22185801008/job/64159725161`
- Fetch logs with:
  ```bash
...
- Optional low-variance mode: pass `-f expb_env="EXPB_SAVM_WARMUP=1"` to enable expb's per-block SAVM warmup (`sil_simulateV1` before each measured block). It serves the measured block's reads from warm caches, which lowers both run-to-run CV (~1.8%→~0.55% on flat-realblocks) and AVG. Pair it with a raised RPC gas cap — `-f additional_extra_flags="--JsonRpc.GasCap=1000000000000"` — otherwise the per-request gas budget (default 100M) is exhausted on dense blocks and the warmup `sil_simulateV1` calls fail with `-38013` (intrinsic gas), silently leaving those blocks un-warmed. Caveat: warmup minimizes cold RocksDB/storage interaction, so it is a low-variance *compute* signal, not a substitute for the default cold benchmark — don't use it when measuring storage-layer changes.
