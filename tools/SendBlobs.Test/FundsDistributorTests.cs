...
public class FundsDistributorTests
{
    ...
    public async Task DistributeFunds_OnSuccessfulRun_TargetFileContainsAllGeneratedKeys(ulong maxFee, int expectedGasPriceCalls)
    {
        ...
        await rpcClient.Received(expectedGasPriceCalls).Post<string>("sil_gasPrice", Arg.Any<object?[]>());
    }
    ...
    private static IJsonRpcClient BuildClientReturningOkForEverySend()
    {
        ...
        rpcClient.Post<string>("sil_sendRawTransaction", Arg.Any<object?[]>()).Returns(OkTxHashHex);
        return rpcClient;
    }
    ...
    private static IJsonRpcClient BuildClientThatFailsOnNthSend(int failOnSendIndex)
    {
        IJsonRpcClient rpcClient = BuildBaseClient();
        int sendCallCount = 0;
        rpcClient.Post<string>("sil_sendRawTransaction", Arg.Any<object?[]>())
    ...
    private static IJsonRpcClient BuildBaseClient()
    {
        ...
        rpcClient.Post<string>("sil_getBalance", Arg.Any<object?[]>()).Returns(OneEtherHex);
        rpcClient.Post<string>("sil_getTransactionCount", Arg.Any<object?[]>()).Returns(ZeroNonceHex);
        rpcClient.Post<string>("sil_gasPrice", Arg.Any<object?[]>()).Returns(OneGweiHex);
        rpcClient.Post<string>("sil_maxPriorityFeePerGas", Arg.Any<object?[]>()).Returns(OneGweiHex);
        return rpcClient;
    }
}
