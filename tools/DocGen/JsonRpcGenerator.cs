...
internal static class JsonRpcGenerator
{
    ...
    internal static void Generate(string path)
    {
        ...
        string?[] excluded = new[] {
            ...
            typeof(ISavmRpcModule).FullName,
        ...
        foreach (string file in Directory.EnumerateFiles(path))
        {
            if (file.EndsWith(".md", StringComparison.Ordinal) &&
                // Skip sil_subscribe.md and sil_unsubscribe.md
        ...
        if (methodMap.TryGetValue("sil", out IEnumerable<MethodInfo>? silMethods))
        {
            // Inject the `subscribe` methods into `sil`
            methodMap["sil"] = silMethods!
    ...
    private static void WriteMarkdown(string path, string ns, IEnumerable<MethodInfo> methods, int sidebarIndex)
    {
        ...
        foreach (MethodInfo method in methods)
        {
            ...
            if (method.Name.Equals("sil_subscribe", StringComparison.Ordinal) ||
                method.Name.Equals("sil_unsubscribe", StringComparison.Ordinal))
            {
