...
internal static class InputGenerator
{
    ...
    {
        SilaJsonSerializer serializer = new([new OwnedReadOnlyListConverter()]);
        ...
        await AnsiConsole
            ...
            .StartAsync($"[orange1]Fetching block `{blockParam}`[/]", async ctx =>
            {
                ...
                chainId = await client.Post<ulong?>("sil_chainId");
    ...
    private static string GetChainName(ulong chainId) => chainId switch
    {
        ...
        BlockchainIds.SilaMainnet => "SilaMainnet",
        BlockchainIds.SilaSepolia => "SilaSepolia",
        _ => $"Unknown ({chainId})"
    };
