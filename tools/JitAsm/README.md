...
```bash
dotnet build src/Nethermind/Nethermind.Savm -c Release
```

Assemblies are output to `src/Nethermind/artifacts/bin/{ProjectName}/release/{ProjectName}.dll`. For example:
- `src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll`
...
```bash
# Good (Windows)
dotnet run --project tools/JitAsm -c Release -- -a D:/GitHub/nethermind/src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmStack -m PushBytesRef

# Bad (may fail on Windows)
dotnet run --project tools/JitAsm -c Release -- \
    -a "src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll" \
    -t "Nethermind.Savm.SavmStack" \
    -m PushBytesRef
```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -m PushBytesRef
```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmStack -m PushBytesRef
```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmStack -m PushBytes --type-params Nethermind.Core.OffFlag
```
...
```bash
# TransactionProcessorBase<SilaGasPolicy>.Execute<OffFlag>(...)
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.TransactionProcessing.TransactionProcessorBase -m Execute --class-type-params Nethermind.Savm.GasPolicy.SilaGasPolicy --type-params Nethermind.Core.OffFlag
```
...
```bash
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -m SomeGenericMethod --type-params System.Int32,System.String
```

### SAVM Instruction Example

Disassemble the MUL opcode implementation with specific gas policy and tracing flags:

```bash
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmInstructions -m InstructionMath2Param --type-params Nethermind.Savm.GasPolicy.SilaGasPolicy,Nethermind.Savm.SavmInstructions+OpMul,Nethermind.Core.OffFlag
```

This shows the JIT output for `InstructionMath2Param<SilaGasPolicy, OpMul, OffFlag>`:
- `SilaGasPolicy` - Standard Sila gas accounting
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -m PushBytesRef -v
```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -m SomeMethod --skip-cctor-detection
```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.BlockExecutionContext -m GetBlobBaseFee > tier1.asm 2>&1

# FullOpts — single-pass, no PGO
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.BlockExecutionContext -m GetBlobBaseFee --fullopts > fullopts.asm 2>&1
...
3. **View the generated assembly:**
   ```bash
   dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -m YourMethod
   ```
...
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmInstructions -m InstructionMath2Param --type-params Nethermind.Savm.GasPolicy.SilaGasPolicy,Nethermind.Savm.SavmInstructions+OpAdd,Nethermind.Core.OffFlag > off.asm 2>&1

# Generate ASM for tracing enabled
dotnet run --project tools/JitAsm -c Release -- -a src/Nethermind/artifacts/bin/Nethermind.Savm/release/Nethermind.Savm.dll -t Nethermind.Savm.SavmInstructions -m InstructionMath2Param --type-params Nethermind.Savm.GasPolicy.SilaGasPolicy,Nethermind.Savm.SavmInstructions+OpAdd,Nethermind.Core.OnFlag > on.asm 2>&1
