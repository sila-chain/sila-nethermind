    ...
    private static IEnumerable<MethodInfo> FindMethods(Type type, string methodName, bool includeInherited = false)
    {
        ...
        if (verbose)
        {
            ...
            if (methods.Count == 0)
            {
                ...
                IEnumerable<MethodInfo> allMethods = type.GetMethods(flags).Where(m => m.Name.Contains("Savm") || m.Name.Contains("Execute")).Take(10);
                Console.Error.WriteLine($"[DEBUG] Sample methods containing 'Savm' or 'Execute': {string.Join(", ", allMethods.Select(m => m.Name))}");
            }
        }
