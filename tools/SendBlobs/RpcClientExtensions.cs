...
internal static class RpcClientExtensions
{
    ...
    extension(IJsonRpcClient rpcClient)
    {
        ...
        {
            string chainIdString = await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_chainId));
        ...
        {
            string nonceString = await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_getTransactionCount), address, Latest);
        ...
        {
            string gasPriceRes = await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_gasPrice));
        ...
        {
            string maxPriorityFeeRes = await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_maxPriorityFeePerGas));
        ...
        {
            object syncingResult = await PostWithRetry<object>(rpcClient, nameof(ISilRpcModule.sil_syncing));

            return syncingResult is bool isSyncing && !isSyncing;
        }

        public async Task<string> GetBalanceAsync(Address address, string blockTag = Latest) => await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_getBalance), address, blockTag);

        public async Task<string> SendRawTransactionAsync(string txRlpHex) => await PostWithRetry<string>(rpcClient, nameof(ISilRpcModule.sil_sendRawTransaction), txRlpHex);

        public async Task<BlockModel<Hash256>?> GetBlockByNumberAsync(object blockNumberOrTag, bool fullTxObjects) => await PostWithRetryNullable<BlockModel<Hash256>>(rpcClient, nameof(ISilRpcModule.sil_getBlockByNumber), blockNumberOrTag, fullTxObjects);
