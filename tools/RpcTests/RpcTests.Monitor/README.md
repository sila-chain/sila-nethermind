...
Continuously monitors an Sila RPC node by conditionally replaying parameterized test cases,
...
| `--tests` | `-g` | `sila-mainnet/**/*` | Glob pattern(s) for test files under `tests/`, repeatable                    |
...
Monitor sila-mainnet node against a Geth reference:
...
dotnet run -- \
  -t localhost:8545 \
  -r geth-archive:8545 \
  -g "sila-mainnet/sil_call/*" \
  -g "sila-mainnet/sil_getLogs/*" \
  -p 8
```
...
export RPC_MONITOR_BOT_TOKEN="xoxb-..."
export RPC_MONITOR_CHANNEL_ID="C1234567890"
dotnet run -- -t localhost:8545 -g "sila-mainnet/sil_call/*"
```
...
[
  {
    "run": "EveryBlocks(5)",
    "test": {
      "description": "WSIL transfers to Uniswap V3"
    },
    "request": {
      "jsonrpc": "2.0",
      "method": "sil_getLogs",
...
## Execution
The monitor subscribes to the new block events from the target node via WebSocket (`sil_subscribe("newHeads")`).
