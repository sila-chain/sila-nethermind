...
public class BlockDataFetcher(HttpClient httpClient, ILogManager logManager, HttpClient? consensusClient = null)
{
    ...
    public virtual async Task<JsonObject?> GetBeaconBlockHeader()
    {
        ...
        try
        {
            HttpRequestMessage requestMessage = new(HttpMethod.Get, "/sil/v1/beacon/headers/head");

            _logger.Info("PR -> CL|B|/sil/v1/beacon/headers/head");
            ...
            string responseJson = await response.Content.ReadAsStringAsync();
            _logger.Info($"CL -> PR|B|/sil/v1/beacon/headers/head|{responseJson}");
    ...
    public virtual async Task<JsonObject?> GetBeaconBlock(string blockId)
    {
        ...
        try
        {
            HttpRequestMessage requestMessage = new(HttpMethod.Get, $"/sil/v1/beacon/blocks/{blockId}");

            _logger.Info($"PR -> CL|B|/sil/v1/beacon/blocks/{blockId}");
            HttpResponseMessage response = await _consensusClient.SendAsync(requestMessage);
            if (!response.IsSuccessStatusCode)
            {
            ...
            string responseJson = await response.Content.ReadAsStringAsync();
            _logger.Info($"CL -> PR|B|/sil/v1/beacon/blocks/{blockId}|{responseJson.Substring(0, Math.Min(200, responseJson.Length))}...");
