...
public static class T8nValidator
{
    ...
    {
        if (!spec.IsSip1559Enabled) return;
        if (env.CurrentBaseFee is not null) return;

        if (!env.ParentBaseFee.HasValue || env.CurrentNumber == 0)
        {
            throw new T8nException("SIP-1559 config but missing 'parentBaseFee' in env section",
    ...
    {
        if (!spec.IsSip4895Enabled) return;
        if (env.Withdrawals is null)
        {
            throw new T8nException("SilaShanghai config but missing 'withdrawals' in env section",
                T8nErrorCodes.ErrorConfig);
        }
    }
