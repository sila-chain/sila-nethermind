...
{
    private static readonly SilaJsonSerializer SilaJsonSerializer = new();
    ...
    private static T LoadDataFromFile<T>(string filePath, string description)
    {
        try
        {
            ...
            return SilaJsonSerializer.Deserialize<T>(fileContent);
        }
    ...
    private static InputData ReadStdInput()
    {
        ...
        try
        {
            return SilaJsonSerializer.Deserialize<InputData>(reader.ReadToEnd());
        }
        catch (Exception e)
        {
