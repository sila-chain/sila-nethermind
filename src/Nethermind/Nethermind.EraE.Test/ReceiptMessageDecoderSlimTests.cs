...
public class ReceiptMessageDecoderSlimTests
{
    ...
    public void Encode_WithSkipBloom_ProducesSmallerPayloadThanFull()
    {
        TxReceipt receipt = Build.A.Receipt
            .WithTxType(TxType.SIP1559)
            .WithLogs(Build.A.LogEntry.WithData([1, 2, 3]).TestObject)
            .TestObject;

        byte[] slimBytes = SlimDecoder.EncodeNew(receipt, RlpBehaviors.Sip658Receipts);
        byte[] fullBytes = FullDecoder.EncodeNew(receipt, RlpBehaviors.Sip658Receipts);
    ...
    public void Decode_AfterSlimEncode_PreservesStatusGasAndLogs()
    {
        ...
        byte[] encoded = SlimDecoder.EncodeNew(original, RlpBehaviors.Sip658Receipts);

        RlpReader ctx = new(encoded.AsSpan());
        TxReceipt decoded = SlimDecoder.Decode(ref ctx, RlpBehaviors.Sip658Receipts)!;
    ...
    public void Encode_WithEmptyLogs_DecodesWithoutError()
    {
        ...
        byte[] encoded = SlimDecoder.EncodeNew(receipt, RlpBehaviors.Sip658Receipts);
        RlpReader ctx = new(encoded.AsSpan());
        TxReceipt decoded = SlimDecoder.Decode(ref ctx, RlpBehaviors.Sip658Receipts)!;
