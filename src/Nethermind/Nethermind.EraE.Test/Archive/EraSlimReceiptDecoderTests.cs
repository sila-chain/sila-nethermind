...
/// <summary>
/// Tests for <see cref="EraSlimReceiptDecoder"/> covering the go-sila 4-field ERA receipt
/// format, which is what ethpandaops and go-sila-based providers emit.
/// </summary>
internal class EraSlimReceiptDecoderTests
{
    // go-sila 4-field format: outer_list { receipt_list { tx_type, status, gas, logs } }
    // Pre-Byzantium: the "status" field is a 32-byte PostTransactionState (state root), not a status code.
    // Post-Byzantium (SIP-658): the "status" field is 0x00 or 0x01 (1 byte).

    [Test]
    public void Decode_GethFormat_PreByzantium_SetsPostTransactionStateNotStatusCode()
    {
        // Arrange: build raw go-sila 4-field receipt bytes with a 32-byte state root.
        ...
        Assert.That(receipts[0].PostTransactionState, Is.EqualTo(expectedStateRoot), "pre-Byzantium go-sila receipts encode the state root in the status field; " +
    ...
    {
        // status = 0x80 (empty bytes = 0/failure in go-sila encoding)
    ...
    // go-sila receipt encoding: outer_list { receipt_list { content } }
