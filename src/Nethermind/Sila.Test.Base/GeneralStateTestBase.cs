...
namespace Sila.Test.Base
{
    public abstract class GeneralStateTestBase
    {
        ...
        protected SilaTestResult RunTest(GeneralStateTest test) => RunTest(test, NullTxTracer.Instance);

        protected SilaTestResult RunTest(GeneralStateTest test, ITxTracer txTracer)
        {
            ...
            BlockHeader header = new(
                ...
                [])
            {
                BaseFeePerGas = test.Fork.IsSip1559Enabled ? test.CurrentBaseFee ?? _defaultBaseFeeForStateTest : UInt256.Zero,
                ...
                ExcessBlobGas = test.CurrentExcessBlobGas ?? (test.Fork.IsSip4844Enabled ? 0ul : null),
                SlotNumber = test.CurrentSlotNumber ?? (test.Fork.IsSip7843Enabled ? 0ul : null),
                ...
                BlockAccessListHash = spec.IsSip7928Enabled ? Keccak.OfAnEmptySequenceRlp : null,
            ...
            SilaTestResult testResult = new(test.Name, test.ForkName, differences.Count == 0)
            {
