...
public abstract class BlockchainTestBase
{
    ...
    protected async Task<SilaTestResult> RunTest(BlockchainTest test, Stopwatch? stopwatch = null, bool failOnInvalidRlp = true, ITestBlockTracer? tracer = null)
    {
        ...
        // SIP-7928 introduces BlockAccessListHash in the block header, which must be computed
        ...
        bool genesisUsesTargetFork = test.Network.IsSip7928Enabled;
        ...
        if (test.Network.IsSip4844Enabled || test.NetworkAfterTransition?.IsSip4844Enabled is true)
        {
        ...
        IDifficultyCalculator difficultyCalculator = new SilashDifficultyCalculator(specProvider);
        ...
        ContainerBuilder containerBuilder = new ContainerBuilder()
            ...
            // Replace NullSealEngine with a validator that enforces pre-Merge Silash difficulty
        ...
        // Wire in the merge module for any post-Merge test (engine API flow OR post-SilaParis
        // RLP-fed blockchain test). The merge module decorates IHeaderValidator with the
        // SIP-3675 post-Merge field rules (difficulty=0, nonce=0, empty UnclesHash) which the
        ...
        try
        {
            ...
            using (stateProvider.BeginScope(null))
            {
                ...
                test.GenesisRlp ??= Rlp.Encode(new Block(JsonToSilaTest.Convert(test.GenesisBlockHeader)));
            ...
            {
                return new SilaTestResult(test.Name, test.ForkName, false) { Error = "head block is null" };
            }
            ...
            // zkSAVM witness assertions. Engine-path diffs were gathered while driving
            ...
            return new SilaTestResult(test.Name, test.ForkName, testPassed)
            {
    ...
    /// </summary>
    /// <param name="witnessDifferences">Collector for zkSAVM witness mismatches found while driving engine_newPayloadWithWitness.</param>
    /// <returns>
    ...
    private static async Task<Result<string>> RunNewPayloads(TestEngineNewPayloadsJson[]? newPayloads, IJsonRpcService rpcService, JsonRpcContext rpcContext, Hash256 initialHeadHash, List<string> witnessDifferences)
    {
        ...
        foreach (TestEngineNewPayloadsJson enginePayload in newPayloads)
        {
            ...
            string? validationError = JsonToSilaTest.ParseValidationError(enginePayload, newPayloadVersion);

            // Only an unmutated, expected-VALID reference witness exercises engine_newPayloadWithWitness; SIP-8025
    ...
    [
        ...
        ("TransactionException.TYPE_4_EMPTY_AUTHORIZATION_LIST", "SIP-7702 transaction with empty auth list"),
        ("TransactionException.TYPE_4_TX_CONTRACT_CREATION", "SIP-7702 transaction cannot be used to create contract"),
    ...
    private static List<string> RunAssertions(BlockchainTest test, Block headBlock, IWorldState stateProvider)
    {
        ...
        {
            BlockHeader testHeader = JsonToSilaTest.Convert(testHeaderJson);
        ...
        List<string> differences)
    {
        ...
        foreach (TestBlockJson testBlockJson in test.Blocks)
        {
            ...
            {
                SilaEcdsa ecdsa = new(test.ChainId);
