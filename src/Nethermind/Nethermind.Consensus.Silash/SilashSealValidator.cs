...
[assembly: InternalsVisibleTo("Sila.PoW.Test")]

namespace Nethermind.Consensus.Silash
{
    public class SilashSealValidator : ISealValidator
    {
        private readonly IDifficultyCalculator _difficultyCalculator;
        private readonly ICryptoRandom _cryptoRandom;
        private readonly ISilash _silash;
        private readonly ITimestamper _timestamper;
        private readonly ILogger _logger;

        private readonly LruCache<ValueHash256, bool> _sealCache = new(2048, 2048, "silash seals");
        ...
        public SilashSealValidator(ILogManager logManager, IDifficultyCalculator difficultyCalculator, ICryptoRandom cryptoRandom, ISilash silash, ITimestamper timestamper)
        {
            _difficultyCalculator = difficultyCalculator ?? throw new ArgumentNullException(nameof(difficultyCalculator));
            _cryptoRandom = cryptoRandom ?? throw new ArgumentNullException(nameof(cryptoRandom));
            _silash = silash ?? throw new ArgumentNullException(nameof(silash));
            _timestamper = timestamper ?? throw new ArgumentNullException(nameof(timestamper));
            _logger = logManager?.GetClassLogger<SilashSealValidator>() ?? throw new ArgumentNullException(nameof(logManager));
        ...
        public bool ValidateSeal(BlockHeader header, bool force)
        {
            ...
            bool result = _silash.Validate(header);
            _sealCache.Set(header.Hash, result);

            return result;
        }

        public void HintValidationRange(Guid guid, ulong start, ulong end) => _silash.HintRange(guid, start, end);
