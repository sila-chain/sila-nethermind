...
{
    internal class SilashCache : ISilashDataSet
    {
        ...
        public SilashCache(uint cacheSize, ReadOnlySpan<byte> seed)
        {
            uint cachePageCount = cacheSize / Silash.HashBytes;
            Size = cachePageCount * Silash.HashBytes;
            ...
            // RandMemoHash
            for (int _ = 0; _ < Silash.CacheRounds; _++)
            {
        ...
        {
            uint n = Size / Silash.HashBytes;
            int r = Silash.HashBytes / Silash.WordBytes;

            uint[] mixInts = new uint[Silash.HashBytes / Silash.WordBytes];
            Data[i % n].AsUInts().CopyTo(mixInts);

            mixInts[0] = i ^ mixInts[0];
            Keccak512.ComputeUIntsToUInts(mixInts, mixInts);

            for (uint j = 0; j < Silash.DataSetParents; j++)
            {
                ulong cacheIndex = Silash.Fnv(i ^ j, mixInts[j % r]);
                Silash.Fnv(mixInts, MemoryMarshal.Cast<Bucket, uint>(MemoryMarshal.CreateSpan(ref Data[cacheIndex % n], 1)));
            }
