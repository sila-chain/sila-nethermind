...
[assembly: InternalsVisibleTo("Nethermind.Silash.Test")]
[assembly: InternalsVisibleTo("Nethermind.Blockchain.Test")]
[assembly: InternalsVisibleTo("Sila.Test.Base")]
[assembly: InternalsVisibleTo("Nethermind.Benchmark")]

namespace Nethermind.Consensus.Silash
{
    internal class Silash : ISilash
    {
        private readonly HintBasedCache _hintBasedCache;

        private readonly ILogger _logger;

        public Silash(ILogManager logManager)
        {
            _logger = logManager?.GetClassLogger<Silash>() ?? throw new ArgumentNullException(nameof(logManager));
        ...
        public (Hash256 MixHash, ulong Nonce) Mine(BlockHeader header, ulong? startNonce = null)
        {
            ...
            ISilashDataSet dataSet = _hintBasedCache.Get(epoch);
            if (dataSet is null)
            {
                if (_logger.IsTrace) _logger.Trace($"Silash cache miss for block {header.ToString(BlockHeader.Format.Short)}");
        ...
        public bool Validate(BlockHeader header)
        {
            ...
            if (dataSet is null)
            {
                if (_logger.IsDebug) _logger.Debug($"Silash cache miss for block {header.ToString(BlockHeader.Format.Short)}");
        ...
        private ISilashDataSet BuildCache(uint epoch)
        {
            uint cacheSize = GetCacheSize(epoch);
            Hash256 seed = GetSeedHash(epoch);
            if (_logger.IsDebug) _logger.Debug($"Building silash cache for epoch {epoch}");
            _cacheStopwatch.Restart();
            ISilashDataSet dataSet = new SilashCache(cacheSize, seed.Bytes);
        ...
        public static (byte[], ValueHash256, bool) Hashimoto(ulong fullSize, ISilashDataSet dataSet, Hash256 headerHash, Hash256 expectedMixHash, ulong nonce)
        {
