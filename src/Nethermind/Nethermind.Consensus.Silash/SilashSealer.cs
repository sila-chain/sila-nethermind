...
{
    internal class SilashSealer : ISealer
    {
        private readonly ISilash _silash;
        private readonly ISigner _signer;
        private readonly ILogger _logger;

        internal SilashSealer(ISilash? silash, ISigner? signer, ILogManager? logManager)
        {
            _logger = logManager?.GetClassLogger<SilashSealer>() ?? throw new ArgumentNullException(nameof(logManager));
        ...
        {
            (Hash256 MixHash, ulong Nonce) = _silash.Mine(block.Header, startNonce);
