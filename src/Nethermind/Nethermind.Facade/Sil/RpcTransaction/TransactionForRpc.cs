...
namespace Nethermind.Facade.Sil.RpcTransaction;
...
public abstract class TransactionForRpc
{
    ...
    {
        // Pre-Berlin only knows Legacy; defaulted-type requests downgrade to avoid SAVM rejection.
        TxType type = Type ?? default;
        return spec is not null && !spec.IsSip2930Enabled && IsTypeDefaulted ? TxType.Legacy : type;
    }

    /// <summary>
    /// Validates fields required for signing (gas, fee, nonce), promotes type-defaulted
    /// transactions to SIP-1559, and returns the resulting <see cref="Transaction"/>.
    /// </summary>
    public Result<Transaction> ToSignableTransaction()
    {
        ...
        // All concrete tx subtypes (AccessList, SIP1559, Blob, SetCode) derive from LegacyTransactionForRpc.
        if (this is not LegacyTransactionForRpc { Nonce: not null })
            return Result<Transaction>.Fail("nonce not specified");

        return PromoteToSip1559IfTypeDefaulted().ToTransaction(validateUserInput: true);
    }

    private static bool HasFeeFields(TransactionForRpc rpcTx) =>
        rpcTx is SIP1559TransactionForRpc { MaxFeePerGas: not null, MaxPriorityFeePerGas: not null }
            or LegacyTransactionForRpc { GasPrice: not null };

    public TransactionForRpc PromoteToSip1559IfTypeDefaulted()
    {
        if (!IsTypeDefaulted) return this;
        // AccessList and its descendants (SIP1559/Blob/SetCode) are already typed — only plain Legacy promotes.
        if (this is AccessListTransactionForRpc) return this;
        if (this is not LegacyTransactionForRpc legacy) return this;

        return new SIP1559TransactionForRpc
        {
            From = legacy.From,
            To = legacy.To,
            Value = legacy.Value,
    ...
    internal class TransactionJsonConverter : JsonConverter<TransactionForRpc>
    {
        ...
        static TransactionJsonConverter()
        {
            ...
            RegisterTransactionType<SIP1559TransactionForRpc>();
        ...
        private Type DeriveTxType(JsonObject untyped, JsonSerializerOptions options, out bool isDefaulted)
        {
            ...
            isDefaulted = true;
            return typeof(SIP1559TransactionForRpc);
        }
