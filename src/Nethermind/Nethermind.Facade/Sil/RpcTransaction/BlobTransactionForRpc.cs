...
public class BlobTransactionForRpc : SIP1559TransactionForRpc, IFromTransaction<BlobTransactionForRpc>
{
    ...
    public override Result<Transaction> ToTransaction(bool validateUserInput = false, ulong? gasCap = null, IReleaseSpec? spec = null)
    {
        ...
        {
            if (hash is null || hash.Length != Sip4844Constants.BytesPerBlobVersionedHash)
        ...
        if (validateUserInput && spec?.IsSip4844Enabled == true)
        {
    ...
    public override Result FillDefaults(in TxFillContext context)
    {
        ...
        {
            // Fail rather than default to 0x0, which is below the SIP-4844 minimum blob base fee of 1.
