...
public class LegacyTransactionForRpc : SignableTransactionForRpc, ITxTyped, IFromTransaction<LegacyTransactionForRpc>
{
    ...
    // NOTE: This field exists only during deserialization according to the Sila JSON-RPC spec.
    ...
    // See: https://github.com/sila-chain/sila-nethermind/pull/6067
        ...
        if ((R?.IsZero == false || S?.IsZero == false) && (R is not null || S is not null))
        {
            ulong v = V is null ? 0
                : V.Value > 1
                    ...
                    : SilaEcdsaExtensions.CalculateV(ChainId ?? 0, V.Value == 1); // protected
