...
public class BlockHeaderForRpc
{
    ...
    public BlockHeaderForRpc(BlockHeader header, ISpecProvider? specProvider = null)
    {
        ...
        if (specProvider is not null)
        {
            ...
            if (spec.IsSip1559Enabled) BaseFeePerGas = header.BaseFeePerGas;
            if (spec.IsSip4844Enabled)
            {
                BlobGasUsed = header.BlobGasUsed;
                ExcessBlobGas = header.ExcessBlobGas;
            }
            if (spec.IsSip4788Enabled) ParentBeaconBlockRoot = header.ParentBeaconBlockRoot;
            if (spec.IsSip7843Enabled) SlotNumber = header.SlotNumber;
        }
    }
