...
public class SimulateBridgeHelper(IBlocksConfig blocksConfig, ISpecProvider specProvider)
{
    ...
    {
        // state-override commits must not trigger SIP-158 deletion on accounts whose
        // code/nonce were zeroed while storage remains — SIP-7610 collision checks need that storage.
        releaseSpec = releaseSpec.WithoutSip158();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (payload.BlockStateCalls is not null)
        {
            ...
            foreach (BlockStateCall<TransactionWithSourceDetails> blockCall in payload.BlockStateCalls)
            {
                ...
                env.SimulateRequestState.BlobBaseFeeOverride = spec.IsSip4844Enabled ? blockCall.BlockOverrides?.BlobBaseFee : null;
        ...
        bool validate)
    {
        ...
        // This is because it creates an edge case in SAVM where gasPrice < baseFee.
        ...
        result.ExcessBlobGas = spec.IsSip4844Enabled ? BlobGasCalculator.CalculateExcessBlobGas(parent, spec) : null;
