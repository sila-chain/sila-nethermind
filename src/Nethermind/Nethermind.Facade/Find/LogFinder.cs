...
namespace Nethermind.Facade.Find
{
        ...
        : ILogFinder
    {
        ...
        private IEnumerable<FilterLog> RunParallel<T>(IEnumerable<T> source, Func<T, IEnumerable<FilterLog>> worker, bool tryParallel, CancellationToken cancellationToken)
        {
            ...
            // we want to support one parallel sil_getLogs call for maximum performance
            // we don't want support more than one sil_getLogs call so we don't starve CPU and threads
            ...
            if (canRunParallel)
            {
                if (_logger.IsTrace) _logger.Trace($"Allowing parallel sil_getLogs, already parallel executions: {parallelExecutions}.");
            ...
            {
                if (_logger.IsTrace) _logger.Trace($"Not allowing parallel sil_getLogs, already parallel executions: {parallelExecutions}.");
            }
        ...
        protected BlockHeader? FindHeaderOrLogError(ulong blockNumber, CancellationToken token)
        {
            ...
            {
                _logger.Error($"Could not find block {blockNumber} in database. sil_getLogs will return incomplete results.");
            }
