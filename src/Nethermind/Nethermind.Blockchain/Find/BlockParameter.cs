    ...
    public class BlockParameterConverter : JsonConverter<BlockParameter>
    {
        ...
        public override BlockParameter? Read(
            ...
            reader.TokenType switch
            {
                ...
                JsonTokenType.Number when !SilaJsonSerializer.StrictHexFormat =>
        ...
        private static BlockParameter ReadStringFormatOther(ReadOnlySpan<byte> span)
        {
            ...
            if (span.Length >= 2 && span.StartsWith("0x"u8))
            {
                ...
                if (SilaJsonSerializer.StrictHexFormat)
                {
                    // SIP-1474 quantity: the empty "0x" is not a valid block number.
                    if (span.Length == 0)
                    {
                        throw new BlockParameterParseException($"hex string \"{Bytes.EmptyHexValue}\"");
                    }
                    // SIP-1474 quantity: no leading-zero digits (only "0x0" represents zero).
            ...
            if (!SilaJsonSerializer.StrictHexFormat && Utf8Parser.TryParse(span, out ulong decimalValue, out _))
            {
