...
public interface ISyncConfig : IConfig
{
    ...
    [ConfigItem(
        Description = "Whether to use the Fast sync mode (the sil/63 synchronization algorithm).",
        DefaultValue = "false")]
    ...
    [ConfigItem(Description = "Whether to make smaller requests, in Fast Blocks mode, to avoid Geth from disconnecting. On the Geth-heavy networks (e.g., SilaMainnet), it's  a desired behavior while on Nethermind- or OpenSila-heavy networks (Aura), it slows down the sync by a factor of ~4.", DefaultValue = "true")]
