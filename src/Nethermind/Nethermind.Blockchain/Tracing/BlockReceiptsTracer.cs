...
public class BlockReceiptsTracer(bool parallel = false) : IBlockTracer, ITxTracer, IJournal<int>, ITxTracerWrapper
{
    ...
    /// SIP-7778: Block gas uses pre-refund values for gas limit accounting,
    ...
    {
        // Track cumulative block gas for restore (regular + SIP-8037 state)
        ...
        // SIP-8037: block gasUsed = max(sum_regular, sum_state). Override header accumulation.
        if (!parallel)
        {
            Block.Header.GasUsed = SilaGasPolicy.CombineBlockGas(cumulativeBlockGas, cumulativeBlockStateGas);
        }
    ...
    protected virtual TxReceipt BuildReceipt(Address recipient, in GasConsumed gasConsumed, byte statusCode, LogEntry[] logEntries, Hash256? stateRoot)
    {
        ...
        // Diagnostic-only: effective gas price after SIP-1559 baseFee adjustment.
        ...
        UInt256 effectiveGasPrice = transaction.CalculateEffectiveGasPrice(sip1559Enabled: baseFee > 0, baseFee);
        ...
        // SIP-7778: regular-dimension block accounting introduces the
        // pre-refund/post-refund split. BlockGasUsed is pre-refund; ExecutionGasUsed
        // (= OperationGas) is post-refund without SIP-7976 floor.
        ...
        // SIP-8037: state-dim block accounting. Only set when the tx actually consumed
    ...
    public void ReportActionError(SavmExceptionType exceptionType) =>
    ...
    private readonly List<(ulong Regular, ulong State)> _cumulativeBlockGasPerTx = [];  // Track pre-refund block gas for restore (regular + SIP-8037 state)
    ...
    /// <summary>
    /// SIP-8037: cumulative state gas for the last tracked tx.
    ...
    public void Restore(int snapshot)
    {
        ...
        // Restore block gas from tracking: max(cumulative_regular, cumulative_state) for SIP-8037
        (ulong cumulativeRegular, ulong cumulativeState) = _cumulativeBlockGasPerTx.Count > 0 ? _cumulativeBlockGasPerTx[^1] : (0, 0);
        Block.Header.GasUsed = SilaGasPolicy.CombineBlockGas(cumulativeRegular, cumulativeState);
