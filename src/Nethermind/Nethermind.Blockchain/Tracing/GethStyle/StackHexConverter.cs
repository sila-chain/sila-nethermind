...
public sealed class StackHexConverter : JsonConverter<ReadOnlyMemory<byte>>
{
    public override void Write(Utf8JsonWriter writer, ReadOnlyMemory<byte> value, JsonSerializerOptions options)
    {
        ...
        for (int offset = 0; offset < span.Length; offset += SavmStack.WordSize)
            HexWriter.WriteUInt256HexRawValue(writer,
                new UInt256(span.Slice(offset, SavmStack.WordSize), isBigEndian: true),
    ...
    public override ReadOnlyMemory<byte> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        Span<byte> wordBuf = stackalloc byte[SavmStack.WordSize];
        while (reader.Read() && reader.TokenType == JsonTokenType.String)
        {
            ReadOnlySpan<byte> decoded = Bytes.FromHexString(reader.GetString()!);
            wordBuf.Clear();
            decoded.CopyTo(wordBuf.Slice(SavmStack.WordSize - decoded.Length));
