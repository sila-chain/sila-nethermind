...
public sealed class GethLikeTxDirectStreamingTracer : GethLikeTxTracer
{
    ...
    private const int SavmWordSize = SavmPooledMemory.WordSize;
    private static readonly JsonEncodedText ZeroMemoryWord = JsonEncodedText.Encode("0x" + new string('0', SavmWordSize * 2));
    ...
    public override void SetOperationStack(TraceStack stack)
    {
        ...
        int needed = stack.Count * SavmWordSize;
        if (needed == 0) { _stackByteCount = 0; return; }

        EnsureBuffer(ref _stackBuffer, needed);
        for (int i = 0; i < stack.Count; i++)
        {
            stack[i].Span.CopyTo(_stackBuffer.AsSpan(i * SavmWordSize, SavmWordSize));
        }
    ...
    public override void SetOperationMemory(TraceMemory memoryTrace)
    {
        ...
        int wordCount = (int)((memoryTrace.Size + SavmWordSize - 1) / SavmWordSize);
        if (wordCount == 0) { _memoryByteCount = 0; return; }

        int needed = wordCount * SavmWordSize;
    ...
    private void WriteStackArrayIfPresent()
    {
        ...
        for (int offset = 0; offset < _stackByteCount; offset += SavmWordSize)
        {
            ReadOnlySpan<byte> word = _stackBuffer!.AsSpan(offset, SavmWordSize);
    ...
    private void WriteMemoryArrayIfPresent()
    {
        ...
        for (int offset = 0; offset < _memoryByteCount; offset += SavmWordSize)
        {
            ReadOnlySpan<byte> slot = _memoryBuffer!.AsSpan(offset, SavmWordSize);
    ...
    // children), mirroring go-sila's journaled refund counter.
