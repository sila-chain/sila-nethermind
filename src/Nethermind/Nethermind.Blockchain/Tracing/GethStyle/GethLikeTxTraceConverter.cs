...
public class GethLikeTxTraceConverter : JsonConverter<GethLikeTxTrace>
{
        ...
        IDictionary<UInt256, UInt256>? storage)
    {
        ...
        if (entry.Stack is { } stack)
        {
            ...
            for (int i = 0; i < stackSpan.Length; i += SavmStack.WordSize)
                HexWriter.WriteUInt256HexRawValue(writer,
                    new UInt256(stackSpan.Slice(i, SavmStack.WordSize), isBigEndian: true));
            writer.WriteEndArray();
        }
        ...
        if (entry.Memory is { } memory)
        {
            ...
            for (int i = 0; i < memSpan.Length; i += SavmPooledMemory.WordSize)
                HexWriter.WriteFixed32HexRawValue(writer,
                    memSpan.Slice(i, SavmPooledMemory.WordSize), addHexPrefix: true);
            writer.WriteEndArray();
        }
