...
public static class JavaScriptConverter
{
    ...
    {
        string hexString => Bytes.FromHexString(hexString, SavmPooledMemory.WordSize),
        _ => ListToWord(input)
    };

    private static byte[] ListToWord(object input) => input.ToBytes().PadLeft(SavmPooledMemory.WordSize);
