...
// The result will be a nested list of call frames, resembling how the SAVM works.
...
public sealed class NativeCallTracer : GethLikeNativeTxTracer
{
    ...
    private SavmExceptionType? _error;
        ...
        GethTraceOptions options) : base(options)
    {
        ...
        _config = options.TracerConfig?.Deserialize<NativeCallTracerConfig>(SilaJsonSerializer.JsonOptions) ?? new NativeCallTracerConfig();
    ...
    {
        _error = SavmExceptionType.Revert;
    ...
    public override void MarkAsFailed(Address recipient, in GasConsumed gasSpent, byte[] output, string? error, Hash256? stateRoot = null)
    {
        ...
        {
            SavmExceptionType errorType = _error.Value;
            firstCallFrame.Error = errorType.GetSavmExceptionDescription();
            if (errorType == SavmExceptionType.Revert && error is not TransactionSubstate.Revert)
            {
    ...
    private void OnExit(ulong gas, ReadOnlyMemory<byte>? output, SavmExceptionType? error = null)
    {
        if (!_config.OnlyTopCall && Depth > 0)
        {
            NativeCallTracerCallFrame callFrame = _callStack[^1];
    ...
    private static void ProcessOutput(NativeCallTracerCallFrame callFrame, ReadOnlyMemory<byte>? output, SavmExceptionType? error)
    {
        if (error is not null)
        {
            callFrame.Error = error.Value.GetSavmExceptionDescription();
            if (callFrame.Type is Instruction.CREATE or Instruction.CREATE2)
            {
                callFrame.To = null;
            }

            if (error == SavmExceptionType.Revert && output?.Length != 0)
            {
