...
public class EstimateGasTracer : TxTracer
{
    ...
    public override void ReportActionError(SavmExceptionType exceptionType)
    {
        ReportOperationError(exceptionType);
        UpdateAdditionalGas();
    }

    public void ReportActionError(SavmExceptionType exceptionType, ulong gasLeft)
    {
        ReportOperationError(exceptionType);
    ...
    public override void ReportOperationError(SavmExceptionType error)
    {
        ...
        {
            OutOfGas |= error == SavmExceptionType.OutOfGas;

            if (error == SavmExceptionType.Revert)
            {
                TopLevelRevert = true;
            }
        }
