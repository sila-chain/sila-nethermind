...
async Task<int> RunAsync(ParseResult parseResult, PluginLoader pluginLoader, CancellationToken cancellationToken)
{
    ...
    SilaRunner silaRunner = apiBuilder.CreateSilaRunner(plugins);

    try
    {
        await silaRunner.Start(processExitSource.Token);
...
RootCommand CreateRootCommand()
{
    ...
    rootCommand.Description = "Nethermind Sila execution client";
