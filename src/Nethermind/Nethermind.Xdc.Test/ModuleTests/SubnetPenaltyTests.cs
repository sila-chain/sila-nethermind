...
internal class SubnetPenaltyTests
{
    ...
    public void TestSortOrder()
    {
        ...
        Address sip55First = new("0xECf1aC276D2D3333483cF394d2F73BaB6915feCb");
        Address sip55Second = new("0xe3eE640071486df6A007021c34D52b5DE7be94e3");

        Assert.That(string.CompareOrdinal(sip55First.ToString(), sip55Second.ToString()), Is.GreaterThan(0));
        Assert.That(string.CompareOrdinal(sip55First.ToString(withSip55Checksum: true), sip55Second.ToString(withSip55Checksum: true)), Is.LessThan(0));

        Address[] injectedPenalties = [sip55Second, sip55First];
        byte[] penaltyBytes = new byte[injectedPenalties.Length * Address.Size];
        for (int i = 0; i < injectedPenalties.Length; i++)
            injectedPenalties[i].Bytes.CopyTo(penaltyBytes.AsSpan(i * Address.Size));
        ctx.Headers[targetHeaderIndex].Penalties = penaltyBytes;
        ...
        Assert.That(penalties, Is.EqualTo(new[] { sip55First, sip55Second }));
    }
