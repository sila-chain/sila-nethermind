...
public class XdcTestBlockchain : TestBlockchain
{
    ...
    protected override ContainerBuilder ConfigureContainer(ContainerBuilder builder, IConfigProvider configProvider)
    {
        ContainerBuilder container = base.ConfigureContainer(builder, configProvider)
            ...
            new TestSpecProvider(WrapReleaseSpec(SilaShanghai.Instance))
            {
            ...
            {
                Nethermind.TxPool.TxPool txPool = new(ctx.Resolve<ISilaEcdsa>()!,
    ...
    private IXdcReleaseSpec WrapReleaseSpec(IReleaseSpec spec)
    {
        ...
        xdcSpec.MasternodeReward = (UInt256)2 * Unit.Sila; // 2 Sila in Wei per masternode
        xdcSpec.ProtectorReward = Unit.Sila;               // 1 Sila in Wei per protector
        xdcSpec.ObserverReward = Unit.Sila / 2;            // 0.5 Sila in Wei per observer
    ...
    ) : IGenesisBuilder
    {
        public Block Build()
        {
            ...
            byte[] dummyCode = Prepare.SavmCode
                .STOP()
                .Done;
