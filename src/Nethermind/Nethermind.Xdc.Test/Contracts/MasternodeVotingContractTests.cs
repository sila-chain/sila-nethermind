...
internal class MasternodeVotingContractTests
{
    ...
    public void GetCandidatesAndStake_GenesisSetup_CanReadExpectedCandidates()
    {
        ...
        ISpecProvider specProvider = new TestSpecProvider(SilaShanghai.Instance);
        ...
        {
            stateProvider.CreateAccount(sender.Address, 1.Sila);
            byte[] code = XdcContractData.XDCValidatorBin();
            stateProvider.CreateAccountIfNotExists(codeSource, 0);
            stateProvider.InsertCode(codeSource, ValueKeccak.Compute(code), code, SilaShanghai.Instance);
        ...
        SilaTransactionProcessor transactionProcessor = new(BlobBaseFeeCalculator.Instance, specProvider, stateProvider, virtualMachine, codeInfoRepository, LimboLogs.Instance);
        ...
        foreach (Address candidate in candidates)
        {
            ...
            Assert.That(stake, Is.EqualTo(10_000_000.Sila));
        }
    }
