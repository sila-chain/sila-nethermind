...
public class XdcProtocolHandlerTests
{
    ...
    {
        ZeroPacket packet = CreatePacket(Sil62MessageCode.Status);
    ...
    public void SendNewTransactions_UsesTransactionsMessage_NotNewPooledTransactionHashes(bool sendFullTx)
    {
        ...
        // never via NewPooledTransactionHashesMessage (0x08 from Sil65) which conflicts with XDC OrderTxMsg.
        (XdcProtocolHandler handler, _, ISession session, _, _, _) = CreateAll();
        using (handler)
        {
            ...
            session.Received(1).DeliverMessage(Arg.Is<P2PMessage>(m => m.PacketType == Sil62MessageCode.Transactions));
            session.DidNotReceive().DeliverMessage(Arg.Is<P2PMessage>(m => m.PacketType == Sil65MessageCode.NewPooledTransactionHashes));
        }
    }
}
