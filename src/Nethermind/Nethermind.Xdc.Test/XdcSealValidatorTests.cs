...
internal class XdcSealValidatorTests
{
    ...
    public static IEnumerable<TestCaseData> InvalidSignatureCases()
    {
        ...
        byte[] keyASig = new SilaEcdsa(0).Sign(TestItem.PrivateKeyA, header).BytesWithRecovery;
        ...
        header = Build.A.XdcBlockHeader().TestObject;
        header.Beneficiary = TestItem.AddressA;
        byte[] keyBSig = new SilaEcdsa(0).Sign(TestItem.PrivateKeyB, header).BytesWithRecovery;
        yield return new TestCaseData(header, keyBSig)
            .SetName("WrongSignerSignature");

        header = Build.A.XdcBlockHeader().TestObject;
        header.Beneficiary = TestItem.AddressA;
        Signature canonicalSig = new SilaEcdsa(0).Sign(TestItem.PrivateKeyA, header);
