...
namespace Nethermind.TxPool
{
    ...
    public class TxPool : ITxPool, IAsyncDisposable
    {
        ...
        public TxPool(ISilaEcdsa ecdsa,
        ...
        private void ReAddReorganisedTransactions(Block? previousBlock)
        {
            if (previousBlock is not null)
            {
                ...
                bool isSip155Enabled = _specProvider.GetSpec(previousBlock.Header).IsSip155Enabled;
                ...
                for (int i = 0; i < txs.Length; i++)
                {
                    ...
                    SubmitTx(tx, isSip155Enabled ? TxHandlingOptions.None : TxHandlingOptions.PreSip155Signing);
                }
                    ...
                    && blobTxs is not null)
                {
                    foreach (Transaction blobTx in blobTxs)
                    {
                        ...
                        SubmitTx(blobTx, isSip155Enabled ? TxHandlingOptions.None : TxHandlingOptions.PreSip155Signing);
                    }
        ...
        private void RemoveProcessedTransactions(Block block)
        {
            ...
            if (transactionsInBlock != 0)
            {
                ...
                Metrics.Sip1559TransactionsRatio = (float)sip1559Txs / transactionsInBlock;
                Metrics.Sip7702TransactionsInBlock = sip7702Txs;
        ...
        {
            bool sip1559Enabled = _specProvider.GetCurrentHeadSpec().IsSip1559Enabled;
            UInt256 effectiveGasPrice = tx.CalculateEffectiveGasPrice(sip1559Enabled, _headInfo.CurrentBaseFee);
            ...
            UpdateTransactionDelegate updateTx)
        {
            ...
            bool isSip1559 = headSpec.IsSip1559Enabled;
                ...
                try
                {
                    ...
                    else if (tx.Nonce == currentNonce + (ulong)i)
                    {
                        UInt256 effectiveGasPrice =
                            tx.CalculateEffectiveGasPrice(isSip1559,
...
* Sip1559 Added:        {Metrics.Pending1559TransactionsAdded,24:N0}
...
* Sip1559 Transactions: {Metrics.Sip1559TransactionsRatio,24:P5}
...
* Sip7702 txs:          {Metrics.Sip7702TransactionsInBlock,24:N0}
------------------------------------------------
Db usage:
