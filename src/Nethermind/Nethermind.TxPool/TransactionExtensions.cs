...
namespace Nethermind.TxPool
{
    ...
    {
        private static readonly long MaxSizeOfTxForBroadcast = 4.KiB; //4KB, as in Geth https://github.com/sila-chain/go-sila/pull/27618
        ...
        internal static UInt256 CalculateGasPrice(this Transaction tx, bool sip1559Enabled, in UInt256 baseFee)
        {
            if (sip1559Enabled && tx.Supports1559)
            {
                if (tx.GasLimit > 0)
                {
                    return tx.CalculateEffectiveGasPrice(sip1559Enabled, baseFee);
                }
        ...
        internal static UInt256 CalculateAffordableGasPrice(this Transaction tx, bool sip1559Enabled, in UInt256 baseFee, in UInt256 balance)
        {
            if (sip1559Enabled && tx.Supports1559)
            {
                if (balance > tx.Value && tx.GasLimit > 0)
                {
                    UInt256 effectiveGasPrice = tx.CalculateEffectiveGasPrice(sip1559Enabled, baseFee);
        ...
        internal static bool IsOverflowWhenAddingTxCostToCumulative(this Transaction tx, UInt256 currentCost, out UInt256 cumulativeCost)
        {
            ...
            if (tx.SupportsBlobs)
            {
                ...
                overflow |= UInt256.MultiplyOverflow(Sip4844Constants.GasPerBlob, (UInt256)tx.BlobVersionedHashes!.Length, out UInt256 blobGas);
