...
{
    /// <summary>
    ///     Sil sync peer pool is capable of returning a sync peer allocation that is best suited for the requesting
...
#pragma warning restore 4014
        ...
        {
            if (_logger.IsDebug) _logger.Debug("Starting sil sync peer upgrade timer");
        ...
        private async Task ExecuteRefreshTask(RefreshTotalDiffTask refreshTotalDiffTask, CancellationToken token)
        {
            ...
            Task delayTask = Task.Delay(Timeouts.Sil, linkedSource.Token);
