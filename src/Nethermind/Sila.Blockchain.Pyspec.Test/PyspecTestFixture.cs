...
namespace Sila.Blockchain.Pyspec.Test;
...
// true            = also honor TEST_SKIP_HEAVY=1 for engine/sync/SilaAmsterdam variants.
...
// SIP-7928 (SilaAmsterdam) parallel-BAL / batch-read prewarm matrix.
...
// exsrcising sync-mode validation; we run the engine payload through the standard harness
...
// Loads only `for_amsterdam` because parallel-BAL execution is gated on SIP-7928.
...
// Engine-payload variant of the SilaAmsterdam fixture; loads from `for_amsterdam` engine tree.
...
{
    public static IEnumerable<T> Load<T, TSelf>(string root, string suffix) where T : SilaTest =>
    ...
    public static IEnumerable<TestCaseData> LoadCases<T, TSelf>(string root, string suffix) where T : SilaTest =>
        ToTestCases(Load<T, TSelf>(root, suffix));

    public static IEnumerable<TestCaseData> ToTestCases<T>(IEnumerable<T> tests) where T : SilaTest
    {
        int index = 0;
    ...
    private static string GetTestCaseName(SilaTest test, int index)
    {
...
internal static class CiRunnerGuard
{
    ...
    public static void SkipIfNotLinuxX64()
    {
        ...
        if (s_isCi && !s_isLinuxX64)
            Assert.Ignore("Skipped in CI - engine/SilaAmsterdam tests only run on Linux x64");
    }
