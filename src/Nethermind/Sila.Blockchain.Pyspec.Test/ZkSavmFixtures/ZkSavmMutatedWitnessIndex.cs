...
/// <summary>
/// Indexes which SIP-8025 "mutated" (deliberately corrupted) witnesses each zkSAVM test carries. The marker
...
internal static class ZkSavmMutatedWitnessIndex
{
                ...
                && index.TryGetValue(Key(test.Category, test.Name), out MutatedInfo info))
            {
                if (blocks.Length != info.PayloadCount)
                    throw new InvalidOperationException(
                        $"zkSAVM mutated-witness index misaligned for '{Key(test.Category, test.Name)}': " +
    ...
    private static IReadOnlyDictionary<string, MutatedInfo> Build()
    {
        ...
        string cacheDir = TestFixtureDownloader.EnsureDownloaded(
            "PyTests", global::Sila.Blockchain.Pyspec.Test.Constants.ARCHIVE_URL_TEMPLATE, Constants.ArchiveVersion, Constants.ArchiveName);
