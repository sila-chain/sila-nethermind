...
public sealed class BlobTxDecoder<T>(Func<T>? transactionFactory = null)
    : BaseSIP1559TxDecoder<T>(TxType.Blob, transactionFactory) where T : Transaction, new()
{
    ...
    protected override int GetContentLength(Transaction transaction, RlpBehaviors rlpBehaviors, bool forSigning,
        bool isSip155Enabled = false, ulong chainId = 0)
    {
        int contentLength = base.GetContentLength(transaction, rlpBehaviors, forSigning, isSip155Enabled, chainId);
