    ...
    : ITxDecoder where T : Transaction, new()
{
    ...
    public virtual void Encode<TWriter>(Transaction transaction, ref TWriter writer, RlpBehaviors rlpBehaviors = RlpBehaviors.None, bool forSigning = false, bool isSip155Enabled = false, ulong chainId = 0)
        where TWriter : struct, IRlpWriteBackend, allows ref struct
    {
        int contentLength = GetContentLength(transaction, rlpBehaviors, forSigning, isSip155Enabled, chainId);

        writer.StartSequence(contentLength);
        EncodePayload(transaction, ref writer);
        EncodeSignature(transaction.Signature, ref writer, forSigning, isSip155Enabled, chainId);
    }

    public virtual int GetLength(Transaction transaction, RlpBehaviors rlpBehaviors, bool forSigning = false, bool isSip155Enabled = false, ulong chainId = 0)
    {
        int txContentLength = GetContentLength(transaction, rlpBehaviors, forSigning, isSip155Enabled, chainId);
    ...
    protected virtual int GetContentLength(Transaction transaction, RlpBehaviors rlpBehaviors, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0) =>
        GetPayloadLength(transaction) + GetSignatureLength(transaction.Signature, forSigning, isSip155Enabled, chainId);
    ...
    protected virtual int GetSignatureLength(Signature? signature, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0)
    {
        int contentLength = 0;
    ...
    protected virtual void EncodeSignature<TWriter>(Signature? signature, ref TWriter writer, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0)
        where TWriter : struct, IRlpWriteBackend, allows ref struct
    {
        if (!forSigning)
        {
