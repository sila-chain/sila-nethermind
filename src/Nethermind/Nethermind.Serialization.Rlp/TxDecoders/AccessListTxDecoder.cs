...
using Nethermind.Serialization.Rlp.Sip2930;
    ...
    : BaseTxDecoder<T>(txType, transactionFactory) where T : Transaction, new()
{
    public override void Encode<TWriter>(Transaction transaction, ref TWriter writer, RlpBehaviors rlpBehaviors = RlpBehaviors.None,
        bool forSigning = false, bool isSip155Enabled = false, ulong chainId = 0)
    {
    ...
    public override int GetLength(Transaction transaction, RlpBehaviors rlpBehaviors, bool forSigning = false, bool isSip155Enabled = false,
        ulong chainId = 0)
    {
        int txPayloadLength = base.GetLength(transaction, rlpBehaviors, forSigning, isSip155Enabled, chainId);
