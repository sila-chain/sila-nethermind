...
{
    private static bool IncludeSigChainIdHack(bool isSip155Enabled, ulong chainId) => isSip155Enabled && chainId != 0;

    protected override ulong GetSignatureFirstElement(Signature signature) => signature.V;

    protected override void EncodeSignature<TWriter>(Signature? signature, ref TWriter writer, bool forSigning, bool isSip155Enabled, ulong chainId)
    {
        if (forSigning)
        {
            if (IncludeSigChainIdHack(isSip155Enabled, chainId))
            {
        ...
        {
            base.EncodeSignature(signature, ref writer, false, isSip155Enabled, chainId);
        }
    }
    ...
    protected override int GetSignatureLength(Signature? signature, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0)
    {
        if (forSigning)
        {
            int contentLength = 0;
        ...
        return base.GetSignatureLength(signature, false, isSip155Enabled, chainId);
    }
}
