...
public class NodeRecordSignerTests
{
    ...
    [Test(Description = "https://sips.sila.org/SIPS/sip-778")]
    public void Is_correct_on_sip_test_vector()
    {
    ...
    public void Can_serialize_eth_entry_as_nested_fork_id_list()
    {
        ...
        NodeRecordSigner signer = new(ecdsa, privateKey);
        NodeRecord nodeRecord = new();
        nodeRecord.SetEntry(new SilEntry(forkHash, next));
        ...
        ForkId? forkId = decoded.GetValue<ForkId>(EnrContentKey.Sil);
    ...
    [TestCase(3)]
    [TestCase(5)]
    public void Sil_entry_rejects_fork_hash_with_wrong_length(int forkHashLength)
    {
        byte[] forkHash = new byte[forkHashLength];
        Assert.That(() => new SilEntry(forkHash, 0UL), Throws.TypeOf<ArgumentException>());
    }
    ...
    private sealed class FutureEthEntry(byte[] forkHash, ulong next) : EnrContentEntry<ForkId>(new ForkId(forkHash, next))
    {
        ...
        public override string Key => EnrContentKey.Sil;
