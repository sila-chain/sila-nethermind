...
public class ForkTests
{
    ...
    {
        // Load foundation.json — source of truth for sila-mainnet forks
    ...
    public void Bpo_forks_are_blob_only_and_do_not_inherit_Amsterdam()
    {
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(BPO3.Instance.IsSip7928Enabled, Is.False);
            Assert.That(BPO4.Instance.IsSip7928Enabled, Is.False);
            Assert.That(BPO5.Instance.IsSip7928Enabled, Is.False);
            Assert.That(SilaAmsterdam.Instance.IsSip7928Enabled, Is.True);
        }
    }
}
