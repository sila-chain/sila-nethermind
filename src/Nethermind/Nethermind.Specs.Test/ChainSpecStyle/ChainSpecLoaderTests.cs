...
public class ChainSpecLoaderTests
{
    ...
    public void Can_load_mainnet()
    {
        ...
        {
            Assert.That(chainSpec.Parameters.Sip1559BaseFeeInitialValue, Is.EqualTo(1.GWei), $"fork base fee");
            Assert.That(chainSpec.NetworkId, Is.EqualTo(1), $"{nameof(chainSpec.NetworkId)}");
            Assert.That(chainSpec.Name, Is.EqualTo("Sila"), $"{nameof(chainSpec.Name)}");
            Assert.That(chainSpec.DataDir, Is.EqualTo("sila"), $"{nameof(chainSpec.Name)}");
            Assert.That(chainSpec.SealEngineType, Is.EqualTo(SealEngineType.Silash), "engine");
    ...
    public void Can_load_sepolia()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(chainSpec.Name, Is.EqualTo("SilaSepolia Testnet"), $"{nameof(chainSpec.Name)}");
    ...
    [Test]
    public void Can_load_posdao_with_opensila_pricing_transitions()
    {
        // TODO: modexp 2565
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(chainSpec.Parameters.Sip152Transition, Is.EqualTo(15));
            Assert.That(chainSpec.Parameters.Sip1108Transition, Is.EqualTo(10));
        }
    }
    ...
    public void All_ChainSpecParamsJson_properties_should_be_mapped_in_loader()
    {
        ...
        // - Sip1706Transition: throws when set together with Sip2200Transition
        // - Sip1283ReenableTransition: must equal Sip1706Transition when Sip1283DisableTransition is set
        HashSet<string> excludedProperties = ["Sip1706Transition", "Sip1283ReenableTransition"];
        ...
        // including those with non-zero field initializers (e.g. Sip2935RingBufferSize)
