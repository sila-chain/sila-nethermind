...
public class NetworkModuleTest
{
    ...
    private static WeakReference CreateDisposeAndTrack(IProtocolHandlerFactory handlerFactory)
    {
        ...
        // versioned factories match their specific version (e.g. Sil68 = 68)
    ...
    public void Registers_eth71_protocol_handler_factory()
    {
        ...
        IProtocolHandler handler = cont.Resolve<IProtocolHandlerFactory[]>()
            .Where(static factory => factory.ProtocolCode == Protocol.Sil)
            .Select(factory =>
            {
                factory.TryCreate(session, SilVersions.Sil71, out IProtocolHandler createdHandler);
                return createdHandler;
            })
            .FirstOrDefault(static handler => handler is Sil71ProtocolHandler);

        Assert.That(handler, Is.TypeOf<Sil71ProtocolHandler>());
        handler?.Dispose();
    }
