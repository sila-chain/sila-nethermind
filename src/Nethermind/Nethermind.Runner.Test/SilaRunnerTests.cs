...
using Nethermind.Runner.Sila;
using Nethermind.Runner.Sila.Api;
...
using Build = Nethermind.Runner.Test.Sila.Build;

namespace Nethermind.Runner.Test;

[TestFixture, Parallelizable(ParallelScope.None)]
public class SilaRunnerTests
{
    static SilaRunnerTests()
    {
    ...
    private static ICollection<(string file, ConfigProvider configProvider)> InitOnce()
    {
...

        {
            ...
            ConfigProvider configProvider = new();
            configProvider.AddSource(new JsonConfigSource("configs/sila-mainnet.json"));
            configProvider.Initialize();
            configProvider.GetConfig<ISyncConfig>().VerifyTrieOnStateSyncFinished = true;
            result.Add(("sila-mainnet-verify-trie-starter", configProvider));
        }
    ...
    public async Task Smoke_CanResolveAllSteps((string file, ConfigProvider configProvider) testCase, int testIndex)
    {
        ...
        SilaRunner runner = builder.CreateSilaRunner(plugins);
        ...
        try
        {
            ISilaStepsLoader stepsLoader = runner.LifetimeScope.Resolve<ISilaStepsLoader>();
