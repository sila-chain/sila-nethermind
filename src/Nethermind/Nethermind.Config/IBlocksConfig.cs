...
public interface IBlocksConfig : IConfig
{
    ...
    [ConfigItem(Description = "The ticker that gas rewards are denominated in for processing logs", DefaultValue = "SIL", HiddenFromDocs = true)]
    ...
    [ConfigItem(
        Description = "Parallelize transaction execution when Block Level Access Lists are available. Experimental SilaAmsterdam/BAL path; disabling falls back to sequential execution and the option is ignored for blocks without BAL bodies.",
        DefaultValue = "true")]
    bool ParallelExecution { get; set; }

    [ConfigItem(
        Description = "Use parallel state reads when Block Level Access Lists are available. Experimental SilaAmsterdam/BAL path; disabling falls back to sequential reads and the option is ignored for blocks without BAL bodies.",
        DefaultValue = "true")]
