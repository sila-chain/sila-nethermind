...
) : INodeRecordProvider
{
    ...
    private readonly NodeRecordSigner _enrSigner = new(silaEcdsa, nodeKey.Unprotect());
    ...
    private LocalNodeRecord CreateSignedRecord(LocalNodeRecordState state, ulong sequence)
    {
        ...
        selfNodeRecord.SetEntry(new SilEntry(state.ForkId.HashBytes, state.ForkId.Next));
