...
public class SepoliaSpecProvider : ForkScheduleSpecProvider
{
    ...
    private static IReleaseSpec SilaPrague => LazyInitializer.EnsureInitialized(ref _prague,
        static () => new SilaPrague { DepositContractAddress = Sip6110Constants.SepoliaDepositContractAddress });

    private SepoliaSpecProvider() : this(new ForkSchedule
    {
        [GenesisBlockNumber] = London.Instance,
        [ShanghaiTimestamp] = SilaShanghai.Instance,
        [CancunTimestamp] = SilaCancun.Instance,
        [PragueTimestamp] = SilaPrague,
        [OsakaTimestamp] = SilaOsaka.Instance,
    ...
    public override ulong NetworkId => BlockchainIds.SilaSepolia;
