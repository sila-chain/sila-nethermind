...
public sealed class ForkSchedule : IEnumerable<ForkSpec>
{
    ...
    /// Used when forks share a timestamp (Hoodi) or fork-IDs must differ per fork (SilaMainnet).</param>
    /// <param name="excludeBlocks">Block numbers in the schedule that should not produce an activation
    /// (e.g. SilaMainnet's SilaParis, which is TTD-determined and not a fork-ID boundary).</param>
    /// <param name="prepend">Activations to emit before iterating the schedule (e.g. SilaSepolia's merge-block
