...
/// <summary>
/// Loader for Geth-style genesis.json files as defined in SIP-7949.
...
public class GethGenesisLoader(IJsonSerializer serializer) : IChainSpecLoader
{
    ...
    private void LoadParameters(GethGenesisJson gethGenesis, ChainSpec chainSpec)
    {
        ...
        chainSpec.Parameters = new ChainParameters
        {
            ...
            Sip7Transition = config.HomesteadBlock ?? 0,

            // MaxCodeSize (SIP-170) is standard on all networks since Spurious Dragon
            MaxCodeSize = 0x6000,
            MaxCodeSizeTransition = config.Sip158Block ?? config.SpuriousDragonBlock ?? 0,

            Sip150Transition = config.Sip150Block ?? config.TangerineWhistleBlock ?? 0,

            Sip160Transition = config.Sip155Block ?? config.SpuriousDragonBlock ?? 0,
            Sip161abcTransition = config.Sip158Block ?? config.SpuriousDragonBlock ?? 0,
            Sip161dTransition = config.Sip158Block ?? config.SpuriousDragonBlock ?? 0,
            Sip155Transition = config.Sip155Block ?? config.SpuriousDragonBlock ?? 0,

            Sip140Transition = config.ByzantiumBlock,
            Sip211Transition = config.ByzantiumBlock,

            ValidateChainIdTransition = config.Sip155Block ?? config.SpuriousDragonBlock,

            Sip214Transition = config.ByzantiumBlock,
            Sip658Transition = config.ByzantiumBlock,

            Sip145Transition = config.ConstantinopleBlock,

            ValidateReceiptsTransition = config.ByzantiumBlock,
            Sip1014Transition = config.ConstantinopleBlock,
            Sip1052Transition = config.ConstantinopleBlock,
            Sip1283Transition = config.ConstantinopleBlock,

            Sip1283DisableTransition = config.PetersburgBlock,

            Sip152Transition = config.IstanbulBlock,
            Sip1108Transition = config.IstanbulBlock,
            Sip1344Transition = config.IstanbulBlock,
            Sip1884Transition = config.IstanbulBlock,
            Sip2028Transition = config.IstanbulBlock,
            Sip2200Transition = config.IstanbulBlock,

            Sip2565Transition = config.BerlinBlock,
            Sip2929Transition = config.BerlinBlock,
            Sip2930Transition = config.BerlinBlock,

            Sip1559Transition = config.LondonBlock,
            Sip3198Transition = config.LondonBlock,
            Sip3529Transition = config.LondonBlock,
            Sip3541Transition = config.LondonBlock,
            Sip3607Transition = config.LondonBlock,

            MergeForkIdTransition = config.MergeNetsplitBlock,
            TerminalTotalDifficulty = config.TerminalTotalDifficulty,

            // Post-merge per-SIP timestamp fan-out is driven off `HardforkLabels.All` below; only
            // the side-effects that don't fit the bulk-label pattern stay here as conditional gates.
            Sip4788ContractAddress = config.CancunTime is null ? null : Sip4788Constants.BeaconRootsAddress,
            Sip2935ContractAddress = config.PragueTime is null ? null : Sip2935Constants.BlockHashHistoryAddress,
            DepositContractAddress = config.PragueTime is null ? null : config.DepositContractAddress ?? Address.Zero,
            Sip7002ContractAddress = config.PragueTime is null ? null : Sip7002Constants.WithdrawalRequestPredeployAddress,
            Sip7251ContractAddress = config.PragueTime is null ? null : Sip7251Constants.ConsolidationRequestPredeployAddress,
            Sip7934MaxRlpBlockSize = Sip7934Constants.DefaultMaxRlpBlockSize,

            BlobSchedule = blobSchedule
        };

        // Fan out SilaShanghai/SilaCancun/SilaPrague/SilaOsaka/SilaAmsterdam timestamps via the shared HardforkLabels
    ...
    private static readonly Dictionary<string, BlobScheduleFork> _blobScheduleForks =
        ...
        {
            [nameof(SilaCancun)] = new(0),
            [nameof(SilaPrague)] = new(1),
            [nameof(SilaOsaka)] = new(2),
            [nameof(BPO1)] = new(3),
            [nameof(BPO2)] = new(4),
            [nameof(SilaAmsterdam)] = new(5),
            [nameof(BPO3)] = new(6),
            [nameof(BPO4)] = new(7),
            [nameof(BPO5)] = new(8),
        };
    ...
    private static void LoadGenesis(GethGenesisJson gethGenesisJson, ChainSpec chainSpec)
    {
        ...
        UInt256 baseFee = gethGenesisJson.Config.LondonBlock switch
        {
            ...
            0 => gethGenesisJson.BaseFeePerGas ?? Sip1559Constants.DefaultForkBaseFee,
            _ => UInt256.Zero,
        };
    ...
    {
        chainSpec.HomesteadBlockNumber = chainSpec.Parameters.Sip7Transition;
        ...
        chainSpec.ConstantinopleBlockNumber = chainSpec.Parameters.Sip145Transition;
        chainSpec.ConstantinopleFixBlockNumber = chainSpec.Parameters.Sip1283DisableTransition ?? chainSpec.Parameters.Sip145Transition;
...
/// <summary>
/// Silash engine parameters provider for Geth-style genesis files.
/// </summary>
internal sealed class GethGenesisEngineParametersProvider(GethGenesisConfigJson config) : IChainSpecParametersProvider
{
    private readonly GethSilashChainSpecEngineParameters _engineParameters =
    ...
    public T GetChainSpecParameters<T>() where T : IChainSpecEngineParameters
    {
        ...
        if (target.EngineName != Core.SealEngineType.Silash)
        {
    ...
    private sealed class GethSilashChainSpecEngineParameters(GethGenesisConfigJson config) : IChainSpecEngineParameters
    {
        ...
        public ulong? Sip100bTransition { get; } = config.ByzantiumBlock;
        ...
        public void AddTransitions(SortedSet<ulong> blockNumbers, SortedSet<ulong> timestamps)
        {
            ...
            if (Sip100bTransition is not null)
            {
                blockNumbers.Add(Sip100bTransition.Value);
            }
        }
        ...
        public void ApplyToReleaseSpec(ReleaseSpec spec, ulong startBlock, ulong? startTimestamp)
        {
            ...
            spec.IsSip2Enabled = HomesteadTransition <= startBlock;
            spec.IsSip7Enabled = HomesteadTransition <= startBlock;
            spec.IsSip100Enabled = (Sip100bTransition ?? 0) <= startBlock;
