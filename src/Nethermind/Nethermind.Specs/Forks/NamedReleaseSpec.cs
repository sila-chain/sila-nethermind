...
/// Concrete Gnosis forks call <c>base.Apply(spec)</c> to get the sila-mainnet delta.
/// </para>
/// </summary>
public abstract class NamedReleaseSpec : ReleaseSpec
{
    ...
    /// merge-boundary forks (<see cref="SilaParis"/> on sila-mainnet, <see cref="GnosisForks.ShanghaiGnosis"/>
    /// on Gnosis since it skipped a SilaParis-equivalent); descendants inherit because each ancestor's
    ...
    /// corresponding endpoint (e.g. pre-merge forks have no <c>engine_*</c> at all; SilaParis has no
    ...
    /// Each concrete fork overrides this to configure the SIPs and parameters it introduces.
    /// Gnosis forks should call <c>base.Apply(spec)</c> first to include the sila-mainnet delta.
    /// </summary>
...
/// Concrete forks inherit as: <c>class SilaCancun() : NamedReleaseSpec&lt;SilaCancun&gt;(SilaShanghai.Instance)</c>.
/// </summary>
