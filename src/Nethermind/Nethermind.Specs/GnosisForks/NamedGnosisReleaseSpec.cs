...
/// <list type="bullet">
///   <item><c>mainnetFork</c> — the sila-mainnet fork whose SIP delta this Gnosis fork incorporates
///     (e.g. <see cref="SilaCancun"/> for <c>CancunGnosis</c>).</item>
...
///     <c>mainnetFork.Parent</c> (i.e. the sila-mainnet fork just before the divergence point).</item>
/// </list>
///
/// <para>
/// <see cref="Apply"/> calls <c>mainnetFork.Apply(spec)</c> to include the sila-mainnet delta.
...
///     → ShanghaiGnosis.Apply(base=SilaShanghai) → CancunGnosis.Apply(base=SilaCancun + blob overrides)
/// </code>
/// </para>
/// </summary>
...
{
    /// <summary>
    /// Applies the corresponding sila-mainnet fork's delta. Concrete Gnosis forks should call
...
/// <c>class CancunGnosis() : NamedGnosisReleaseSpec&lt;CancunGnosis&gt;(SilaCancun.Instance, ShanghaiGnosis.Instance)</c>.
/// </summary>
