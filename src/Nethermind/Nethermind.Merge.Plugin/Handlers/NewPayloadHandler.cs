...
/// <a href="https://github.com/sila-chain/execution-apis/blob/main/src/engine/shanghai.md#engine_newpayloadv2">
/// SilaShanghai</a> specification.
/// </summary>
public sealed class NewPayloadHandler : IAsyncHandler<ExecutionPayload, PayloadStatusV1>, IDisposable
{
        ...
        ILogManager logManager)
    {
        ...
        _defaultProcessingOptions = receiptConfig.StoreReceipts ? ProcessingOptions.SilaMerge | ProcessingOptions.StoreReceipts : ProcessingOptions.SilaMerge;
    ...
    /// 1. The block parent was already processed. Then we process with ProcessingOptions.SilaMerge with potentially also StoringReceipts.
    ...
    private bool ShouldProcessBlock(Block block, BlockHeader parent, out ProcessingOptions processingOptions)
    {
        ...
        if (!parentProcessed && processTerminalBlock) // so if parent wasn't processed
        {
            ...
            // this option is part of ProcessingOptions.SilaMerge option
