...
public sealed class SszMiddleware
{
    ...
    /// endpoints, per execution-apis#793 (e.g. <c>Sil-Execution-Version: cancun</c>).
    /// </summary>
    public const string ForkHeaderName = "Sil-Execution-Version";
    ...
    /// SSZ-REST spec capabilities response (see https://github.com/sila-chain/execution-apis/pull/793).
    /// </summary>
        ...
        out ReadOnlyMemory<char> pathSegment, out bool forkScoped)
    {
        ...
        switch (SszRestPaths.GetScoping(resource))
        {
            ...
            default:
                // Fork-scoped: fork (and thus version) is resolved later from the Sil-Execution-Version header.
                forkScoped = true;
                pathSegment = path.AsMemory(offset, pathLen - offset);
                return true;
        }
