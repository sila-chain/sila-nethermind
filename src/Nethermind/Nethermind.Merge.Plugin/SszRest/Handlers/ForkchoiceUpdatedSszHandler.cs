...
/// <c>engine_forkchoiceUpdatedV{N}</c> (the version is selected by the <c>Sil-Execution-Version</c>
    ...
    where TWire : struct, ISszCodec<TWire>
{
    ...
    /// the <c>Sil-Execution-Version</c> header, or <c>null</c> when the check doesn't apply (no
    ...
    private string? GetForkMismatchMessage(HttpContext ctx, ulong? timestamp)
    {
        ...
        return string.Equals(payloadForkSegment, requestedFork, StringComparison.OrdinalIgnoreCase)
            ? null
            : $"Sil-Execution-Version fork '{requestedFork}' does not match the fork for timestamp {timestamp.Value}";
    }
}
