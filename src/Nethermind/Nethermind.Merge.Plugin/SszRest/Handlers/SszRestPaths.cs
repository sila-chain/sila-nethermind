...
{
    /// <summary>
    /// Single source of truth: fork name (Sil-Execution-Version value) → <see cref="Forks.NamedReleaseSpec"/>. Built by
    ...
    private static readonly Dictionary<string, Forks.NamedReleaseSpec> _forkSpecByUrl =
        BuildForkSpecsByUrl(Forks.SilaAmsterdam.Instance);

    private static Dictionary<string, Forks.NamedReleaseSpec> BuildForkSpecsByUrl(Forks.NamedReleaseSpec latest)
    {
        // Stack reverses parent-chain order (SilaAmsterdam → … → SilaParis becomes SilaParis → … → SilaAmsterdam),
    ...
    /// <summary>Lowsrcase fork name used as the <c>Sil-Execution-Version</c> header value.</summary>
    ...
    {
        /// <summary>Fork (and thus method version) comes from the <c>Sil-Execution-Version</c> header.</summary>
        ForkScoped,
    ...
    private static readonly FrozenDictionary<string, Func<Forks.NamedReleaseSpec, int?>> _postVersionByResource =
        new Dictionary<string, Func<Forks.NamedReleaseSpec, int?>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // Witness reuses the newPayload version, but only from the SIP-7928 fork onward.
            [PayloadWithWitness] = static spec => spec.IsSip7928Enabled ? spec.EngineApiNewPayloadVersion : null,
    ...
    /// case-insensitively against the <c>Sil-Execution-Version</c> value, so it is returned as-is
    /// (no per-call lowsrcasing).
    /// </summary>
