...
public partial class FlashbotsModuleTests
{
    ...
    {
        using BaseEngineModuleTests.MergeTestBlockchain chain = await CreateBlockChain(releaseSpec: SilaCancun.Instance);
    ...
    private Block CreateBlock(EngineModuleTests.MergeTestBlockchain chain)
    {
        ...
        Transaction[] transactions = [
            Build.A.Transaction.WithShardBlobTxTypeAndFields(1, spec: SilaPrague.Instance).WithMaxFeePerGas(1.GWei).WithMaxPriorityFeePerGas(1).SignedAndResolved(TestItem.PrivateKeyA).TestObject,
            Build.A.Transaction.WithShardBlobTxTypeAndFields(2, spec: SilaOsaka.Instance).WithMaxFeePerGas(1.GWei).WithMaxPriorityFeePerGas(0).SignedAndResolved(TestItem.PrivateKeyB).TestObject,
            Build.A.Transaction
                .WithMaxFeePerGas(0)
        ...
        return new(
            ...
            {
                BlobGasUsed = 3 * Sip4844Constants.GasPerBlob,
                ExcessBlobGas = 0,
                BaseFeePerGas = 0,
                Bloom = Bloom.Empty,
                GasUsed = 0,
