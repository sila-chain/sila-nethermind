...
public class ConfigProvider_FindIncorrectSettings_Tests
{
    ...
    public void CorrectSettingNames_CaseInsensitive()
    {
        ...
        Dictionary<string, string> envVars = new() { { "NETHERMIND_NETWORKCONFIG_MAXCANDIDATEPESRCOUNT", "500" } };
    ...
    public void SettingWithTypos()
    {
        ...
        _env.GetEnvironmentVariables().Returns(new Dictionary<string, string>() {
            { "NETHERMIND_NETWORKCONFIG_MAXCANDIDATEPSRCOUNT", "500" }  // incorrect, should be NETHERMIND_NETWORKCONFIG_MAXCANDIDATEPESRCOUNT
        });
        EnvConfigSource? envSource = new(_env);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(Errors[2].Name, Is.EqualTo("MAXCANDIDATEPSRCOUNT"));
            Assert.That(ErrorMsg, Is.EqualTo($"ConfigType:JsonConfigFile|Category:DiscoveRyConfig|Name:Concurrenc{Environment.NewLine}ConfigType:JsonConfigFile|Category:BloomsConfig|Name:IndexLevelBucketSizes{Environment.NewLine}ConfigType:EnvironmentVariable(NETHERMIND_*)|Category:NETWORKCONFIG|Name:MAXCANDIDATEPSRCOUNT"));
        });
    }
    ...
    {
        _env.GetEnvironmentVariables().Returns(new Dictionary<string, string>() {
            { "NETHERMIND_NETWORKCONFIGMAXCANDIDATEPESRCOUNT", "500" }  // incorrect, should be NETHERMIND_NETWORKCONFIG_MAXCANDIDATEPESRCOUNT
        });
        EnvConfigSource? envSource = new(_env);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(Errors[0].Name, Is.EqualTo("NETWORKCONFIGMAXCANDIDATEPESRCOUNT"));
            Assert.That(ErrorMsg, Is.EqualTo($"ConfigType:EnvironmentVariable(NETHERMIND_*)|Category:|Name:NETWORKCONFIGMAXCANDIDATEPESRCOUNT"));
        });
    }
