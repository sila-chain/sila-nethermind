...
{
    private static readonly IReleaseSpec Spec = SilaAmsterdam.Instance;
    ...
    public void AccountExists_AccountCreatedByCurrentTxOnly_ReturnsFalseBeforeCurrentTx()
    {
        ...
        // must be false.
        byte[] currentTxCode = [.. Sip7702Constants.DelegationHeader, .. TestItem.AddressB.Bytes];
    ...
    {
        byte[] priorTxCode = [.. Sip7702Constants.DelegationHeader, .. TestItem.AddressB.Bytes];
        ReadOnlyAccountChanges ac = Build.An.AccountChanges
            .WithAddress(TestItem.AddressA)
            .WithNonceChanges(new NonceChange(23, 1))
