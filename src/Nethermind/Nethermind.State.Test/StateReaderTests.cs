...
namespace Nethermind.Store.Test
{
    ...
    public class StateReaderTests(bool useFlat)
    {
        ...
        {
            using Context ctx = new(useFlat);
            BlockHeader header = ctx.CommitAndCapture(state => state.CreateAccount(TestItem.AddressA, 1.Sila));

            verify(ctx.Reader, header);
        }

        private (Context ctx, IReleaseSpec releaseSpec, IDisposable scope) SetupContractSenderTest(
            bool sip3607Enabled, bool sip7702Enabled, byte[]? code = null)
        {
            IReleaseSpec releaseSpec = ReleaseSpecSubstitute.Create();
            releaseSpec.IsSip3607Enabled.Returns(sip3607Enabled);
            releaseSpec.IsSip7702Enabled.Returns(sip7702Enabled);
        ...
        public void IsInvalidContractSender_BasicCases(bool sip3607, bool sip7702, byte[]? code, bool delegated, bool expected)
        {
            byte[]? effectiveCode = delegated ? [.. Sip7702Constants.DelegationHeader, .. new byte[20]] : code;
            (Context ctx, IReleaseSpec releaseSpec, IDisposable scope) = SetupContractSenderTest(sip3607, sip7702, effectiveCode);
            using (ctx)
            using (scope)
            {
        ...
        {
            (Context ctx, IReleaseSpec releaseSpec, IDisposable scope) = SetupContractSenderTest(sip3607Enabled: true, sip7702Enabled: true, new byte[20]);
            using (ctx)
            using (scope)
            {
