...
public class StateProviderTests(bool useFlat)
{
    ...
    [Test]
    public void Sip_158_zero_value_transfer_deletes()
    {
    ...
    [Test]
    public void Sip_158_touch_zero_value_system_account_is_not_deleted()
    {
        using Context ctx = new(useFlat);
        ...
        ReleaseSpec releaseSpec = new() { IsSip158Enabled = true, Sip158IgnoredAccount = systemUser };
    ...
    public void Update_balance_on_non_existing_account_throws()
    {
        ...
        IWorldState provider = ctx.WorldState;
        using IDisposable _ = provider.BeginScope(IWorldState.PreGenesis);
        Assert.Throws<InvalidOperationException>(() => provider.AddToBalance(TestItem.AddressA, 1.Sila, Olympic.Instance));
    }
