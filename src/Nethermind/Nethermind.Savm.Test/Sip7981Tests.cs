...
/// <summary>
/// Tests for SIP-7981: access list token floor pricing.
/// </summary>
[TestFixture]
public class Sip7981Tests
{
    private static readonly IReleaseSpec Spec = new OverridableReleaseSpec(SilaAmsterdam.Instance) { IsSip7976Enabled = true, IsSip7981Enabled = true };
    ...
    public void Access_list_token_pricing(AccessList accessList, ulong expectedTokens)
    {
        ...
        ulong expectedStandard = GasCostOf.TransactionSip2780
            + Sip8038Constants.ColdAccountAccess
            + (ulong)addressCount * Sip8038Constants.AccessListAddressCost
            + (ulong)storageKeyCount * Sip8038Constants.AccessListStorageKeyCost
            + FloorPerToken * expectedTokens;
        ulong expectedFloor = GasCostOf.TransactionSip2780 + FloorPerToken * expectedTokens;

        Assert.That(cost, Is.EqualTo(new SilaIntrinsicGas(Standard: expectedStandard, FloorGas: expectedFloor)));
    }
    ...
    public void Null_access_list_contributes_zero_tokens()
    {
        ...
        SilaIntrinsicGas cost = IntrinsicGasCalculator.Calculate(transaction, Spec);
        Assert.That(cost, Is.EqualTo(new SilaIntrinsicGas(
            Standard: GasCostOf.TransactionSip2780 + Sip8038Constants.ColdAccountAccess,
            FloorGas: GasCostOf.TransactionSip2780)));
    }

    [Test]
    public void Token_floor_not_applied_before_sip7981()
    {
        AccessList accessList = new AccessList.Builder().AddAddress(Address.Zero).Build();
        Transaction transaction = new() { To = Address.Zero, AccessList = accessList };
        SilaIntrinsicGas cost = IntrinsicGasCalculator.Calculate(transaction, SilaPrague.Instance);
        Assert.That(cost, Is.EqualTo(new SilaIntrinsicGas(
