...
{
    [TestFixture]
    public class Sip1014Tests : VirtualMachineTestsBase
    {
        ...
        private static readonly byte[] _defaultInitCode = Prepare.SavmCode.ForInitOf(_defaultDeployedCode).Done;

        private void AssertSip1014(Address address, byte[] code) => AssertCodeHash(address, Keccak.Compute(code));
        ...
        {
            byte[] createCode = Prepare.SavmCode.Create2(initCode, salt, 0).Done;
            ...
            byte[] callCode = Prepare.SavmCode.Call(TestItem.AddressC, callGas).Done;
            return (expectedAddress, callCode);
        }

        [Test]
        public void TestHive() =>
            Execute(Prepare.SavmCode
        ...
        public void Test()
        {
            ...
            Execute(callCode);
            AssertSip1014(expectedAddress, _defaultDeployedCode);
        }
        ...
        public void Test_out_of_gas_existing_account(bool withStorage)
        {
            ...
            TestState.CreateAccount(expectedAddress, 1.Sila);
            ...
            Assert.That(account.Balance, Is.EqualTo(1.Sila));
            AssertSip1014(expectedAddress, []);
        }
        ...
        public void Test_collision_with_7702_delegated_account_is_rejected()
        {
            ...
            Assert.That(Sip7702Constants.IsDelegatedCode(TestState.GetCode(expectedAddress)), Is.True, "original delegation should remain");
        }
        ...
        public void Examples_from_sip_spec_are_executed_correctly(string addressHex, string saltHex, string initCodeHex, long gas, string resultHex)
        {
            (_, byte[] callCode) = PrepareCreate2(Bytes.FromHexString(saltHex), Bytes.FromHexString(initCodeHex));
            ExecuteAndTrace(callCode);
            AssertSip1014(new(resultHex), []);
        }
    }
}
