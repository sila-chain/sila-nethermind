...
/// <summary>
/// End-to-end regression tests for SIP-8037 state-gas spill-refund accounting.
/// </summary>
/// <remarks>
...
/// </remarks>
[TestFixture]
public class Sip8037GasAccountingTests : VirtualMachineTestsBase
{
    ...
    private static byte[] SstoreSetThenRevert() =>
        Prepare.SavmCode.PushData(1).PushData(0).Op(Instruction.SSTORE).Revert(0, 0).Done;
    ...
    public static IEnumerable<Scenario> Scenarios()
    {
        ...
        yield return new Scenario(
            ...
            1_000_000,
            Prepare.SavmCode.PushData(1).PushData(0).Op(Instruction.SSTORE).Op(Instruction.STOP).Done,
            [],
        ...
        // Tx gas above the SIP-7825 cap seeds the reservoir (R0 > 0): the SSTORE state charge
        ...
        {
            byte[] pCode = Prepare.SavmCode
                .PushData(1).PushData(0).Op(Instruction.SSTORE)
                .Op(Instruction.INVALID).Done;
            byte[] aCode = Prepare.SavmCode
        ...
        {
            byte[] aMain = Prepare.SavmCode
                .PushData(1).PushData(0).Op(Instruction.SSTORE)
                .Call(M, 600_000).Op(Instruction.POP)
                .Op(Instruction.STOP).Done;
            byte[] aClear = Prepare.SavmCode
                .PushData(0).PushData(0).Op(Instruction.SSTORE)
                .Op(Instruction.STOP).Done;
            byte[] mCode = Prepare.SavmCode
        ...
        // gross spill merge docked the parent reservoir, ovsrcharging the sender.
        {
            byte[] pCode = Prepare.SavmCode
        ...
        // a same-frame NEW_ACCOUNT refund (soft-failed value call), returns into P, P halts.
        {
            ...
            byte[] aHaltCode = Prepare.SavmCode
                .Call(P, 1_200_000).Op(Instruction.POP)
                .Op(Instruction.INVALID).Done;
            yield return new Scenario(
        ...
        {
            byte[] childInitCode = Prepare.SavmCode.Op(Instruction.STOP).Done;
        ...
        // CREATE deposit fails on invalid code (SIP-3541): the child create frame halts and
    ...
    public void Sip8037_gas_accounting_matches_expected_split(Scenario scenario)
    {
