...
public class Sip8037RegressionTests : VirtualMachineTestsBase
{
    ...
    private static Prepare BuildCreateFactory(byte[] initCode, UInt256 value, bool create2, byte[]? salt = null) =>
        create2
            ? Prepare.SavmCode.Create2(initCode, salt ?? DefaultCreate2Salt, value)
            : Prepare.SavmCode.Create(initCode, value);

    [Test]
    public void Sip8037_rejects_tx_when_calldata_floor_exceeds_tx_max_regular_gas()
    {
        ...
        Transaction transaction = Build.A.Transaction
            ...
            .WithData(calldata)
            .To(Recipient)
            .SignedAndResolved(new SilaEcdsa(SpecProvider.ChainId), SenderKey)
            .TestObject;
        ...
        IntrinsicGas<SilaGasPolicy> intrinsicGas = SilaGasPolicy.CalculateIntrinsicGas(transaction, Spec);
        Assert.That(intrinsicGas.FloorGas.Value, Is.GreaterThan(Sip7825Constants.DefaultTxGasLimitCap));
    ...
    [Test]
    public void Sip8037_soft_failed_call_refunds_spilled_new_account_state_gas_to_gas_left()
    {
        byte[] code = Prepare.SavmCode
            .CallWithValue(TestItem.AddressF, long.MaxValue, 101.Sila)
            .Op(Instruction.POP)
    ...
    /// </summary>
    [TestCase(false, TestName = "Sip8037_nested_create_code_deposit_must_not_borrow_parent_regular_gas_CREATE")]
    [TestCase(true, TestName = "Sip8037_nested_create_code_deposit_must_not_borrow_parent_regular_gas_CREATE2")]
    public void Sip8037_nested_create_code_deposit_must_not_borrow_parent_regular_gas(bool create2)
    {
    ...
    [TestCase(false, TestName = "Sip8037_nested_create_code_deposit_failure_must_refund_parent_create_state_CREATE")]
    [TestCase(true, TestName = "Sip8037_nested_create_code_deposit_failure_must_refund_parent_create_state_CREATE2")]
    public void Sip8037_nested_create_code_deposit_failure_must_refund_parent_create_state(bool create2)
    {
        byte[] childInitCode = Prepare.SavmCode
            .PushData(33_000)
            .PushData(0)
            .Op(Instruction.RETURN)
            .Done;
    ...
    /// </summary>
    [Test]
    public void Sip8037_code_deposit_failure_must_keep_initcode_state_gas_in_state_dimension()
    {
        // Initcode:
        //   1) CALL a dead account with value=1 - charges NewAccountState
        //   2) RETURN 33,000 bytes - exceeds MaxCodeSizeSip7954 (32,768), triggers deposit failure
        byte[] initCode = Prepare.SavmCode
        ...
        TransactionProcessor<SilaGasPolicy> parallelProcessor = new(
    ...
    [Test]
    public void Sip8037_oversized_initcode_create_must_keep_create_state_out_of_block_state_gas()
    {
        byte[] createFromCalldataCode = Prepare.SavmCode
            .CALLDATASIZE()
            .CALLDATACOPY(0, 0)
            .CALLDATASIZE()
            .CREATEx(1, 0, 0, null)
        ...
        ulong gasLimit = Sip7825Constants.DefaultTxGasLimitCap + GasCostOf.CreateState;
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(tracer.StatusCode, Is.EqualTo(StatusCode.Failure));
            Assert.That(tracer.GasConsumedResult.SpentGas, Is.EqualTo(Sip7825Constants.DefaultTxGasLimitCap));
            Assert.That(tracer.GasConsumedResult.BlockStateGas, Is.Zero);
            Assert.That(tracer.GasConsumedResult.EffectiveBlockGas, Is.EqualTo(Sip7825Constants.DefaultTxGasLimitCap));
        }
    }

    [Test]
    public void Sip8037_top_level_create_collision_must_still_pay_calldata_floor()
    {
        byte[] initCode = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
    ...
    [Test]
    public void Sip8037_authorization_refund_excludes_existing_authority_from_block_state_gas()
    {
    ...
    [TestCase(false, 410_397L, TestName = "Sip8037_nested_create_collision_refunds_state_gas_and_burns_regular_gas_CREATE")]
    [TestCase(true, 410_397L, TestName = "Sip8037_nested_create_collision_refunds_state_gas_and_burns_regular_gas_CREATE2")]
    public void Sip8037_nested_create_collision_refunds_state_gas_and_burns_regular_gas(bool create2, long expectedBlockGas)
    {
        byte[] initCode = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
        ...
        Prepare codeBuilder = create2
            ? Prepare.SavmCode.Create2(initCode, salt, UInt256.Zero)
            : Prepare.SavmCode.Create(initCode, UInt256.Zero);
    ...
    [TestCase(false, TestName = "Sip8037_top_level_create_revert_must_refund_inner_create_state_gas")]
    [TestCase(true, TestName = "Sip8037_top_level_create_halt_must_refund_inner_create_state_gas")]
    public void Sip8037_top_level_create_failure_must_refund_inner_create_state_gas(bool exceptionalHalt)
    {
        byte[] childInitCode = Prepare.SavmCode
            .ForInitOf(Prepare.SavmCode.Op(Instruction.STOP).Done)
            .Done;

        Prepare initCodeBuilder = Prepare.SavmCode
    ...
    [Test]
    public void Sip8037_top_level_halt_burns_reverted_inner_create_state_gas()
    {
        byte[] childInitCode = Prepare.SavmCode
            .Create([], UInt256.One)
            .Op(Instruction.POP)
    ...
    [Test]
    public void Sip8037_top_level_halt_must_refund_successful_inner_create_spill()
    {
        byte[] childInitCode = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
    ...
    [
        new TestCaseData(
            ...
            0,
            0xc7510UL)
            .SetName("Sip8037_block_957_failed_create_must_not_double_refund_top_level_create_state_gas"),
        new TestCaseData(
            ...
            .SetName("Sip8037_block_1194_failed_create_must_exclude_top_level_create_state_gas"),
        new TestCaseData(
            ...
            0xf8cc,
            0xc7510UL)
            .SetName("Sip8037_block_2367_failed_create_must_preserve_inner_reservoir_and_refund_top_level_create_state_gas"),
    ];

    [TestCaseSource(nameof(FailedCreateRegressionCases))]
    public void Sip8037_failed_create_must_exclude_top_level_create_state_gas(
        string scenario,
        string initCodeHex,
        ulong gasLimit,
        int value,
    ...
    [Test]
    public void Sip8037_block_15149_failed_create_must_not_keep_inner_create_refund_in_state_dimension()
    {
    ...
    [Test]
    public void Sip8037_create_memory_oog_must_not_charge_create_state_gas()
    {
        byte[] code = Prepare.SavmCode
            .PushData(16_777_216)
            .PushData(0)
    ...
    [Test]
    public void Sip8037_create_state_refund_helpers_are_disabled_before_amsterdam()
    {
        byte[] childInitCode = Prepare.SavmCode
            .PushData(33_000)
            .PushData(0)
    ...
    [TestCase(false, TestName = "Sip8037_create_in_static_context_must_not_charge_state_gas_or_increment_nonce_CREATE")]
    [TestCase(true, TestName = "Sip8037_create_in_static_context_must_not_charge_state_gas_or_increment_nonce_CREATE2")]
    public void Sip8037_create_in_static_context_must_not_charge_state_gas_or_increment_nonce(bool create2)
    {
        Address createdAddress = SetupStaticCreateAttempt(create2);

        byte[] outerCode = Prepare.SavmCode
    ...
    [TestCase(false, TestName = "Sip8037_static_create_followed_by_parent_sstore_must_not_leak_create_state_gas_CREATE")]
    [TestCase(true, TestName = "Sip8037_static_create_followed_by_parent_sstore_must_not_leak_create_state_gas_CREATE2")]
    public void Sip8037_static_create_followed_by_parent_sstore_must_not_leak_create_state_gas(bool create2)
    {
        Address createdAddress = SetupStaticCreateAttempt(create2);
    ...
    [Test]
    public void Sip8037_delegatecall_sstore_restoration_refund_credits_local_reservoir()
    {
        byte[] childCode = Prepare.SavmCode
            .PushData(0)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .PushData(0)
        ...
        byte[] parentCode = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .PushData(1)
    ...
    [Test]
    public void Sip8037_reverted_ancestor_discards_descendant_storage_refund_credit()
    {
        byte[] descendantCode = Prepare.SavmCode
            .PushData(0)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .Op(Instruction.STOP)
        ...
        byte[] intermediateCode = Prepare.SavmCode
            .DelegateCall(TestItem.AddressC, 400_000)
            .Op(Instruction.POP)
            .Revert(0, 0)
            .Done;
    ...
    private Address SetupStaticCreateAttempt(bool create2)
    {
        ...
        byte[] createAttemptCode = create2
            ? Prepare.SavmCode.Create2(childInitCode, [0x01], UInt256.Zero).Op(Instruction.STOP).Done
            : Prepare.SavmCode.Create(childInitCode, UInt256.Zero).Op(Instruction.STOP).Done;
    ...
    /// </summary>
    [Test]
    public void Sip8037_top_level_exceptional_halt_burns_spilled_state_gas()
    {
        Prepare codeBuilder = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
            .Op(Instruction.SSTORE);
        ...
        Assert.That(tracer.Error, Is.EqualTo(nameof(SavmExceptionType.OutOfGas)));
    ...
    /// </summary>
    [Test]
    public void Sip8037_top_level_revert_refunds_full_spilled_state_gas()
    {
        byte[] code = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
    ...
    /// </summary>
    [Test]
    public void Sip8037_top_level_exceptional_halt_burns_spilled_child_state_gas()
    {
        byte[] code = Prepare.SavmCode
    ...
    [Test]
    public void Sip8037_exceptional_halt_must_restore_child_inline_state_refund()
    {
        byte[] childCode = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
        ...
        ulong gasLimit = Sip7825Constants.DefaultTxGasLimitCap + GasCostOf.SSetState;
        TestAllTracerWithOutput tracer = Execute(Activation, gasLimit, code, blockGasLimit: DynamicStatePricingBlockGasLimit);
    ...
    /// </summary>
    [Test]
    public void Sip8037_reservoir_restored_after_child_spill_and_halt()
    {
    ...
    [Test]
    public void Sip8037_block_validation_must_not_use_header_max_gas_used_as_remaining_tx_budget()
    {
        byte[] stateHeavyCode = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .Op(Instruction.STOP)
    ...
    [Test]
    public void Sip8037_block_validation_must_allow_tx_gas_limit_above_remaining_regular_budget_when_2d_budget_fits()
    {
        byte[] stopCode = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
        ...
        Assert.That(secondResult, Is.EqualTo(TransactionResult.Ok),
            "SilaAmsterdam should admit the tx because its capped regular contribution and state contribution both fit.");
        Assert.That(secondTracer.GasConsumedResult.BlockStateGas, Is.EqualTo(GasCostOf.SSetState));
        Assert.That(block.Header.GasUsed, Is.LessThanOrEqualTo(block.Header.GasLimit));
    }

    [TestCase(false, TestName = "Sip8037_selfdestruct_to_new_beneficiary_without_balance_charges_no_state_gas")]
    [TestCase(true, TestName = "Sip8037_selfdestruct_to_new_beneficiary_with_balance_charges_new_account_state_gas")]
    public void Sip8037_selfdestruct_to_new_beneficiary_charges_state_gas_only_for_balance_transfer(bool fundContract)
    {
    ...
    [TestCase(false, TestName = "Sip8037_selfdestruct_to_new_beneficiary_charges_state_gas_when_cold")]
    [TestCase(true, TestName = "Sip8037_selfdestruct_to_new_beneficiary_charges_state_gas_when_warm")]
    public void Sip8037_selfdestruct_to_new_beneficiary_state_gas_does_not_depend_on_warmth(bool warmBeneficiary)
    {
        Address beneficiary = TestItem.AddressC;
        Prepare codeBuilder = Prepare.SavmCode;
        if (warmBeneficiary)
        ...
        Assert.That(TestState.GetBalance(beneficiary), Is.EqualTo(100.Sila + UInt256.One));
    }

    [TestCase(false, 0, GasCostOf.CreateState, TestName = "Sip8037_same_tx_created_selfdestruct_to_new_beneficiary_with_zero_balance_CREATE")]
    [TestCase(false, 100, GasCostOf.CreateState + GasCostOf.NewAccountState, TestName = "Sip8037_same_tx_created_selfdestruct_to_new_beneficiary_with_balance_CREATE")]
    [TestCase(true, 0, GasCostOf.CreateState, TestName = "Sip8037_same_tx_created_selfdestruct_to_new_beneficiary_with_zero_balance_CREATE2")]
    [TestCase(true, 100, GasCostOf.CreateState + GasCostOf.NewAccountState, TestName = "Sip8037_same_tx_created_selfdestruct_to_new_beneficiary_with_balance_CREATE2")]
    public void Sip8037_same_tx_created_selfdestruct_to_new_beneficiary_keeps_created_account_state_gas(
        bool create2,
        int createdBalance,
    ...
    [Test]
    public void Sip8037_selfdestruct_to_same_tx_created_beneficiary_must_not_charge_new_account_state_again()
    {
        byte[] beneficiaryRuntime = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
        byte[] beneficiaryInitCode = Prepare.SavmCode
            .ForInitOf(beneficiaryRuntime)
            .Done;
        Address beneficiary = ContractAddress.From(Recipient, 0);
        byte[] victimInitCode = Prepare.SavmCode
            .SELFDESTRUCT(beneficiary)
            .Done;
    ...
    [TestCase(false, TestName = "Sip8037_same_tx_selfdestruct_must_not_refund_created_storage_state_gas_CREATE")]
    [TestCase(true, TestName = "Sip8037_same_tx_selfdestruct_must_not_refund_created_storage_state_gas_CREATE2")]
    public void Sip8037_same_tx_selfdestruct_must_not_refund_created_storage_state_gas(bool create2)
    {
        byte[] childInitCode = Prepare.SavmCode
            .SSTORE(0, new byte[] { 1 })
    ...
    [TestCase(false, TestName = "Sip8037_same_tx_selfdestruct_must_not_refund_code_deposit_state_gas_CREATE")]
    [TestCase(true, TestName = "Sip8037_same_tx_selfdestruct_must_not_refund_code_deposit_state_gas_CREATE2")]
    public void Sip8037_same_tx_selfdestruct_must_not_refund_code_deposit_state_gas(bool create2)
    {
        byte[] selfDestructRuntime = Prepare.SavmCode
            .Op(Instruction.ADDRESS)
            .Op(Instruction.SELFDESTRUCT)
            .Done;
        byte[] childInitCode = Prepare.SavmCode
    ...
    [Test]
    public void Sip8037_same_tx_selfdestruct_must_not_refund_multiple_created_accounts()
    {
        byte[] childInitCode = Prepare.SavmCode
            .Op(Instruction.ADDRESS)
            .Op(Instruction.SELFDESTRUCT)
    ...
    [Test]
    public void Sip8037_top_level_create_selfdestruct_must_keep_intrinsic_create_state_gas_in_block_state_gas()
    {
        byte[] initCode = Prepare.SavmCode
            .Op(Instruction.ADDRESS)
            .Op(Instruction.SELFDESTRUCT)
    ...
    [TestCase(0UL, SelfDestructBeneficiaryKind.Self, GasCostOf.CreateState, TestName = "Sip8037_create_tx_selfdestruct_to_self_keeps_create_state_gas")]
    [TestCase(100UL, SelfDestructBeneficiaryKind.Self, GasCostOf.CreateState, TestName = "Sip8037_create_tx_selfdestruct_to_self_with_value_keeps_create_state_gas")]
    [TestCase(100UL, SelfDestructBeneficiaryKind.Existing, GasCostOf.CreateState, TestName = "Sip8037_create_tx_selfdestruct_to_existing_with_value_keeps_create_state_gas")]
    [TestCase(100UL, SelfDestructBeneficiaryKind.Nonexistent, GasCostOf.CreateState + GasCostOf.NewAccountState, TestName = "Sip8037_create_tx_selfdestruct_to_nonexistent_with_value_keeps_create_and_beneficiary_state_gas")]
    public void Sip8037_create_tx_selfdestruct_initcode_keeps_create_state_gas(
        ulong txValue,
        ...
        long expectedStateGas)
    {
        ...
        TestState.CreateAccount(Sender, 100.Sila);
    ...
    [TestCase(16_777_216ul, TestName = "Sip8037_subcall_set_clear_revert_pays_no_state_gas_spill")]
    [TestCase(16_875_136ul, TestName = "Sip8037_subcall_set_clear_revert_pays_no_state_gas_reservoir")]
    public void Sip8037_subcall_set_clear_revert_pays_no_state_gas(ulong gasLimit)
    {
    ...
    /// </summary>
    [Test]
    public void Sip8037_failed_child_sstore_must_not_inflate_parent_state_reservoir()
    {
        byte[] childCode = Prepare.SavmCode
            .PushData(1)
            .PushData(0)
