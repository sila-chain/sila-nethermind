...
namespace Nethermind.Savm.Test.CodeAnalysis;
...
// Mutates process-wide StreamInterpreter statics, so it must not run alongside other SAVM tests.
...
public class StreamInterpreterDifferentialTests : VirtualMachineTestsBase
{
    ...
    private static readonly byte[] ArithmeticChain = Prepare.SavmCode
    ...
    private static readonly byte[] JumpLoop = Prepare.SavmCode
        .PushData(5)                                  // counter
    ...
    private static readonly byte[] StoreAndReturn = Prepare.SavmCode
        .PushData(42).PushData(0).Op(Instruction.MSTORE)
        .PushData(32).PushData(0).Op(Instruction.RETURN)
        .Done;

    private static readonly byte[] StackUnderflow = Prepare.SavmCode
        .Op(Instruction.ADD)
        .Done;

    private static readonly byte[] NestedCallWithResume = Prepare.SavmCode
    ...
    private static readonly byte[] CreateWithInitCode = Prepare.SavmCode
        .Create(Prepare.SavmCode
    ...
    private static readonly byte[] BitwiseFusionWithConstant = Prepare.SavmCode
        .PushData(0xFF)
        .PushData(0x0F)
        .Op(Instruction.AND)
        .PushData(0)
    ...
    // sila-mainnet stream divergence (CREATE child contract, then CALL into it).
    ...
    private static readonly byte[] SolidityExampleCall = Prepare.SavmCode
    ...
    private static readonly byte[] CalleeCode = Prepare.SavmCode
    ...
    public void StreamInterpreter_OutOfGas_MatchesByteCodeLoop(byte[] code, ulong gasLimit)
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(baseline.StatusCode, Is.EqualTo((byte)Savm.StatusCode.Failure), "precondition: the bytecode loop must run out of gas");
    ...
    private sealed class ReceiptCaptureTracer : Savm.Tracing.TxTracer
    {
