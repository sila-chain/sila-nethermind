...
public class SilaGasPolicyTests
{
    ...
    public void ConsumeDataCopyGas_charges_base_access_plus_per_word_copy(bool isExternalCode, ulong words)
    {
        ...
        SilaGasPolicy gas = SilaGasPolicy.FromULong(initial);
        SilaGasPolicy.ConsumeDataCopyGas(ref gas, SilaCancun.Instance, isExternalCode, words);

        ulong baseCost = isExternalCode ? SilaCancun.Instance.GasCosts.ExtCodeCost : GasCostOf.VeryLow;
        ulong expected = baseCost + GasCostOf.Memory * words;
        Assert.That(initial - SilaGasPolicy.GetRemainingGas(in gas), Is.EqualTo(expected));
    }
    ...
    public void Default_gas_policy_implementations_are_aggressively_inlined()
    {
        ...
        foreach (MethodInfo method in typeof(IGasPolicy<>).GetMethods(BindingFlags.Public | BindingFlags.Static))
        {
            ...
            Assert.That(
                ...
                "charges compile to real calls in no-dynamic-PGO regimes (e.g. the NativeAOT zkSAVM guest).");
        }
