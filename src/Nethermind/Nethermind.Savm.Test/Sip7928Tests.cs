...
/// <summary>
/// Tests for SIP-7928 Block Access Lists.
/// Verifies that executing SAVM code correctly records state accesses into a
/// <see cref="BlockAccessListAtIndex"/> via <see cref="TracedAccessWorldState"/>.
/// </summary>
[TestFixture(false)]
[TestFixture(true)]
public class Sip7928Tests(bool parallel) : VirtualMachineTestsBase
{
    protected override ulong BlockNumber => MainnetSpecProvider.ParisBlockNumber;
    protected override ulong Timestamp => MainnetSpecProvider.AmsterdamBlockTimestamp;

    private static readonly SilaEcdsa _ecdsa = new(0);
    private static readonly UInt256 _accountBalance = 10.Sila;
    private static readonly UInt256 _testAccountBalance = 1.Sila;
    ...
    private static readonly byte[] _delegatedCode = Prepare.SavmCode
    ...
    {
        yield return new TestCaseData(SilaShanghai.Instance, 0)
            .SetName("SIP7928_pre_SIP6780_selfdestruct_to_sender_zero_balance");
        yield return new TestCaseData(SilaShanghai.Instance, 100)
            .SetName("SIP7928_pre_SIP6780_selfdestruct_to_sender_nonzero_balance");
        yield return new TestCaseData(SilaAmsterdam.Instance, 0)
            .SetName("SIP7928_SIP6780_selfdestruct_to_sender_zero_balance");
        yield return new TestCaseData(SilaAmsterdam.Instance, 100)
            .SetName("SIP7928_SIP6780_selfdestruct_to_sender_nonzero_balance");
    }

    private (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor) CreateTracedProcessor(
        ...
        bool wrapPrecompileCache = false)
    {
        ...
        SilaCodeInfoRepository baseRepo = new(tracedState);
        ICodeInfoRepository codeInfoRepo = wrapPrecompileCache
            ? new PrecompileCachedCodeInfoRepository(tracedState, new SilaPrecompileProvider(), baseRepo, precompileCaches: null)
            : baseRepo;
        SilaVirtualMachine machine = new(blockhashProvider, SpecProvider, logManager);
        TransactionProcessor<SilaGasPolicy> processor = new(
    ...
    private static Transaction BuildContractTx(byte[] code, ulong executionGas, UInt256 value, BlockHeader header)
    {
        Transaction templateTx = Build.A.Transaction
            ...
            .WithValue(value)
            .TestObject;
        ulong intrinsicGas = IntrinsicGasCalculator.Calculate(templateTx, SilaAmsterdam.Instance, header.GasLimit).MinimalGas;
    ...
    private (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor, Block block) SetupPrecompileBalScenario(
        Address? delegationTarget = null)
    {
        InitWorldState(TestState, delegationTarget: delegationTarget);
        (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor) =
            CreateTracedProcessor(wrapPrecompileCache: true);
        Block block = Build.A.Block.TestObject;
        processor.SetBlockExecutionContext(new BlockExecutionContext(block.Header, SilaAmsterdam.Instance));
    ...
    {
        SilaEcdsa ecdsa = new(SpecProvider.ChainId);
    ...
    private static byte[] BuildDelegationCode(Address target) =>
        [.. Sip7702Constants.DelegationHeader, .. target.Bytes];
    ...
    private static byte[] BuildCallOpcodeResultStorageWriteCode(Instruction callOpcode, Address target, UInt256 slot) =>
        callOpcode switch
        {
            Instruction.CALL => Prepare.SavmCode.Call(target, 50_000).PushData(slot).Op(Instruction.SSTORE).Done,
            Instruction.STATICCALL => Prepare.SavmCode.StaticCall(target, 50_000).PushData(slot).Op(Instruction.SSTORE).Done,
            Instruction.DELEGATECALL => Prepare.SavmCode.DelegateCall(target, 50_000).PushData(slot).Op(Instruction.SSTORE).Done,
            Instruction.CALLCODE => Prepare.SavmCode.CallCode(target, 50_000).PushData(slot).Op(Instruction.SSTORE).Done,
    ...
    private static byte[] BuildCreateThenPopCode(Instruction createOpcode, byte[] initCode, byte[] salt, UInt256 value) =>
        (createOpcode == Instruction.CREATE2
            ? Prepare.SavmCode.Create2(initCode, salt, value)
            : Prepare.SavmCode.Create(initCode, value))
        .Op(Instruction.POP)
        .Op(Instruction.STOP)
        .Done;
    ...
    {
        byte[] returnFlagCode = Prepare.SavmCode
            .PushData(0)
            .Op(Instruction.SLOAD)
            .PushData(0)
        ...
        byte[] prefix = Prepare.SavmCode
            .Op(Instruction.CALLDATASIZE)
            .Op(Instruction.ISZERO)
            .PushData(createJumpDestination)
            .Op(Instruction.JUMPI)
            .Done;
        byte[] loadFlagCode = Prepare.SavmCode
            .PushData(0)
            .Op(Instruction.SLOAD)
            .Done;
        byte[] createAndSetFlagCode = Prepare.SavmCode
        ...
        byte[] branchCode = Prepare.SavmCode
            .PushData(createOnlyJumpDestination)
            .Op(Instruction.JUMPI)
            .Done;
        byte[] createOnlyCode = Prepare.SavmCode
            .ForCreate2Of(initCode)
            .Op(Instruction.POP)
            .Op(Instruction.STOP)
            .Done;
    ...
    private BlockAccessListAtIndex ExecuteSetCodeCall(bool wrapPrecompileCache, params AuthorizationTuple[] authorizationList)
    {
        ...
        processor.SetBlockExecutionContext(new BlockExecutionContext(header, SilaAmsterdam.Instance));
    ...
    {
        (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor) = CreateTracedProcessor();
        BlockHeader header = Build.A.BlockHeader
            .WithGasLimit(120_000_000)
            .WithBaseFee(1)
            .TestObject;
        ...
        bool revert)
    {
        ...
        Transaction templateTx = Build.A.Transaction
            ...
            .WithValue(value)
            .TestObject;
        ulong gasLimit = IntrinsicGasCalculator.Calculate(templateTx, SilaAmsterdam.Instance, block.Header.GasLimit).MinimalGas + _gasLimit;
        ...
        // With SIP-8037's higher CostPerStateByte, some CREATE/SELFDESTRUCT cases now run out
    ...
    public async Task Constructs_BAL_when_processing_code_exception(
        ...
        byte[]? extraCode,
        ulong executionGas,
        SavmExceptionType expectedException)
    {
        InitWorldState(TestState, extraCode);
        ...
        Transaction templateTx = Build.A.Transaction
            ...
            .TestObject;
        ulong intrinsicGas = IntrinsicGasCalculator.Calculate(templateTx, SilaAmsterdam.Instance, block.Header.GasLimit).MinimalGas;
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(res.SavmExceptionType, Is.EqualTo(expectedException));
            AssertEqual(accountChangesA, bal.GetAccountChanges(TestItem.AddressA));
            AssertEqual(accountChangesZero, bal.GetAccountChanges(Address.Zero));
            Assert.That(bal.AccountCount, Is.EqualTo(expected.Count() + 2));
        }
    ...
    public void Delegated_precompile_target_is_recorded_in_BAL_under_PrecompileCachedCodeInfoRepository()
    {
        ...
        (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor, Block block) =
            SetupPrecompileBalScenario(delegationTarget: precompileAddress);

        byte[] code = Prepare.SavmCode.Call(_callTargetAddress, 50_000).Done;
    ...
    /// <summary>
    /// SIP-7702: delegation to a precompile must NOT execute the precompile (FastCall returns 1).
    /// </summary>
    /// <remarks>
    /// Inner gas is 0 to discriminate: precompile execution OOGs and pushes 0, FastCall pushes 1 regardless of forwarded gas.
    /// </remarks>
    [Test]
    public void Calling_account_delegated_to_precompile_uses_FastCall_per_SIP_7702()
    {
        Address precompileAddress = Sha256Precompile.Address;
        (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor, Block block) =
            SetupPrecompileBalScenario(delegationTarget: precompileAddress);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(change, Is.Not.Null,
                "SIP-7702 FastCall must succeed and propagate via SSTORE; missing slot 0 entry indicates the call failed.");
            Assert.That(change!.Value.Value, Is.EqualTo(UInt256.One.ToBigEndianWord()),
                "SIP-7702: delegation to a precompile must NOT execute the precompile - FastCall returns 1 regardless of forwarded gas.");
        }
    }

    /// <summary>
    /// SIP-7928: DELEGATECALL to a precompile records the precompile (codeSource) in BAL.
    /// </summary>
    /// <remarks>
    ...
    public void DelegateCall_to_precompile_records_codeSource_in_BAL_under_PrecompileCachedCodeInfoRepository()
    {
        ...
        byte[] code = Prepare.SavmCode.DelegateCall(precompileAddress, 50_000).Done;
        Transaction tx = BuildContractTx(code, _gasLimit, _testAccountBalance, block.Header);
    ...
    /// <summary>
    /// SIP-7928: top-level transaction with <c>tx.to == precompile_address</c> records the recipient in BAL.
    /// </summary>
    /// <remarks>
    /// <see cref="TransactionProcessor"/>'s <c>BuildExecutionEnvironment</c> only calls <c>accessTracker.WarmUp</c> (SIP-2929)
    ...
    {
        Address precompileAddress = Sha256Precompile.Address;
        (TracedAccessWorldState tracedState, TransactionProcessor<SilaGasPolicy> processor, _) =
            SetupPrecompileBalScenario();
    ...
    private static IEnumerable<TestCaseData> PreValidationRejectionCases()
    {
        yield return new TestCaseData(
            ...
            .SetName("SIP7702_authorization_max_nonce_keeps_authority_out_of_BAL_account_nonce_zero");
        yield return new TestCaseData(
                ulong.MaxValue,
                (Func<ulong, Address, AuthorizationTuple>)((chainId, authority) =>
                    new(chainId, _delegationTargetAddress, ulong.MaxValue, 0, UInt256.One, UInt256.One, authority)))
            .SetName("SIP7702_authorization_max_nonce_keeps_authority_out_of_BAL_account_nonce_max");
        yield return new TestCaseData(
                0ul,
                (Func<ulong, Address, AuthorizationTuple>)((chainId, authority) =>
                    new(chainId, _delegationTargetAddress, 0, 0, UInt256.One, SecP256k1Curve.HalfNPlusOne, authority)))
            .SetName("SIP7702_authorization_high_s_keeps_authority_out_of_BAL");
    }

    [TestCaseSource(nameof(PreValidationRejectionCases))]
    public void Sip7702_authorization_pre_validation_rejection_does_not_record_authority_or_target(
    ...
    [Test]
    public void Sip7702_authorization_existing_code_rejection_records_authority_only()
    {
        byte[] entryCode = BuildStorageWriteCode(UInt256.Zero, UInt256.One);
    ...
    [TestCase(true, TestName = "SIP7702_authorization_valid_nonce_records_signer_target_and_entry_change")]
    [TestCase(false, TestName = "SIP7702_authorization_invalid_nonce_records_signer_read_and_noop_sstore_read")]
    public void Sip7702_authorization_nonce_validity_records_bal(bool validAuthorizationNonce)
    {
        Address authority = TestItem.AddressB;
    ...
    [Test]
    public void Sip7702_null_address_delegation_to_empty_code_records_nonce_without_code_change()
    {
    ...
    [Test]
    public void Sip7702_multi_hop_delegation_resolves_one_hop_in_bal()
    {
        Address authority = TestItem.AddressB;
    ...
    [Test]
    public void Sip7702_self_delegation_resolves_one_hop_to_designator_code()
    {
        Address authority = TestItem.AddressB;
        byte[] entryCode = BuildCallResultStorageWriteCode(authority, UInt256.Zero);
    ...
    [Test]
    public void Sip7702_same_tx_delegation_chain_resolves_one_hop_in_bal()
    {
        Address authority = TestItem.AddressB;
        Address intermediate = TestItem.AddressE;
        Address finalTarget = TestItem.AddressF;
    ...
    [TestCase(Instruction.CALL, TestName = "SIP7702_set_code_to_precompile_records_BAL_for_CALL")]
    [TestCase(Instruction.STATICCALL, TestName = "SIP7702_set_code_to_precompile_records_BAL_for_STATICCALL")]
    [TestCase(Instruction.DELEGATECALL, TestName = "SIP7702_set_code_to_precompile_records_BAL_for_DELEGATECALL")]
    [TestCase(Instruction.CALLCODE, TestName = "SIP7702_set_code_to_precompile_records_BAL_for_CALLCODE")]
    public void Sip7702_set_code_to_precompile_records_bal_for_call_opcode(Instruction callOpcode)
    {
        Address authority = TestItem.AddressB;
    ...
    [TestCase(Instruction.CREATE, Instruction.SLOAD, TestName = "SIP7928_create_sload_selfdestruct_records_storage_read")]
    [TestCase(Instruction.CREATE, Instruction.SSTORE, TestName = "SIP7928_create_sstore_selfdestruct_records_storage_read")]
    [TestCase(Instruction.CREATE2, Instruction.SLOAD, TestName = "SIP7928_create2_sload_selfdestruct_records_storage_read")]
    [TestCase(Instruction.CREATE2, Instruction.SSTORE, TestName = "SIP7928_create2_sstore_selfdestruct_records_storage_read")]
    public void Sip7928_same_tx_created_selfdestruct_records_storage_as_read(
    ...
    [TestCase(0, TestName = "SIP7928_create2_selfdestruct_then_recreate_same_block_zero_balance")]
    [TestCase(100, TestName = "SIP7928_create2_selfdestruct_then_recreate_same_block_nonzero_balance")]
    public void Sip7928_create2_selfdestruct_then_recreate_same_block_records_fresh_changes(int firstCreateBalance)
    {
        Address beneficiary = TestItem.AddressB;
    ...
    public void Sip7928_selfdestruct_to_sender_coalesces_sender_changes(IReleaseSpec spec, int victimBalance)
    {
        byte[] selfdestructCode = Prepare.SavmCode
    ...
    [Test]
    public void Sip7928_extcodehash_records_boundary_account_reads()
    {
        Address emptyAccount = TestItem.AddressB;
        byte[] selfdestructInitCode = Prepare.SavmCode
        ...
        byte[] factoryCode = Prepare.SavmCode
    ...
    [Test]
    public void Sip7928_failed_create_transaction_converts_constructor_storage_writes_to_reads()
    {
        ...
        UInt256 callTargetBalance = default)
    {
        ...
        worldState.CreateAccount(Sip2935Constants.BlockHashHistoryAddress, 0, Sip2935TestConstants.Nonce);
        worldState.InsertCode(Sip2935Constants.BlockHashHistoryAddress, Sip2935TestConstants.CodeHash, Sip2935TestConstants.Code, SpecProvider.GenesisSpec);

        worldState.CreateAccount(Sip4788Constants.BeaconRootsAddress, 0, Sip4788TestConstants.Nonce);
        worldState.InsertCode(Sip4788Constants.BeaconRootsAddress, Sip4788TestConstants.CodeHash, Sip4788TestConstants.Code, SpecProvider.GenesisSpec);

        worldState.CreateAccount(Sip7002Constants.WithdrawalRequestPredeployAddress, 0, Sip7002TestConstants.Nonce);
        worldState.InsertCode(Sip7002Constants.WithdrawalRequestPredeployAddress, Sip7002TestConstants.CodeHash, Sip7002TestConstants.Code, SpecProvider.GenesisSpec);

        worldState.CreateAccount(Sip7251Constants.ConsolidationRequestPredeployAddress, 0, Sip7251TestConstants.Nonce);
        worldState.InsertCode(Sip7251Constants.ConsolidationRequestPredeployAddress, Sip7251TestConstants.CodeHash, Sip7251TestConstants.Code, SpecProvider.GenesisSpec);
        ...
        else
        {
            ...
            byte[] delegationCode = [.. Sip7702Constants.DelegationHeader, .. target.Bytes];
    ...
    /// <summary>
    /// When an outer CALL into an SIP-7702-delegated EOA OOGs at the cold-access gas charge
    ...
    public void Call_into_7702_delegated_eoa_oog_at_delegation_cold_access_does_not_record_delegation_target()
    {
        ...
        ulong executionGas = pushOperandsCost + GasCostOf.Call + Sip8038Constants.ColdAccountAccess + GasCostOf.WarmStateRead - 1;
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(bal.GetAccountChanges(_callTargetAddress), Is.Not.Null,
                "SIP-7702 delegated EOA must be recorded as the CALL target");
            Assert.That(bal.GetAccountChanges(_delegationTargetAddress), Is.Null,
                "SIP-7702 delegation target must not be recorded when CALL OOGs before its code is loaded");
        }
    }

    [TestCase(120_000_000ul, 30_000_000ul, true, TestName = "SIP2935_system_call_records_storage_change_when_state_gas_affordable")]
    [TestCase(120_000_000ul, 30_000ul, false, TestName = "SIP2935_system_call_records_no_storage_access_when_state_gas_not_affordable")]
    public void Sip2935_system_call_bal_respects_sip8037_state_gas(ulong blockGasLimit, ulong systemCallGasLimit, bool shouldStoreParentHash)
    {
        InitWorldState(TestState);
        ...
        SystemCall systemCall = new()
        {
            ...
            To = Sip2935Constants.BlockHashHistoryAddress,
            Value = 0ul,
        };
        ...
        AccountChangesAtIndex? accountChanges = tracedState.GetGeneratingBlockAccessList()!.GetAccountChanges(Sip2935Constants.BlockHashHistoryAddress);
        Assert.That(accountChanges, Is.Not.Null);
        ...
        {
            // Under SIP-8037, an unaffordable state-gas attempt records neither a storage
            // change nor a read — see upstream's "Fix SIP-8037 reverted state gas accounting".
            using (Assert.EnterMultipleScope())
            {
                Assert.That(accountChanges!.StorageChangeCount, Is.EqualTo(0));
                Assert.That(accountChanges.StorageReads, Is.Empty);
    ...
    private static IEnumerable<TestCaseData> CodeTestSource
    {
        get
        {
            ...
            code = Prepare.SavmCode
                ...
                .Op(Instruction.SELFDESTRUCT)
                .Done;
            // Under SIP-8037's higher state-gas cost, the contract-creation tx runs out of
            ...
            code = Prepare.SavmCode
                ...
                .Create(
                    Prepare.SavmCode
                        .ForInitOf(Prepare.SavmCode.Op(Instruction.STOP).Done)
                        .Done,
                    0)
                .Op(Instruction.POP)
            ...
            byte[] revertToPreviousCode = Prepare.SavmCode
                .PushData(0)
                .PushData(slot)
                .Op(Instruction.SSTORE)
                .PushData(0)
            ...
            byte[] callTargetCode = Prepare.SavmCode
                .PushData(slot)
                .Op(Instruction.SLOAD)
                .Done;
            code = Prepare.SavmCode
            ...
            byte[] returnValueCode = Prepare.SavmCode
                .PushData(0)
                .PushData(0)
                .PushData(0)
                .PushData(0)
            ...
            byte[] createdRuntimeCode = Prepare.SavmCode
                .Op(Instruction.STOP)
                .Done;
            byte[] createInitCode = Prepare.SavmCode
                .PushData(slot)
                .Op(Instruction.SLOAD)
            ...
            code = Prepare.SavmCode
                ...
                .Done;
            // Under SIP-8037's higher state-gas cost the nested CREATE runs out of state gas;
    ...
    private static IEnumerable<TestCaseData> ExceptionTestSource
    {
        get
        {
            ...
            code = Prepare.SavmCode
                ...
                .Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.ColdAccountAccess - 1, SavmExceptionType.OutOfGas)
            ...
            code = Prepare.SavmCode
                ...
                .Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.SelfBalance, SavmExceptionType.OutOfGas)
            ...
            byte[] callTargetCode = Prepare.SavmCode.Op(Instruction.STOP).Done;
            code = Prepare.SavmCode
                .Call(_callTargetAddress, 0)
                .PushData(0)
                .PushData(0)
            ...
            code = Prepare.SavmCode
                ...
                .Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.ColdSLoad - 1, SavmExceptionType.OutOfGas)
            ...
            yield return new TestCaseData(
                ...
                Sip8038Constants.ColdStorageAccess + GasCostOf.VeryLow + GasCostOf.Memory - 1,
                SavmExceptionType.OutOfGas)
            ...
            yield return new TestCaseData(
                ...
                code,
                null,
                GasCostOf.SelfDestructSip150 + GasCostOf.ColdAccountAccess + GasCostOf.VeryLow - 1,
                SavmExceptionType.OutOfGas)
            ...
            yield return new TestCaseData(
                ...
                GasCostOf.SelfDestructSip150 + Sip8038Constants.ColdAccountAccess + GasCostOf.VeryLow,
                SavmExceptionType.OutOfGas)
            { TestName = "selfdestruct_oog_post_state_access" };

            code = Prepare.SavmCode.Op(Instruction.BALANCE).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "balance_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.EXTCODESIZE).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "extcodesize_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.EXTCODEHASH).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "extcodehash_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.EXTCODECOPY).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "extcodecopy_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.SLOAD).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "sload_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.SSTORE).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.CallStipend + 1, SavmExceptionType.StackUnderflow)
            { TestName = "sstore_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.CALL).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "call_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.CALLCODE).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "callcode_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.DELEGATECALL).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "delegatecall_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.STATICCALL).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "staticcall_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.CREATE).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "create_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.CREATE2).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.VeryLow, SavmExceptionType.StackUnderflow)
            { TestName = "create2_stack_underflow" };

            code = Prepare.SavmCode.Op(Instruction.SELFDESTRUCT).Done;
            changes = [testAccount];
            yield return new TestCaseData(changes, code, null, GasCostOf.SelfDestructSip150, SavmExceptionType.StackUnderflow)
    ...
    public void CodeInfoRepository_getcachedcodeinfo_records_account_read_in_bal(string address)
    {
        ...
        CodeInfoRepository repo = new(tracedState, new SilaPrecompileProvider());

        repo.GetCachedCodeInfo(new(address), false, SilaAmsterdam.Instance, out Address? delegationAddress);
    ...
    {
        (_, TransactionProcessor<SilaGasPolicy> processor) = CreateTracedProcessor(parallelOverride: true);

        TestState.CreateAccount(TestItem.AddressA, 10.Sila);
        TestState.Commit(SpecProvider.GenesisSpec);
    ...
    public void CodeInfoRepository_getcachedcodeinfo_delegated_records_account_read_in_bal()
    {
        ...
        byte[] delegationCode = [.. Sip7702Constants.DelegationHeader, .. delegationTarget.Bytes];
        ...
        CodeInfoRepository repo = new(tracedState, new SilaPrecompileProvider());
        CodeInfo result = repo.GetCachedCodeInfo(delegatedAccount, true, SilaAmsterdam.Instance, out Address? delegationAddress);
    ...
    public void CacheCodeInfoRepository_reads_prior_code_change_from_bal_world_state()
    {
        ...
        CacheCodeInfoRepository repo = new(balWorldState, new SilaPrecompileProvider(), StaticCodeCache.Instance);
        CodeInfo result = repo.GetCachedCodeInfo(TestItem.AddressA, false, SilaAmsterdam.Instance, out Address? delegationAddress);
    ...
    public void CacheCodeInfoRepository_tracing_records_account_read_in_bal()
    {
        ...
        CacheCodeInfoRepository repo = new(tracedState, new SilaPrecompileProvider(), StaticCodeCache.Instance);
        CodeInfo result = repo.GetCachedCodeInfo(TestItem.AddressB, false, SilaAmsterdam.Instance, out Address? delegationAddress);
