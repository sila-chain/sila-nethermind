...
{
    [TestFixture]
    public class Sip145Tests : VirtualMachineTestsBase
    {
        ...
        private void AssertSip145(TestAllTracerWithOutput receipt, string result) => AssertSip145(receipt, Bytes.FromHexString(result));

        private void AssertSip145(TestAllTracerWithOutput receipt, ReadOnlySpan<byte> result)
        {
            AssertStorage(0, result);
            AssertGas(receipt, result.IsZero() ? ZeroResultGas : NonZeroResultGas);
        }

        private const ulong ZeroResultGas = GasCostOf.Transaction + 4 * GasCostOf.VeryLow + GasCostOf.SStoreNetMeteredSip1283;
