...
public class ParityLikeTxTracerTests : VirtualMachineTestsBase
{
    ...
    {
        byte[] deployedCode = Prepare.SavmCode
            .Call(IdentityPrecompile.Address, 50000)
            .CallWithValue(IdentityPrecompile.Address, 50000, 1.Sila)
            .Op(Instruction.STOP)
            .Done;
    ...
    public void Can_trace_balance_changes()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(trace.StateChanges.ContainsKey(Sender), Is.True, "sender");
            Assert.That(trace.StateChanges.ContainsKey(Miner), Is.True, "miner");
            Assert.That(trace.StateChanges[Sender].Balance.Before, Is.EqualTo(100.Sila), "sender before");
            Assert.That(trace.StateChanges[Sender].Balance.After, Is.EqualTo(100.Sila - 21001), "sender after");
            Assert.That(trace.StateChanges[Recipient].Balance.Before, Is.EqualTo(100.Sila), "recipient before");
            Assert.That(trace.StateChanges[Recipient].Balance.After, Is.EqualTo(100.Sila + 1), "recipient after");
