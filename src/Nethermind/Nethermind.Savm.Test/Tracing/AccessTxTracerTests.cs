    ...
    public class AccessTxTracerTests : VirtualMachineTestsBase
    {
        ...
        public void Reverted_call_target_address_is_still_captured_in_access_list()
        {
            ...
            Assert.That(
                ...
                "addresses accessed inside reverted frames must survive for sil_createAccessList");
        }
        ...
        public void Reverted_sub_frame_sload_storage_key_is_still_captured_in_access_list()
        {
            ...
            byte[] addressCCode = Prepare.SavmCode
                .PushData(7)
            ...
            byte[] recipientCode = Prepare.SavmCode
                .Call(TestItem.AddressC, 50000)
                .Op(Instruction.STOP)
                .Done;
        ...
        public void Outer_committed_and_inner_reverted_call_both_captured_in_access_list()
        {
            ...
            TestState.InsertCode(TestItem.AddressE, Prepare.SavmCode.Op(Instruction.STOP).Done, SpecProvider.GenesisSpec);
            ...
            // Recipient code: CALL AddressC (succeeds at SAVM level but AddressC reverts internally)
            byte[] recipientCode = Prepare.SavmCode
                .Call(TestItem.AddressC, 50000)
                .Op(Instruction.STOP)
                .Done;
