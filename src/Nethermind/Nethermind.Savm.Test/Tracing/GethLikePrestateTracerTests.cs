...
{
    private static readonly JsonSerializerOptions SerializerOptions = SilaJsonSerializer.JsonOptionsIndented;
    ...
    {
        TestState.CreateAccount(Address.Zero, 100.Sila);
    ...
    public void Test_PrestateTrace_NestedCalls(string? config, string expectedTrace)
    {
        ...
        byte[] createCode = Prepare.SavmCode
            ...
            .Op(Instruction.STOP)
            .Done;
        byte[] nestedCode = Prepare.SavmCode
    ...
    {
        TestState.CreateAccount(Address.Zero, 100.Sila);
        TestState.CreateAccount(TestItem.AddressC, 5.Sila);
    ...
    private static byte[] Balance => Prepare.SavmCode
        .PushData(TestItem.AddressC.ToString(false, false).PadLeft(64, '0'))
        .Op(Instruction.BALANCE)
        .Op(Instruction.STOP)
        .Done;

    private static byte[] SelfDestruct => Prepare.SavmCode
        .PushData(TestItem.AddressC.ToString(false, false).PadLeft(64, '0'))
