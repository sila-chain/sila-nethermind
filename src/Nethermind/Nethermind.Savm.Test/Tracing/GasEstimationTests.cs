...
namespace Nethermind.Savm.Test.Tracing
{
    ...
    public class GasEstimationTests(bool useCreates)
    {
        ...
        public void Handles_well_errors()
        {
            ...
            if (_executionType.IsAnyCreate())
            {
                testEnvironment.tracer.ReportActionError(SavmExceptionType.Other);
        ...
        public void Handles_well_revert()
        {
            ...
            testEnvironment.tracer.ReportActionError(SavmExceptionType.Revert, 96000000ul);
            testEnvironment.tracer.ReportActionError(SavmExceptionType.Revert, 98000000ul);
            testEnvironment.tracer.ReportActionError(SavmExceptionType.Revert, 99000000ul);
        ...
        public void Handles_well_precompile_out_of_gas()
        {
            ...
            Action reportError = () => testEnvironment.tracer.ReportActionError(SavmExceptionType.OutOfGas);
        ...
        public void Estimate_uses_next_block_spec_for_execution_across_fork_boundary()
        {
            ...
            Assert.That(captured!.Value.Spec.IsSip4844Enabled, Is.True);
        }
        ...
        public void Should_return_zero_when_out_of_gas_detected_during_estimation()
        {
            ...
            testEnvironment.tracer.ReportActionError(SavmExceptionType.OutOfGas);
        ...
        public void Should_return_zero_with_insufficient_balance_error_when_sender_is_address_zero_with_value_transfer()
        {
            ...
            Transaction tx = Build.A.Transaction
                ...
                .WithValue(1.Sila) // Value transfer with zero balance
                .TestObject;
            Block block = Build.A.Block.WithNumber(1).WithTransactions(tx).TestObject;
        ...
        public void Should_return_zero_with_out_of_gas_error_when_address_zero_runs_out_of_gas()
        {
            ...
            tracer.ReportActionError(SavmExceptionType.OutOfGas);
        ...
        {
            // Reproduces https://github.com/sila-chain/sila-nethermind/issues/10552
            ...
            byte[] reverterCode = Prepare.SavmCode
                .PushData(0x00)
                .PushData(0x00)
            ...
            byte[] callerCode = Prepare.SavmCode
        ...
        public void Should_estimate_gas_for_create2_with_setup_call_pattern()
        {
            ...
            byte[] proxyRuntimeCode = Prepare.SavmCode
                .PushData(0x42)    // value
                .PushData(0x00)    // key
        ...
        public void Should_estimate_gas_with_multiple_inner_calls_mixed_reverts()
        {
            ...
            byte[] succeederCode = Prepare.SavmCode
                .PushData(0x01)
                .PushData(0x00)
                .Op(Instruction.SSTORE)
                .Op(Instruction.STOP)
        ...
        public void Should_estimate_gas_when_inner_call_runs_out_of_gas_but_caller_handles_it()
        {
            ...
            // See: https://github.com/sila-chain/go-sila/blob/master/sil/gasestimator/gasestimator.go
            using TestEnvironment testEnvironment = new();

            // Contract that consumes all gas via infinite loop (will always OOG)
            Address gasGuzzlerAddress = TestItem.AddressB;
            byte[] gasGuzzlerCode = Prepare.SavmCode
            ...
            byte[] middleCode = Prepare.SavmCode
        ...
        public void Should_estimate_gas_with_delegatecall_that_reverts_internally()
        {
            ...
            byte[] implCode = Prepare.SavmCode
                .Revert(0, 0)
                .Done;
            ...
            byte[] proxyCode = Prepare.SavmCode
        ...
        public void Estimate_subcall_contract_uses_optimistic_multiplier_not_margin_multiplier()
        {
            ...
            byte[] innerCode = Prepare.SavmCode
                .PushData(0x01)
                .PushData(0x00)
                .Op(Instruction.SSTORE)
                .Op(Instruction.STOP)
        ...
        private class TestEnvironment : IDisposable
        {
            ...
            public ISilaEcdsa _silaEcdsa;
            public SilaTransactionProcessor _transactionProcessor;
