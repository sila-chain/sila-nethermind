...
public class GethLikeTxMemoryTracerTests : VirtualMachineTestsBase
{
    ...
    {
        // From https://github.com/sila-chain/sila-nethermind/issues/5717
    ...
    {
        byte[] calleeCode = Prepare.SavmCode
            .PersistData("0x0", HexZero)
            .PushData(0)
            .PushData(0)
    ...
    {
        GethLikeTxTrace trace = ExecuteAndTrace(code);
        using JsonDocument inMemoryDoc = JsonDocument.Parse(new SilaJsonSerializer().Serialize(trace));
