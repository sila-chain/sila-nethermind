...
public class DebugTracerTests : VirtualMachineTestsBase
{
    ...
    private static unsafe Span<byte> AlignedStack(byte[] pinnedArray)
    {
        ...
        int offset = (int)((uint)(-(nint)addr) & (SavmStack.WordSize - 1u));
    ...
    public void Debugger_Can_Alter_Data_Stack(string bytecodeHex)
    {
        ...
        while (vmThread.IsAlive)
        {
            if (tracer.CanReadState)
            {
                ...
                SavmStack stack = new(tracer.CurrentState.DataStackHead, tracer, ref MemoryMarshal.GetReference(AlignedStack(tracer.CurrentState.DataStack)), default);
                if (!stack.PopLimbo()) throw new SavmStackUnderflowException();
    ...
    public void Use_Debug_Tracer_To_Check_Assertion_Live(string bytecodeHex)
    {
        ...
        while (vmThread.IsAlive)
        {
            ...
            {
                // we alter the value stored in memory to force EQ check at the end to fail
                if (gasAvailable_pre_MSTORE is null) gasAvailable_pre_MSTORE = SilaGasPolicy.GetRemainingGas(tracer.CurrentState.Gas);
                else
                {
                    ulong gasAvailable_post_MSTORE = SilaGasPolicy.GetRemainingGas(tracer.CurrentState.Gas);
