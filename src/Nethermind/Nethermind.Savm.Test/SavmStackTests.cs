...
public class SavmStackTests
{
    ...
    private const string PushByte = nameof(SavmStack.PushByte);
    private const string PushOne = nameof(SavmStack.PushOne);
    private const string PushZero = nameof(SavmStack.PushZero);
    private const string PushUInt32 = nameof(SavmStack.PushUInt32);
    private const string PushUInt64 = nameof(SavmStack.PushUInt64);
    private const string PushUInt256 = nameof(SavmStack.PushUInt256);
    private const string PushBytes = nameof(SavmStack.PushBytes);
    private const string Dup = nameof(SavmStack.Dup);
    private const string Swap = nameof(SavmStack.Swap);
    private const string Exchange = nameof(SavmStack.Exchange);
    ...
    private const string PopLimbo = nameof(SavmStack.PopLimbo);
    ...
    {
        using VmState<SilaGasPolicy> vmState = CreateSavmState();
        vmState.InitializeStacks(default, out SavmStack stack);
        ...
        SavmExceptionType result = InvokePush(op, ref stack);

        Assert.That(result, Is.EqualTo(SavmExceptionType.StackOverflow));
        Assert.That(stack.Head, Is.EqualTo(SavmStack.MaxStackSize - 1));
    }
    ...
    public void StackReshuffle_with_insufficient_depth_returns_StackUnderflow_and_preserves_head(string op)
    {
        ...
        SavmExceptionType result = op switch
        {
        ...
        Assert.That(result, Is.EqualTo(SavmExceptionType.StackUnderflow));
    ...
    {
        // SAVM spec: truncated PUSH{n} (where code ends before n bytes of immediate) must push
        ...
        SavmExceptionType result = stack.PushBothPaddedBytes<OffFlag>(
            ref MemoryMarshal.GetArrayDataReference(immediate),
            used,
            pushSize: 32);

        Assert.That(result, Is.EqualTo(SavmExceptionType.None));
    ...
    public void PushUInt256_then_PopUInt256_roundtrip()
    {
        ...
        Assert.That(stack.PushUInt256<OffFlag>(in value), Is.EqualTo(SavmExceptionType.None));
    ...
    public void Three_pushes_then_three_out_pop_returns_top_first()
    {
        ...
        Assert.That(stack.PushUInt256<OffFlag>(in x), Is.EqualTo(SavmExceptionType.None));
        Assert.That(stack.PushUInt256<OffFlag>(in y), Is.EqualTo(SavmExceptionType.None));
        Assert.That(stack.PushUInt256<OffFlag>(in z), Is.EqualTo(SavmExceptionType.None));
    ...
    public void PushNBytes_encodes_big_endian_low_padded_high_zero(int n)
    {
        ...
        SavmExceptionType result = InvokePushNBytes(n, ref stack, ref MemoryMarshal.GetArrayDataReference(immediate));
    ...
    private static SavmExceptionType InvokePushNBytes(int n, ref SavmStack stack, ref byte imm) => n switch
    {
    ...
    private static SavmExceptionType InvokePush(string op, ref SavmStack stack) => op switch
    {
    ...
    private static SavmExceptionType PushUInt256Value(ref SavmStack stack)
    {
        UInt256 value = new(1, 2, 3, 4);
        return stack.PushUInt256<OffFlag>(in value);
    }

    private static bool InvokePopBool(string op, ref SavmStack stack) => op switch
    {
    ...
    private static VmState<SilaGasPolicy> CreateSavmState() =>
        VmState<SilaGasPolicy>.RentTopLevel(
            SilaGasPolicy.FromULong(10_000UL),
            ExecutionType.CALL,
