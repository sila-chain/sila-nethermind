...
namespace Nethermind.Savm.Test
{
    ...
    public class IntrinsicGasCalculatorTests
    {
        ...
        {
            SilaIntrinsicGas gas = IntrinsicGasCalculator.Calculate(testCase.Tx, Berlin.Instance);
            Assert.That(gas, Is.EqualTo(new SilaIntrinsicGas(Standard: testCase.Cost, FloorGas: 0)));
        }
        ...
        public void Intrinsic_cost_is_calculated_properly((List<object> orderQueue, ulong Cost) testCase)
        {
            ...
            void Test(IReleaseSpec spec, bool supportsAccessLists)
            {
                ...
                {
                    SilaIntrinsicGas gas = IntrinsicGasCalculator.Calculate(tx, spec);
                    Assert.That(gas, Is.EqualTo(new SilaIntrinsicGas(Standard: 21000 + testCase.Cost, FloorGas: 0)), spec.Name);
                }
            }
        ...
        public void Intrinsic_cost_of_data_is_calculated_properly((byte[] Data, ulong OldCost, ulong NewCost, ulong FloorCost) testCase)
        {
            ...
            void Test(IReleaseSpec spec, GasOptions options)
            {
                ...
                Assert.That(gas, Is.EqualTo(new SilaIntrinsicGas(
            ...
            Test(SilaShanghai.Instance, GasOptions.AfterRepricing);
            Test(SilaCancun.Instance, GasOptions.AfterRepricing);
            Test(SilaPrague.Instance, GasOptions.AfterRepricing | GasOptions.FloorCostEnabled);
        }
        ...
        public void Calculate_TxHasAuthorizationList_ReturnsExpectedCostOfTx((AuthorizationTuple[] AuthorizationList, ulong ExpectedCost) testCase)
        {
            ...
            SilaIntrinsicGas gas = IntrinsicGasCalculator.Calculate(tx, SilaPrague.Instance);
        ...
        public void Calculate_TxHasAuthorizationListBeforePrague_ThrowsInvalidDataException()
        {
            ...
            Assert.That(() => IntrinsicGasCalculator.Calculate(tx, SilaCancun.Instance), Throws.InstanceOf<InvalidDataException>());
        }

        [Test]
        public void Sip8037_policy_intrinsic_gas_splits_authorization_cost()
        {
            Transaction tx = Build.A.Transaction.SignedAndResolved()
                .WithAuthorizationCode(new AuthorizationTuple(1, TestItem.AddressF, 0, 0, UInt256.One, UInt256.One))
                .TestObject;
            IntrinsicGas<SilaGasPolicy> intrinsicGas = SilaGasPolicy.CalculateIntrinsicGas(tx, SilaAmsterdam.Instance);
            ...
            Assert.That(intrinsicGas.Standard.Value, Is.EqualTo(GasCostOf.TransactionSip2780 + recipientRegular + Sip8038Constants.PerAuthBaseRegular));
            Assert.That(intrinsicGas.Standard.StateReservoir, Is.EqualTo(GasCostOf.NewAccountState + GasCostOf.PerAuthBaseState));
        }

        [Test]
        public void Sip8037_nongeneric_intrinsic_gas_includes_state_gas_for_create()
        {
            Transaction tx = Build.A.Transaction.SignedAndResolved()
                .WithCode(Array.Empty<byte>())
                .TestObject;
            SilaIntrinsicGas gas = IntrinsicGasCalculator.Calculate(tx, SilaAmsterdam.Instance);

            // Create regular = CREATE_ACCESS + TRANSFER_LOG (value endowment); state = NEW_ACCOUNT.
            ulong expectedRegular = GasCostOf.TransactionSip2780 + Sip8038Constants.CreateAccess + GasCostOf.TransferLogSip2780;
        ...
        [Test]
        public void Sip8037_nongeneric_intrinsic_gas_includes_state_gas_for_setcode()
        {
            Transaction tx = Build.A.Transaction.SignedAndResolved()
                .WithAuthorizationCode(new AuthorizationTuple(1, TestItem.AddressF, 0, 0, UInt256.One, UInt256.One))
                .TestObject;
            ...
            ulong recipientRegular = Sip8038Constants.ColdAccountAccess + GasCostOf.TransferLogSip2780 + GasCostOf.TxValueCostSip2780;
            ulong expectedRegular = GasCostOf.TransactionSip2780 + recipientRegular + Sip8038Constants.PerAuthBaseRegular;
            ulong expectedState = GasCostOf.NewAccountState + GasCostOf.PerAuthBaseState;
            Assert.That(gas.Standard, Is.EqualTo(expectedRegular + expectedState));
        }

        [Test]
        public void Sip8037_nongeneric_minimal_gas_is_at_least_regular_plus_state()
        {
            ...
            ulong regularPlusState = GasCostOf.TransactionSip2780 + Sip8038Constants.CreateAccess + GasCostOf.TransferLogSip2780 + GasCostOf.CreateState;
            Assert.That(gas.MinimalGas, Is.GreaterThanOrEqualTo(regularPlusState));
        }

        // SIP-2780 fixed-cost vectors: the intrinsic is state-independent, so the recipient
        // touch and value-move costs are flat for every non-self recipient.
        private const ulong TxBaseSip2780 = GasCostOf.TransactionSip2780;
        private const ulong TransferLogSip2780 = GasCostOf.TransferLogSip2780;
        private const ulong ColdAccess = Sip8038Constants.ColdAccountAccess;
        private const ulong TxValueCost = GasCostOf.TxValueCostSip2780;

        [TestCase(false, 1ul, TxBaseSip2780 + ColdAccess + TxValueCost + TransferLogSip2780, TestName = "Sip2780_intrinsic_value_transfer_21000")]
        [TestCase(true, 1ul, TxBaseSip2780, TestName = "Sip2780_intrinsic_self_transfer_12000")]
        [TestCase(false, 0ul, TxBaseSip2780 + ColdAccess, TestName = "Sip2780_intrinsic_no_transfer_15000")]
        [TestCase(true, 0ul, TxBaseSip2780, TestName = "Sip2780_intrinsic_self_no_transfer_12000")]
        public void Sip2780_intrinsic_gas_is_calculated_properly(bool selfTransfer, ulong value, ulong expectedStandard)
        {
            OverridableReleaseSpec spec = new(SilaPrague.Instance) { IsSip2780Enabled = true, IsSip7708Enabled = true };
            ...
            Assert.That(gas.MinimalGas, Is.EqualTo(Math.Max(expectedStandard, TxBaseSip2780)));
        }

        [Test]
        public void Sip2780_access_list_does_not_discount_the_flat_recipient_touch()
        {
            OverridableReleaseSpec spec = new(SilaPrague.Instance) { IsSip2780Enabled = true, IsSip7708Enabled = true };
            ...
            Assert.That(actual, Is.EqualTo(TxBaseSip2780 + GasCostOf.AccessAccountListEntry + ColdAccess + TxValueCost + TransferLogSip2780));
        }

        [Test]
        public void Sip2780_intrinsic_gas_for_create_charges_transfer_log_only_when_value_positive()
        {
            OverridableReleaseSpec spec = new(SilaPrague.Instance) { IsSip2780Enabled = true, IsSip7708Enabled = true };
            ...
            Assert.That(IntrinsicGasCalculator.Calculate(createZero, spec).Standard,
                Is.EqualTo(TxBaseSip2780 + GasCostOf.TxCreate));
            ...
            Assert.That(IntrinsicGasCalculator.Calculate(createValue, spec).Standard,
                Is.EqualTo(TxBaseSip2780 + GasCostOf.TxCreate + TransferLogSip2780));
        }

        [Test]
        public void Sip2780_reduces_the_calldata_floor_base()
        {
            // Without reducing the floor base, the legacy 21,000 floor would dominate and negate the SIP.
            OverridableReleaseSpec spec = new(SilaPrague.Instance) { IsSip2780Enabled = true, IsSip7708Enabled = true };
            Transaction tx = Build.A.Transaction.WithData([1]).SignedAndResolved().TestObject;

            SilaIntrinsicGas gas = IntrinsicGasCalculator.Calculate(tx, spec);

            // Same per-token floor as SilaPrague (DataTestCaseSource maps [1] to 21,040), rebased to 4,500.
            Assert.That(gas.FloorGas, Is.EqualTo(TxBaseSip2780 + (21040 - GasCostOf.Transaction)));
        }

        [Test]
        public void Sip2780_disabled_keeps_legacy_intrinsic_base()
        {
        ...
        [Test]
        public void Calculate_Sip7623FloorWithoutSip2028_UsesFixedTokenCost()
        {
            ReleaseSpec spec = new()
            {
                IsSip2Enabled = true,
                IsSip7623Enabled = true,
                IsSip2028Enabled = false,
            };
            ...
            ulong expectedFloor = GasCostOf.Transaction
                + GasCostOf.TotalCostFloorPerTokenSip7623 * (zeros + nonZeros * GasCostOf.TxDataNonZeroMultiplierSip2028);
        ...
        public void IntrinsicGasMemo_AtGivenKey_HitsOnlyOnSpecMatch(bool sameSpec, bool sameLimit, bool expectHit)
        {
            ...
            IntrinsicGas<SilaGasPolicy> first = SilaGasPolicy.CalculateIntrinsicGas(tx, SilaPrague.Instance, 30_000_000);
            ...
            IReleaseSpec spec = sameSpec ? SilaPrague.Instance : SilaCancun.Instance;
            ulong limit = sameLimit ? 30_000_000UL : 60_000_000UL;
            IntrinsicGas<SilaGasPolicy> second = SilaGasPolicy.CalculateIntrinsicGas(tx, spec, limit);

            Transaction fresh = Build.A.Transaction.WithData(new byte[1024]).SignedAndResolved().TestObject;
            IntrinsicGas<SilaGasPolicy> expected = SilaGasPolicy.CalculateIntrinsicGas(fresh, spec, limit);
        ...
        public void IntrinsicGasMemo_WhenTheTransactionChanges_IsInvalidated(bool viaPreHash)
        {
            ...
            SilaGasPolicy.CalculateIntrinsicGas(tx, SilaPrague.Instance, 30_000_000);
