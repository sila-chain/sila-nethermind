...
// Tests target Sip8037BlockGasInclusionCheck.Validate directly.
[TestFixture]
public class Sip8037BlockGasInclusionCheckTests
{
    private const ulong CostPerStateByte = 1530;
    private const ulong GasNewAccount = 120; // SIP-8037 GAS_NEW_ACCOUNT
    ...
    [TestCase(0UL, Sip8037BlockGasInclusionCheck.Outcome.Ok, TestName = "Boundary_state_exact_fit_accepts")]
    [TestCase(1UL, Sip8037BlockGasInclusionCheck.Outcome.StateDimensionExceeded, TestName = "Boundary_state_exceeded_by_one_rejects_on_state_dimension")]
    public void Boundary_state(ulong delta, Sip8037BlockGasInclusionCheck.Outcome expected)
    {
        ...
        ulong blockGasLimit = Sip7825Constants.DefaultTxGasLimitCap + tx1State + 100_000;
        ...
        // SIP-8037: the state dimension reserves the full tx.gas (no intrinsic subtraction).
        ulong tx2Gas = stateAvailable + delta;

        Sip8037BlockGasInclusionCheck.Outcome outcome = Sip8037BlockGasInclusionCheck.Validate(
    ...
    public void Creation_tx_regular_check_uses_full_tx_gas_rejects()
    {
        ...
        ulong blockGasLimit = Sip7825Constants.DefaultTxGasLimitCap + remainingRegular;
        ...
        Assert.That(outcome, Is.EqualTo(Sip8037BlockGasInclusionCheck.Outcome.RegularDimensionExceeded));
    }
    ...
    {
        ulong blockGasLimit = Sip7825Constants.DefaultTxGasLimitCap + 100;
        // One over state_available; regular still fits because of the SIP-7825 cap.
        ulong txGas = blockGasLimit + 1;

        Sip8037BlockGasInclusionCheck.Outcome outcome = Sip8037BlockGasInclusionCheck.Validate(
            blockGasLimit, 0, 0, txGas);

        Assert.That(outcome, Is.EqualTo(Sip8037BlockGasInclusionCheck.Outcome.StateDimensionExceeded));
    }
    ...
    public void Creation_tx_state_check_uses_full_tx_gas_rejects_on_state_dimension()
    {
        ...
        ulong worstCaseRegular = Math.Min(Sip7825Constants.DefaultTxGasLimitCap, createTxGas);
    ...
    // SIP-7825 cap: the regular worst-case is clamped to TX_MAX_GAS_LIMIT regardless of tx.gas,
    ...
    {
        ulong blockGasLimit = Sip7825Constants.DefaultTxGasLimitCap + 100; // tiny headroom

        ulong txGas = Sip7825Constants.DefaultTxGasLimitCap * 10;
    ...
    public void Empty_block_simple_call_accepts()
    {
        ...
        Assert.That(outcome, Is.EqualTo(Sip8037BlockGasInclusionCheck.Outcome.Ok));
    }
    ...
    public void Calculate_block_regular_gas_subtracts_state_component()
    {
        ...
        Assert.That(
            Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas: 379_970, blockStateGas: 281_520),
            Is.EqualTo(98_450));
        Assert.That(
            Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas: 133_379, blockStateGas: 97_920),
            Is.EqualTo(35_459));
    }
    ...
    public void Calculate_block_regular_gas_never_negative()
        => Assert.That(
            Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas: 12_625, blockStateGas: 1_566_720),
            Is.EqualTo(0));
    ...
    public void Calculate_block_regular_gas_ignores_calldata_floor()
        => Assert.That(
            Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas: 21_000, blockStateGas: 0),
            Is.EqualTo(21_000));
}
