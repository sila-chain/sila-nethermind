...
public class SavmPooledMemoryTests : SavmMemoryTestsBase
{
    [TestCase(32UL, 1UL)]
    [TestCase(0UL, 0UL)]
    [TestCase(33UL, 2UL)]
    ...
    {
        ulong result = SavmCalculations.Div32Ceiling(input);
        TestContext.Out.WriteLine($"Memory cost (gas): {result}");
        Assert.That(result, Is.EqualTo(expectedResult));
    }

    private const ulong MaxCodeSize = CodeSizeConstants.MaxCodeSizeSip170;
    ...
    public void CalculateMemoryCost_TotalSizeExceedsIntMaxAfterWordAlignment_ShouldReturnOutOfGas()
    {
        ...
        UInt256 maxAllowedSize = (UInt256)(int.MaxValue - SavmPooledMemory.WordSize + 1);
    ...
    {
        decimal maxWords = SavmPooledMemory.MaxMemoryWords;
        ulong expectedCost = decimal.ToUInt64(
            maxWords * GasCostOf.Memory +
            decimal.Floor((maxWords * maxWords) / 512m));

        SavmPooledMemory ulongMemory = new();
        ulong ulongResult = ulongMemory.CalculateMemoryCost(0, SavmPooledMemory.MaxMemorySize, out bool ulongOutOfGas);
        Assert.That(ulongOutOfGas, Is.EqualTo(false));
        Assert.That(ulongResult, Is.EqualTo(expectedCost));

        SavmPooledMemory uint256Memory = new();
        UInt256 maxAllowedSize = (UInt256)SavmPooledMemory.MaxMemorySize;
    ...
    [Test]
    public void Inspect_should_not_change_savm_memory()
    {
        SavmPooledMemory memory = new();
    ...
    public void IncrementalGrowth_preserves_written_data_and_zeroes_new_regions(int step)
    {
        ...
        {
            Assert.That(memory.TryLoadSpan((UInt256)offset, (UInt256)SavmPooledMemory.WordSize, out Span<byte> read), Is.True);
            Assert.That(read.ToArray(), Is.EqualTo(new byte[SavmPooledMemory.WordSize]),
                $"memory at offset {offset} must read as zero after growth to {memory.Size}");
        }

        Assert.That(memory.TryLoadSpan(0, (UInt256)SavmPooledMemory.WordSize, out Span<byte> first), Is.True);
    ...
    private static readonly SilaEcdsa silaEcdsa = new(BlockchainIds.GenericNonRealNetwork);
    private static string Run(byte[] input)
    {
        ...
        SilaCodeInfoRepository codeInfoRepository = new(stateProvider);
        SilaVirtualMachine virtualMachine = new(
            new TestBlockhashProvider(specProvider),
            specProvider,
            LimboLogs.Instance);
        ITransactionProcessor transactionProcessor = new SilaTransactionProcessor(
        ...
        Transaction tx = Build.A.Transaction.
            ...
            WithGasPrice(0).
            WithValue(0).
            SignedAndResolved(silaEcdsa, PrivateKeyD, true).
            TestObject;
...
public class MyTracer : ITxTracer, IDisposable
{
    ...
    public void ReportOperationError(SavmExceptionType error)
    {
    }
    ...
    public void ReportActionError(SavmExceptionType exceptionType) => throw new NotSupportedException();
