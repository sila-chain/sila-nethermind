...
namespace Nethermind.Savm.Test
{
    ...
    public class SecP256r1PrecompileTests : PrecompileTests<SecP256r1Precompile, SecP256r1PrecompileTests>, IPrecompileTests
    {
        ...
        public void Produces_Empty_Output_On_Invalid_Input(string input)
        {
            ...
            (ReadOnlyMemory<byte> output, bool success) = Instance.Run(bytes, SilaPrague.Instance);
        ...
        {
            (ReadOnlyMemory<byte> output, bool success) = SecP256r1Precompile.Instance.Run(input, SilaPrague.Instance);
            using (Assert.EnterMultipleScope())
            {
                Assert.That(success, Is.True);
