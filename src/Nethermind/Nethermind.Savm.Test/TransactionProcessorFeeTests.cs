...
public class TransactionProcessorFeeTests
{
    ...
    public void Check_fees_with_fee_collector(bool isTransactionSip1559, bool withFeeCollector)
    {
        if (withFeeCollector)
        {
        ...
        Transaction tx = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithGasPrice(10).WithMaxFeePerGas(10)
            .WithType(isTransactionSip1559 ? TxType.SIP1559 : TxType.Legacy).WithGasLimit(21000).TestObject;
    ...
    public void Check_fees_with_fee_collector_destroy_coinbase()
    {
        ...
        _stateProvider.CreateAccount(TestItem.AddressB, 100.Sila);
    ...
    public void Check_paid_fees_multiple_transactions(bool withFeeCollector)
    {
        ...
        Transaction tx1 = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithType(TxType.SIP1559)
        ...
        Transaction tx2 = Build.A.Transaction.WithType(TxType.Legacy)
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithNonce(1)
    ...
    public void Check_paid_fees_with_blob(bool withFeeCollector)
    {
        ...
        Transaction tx = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithType(TxType.Blob)
    ...
    public void Check_paid_fees_with_byte_code()
    {
        byte[] byteCode = Prepare.SavmCode
            ...
            .Done;
        Transaction tx1 = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithMaxFeePerGas(3).WithMaxPriorityFeePerGas(2)
            .WithType(TxType.SIP1559).WithGasLimit(21000).TestObject;
        Transaction tx2 = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithNonce(1).WithGasPrice(10)
            .WithType(TxType.Legacy).WithGasLimit(21000).TestObject;
        Transaction tx3 = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithNonce(2).WithMaxFeePerGas(2).WithMaxPriorityFeePerGas(1)
            .WithType(TxType.SIP1559).WithCode(byteCode)
    ...
    public void Check_fees_with_free_transaction()
    {
        Transaction tx1 = Build.A.Transaction
            ...
            .WithMaxFeePerGas(3).WithMaxPriorityFeePerGas(1).WithGasLimit(21000).TestObject;
        Transaction tx2 = Build.A.Transaction
            .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).WithNonce(1).WithIsServiceTransaction(true)
            .WithType(TxType.SIP1559).WithMaxFeePerGas(3)
    ...
    public void CallAndRestore_returns_descriptive_error_when_maxFeePerGas_below_baseFee(TxType txType)
    {
        ...
        Transaction tx = txType == TxType.SIP1559
            ? Build.A.Transaction
                .WithType(TxType.SIP1559)
        ...
        Assert.That(result.ErrorDescription, Does.Contain(TestItem.AddressA.ToString(withSip55Checksum: true)));
    }
