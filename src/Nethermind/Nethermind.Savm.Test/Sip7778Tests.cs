...
/// <summary>
/// Tests for SIP-7778: Block Gas Accounting without Refunds
/// With the revert from sila/execution-specs#2073, receipts now use post-refund gas
/// while block gas accounting still uses pre-refund gas.
/// </summary>
public class Sip7778Tests : VirtualMachineTestsBase
{
    protected override ISpecProvider SpecProvider { get; } = CreateSpecProvider();

    private static TestSpecProvider CreateSpecProvider()
    {
        // Use OverridableReleaseSpec to extend London with SIP-7778 enabled
        OverridableReleaseSpec sip7778Spec = new(London.Instance) { IsSip7778Enabled = true };
        TestSpecProvider provider = new(London.Instance)
        {
            NextForkSpec = sip7778Spec,
            ForkOnBlockNumber = 1
        };
        return provider;
    }
    ...
    [Test]
    public void Block_gas_uses_pre_refund_value_when_sip7778_enabled()
    {
        // This test verifies that block gas accounting uses pre-refund gas
        // when SIP-7778 is enabled
        TestState.CreateAccount(Recipient, 1.Sila);
        ...
        // When SIP-7778 is enabled, block.GasUsed should reflect pre-refund gas
    ...
    [Test]
    public void Receipt_uses_post_refund_gas_when_sip7778_enabled()
    {
        // After the revert (sila/execution-specs#2073), receipts show post-refund gas
    ...
    [Test]
    public void Legacy_behavior_unchanged_when_sip7778_disabled()
    {
        // Use block 0 where SIP-7778 is not enabled
        TestState.CreateAccount(Recipient, 1.Sila);
        TestState.Set(new StorageCell(Recipient, 0), new byte[] { 1 });
        ...
        // When SIP-7778 is disabled, block.GasUsed equals gas spent (post-refund)
        Assert.That(tracer.GasSpent, Is.GreaterThan(0));
        Assert.That(block.Header.GasUsed, Is.EqualTo(tracer.GasSpent));
    }

    [Test]
    public void Receipt_gas_equals_block_gas_when_sip7778_disabled()
    {
        TestState.CreateAccount(Recipient, 1.Sila);
        TestState.Set(new StorageCell(Recipient, 0), new byte[] { 1 });
        TestState.Commit(SpecProvider.GetSpec((0, 0)));
        ...
        TxReceipt receipt = tracer.TxReceipts[0];
        // When SIP-7778 is disabled, receipt and block gas are both post-refund
    ...
    {
        // Use a spec with both SIP-7778 and SIP-7623 enabled
        OverridableReleaseSpec sip7778And7623Spec = new(London.Instance)
        {
            IsSip7778Enabled = true,
            IsSip7623Enabled = true,
            IsSip2028Enabled = true
        };
        TestSpecProvider provider = new(London.Instance)
        {
            NextForkSpec = sip7778And7623Spec,
            ForkOnBlockNumber = 1
        };

        // Create a transaction with significant calldata to have high floor gas
        // Floor gas = 21000 + tokens * 10
        // For non-zero bytes: tokens = length * 4 (SIP-2028 multiplier)
        ...
        SilaIntrinsicGas intrinsicGas = IntrinsicGasCalculator.Calculate(
        ...
        TestState.CreateAccount(TestItem.AddressA, 1.Sila);
        TestState.Commit(provider.GetSpec((1, 0)));

        SilaTransactionProcessor processor = new(BlobBaseFeeCalculator.Instance, provider, TestState, Machine, CodeInfoRepository, LimboLogs.Instance);
    ...
    public void Block_gas_uses_max_of_pre_refund_and_floor_with_refund_scenario()
    {
        ...
        OverridableReleaseSpec sip7778And7623Spec = new(London.Instance)
        {
            ...
            IsSip2028Enabled = true,
            IsSip2200Enabled = true,
            IsSip3529Enabled = true // London refund rules
        };
        TestSpecProvider provider = new(London.Instance)
        {
            NextForkSpec = sip7778And7623Spec,
        ...
        SilaIntrinsicGas intrinsicGas = IntrinsicGasCalculator.Calculate(tx, sip7778And7623Spec);
    ...
    [Test]
    public void Transaction_admission_uses_pre_refund_block_gas_when_sip7778_enabled()
    {
        TestState.CreateAccount(TestItem.AddressA, 1.Sila);
        TestState.CreateAccount(Recipient, 1.Sila);
        TestState.Set(new StorageCell(Recipient, 0), new byte[] { 1 });
        ...
        byte[] refundCode = Prepare.SavmCode
            .PushData(0)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .Done;
    ...
    public void Restore_after_multiple_transactions_maintains_correct_gas_tracking()
    {
        ...
        byte[] codeWithRefund = Prepare.SavmCode
            .PushData(0)
            .PushData(0)
            .Op(Instruction.SSTORE)
            .Done;

        byte[] codeNoRefund = Prepare.SavmCode
            .Op(Instruction.STOP)
            .Done;
    ...
    public void Double_restore_works_correctly()
    {
        ...
        byte[] code = Prepare.SavmCode.Op(Instruction.STOP).Done;
