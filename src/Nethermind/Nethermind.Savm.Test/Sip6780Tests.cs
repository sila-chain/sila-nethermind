...
{
    [TestFixture]
    public class Sip6780Tests : VirtualMachineTestsBase
    {
        ...
        private readonly SilaEcdsa _ecdsa = new(1);
        ...
        {
            byte[] contractCall = Prepare.SavmCode
                .Call(_contractAddress, 100000)
                .Op(Instruction.STOP).Done;
            Transaction initTx = Build.A.Transaction.WithCode(_initCode).WithValue(99.Sila).WithGasLimit(_gasLimit).SignedAndResolved(_ecdsa, TestItem.PrivateKeyA).TestObject;
            ...
            Assert.That(contractBalanceAfterInit, Is.EqualTo(99.Sila));
            AssertSendAll();
        ...
        public void self_destruct_not_in_same_transaction_should_not_burn(ulong timestamp, bool onlyOnSameTransaction)
        {
            ...
            Assert.That(contractBalanceAfterInit, Is.EqualTo(99.Sila));
            if (onlyOnSameTransaction)
                Assert.That(TestState.GetBalance(_contractAddress), Is.EqualTo(99.Sila)); // not burnt
            else
        ...
        public void self_destruct_in_same_transaction()
        {
            ...
            byte[] tx1 = Prepare.SavmCode
                .Create2(_initCode, salt, 99.Sila)
                .Call(_contractAddress, 100000)
                .STOP()
                .Done;

            Transaction createTx = Build.A.Transaction.WithCode(tx1).WithValue(100.Sila).WithGasLimit(_gasLimit).SignedAndResolved(_ecdsa, TestItem.PrivateKeyA).TestObject;
            Block block = Build.A.Block.WithNumber(BlockNumber)
                .WithTimestamp(Timestamp)
        ...
        public void self_destruct_in_initcode_of_create_opcodes()
        {
            ...
            byte[] tx1 = Prepare.SavmCode
                .Create2(_selfDestructCode, salt, 99.Sila)
                .STOP()
                .Done;
        ...
        {
            Transaction createTx = Build.A.Transaction.WithCode(_selfDestructCode).WithValue(99.Sila).WithGasLimit(_gasLimit).SignedAndResolved(_ecdsa, TestItem.PrivateKeyA).TestObject;
            Block block = Build.A.Block.WithNumber(BlockNumber)
                .WithTimestamp(Timestamp)
                .WithTransactions(createTx).WithGasLimit(2 * _gasLimit).TestObject;
        ...
        private void AssertSendAll()
        {
            ...
            Assert.That(TestState.GetBalance(TestItem.PrivateKeyB.Address), Is.EqualTo(99.Sila));
        }
