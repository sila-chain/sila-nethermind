...
public class Sip7954Tests : VirtualMachineTestsBase
{
    ...
    {
        Assert.That(Spec.MaxCodeSize, Is.EqualTo(CodeSizeConstants.MaxCodeSizeSip7954));
        Assert.That(Spec.MaxInitCodeSize, Is.EqualTo(2L * CodeSizeConstants.MaxCodeSizeSip7954));
    }

    [TestCase(true, 55000, ExpectedResult = false, TestName = "InitCode_between_old_and_new_limit_accepted")]
    [TestCase(false, 55000, ExpectedResult = true, TestName = "InitCode_above_old_limit_rejected_before_sip7954")]
    [TestCase(true, 2 * CodeSizeConstants.MaxCodeSizeSip7954 + 1, ExpectedResult = true, TestName = "InitCode_exceeding_new_limit_rejected")]
    public bool InitCode_size_validation(bool sip7954Enabled, int initCodeSize) =>
        ExecuteRawCreateTransaction(sip7954Enabled ? Timestamp : MainnetSpecProvider.ShanghaiBlockTimestamp, initCodeSize) == TransactionResult.TransactionSizeOverMaxInitCodeSize;

    [TestCase(30000, ExpectedResult = StatusCode.Success, TestName = "Code_deposit_between_old_and_new_limit_succeeds")]
    [TestCase(CodeSizeConstants.MaxCodeSizeSip7954 + 1, ExpectedResult = StatusCode.Failure, TestName = "Code_deposit_above_new_limit_fails")]
    ...
    [Test]
    public void Sip8037_floor_gas_enforced_in_validate_gas()
    {
        ...
        SilaIntrinsicGas intrinsic = IntrinsicGasCalculator.Calculate(probeTx, spec, probeBlock.Header.GasLimit);
    ...
    private TestAllTracerWithOutput ExecuteDeployTransaction(ulong timestamp, int deployedCodeSize)
    {
        ...
        _processor.Execute(transaction, new BlockExecutionContext(block.Header, SilaAmsterdam.NoSip8037Instance), tracer);
        return tracer;
    }
}
