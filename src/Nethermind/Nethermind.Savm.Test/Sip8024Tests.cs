...
/// <summary>
/// Tests for SIP-8024: Backward-compatible SWAPN, DUPN, EXCHANGE for legacy code.
/// </summary>
public class Sip8024Tests : VirtualMachineTestsBase
{
    protected override ulong BlockNumber => MainnetSpecProvider.ParisBlockNumber;
    protected override ulong Timestamp => MainnetSpecProvider.OsakaBlockTimestamp;
    protected override ISpecProvider SpecProvider => new TestSpecProvider(new SilaOsaka { IsSip8024Enabled = true });

    private static Prepare PushNValues(int count) => Prepare.SavmCode.For(count, static (p, i) => p.PushData(i + 1));
    private static Prepare PushZeros(int count) => Prepare.SavmCode.For(count, static (p, _) => p.PushData(0));
    ...
    private static IEnumerable<TestCaseData> SuccessTestCases()
    {
        ...
        // SIP test vector: PUSH1 1, PUSH1 0, DUP1 x15, DUPN 0x80 -> duplicates bottom item (1)
        yield return new TestCaseData(Prepare.SavmCode.PushData(1).PushData(0).Dup1Chain(15).Op(Instruction.DUPN).Data(0x80).MSTORE(0).Return(32, 0).Done, 1).SetName("SipTestVector_DupN_18Items");

        // SIP test vector: PUSH1 1, PUSH1 0, DUP1 x15, PUSH1 2, SWAPN 0x80 -> swap top (2) with bottom (1)
        yield return new TestCaseData(Prepare.SavmCode.PushData(1).PushData(0).Dup1Chain(15).PushData(2).Op(Instruction.SWAPN).Data(0x80).MSTORE(0).Return(32, 0).Done, 1).SetName("SipTestVector_SwapN_18Items");

        // SIP test vector: PUSH1 0, PUSH1 1, PUSH1 2, EXCHANGE 0x8e -> swaps positions 1,2, top stays 2
        yield return new TestCaseData(Prepare.SavmCode.PushData(0).PushData(1).PushData(2).Op(Instruction.EXCHANGE).Data(0x8e).MSTORE(0).Return(32, 0).Done, 2).SetName("SipTestVector_Exchange_3Items");
    }
    ...
    private static IEnumerable<TestCaseData> FailureTestCases()
    {
        ...
        // SIP test vector: SWAPN with disallowed immediate 0x5b
        yield return new TestCaseData(new byte[] { 0xe7, 0x5b }).SetName("SipTestVector_InvalidSwapn");

        // SIP test vector: 16 items (PUSH1 0 + DUP1 x15), DUPN depth=17 -> underflow
        yield return new TestCaseData(PushZeros(1).Dup1Chain(15).Op(Instruction.DUPN).Data(0x80).Op(Instruction.STOP).Done).SetName("SipTestVector_DupN_StackUnderflow");
    }
    ...
    [Test]
    public void SipTestVector_JumpOverInvalidDupn_Succeeds()
    {
    ...
    public class Sip8024DisabledTests : VirtualMachineTestsBase
    {
        protected override ulong BlockNumber => MainnetSpecProvider.ParisBlockNumber;
        protected override ulong Timestamp => MainnetSpecProvider.OsakaBlockTimestamp;
        protected override ISpecProvider SpecProvider => new TestSpecProvider(new SilaOsaka() { IsSip8024Enabled = false });

        private static IEnumerable<TestCaseData> DisabledTestCases()
        {
            yield return new TestCaseData(Prepare.SavmCode.For(20, static (p, i) => p.PushData(i + 1)).Op(Instruction.DUPN).Data(0x80).Done).SetName("DupN_WhenDisabled");
            yield return new TestCaseData(Prepare.SavmCode.For(20, static (p, i) => p.PushData(i + 1)).Op(Instruction.SWAPN).Data(0x80).Done).SetName("SwapN_WhenDisabled");
            yield return new TestCaseData(Prepare.SavmCode.For(5, static (p, i) => p.PushData(i + 1)).Op(Instruction.EXCHANGE).Data(0x9d).Done).SetName("Exchange_WhenDisabled");
        }
