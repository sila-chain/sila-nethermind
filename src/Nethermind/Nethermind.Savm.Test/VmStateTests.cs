...
namespace Nethermind.Savm.Test
{
    public class VmStateTests
    {
        ...
        {
            using VmStateScope scope = CreateSavmStateScope();
        ...
        {
            using VmStateScope parent = CreateSavmStateScope();
            using VmStateScope child = CreateSavmStateScope(parent.VmState);
        ...
        {
            using VmStateScope parent = CreateSavmStateScope();
            using VmStateScope _ = CreateSavmStateScope(parent.VmState);
        }
        ...
        {
            using VmStateScope scope = CreateSavmStateScope();
            scope.VmState.InitializeStacks(default, out SavmStack _);
        }

        private static VmStateScope CreateSavmStateScope(VmState<SilaGasPolicy> parentVmState = null) =>
            new(parentVmState is null
                ? VmState<SilaGasPolicy>.RentTopLevel(SilaGasPolicy.FromULong(10000),
                    ExecutionType.CALL,
                    RentExecutionEnvironment(),
                    new StackAccessTracker(),
                    Snapshot.Empty)
                : VmState<SilaGasPolicy>.RentFrame(SilaGasPolicy.FromULong(10000),
                    0,
                    0,
                    ExecutionType.CALL,
                    false,
        ...
        private readonly struct VmStateScope(VmState<SilaGasPolicy> vmState) : System.IDisposable
        {
            public VmState<SilaGasPolicy> VmState { get; } = vmState;
