...
public class SimpleTransferFastPathDifferentialTests
{
    ...
    private static IEnumerable<TestCaseData> Scenarios()
    {
        ...
        foreach (RecipientKind kind in valueTransferKinds)
        {
            ...
            {
                foreach (bool sip7708 in (bool[])[true, false])
                {
                    yield return Case(value, kind, sip7708, dataLength: 0, TracerShape.AllOn, Mode.Execute);
                }
            }
        }

        yield return Case(1_000_000, RecipientKind.Existing, sip7708: true, dataLength: 300, TracerShape.AllOn, Mode.Execute);
        yield return Case(0, RecipientKind.Nonexistent, sip7708: false, dataLength: 300, TracerShape.AllOn, Mode.Execute);

        yield return Case(1_000_000, RecipientKind.Existing, sip7708: true, dataLength: 0, TracerShape.ActionsNoState, Mode.CallAndRestore);
        yield return Case(1_000_000, RecipientKind.Nonexistent, sip7708: false, dataLength: 0, TracerShape.ActionsNoState, Mode.BuildUp);
        yield return Case(1_000_000, RecipientKind.Self, sip7708: true, dataLength: 0, TracerShape.ActionsNoState, Mode.CallAndRestore);

        yield return Case(1_000_000, RecipientKind.Existing, sip7708: true, dataLength: 0, TracerShape.ActionsNoState, Mode.Execute);
        yield return Case(1_000_000, RecipientKind.Nonexistent, sip7708: false, dataLength: 0, TracerShape.None, Mode.Execute);
        yield return Case(0, RecipientKind.Self, sip7708: false, dataLength: 300, TracerShape.None, Mode.Execute);

        yield return Case(1_000_000, RecipientKind.Precompile, sip7708: false, dataLength: 0, TracerShape.AllOn, Mode.Execute);
        yield return Case(0, RecipientKind.Precompile, sip7708: false, dataLength: 300, TracerShape.AllOn, Mode.Execute);
    }

    private static TestCaseData Case(ulong value, RecipientKind kind, bool sip7708, int dataLength, TracerShape tracer, Mode mode) =>
        new TestCaseData(value, kind, sip7708, dataLength, tracer, mode)
            .SetName($"{mode}_{kind}_value{value}_7708{(sip7708 ? "On" : "Off")}_data{dataLength}_{tracer}");

    [TestCaseSource(nameof(Scenarios))]
    public void Fast_path_and_savm_path_are_equivalent(ulong value, RecipientKind recipientKind, bool sip7708, int dataLength, TracerShape tracer, Mode mode)
    {
        ExecutionCapture fast = Run(forceSavmPath: false, value, recipientKind, sip7708, dataLength, tracer, mode);
        ExecutionCapture slow = Run(forceSavmPath: true, value, recipientKind, sip7708, dataLength, tracer, mode);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(slow.FastPathEngaged, Is.Zero, "forced SAVM path did not engage the fast path");
        }
    }
    ...
    private static ExecutionCapture Run(
        ...
        Mode mode)
    {
        OverridableReleaseSpec spec = new(SilaPrague.Instance) { IsSip7708Enabled = sip7708 };
        ...
        state.CreateAccount(TestItem.AddressA, 1.Sila);
        switch (recipientKind)
        {
            case RecipientKind.Existing:
                state.CreateAccount(TestItem.AddressB, 1.Sila);
                break;
            ...
            case RecipientKind.Delegated:
                state.CreateAccount(TestItem.AddressB, 1.Sila);
                byte[] delegationCode = [.. Sip7702Constants.DelegationHeader, .. TestItem.AddressD.Bytes];
        ...
        SilaCodeInfoRepository codeInfoRepository = new(state);
        SilaVirtualMachine virtualMachine = new(new TestBlockhashProvider(specProvider), specProvider, LimboLogs.Instance);
        SilaTransactionProcessor processor = new(BlobBaseFeeCalculator.Instance, specProvider, state, virtualMachine, codeInfoRepository, LimboLogs.Instance);
        ...
        TransactionProcessorBase.ForceSimpleTransferDisabled = forceSavmPath;
        try
        {
    ...
    private sealed class RecordingTracer : TxTracer
    {
        ...
        public override void ReportActionError(SavmExceptionType savmExceptionType) =>
            Events.Add($"ActionError({savmExceptionType})");
