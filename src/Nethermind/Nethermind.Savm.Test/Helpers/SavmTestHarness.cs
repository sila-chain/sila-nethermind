...
namespace Nethermind.Savm.Test.Helpers;

/// <summary>
/// Bundle of the standard pieces a metrics/slow-block integration test needs to drive a
/// transaction through <see cref="SilaTransactionProcessor"/>: spec provider, world state +
...
/// </remarks>
public sealed class SavmTestHarness : IDisposable
{
    public ISpecProvider SpecProvider { get; }
    public IWorldState WorldState { get; }
    public ITransactionProcessor TxProcessor { get; }
    public ISilaEcdsa Ecdsa { get; }
    ...
    /// the default) sets <see cref="ProcessingThread.IsBlockProcessingThread"/> so SAVM
    ...
    /// </summary>
    public SavmTestHarness(IReleaseSpec? spec = null, bool markBlockProcessingThread = true)
    {
        _spec = spec ?? SilaPrague.Instance;
        ...
        SilaCodeInfoRepository codeInfoRepo = new(WorldState);
        SilaVirtualMachine vm = new(new TestBlockhashProvider(SpecProvider), SpecProvider, LimboLogs.Instance);
        TxProcessor = new SilaTransactionProcessor(BlobBaseFeeCalculator.Instance, SpecProvider, WorldState, vm, codeInfoRepo, LimboLogs.Instance);
        Ecdsa = new SilaEcdsa(SpecProvider.ChainId);
    }

    /// <summary>Builds a SilaPrague-timestamped block with the given transactions and a 30M gas limit.</summary>
