...
internal class XdcTransactionProcessor(
    ...
    IMasternodeVotingContract masternodeVotingContract) : SilaTransactionProcessorBase(
        ...
        logManager)
{
    ...
    protected override TransactionResult ValidateGas(Transaction tx, BlockHeader header, IReleaseSpec _, in SilaGasPolicy intrinsicGas, ulong minGasRequired, bool validate)
    {
    ...
    protected override UInt256 CalculateEffectiveGasPrice(Transaction tx, bool sip1559Enabled, in UInt256 baseFee, out UInt256 opcodeGasPrice)
    {
        ...
        return base.CalculateEffectiveGasPrice(tx, sip1559Enabled, in baseFee, out opcodeGasPrice);
    }

    protected override IntrinsicGas<SilaGasPolicy> CalculateIntrinsicGas(Transaction tx, IReleaseSpec spec, ulong blockGasLimit) =>
    ...
    private TransactionResult ExecuteSpecialTransaction(Transaction tx, ITxTracer tracer, ExecutionOptions opts)
    {
        ...
        IntrinsicGas<SilaGasPolicy> intrinsicGas = CalculateIntrinsicGas(tx, spec, header.GasLimit);
