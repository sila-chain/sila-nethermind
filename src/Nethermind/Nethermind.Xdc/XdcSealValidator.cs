...
{
    private readonly SilaEcdsa _silaEcdsa = new(0); //Ignore chainId since we don't sign transactions here
    ...
    public bool ValidateSeal(BlockHeader header)
    {
        ...
        if (header.Author is null)
        {
            ...
            Address signer = _silaEcdsa.RecoverAddress(signature, in hash);
