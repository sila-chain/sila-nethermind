...
{
    private readonly SilaEcdsa _silaEcdsa = new(specProvider.ChainId);
    ...
    public Address[] HandlePenalties(ulong number, Hash256 parentHash, Address[] candidates)
    {
        ...
        for (int i = listBlockNumber.Count - 1; i >= 0; i--)
        {
            ...
            foreach (Transaction tx in signingTxs)
            {
                ...
                tx.SenderAddress ??= _silaEcdsa.RecoverAddress(tx);
        ...
        // SIP-55 checksummed hex is required: lowsrcase byte comparison reorders
        // (e.g. "0xAb..." < "0xaa..." vs "0xab..." > "0xaa...").
        Address[] result = new Address[penalties.Count];
        penalties.CopyTo(result);
        result.AsSpan().Sort(default(AddressBySip55ChecksumOrdinalComparer));
        return result;
    }
}
