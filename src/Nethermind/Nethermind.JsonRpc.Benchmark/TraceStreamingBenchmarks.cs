...
public class TraceStreamingBenchmarks
{
    ...
    public int Throughput_Buffered()
    {
        ...
        JsonSerializer.Serialize(writer, new ParityTxTraceFromReplay(trace, true), SilaJsonSerializer.JsonOptions);
    ...
    public long PeakHeap_BufferedWithTracer()
    {
        ...
        for (int n = 0; n < Concurrency; n++)
        {
            ...
            DiscardingBufferWriter sink = new();
            using Utf8JsonWriter writer = new(sink);
            JsonSerializer.Serialize(writer, new ParityTxTraceFromReplay(tracer.BuildResult(), true), SilaJsonSerializer.JsonOptions);
            live.Add(tracer);
        }
    ...
    {
        const int Concurrency = 16;
        JsonSerializerOptions opts = SilaJsonSerializer.JsonOptions;
        GC.Collect();
        GC.WaitForPendingFinalizers();
        long baseline = GC.GetTotalMemory(true);
    ...
    private static void WriteStreamingEnvelope(Utf8JsonWriter writer, int opcodeCount)
    {
        ...
        JsonSerializer.Serialize(writer, Array.Empty<byte>(), SilaJsonSerializer.JsonOptions);
