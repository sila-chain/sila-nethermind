...
public class DebugTraceStreamingBenchmarks
{
    ...
    public int Throughput_Buffered()
    {
        ...
        JsonSerializer.Serialize(writer, trace, SilaJsonSerializer.JsonOptions);
    ...
    public long PeakHeap_BufferedWithTracer()
    {
        ...
        for (int n = 0; n < Concurrency; n++)
        {
            ...
            DiscardingBufferWriter sink = new();
            using Utf8JsonWriter writer = new(sink);
            JsonSerializer.Serialize(writer, tracer.BuildResult(), SilaJsonSerializer.JsonOptions);
            live.Add(tracer);
        }
    ...
    private static GethTxMemoryTraceEntry BuildEntry(int pc) => new()
    {
        ...
        GasCost = 3,
        Memory = new byte[SavmPooledMemory.WordSize],
    ...
    {
        byte[] bytes = new byte[SavmStack.WordSize * 2];
        bytes[SavmStack.WordSize - 1] = 1;
        bytes[^1] = 2;
        return bytes;
    }
