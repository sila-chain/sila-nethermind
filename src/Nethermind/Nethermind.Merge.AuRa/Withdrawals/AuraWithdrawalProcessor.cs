...
public class AuraWithdrawalProcessor : IWithdrawalProcessor
{
    ...
    public void ProcessWithdrawals(Block block, IReleaseSpec spec)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (_logger.IsTrace) _logger.Trace($"  {(BigInteger)withdrawal.AmountInWei / (BigInteger)Unit.Sila:N3}GNO to account {withdrawal.Address}");
        }
        ...
        catch (Exception ex) when (ex is ArgumentNullException || ex is SavmException)
        {
