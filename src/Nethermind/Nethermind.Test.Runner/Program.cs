...
internal class Program
{
    ...
    public class Options
    {
        ...
        public static Option<bool> ZkSavmTest { get; } =
            new("--zksavmTest") { Description = "Run as zkSAVM stateless-execution test (tests-zksavm fixtures: witness input through StatelessExecutor vs expected output)." };
        ...
        public static Option<bool> GnosisTest { get; } =
            new("--gnosisTest", "-g") { Description = "Set test as gnosisTest. if not, it will be by default assumed a sila-mainnet test." };
    ...
    private static readonly IJsonSerializer _serializer = new SilaJsonSerializer();
    ...
    private static async Task<int> Run(ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        bool isZkSavmTest = parseResult.GetValue(Options.ZkSavmTest);

        int testTypeCount = (isStateTest ? 1 : 0) + (isBlockTest ? 1 : 0) + (isEngineTest ? 1 : 0) + (isTxTest ? 1 : 0) + (isZkSavmTest ? 1 : 0);
        if (testTypeCount != 1)
        {
            Console.WriteLine("Please specify one of: --stateTest, --blockTest, --engineTest, --txTest, or --zksavmTest");
            return 1;
        }
        ...
        while (!string.IsNullOrWhiteSpace(input))
        {
            ...
            if (isEngineTest || isBlockTest)
            {
                ...
                List<SilaTestResult> results = await RunBlockTestFiles(files, runnerOptions, workers);
                Console.Out.Write(_serializer.Serialize(results, true));
            }
            else if (isStateTest)
            {
                List<SilaTestResult> results = RunStateTestFiles(files, whenTrace, traceMemory, !excludeStack, chainId, filter, enableWarmup, workers);
                Console.Out.Write(_serializer.Serialize(results, true));
            }
            else if (isTxTest)
            {
                List<SilaTestResult> results = RunTransactionTestFiles(files, filter, workers);
                Console.Out.Write(_serializer.Serialize(results, true));
            }
            else if (isZkSavmTest)
            {
                List<SilaTestResult> results = RunZkSavmTestFiles(files, filter, workers);
                Console.Out.Write(_serializer.Serialize(results, true));
            }
    ...
    private static async Task<List<SilaTestResult>> RunBlockTestFiles(
        List<string> files, BlockchainTestsRunnerOptions baseOptions, int workers)
    {
        ...
        {
            List<SilaTestResult> allResults = [];
            foreach (string file in files)
            {
                try
                {
                    ...
                    IEnumerable<SilaTestResult> results = await runner.RunTestsAsync();
                ...
                catch (Exception ex)
                {
                    ...
                    allResults.Add(new SilaTestResult(name, ex.Message));
                }
            }
            return allResults;
        }
        ...
        IEnumerable<SilaTestResult>[] resultsByFile = new IEnumerable<SilaTestResult>[files.Count];
        await Parallel.ForEachAsync(
            ...
            async (item, ct) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    WriteFileExceptionStatus(name, ex);
                    resultsByFile[item.index] = [new SilaTestResult(name, ex.Message)];
                }
            });

        List<SilaTestResult> combinedResults = [];
        foreach (IEnumerable<SilaTestResult> results in resultsByFile)
        {
    ...
    private static List<SilaTestResult> RunStateTestFiles(
        ...
        ulong chainId, string filter, bool enableWarmup, int workers)
    {
        ...
        void ReportProgress(SilaTestResult result)
        {
            int completed = Interlocked.Increment(ref completedTests);
            if (!result.Pass)
            {
        ...
        if (workers <= 1 || whenTrace != WhenTrace.Never || enableWarmup)
        {
            ...
            foreach ((int index, GeneralStateTest test) in testCases)
            {
                ...
                SilaTestResult result = runner.RunSingleTest(test);
        ...
        SilaTestResult[] results = new SilaTestResult[testCases.Count];
        Parallel.ForEach(
            testCases,
            ...
            item =>
            {
                ...
                SilaTestResult result = runner.RunSingleTest(item.test);
    ...
    private static List<SilaTestResult> RunTransactionTestFiles(List<string> files, string? filter, int workers)
    {
        Regex? filterRegex = filter is not null ? new Regex($"^({filter})", RegexOptions.Compiled) : null;
        ...
        int completedTests = 0;
        void ReportResult(SilaTestResult result)
        {
            int completed = Interlocked.Increment(ref completedTests);
            if (!result.Pass)
            {
        ...
        SilaTestResult[] results = new SilaTestResult[tests.Count];
        if (workers <= 1)
        {
    ...
    private static List<SilaTestResult> RunZkSavmTestFiles(List<string> files, string? filter, int workers)
    {
        Regex? filterRegex = filter is not null ? new Regex($"^({filter})", RegexOptions.Compiled) : null;

        int completedFiles = 0;
        List<SilaTestResult>[] resultsByFile = new List<SilaTestResult>[files.Count];
        ...
        {
            List<SilaTestResult> fileResults = [];
            try
            {
                ...
                foreach (BlockchainTest test in source.LoadTests<BlockchainTest>())
                {
                    ...
                    fileResults.AddRange(ZkSavmTestsRunner.RunTest(test));
                }
            }
            catch (Exception ex)
            {
                ...
                Console.Error.Flush();
                fileResults.Add(new SilaTestResult(name, ex.Message));
            }

            resultsByFile[index] = fileResults;

            int done = Interlocked.Increment(ref completedFiles);
            foreach (SilaTestResult result in fileResults)
            {
                if (!result.Pass)
                {
        ...
        List<SilaTestResult> combinedResults = [];
        foreach (List<SilaTestResult> fileResults in resultsByFile)
        {
