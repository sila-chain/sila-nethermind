...
public class BlockchainTestsRunner(in BlockchainTestsRunnerOptions options, ITestSourceLoader? testsSource = null) : BlockchainTestBase, IBlockchainTestRunner
{
    ...
    public async Task<IEnumerable<SilaTestResult>> RunTestsAsync()
    {
        ...
        IEnumerable<SilaTest> tests = _testsSource.LoadTests<SilaTest>();
        foreach (SilaTest loadedTest in tests)
        {
            SilaTestResult? result = await ExecuteTestAsync(loadedTest);
            if (result is null)
                continue;
    ...
    public async Task<SilaTestResult> RunSingleTestAsync(BlockchainTest test)
    {
        test.ChainId = _chainId;
        return await RunTest(test);
    }

    private async Task<SilaTestResult?> ExecuteTestAsync(SilaTest loadedTest)
    {
        if (loadedTest is FailedToLoadTest)
            return new SilaTestResult(loadedTest.Name, loadedTest.LoadFailure);
        ...
        if (test.LoadFailure is not null)
            return new SilaTestResult(test.Name, test.LoadFailure);
        ...
        {
            return new SilaTestResult(test.Name, test.ForkName, ex.Message);
        }
    }

    private void ReportResult(SilaTestResult result)
    {
        if (_suppressOutput)
        {
    ...
    private void ReportSuppressedResult(SilaTestResult result)
    {
