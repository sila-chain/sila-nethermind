...
namespace Nethermind.Test.Runner
{
    ...
    public class StateTestsRunner : GeneralStateTestBase, IStateTestRunner
    {
        ...
        private void WriteOut(List<SilaTestResult> testResult)
        {
            if (!_suppressOutput)
        ...
        public SilaTestResult RunSingleTest(GeneralStateTest test)
        {
            test.ChainId = _chainId;
            return ExecuteWithOptionalTrace(test);
        }

        private SilaTestResult ExecuteWithOptionalTrace(GeneralStateTest test)
        {
            SilaTestResult? result = null;
            ...
            if (_whenTrace != WhenTrace.Never && !(result?.Pass ?? false))
            {
                ...
                // SIP-3155 always needs stack; IsTracingStack controls whether
                // the SAVM calls SetOperationStack at all.
