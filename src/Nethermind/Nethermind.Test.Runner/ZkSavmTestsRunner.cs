...
/// <summary>
/// Runs the zkSAVM stateless-execution preview fixtures (the <c>tests-zksavm</c> releases):
...
/// fixture's expected output, mirroring the ZkSavmPreview NUnit fixture.
/// </remarks>
public static class ZkSavmTestsRunner
{
    public static List<SilaTestResult> RunTest(BlockchainTest test)
    {
        ...
        for (int i = 0; i < blocks.Length; i++)
        {
            ...
            results.Add(new SilaTestResult($"{test.Name}_stateless_block_{i}", test.ForkName, error is null) { Error = error });
        }
