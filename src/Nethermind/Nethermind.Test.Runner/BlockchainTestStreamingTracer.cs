...
/// Compatible with go-sila's block test tracing output format.
...
public class BlockchainTestStreamingTracer(GethTraceOptions options, Stream? output = null) : ITestBlockTracer, IDisposable
{
    ...
    private void WriteTraceEntry(GethTxFileTraceEntry entry)
    {
        ...
        if (entry.Stack is { Length: > 0 } stack)
        {
            ...
            for (int i = 0; i < sp.Length; i += SavmStack.WordSize)
                HexWriter.WriteUInt256HexRawValue(writer, new UInt256(sp.Slice(i, SavmStack.WordSize), isBigEndian: true), zeroPadded: false);
