...
public class SlotValueTests
{
    ...
    [Test]
    public void Test_ToSavmBytes_ReturnsCanonicalZeroForZeroValue()
    {
        SlotValue zero = new(ReadOnlySpan<byte>.Empty);
        Assert.That(zero.ToSavmBytes(), Is.EqualTo(new byte[] { 0 }));
    }
    ...
    public void Test_ToSavmBytes_StripsLeadingZerosForNonZeroValue(string inputHex, string expectedHex)
    {
        SlotValue value = SlotValue.FromSpanWithoutLeadingZero(Bytes.FromHexString(inputHex));
        Assert.That(value.ToSavmBytes(), Is.EqualTo(Bytes.FromHexString(expectedHex)));
    }
}
