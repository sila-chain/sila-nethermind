...
using Nethermind.Network.P2P.Subprotocols.Sil.V68.Messages;
...
namespace Nethermind.Network.Test.P2P.Subprotocols.Sil.V68;

public class Sil68ProtocolHandlerTests
{
    ...
    private Sil68ProtocolHandler _handler = null!;
    ...
    public void Setup()
    {
        ...
        _handler = new Sil68ProtocolHandler(
            _session,
            _svc,
    ...
    public void Metadata_correct()
    {
        ...
        Assert.That(_handler.Name, Is.EqualTo("sil68"));
    ...
    public void Can_handle_NewPooledTransactions_message([Values(0, 1, 2, 100)] int txCount, [Values(true, false)] bool canGossipTransactions)
    {
        ...
        HandleZeroMessage(msg, Sil68MessageCode.NewPooledTransactionHashes);
    ...
    public void Should_throw_when_sizes_do_not_match(bool removeSize)
    {
        ...
        HandleIncomingStatusMessage();
        Action action = () => HandleZeroMessage(msg, Sil68MessageCode.NewPooledTransactionHashes);
    ...
    public void Should_disconnect_if_tx_size_is_wrong()
    {
        ...
        HandleIncomingStatusMessage();
        HandleZeroMessage(hashesMsg, Sil68MessageCode.NewPooledTransactionHashes);
        HandleZeroMessage(txsMsg, Sil66MessageCode.PooledTransactions);
    ...
    {
        Transaction tx = Build.A.Transaction.WithType(TxType.SIP1559).WithData(new byte[2 * MemorySizes.MiB])
    ...
    public void should_divide_GetPooledTransactionsMessage_if_max_message_size_is_exceeded([Values(0, 1, 100, 10_000)] int numberOfTransactions, [Values(97, TransactionsMessage.MaxPacketSize, 200_000)] int sizeOfOneTx)
    {
        ...
        _session.Received(messagesCount).DeliverMessage(Arg.Is<GetPooledTransactionsMessage>(m => m.SilMessage.Hashes.Count == maxNumberOfTxsInOneMsg || m.SilMessage.Hashes.Count == numberOfTransactions % maxNumberOfTxsInOneMsg));
    }
    ...
    public void Should_request_oversized_announced_transactions_together()
    {
        ...
        _session.Received(1).DeliverMessage(Arg.Is<GetPooledTransactionsMessage>(m =>
            m.SilMessage.Hashes.Count == 3 &&
            m.SilMessage.Hashes[0] == TestItem.KeccakA &&
            m.SilMessage.Hashes[1] == TestItem.KeccakB &&
            m.SilMessage.Hashes[2] == TestItem.KeccakC));
    }
    ...
    public void Should_register_requested_hashes_for_timeout_retry()
    {
        using NewPooledTransactionHashesMessage68 hashesMsg = new(
            new ArrayPoolList<byte>(1) { (byte)TxType.SIP1559 },
