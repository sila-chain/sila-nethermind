...
public class PooledTransactionsRequestingTests
{
    ...
    public void Setup()
    {
        ...
        TestSingleReleaseSpecProvider specProvider = new(SilaOsaka.Instance);
        ...
        TxPool.TxPool txPool = new(
            new SilaEcdsa(specProvider.ChainId),
            new BlobTxStorage(),
        ...
        Transaction tx = Build.A.Transaction.WithShardBlobTxTypeAndFields(1).WithMaxPriorityFeePerGas(1).WithGasLimit(100)
            .SignedAndResolved(new SilaEcdsa(1), TestItem.PrivateKeyA).TestObject;
        ...
        Sil66ProtocolHandler handler2 = new(
            _session2,
            _svc,
            new NodeStatsManager(_timerFactory, LimboLogs.Instance),
            syncManager,
        ...
        HandleZeroMessage(_handler, hashesMsg1, Sil65MessageCode.NewPooledTransactionHashes);
        HandleZeroMessage(handler2, hashesMsg2, Sil65MessageCode.NewPooledTransactionHashes);
    }
    ...
    public async Task Should_request_from_others_after_timeout()
    {
        ...
        _session2.Received(1).DeliverMessage(Arg.Is<Network.P2P.Subprotocols.Sil.V66.Messages.GetPooledTransactionsMessage>(m => m.SilMessage.Hashes.Contains(_txs[0].Hash)));
    }
    ...
    public async Task Should_not_request_from_others_if_received()
    {
        ...
        HandleZeroMessage(_handler, new Network.P2P.Subprotocols.Sil.V66.Messages.PooledTransactionsMessage(1111, new PooledTransactionsMessage(_txs)), Sil65MessageCode.PooledTransactions);
        await Task.Delay(Timeout);

        _session2.Received(0).DeliverMessage(Arg.Is<Network.P2P.Subprotocols.Sil.V66.Messages.GetPooledTransactionsMessage>(m => m.SilMessage.Hashes.Contains(_txs[0].Hash)));
    }
    ...
    private void HandleIncomingStatusMessage(Sil66ProtocolHandler handler)
    {
    ...
    private void HandleZeroMessage<T>(Sil66ProtocolHandler handler, T msg, int messageCode) where T : MessageBase
    {
