...
public class EncryptionHandshakeServiceTests
{
    ...
    public void SetUp()
    {
        ...
        _messageSerializationService = new MessageSerializationService(
            ...
            SerializerInfo.Create(new AuthSip8MessageSerializer(new Sip8MessagePad(_testRandom))),
            SerializerInfo.Create(new AckMessageSerializer()),
            SerializerInfo.Create(new AckSip8MessageSerializer(new Sip8MessagePad(_testRandom)))
        );
    ...
    private readonly ISilaEcdsa _ecdsa = new SilaEcdsa(BlockchainIds.SilaSepolia); // TODO: separate general crypto signer from Sila transaction signing
    ...
    private void Auth(bool preSip8Format = false) => _auth = _initiatorService.Auth(NetTestVectors.StaticKeyB.PublicKey, _initiatorHandshake, preSip8Format);
    ...
    private void InitializeRandom(bool preSip8Format = false)
    {
    ...
    /// <summary>
    ///     https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-8.md
    /// </summary>
    [Test]
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Agrees_on_secrets(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
        Agree();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Initiator_secrets_are_not_null(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Recipient_secrets_are_not_null(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_ephemeral_key_on_ack(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_ephemeral_key_on_auth(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_initiator_nonce_on_ack(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_initiator_nonce_on_auth(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_recipient_nonce_on_ack(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_recipient_nonce_on_agree(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_remote_ephemeral_key_on_ack(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_remote_ephemeral_key_on_agree(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_remote_public_key_on_ack(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
        Ack();
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Sets_remote_public_key_on_auth(bool preSip8Format)
    {
        InitializeRandom(preSip8Format);
        Auth(preSip8Format);
