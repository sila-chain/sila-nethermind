...
namespace Nethermind.Network.Test.Rlpx.Handshake
{
    ...
    public class NettyHandshakeHandlerTests
    {
        ...
        private readonly Packet _ackPacket = new(NetTestVectors.AckSip8);
        private readonly Packet _authPacket = new(NetTestVectors.AuthSip8);
        ...
        public void Initiator_sends_auth_on_channel_activation()
        {
            ...
            _channelHandlerContext.When(x => x.WriteAndFlushAsync(Arg.Is<IByteBuffer>(b => Bytes.AreEqual(b.Array.Slice(b.ArrayOffset, NetTestVectors.AuthSip8.Length), NetTestVectors.AuthSip8))))
                .Do(c => received = true);
        ...
        public void Recipient_sends_ack_on_receiving_auth()
        {
            ...
            NettyHandshakeHandler handler = CreateHandler();
            _channelHandlerContext.Allocator.Returns(UnpooledByteBufferAllocator.Default);
            _channelHandlerContext.When(x => x.WriteAndFlushAsync(Arg.Is<IByteBuffer>(b => Bytes.AreEqual(b.Array.Slice(b.ArrayOffset, NetTestVectors.AckSip8.Length), NetTestVectors.AckSip8))))
                .Do(_ => received = true);
            handler.ChannelRead(_channelHandlerContext, Unpooled.Buffer(0, 0));
