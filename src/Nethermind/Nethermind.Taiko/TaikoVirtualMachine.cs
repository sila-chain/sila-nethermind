...
/// where <c>L1BlockHeight</c> is 0/null, or <c>sil_call</c> at a block with no stored origin) and
...
) : VirtualMachine<SilaGasPolicy>(blockHashProvider, specProvider, logManager)
{
    ...
    public override void SetBlockExecutionContext(in BlockExecutionContext blockExecutionContext)
    {
        ...
        // instead of hitting the store. null when origin is missing (preconf / sil_call / pre-genesis).
        ...
        IReleaseSpec spec)
    {
        ...
        {
            SilaGasPolicy gas = state.Gas;
            PrecompileExtras extras = new(
                remainingGas: SilaGasPolicy.GetRemainingGas(in gas),
            ...
            // On L1 OOG the user loses the full gas limit — matching standard SAVM sub-call semantics.
            ulong gasConsumed = output.Data.gasConsumed;
            if (gasConsumed > 0 && !SilaGasPolicy.UpdateGas(ref gas, gasConsumed))
            {
            ...
            if (!output)
            {
                return new(
                    ...
                    shouldRevert: true,
                    exceptionType: SavmExceptionType.PrecompileFailure
                )
                {
...
public sealed class TaikoSilaVirtualMachine(
    IBlockhashProvider? blockHashProvider,
    ISpecProvider? specProvider,
    IL1OriginStore l1OriginStore,
    ILogManager? logManager
