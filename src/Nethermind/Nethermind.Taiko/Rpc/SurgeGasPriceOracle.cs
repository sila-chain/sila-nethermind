...
public class SurgeGasPriceOracle : GasPriceOracle
{
    ...
    private const ulong BlobGasPerBlob = Sip4844Constants.GasPerBlob;
    ...
    private async ValueTask<L1FeeHistoryResults?> GetL1FeeHistory()
    {
        try
        {
            return await _l1RpcClient.Post<L1FeeHistoryResults?>("sil_feeHistory",
    ...
    private async ValueTask<string?> CallTaikoInboxFunction(string data)
    {
        try
        {
            return await _l1RpcClient.Post<string>("sil_call", new
            {
