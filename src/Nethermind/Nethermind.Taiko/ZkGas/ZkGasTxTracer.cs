...
public sealed class ZkGasTxTracer : TxTracer
{
    ...
    public override void ReportOperationRemainingGas(ulong gas)
    {
        ...
        if (IsSpawnOpcode(_currentOpcode))
        {
            ...
            //  3. Otherwise (CREATE/CREATE2 post-trace bail on SIP-7610 collision)
    ...
    /// </summary>
    /// <remarks>
    /// The SAVM specification burns all gas forwarded to a precompile when it errors (not reverts),
    ...
    /// <summary>
    /// Adjusts ZK gas for opcodes that fail the static-context check. RSAVM (used by alethia-reth)
    ...
    ///
    /// For sila-mainnet semantics this is invisible (the entire failed call frame's gas is consumed
    /// regardless), but for Taiko ZK gas accounting the per-opcode gas delta drives the
    /// consensus-relevant block.Header.Difficulty value, so we must match RSAVM's behaviour
    /// exactly. Charge the missing constant gas here so the resulting zk gas equals what
    /// alethia-reth produces.
    /// </summary>
    public override void ReportOperationError(SavmExceptionType error)
    {
        ...
        if (error == SavmExceptionType.StaticCallViolation)
        {
            ulong staticGas = GetRsavmStaticGasForOpcode(_currentOpcode);
            if (staticGas > 0)
            {
    ...
    /// <summary>
    /// RSAVM's per-opcode constant gas cost charged in the interpreter loop before dispatch.
    /// Source: rsavm-interpreter <c>instructions.rs</c> instruction-table entries.
    /// Only opcodes that can fail the static-context check are listed; other opcodes never
    /// reach <see cref="SavmExceptionType.StaticCallViolation"/>.
    /// </summary>
    private static ulong GetRsavmStaticGasForOpcode(byte opcode) => opcode switch
    {
        0x5d => 100,    // TSTORE — WarmStorageReadCostSIP2929 (SIP-1153)
        ...
        0xa3 => 375,    // LOG3
        0xa4 => 375,    // LOG4
        0xf0 => 0,      // CREATE — RSAVM dispatches with 0 static gas; rsavm-interpreter::contract::create handles internally
        0xf5 => 0,      // CREATE2
    ...
    ///    it must have been a post-trace bail (SIP-7610 collision);
    ///    RSAVM treats this as spawned too → spawn estimate.
