    ...
    : HeaderValidator(blockTree, sealValidator, specProvider, logManager)
{
    ...
    // SIP-4396 calculation parameters.
    ...
    protected override bool Validate1559(BlockHeader header, BlockHeader parent, IReleaseSpec spec, ref string? error)
    {
        ...
        {
            return ValidateSip4396Header(header, parent, spec, ref error);
        }

        return true;
    }

    private bool ValidateSip4396Header(BlockHeader header, BlockHeader parent, IReleaseSpec spec, ref string? error)
    {
        ...
        if (header.Number > 1)
        {
            ...
            {
                // Skip SIP-4396 verification due to unknown ancestor
                if (_logger.IsWarn)
                {
                    _logger.Warn($"Skipping SIP-4396 verification due to unknown ancestor, parent: {parent.Hash}, number: {parent.Number}");
                }
                return true;
            }
        ...
        // Calculate expected base fee using SIP-4396
        UInt256 expectedBaseFee = CalculateSip4396BaseFee(parent, parentBlockTime, spec);
    ...
    private static UInt256 CalculateSip4396BaseFee(BlockHeader parent, ulong parentBlockTime, IReleaseSpec spec)
    {
        ...
        ISip1559Spec sip1559Spec = spec;
        ulong parentGasTarget = parent.GasLimit / sip1559Spec.ElasticityMultiplier;
        ...
        UInt256 baseFee;
        UInt256 baseFeeChangeDenominator = sip1559Spec.BaseFeeMaxChangeDenominator;
        ...
        return ClampSip4396BaseFeeShasta(baseFee);
    }

    private static UInt256 ClampSip4396BaseFeeShasta(UInt256 baseFee)
    {
    ...
    protected override bool ValidateRequestsHash(BlockHeader header, IReleaseSpec spec, ref string? error)
    {
        ...
        if (taikoSpec.IsUnzenEnabled)
        {
            // Unzen pins RequestsHash to EMPTY_REQUESTS_HASH even though SilaPrague system contracts aren't active
