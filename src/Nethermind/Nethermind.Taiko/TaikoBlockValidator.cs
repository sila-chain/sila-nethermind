    ...
    ILogManager logManager) : BlockValidator(txValidator, headerValidator, unclesValidator, specProvider, logManager)
{
    ...
    // Blob transactions are explicitly rejected in ValidateTransactions below; no extra SIP-4844 field
    // validation is needed on Taiko L2.
    protected override bool ValidateSip4844Fields(Block block, IReleaseSpec spec, ref string? error) => true;
    ...
    private bool ValidateAnchorTransaction(Transaction tx, Block block, ITaikoReleaseSpec spec, out string? errorMessage)
    {
        ...
        if (tx.Type != TxType.SIP1559)
        {
            errorMessage = "Anchor transaction must be of type SIP-1559";
            return false;
        }
