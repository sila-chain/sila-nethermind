...
/// that fields required by SIP-4844 (<c>BlobGasUsed</c>, <c>ExcessBlobGas</c>) and SIP-4788
...
///
/// On a standard Sila chain these fields are populated by the block processor during normal
...
{
    /// <summary>
    /// Builds the genesis block via the wrapped builder and then stamps any zero-valued SIP-4844
    /// and SIP-4788 fields that must be present in the header when those forks are active at genesis.
    /// </summary>
    public Block Build()
    {
        ...
        if (genesisSpec.IsSip4844Enabled)
        {
            genesis.Header.BlobGasUsed ??= 0;
            genesis.Header.ExcessBlobGas ??= 0;
        }

        if (genesisSpec.IsSip4788Enabled)
        {
