...
/// <summary>
/// Handles retrospective opcode tracing by replaying historical transactions with actual SAVM execution.
...
public sealed class RetrospectiveExecutionTracer
{
    ...
    /// <summary>
    /// Processes a single block by replaying all its transactions with the SAVM.
    ...
    private long[] ProcessBlock(Block block, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (spec.IsSip4844Enabled && tracingHeader.ExcessBlobGas.HasValue)
            {
