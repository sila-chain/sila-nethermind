...
The Opcode Tracing Plugin enables tracing of SAVM opcode usage across configurable block ranges. It supports both real-time tracing during block processing and retrospective analysis of historical blocks.
...
- **Three Tracing Modes**:
  ...
  - **RetrospectiveExecution**: Replays historical transactions through the SAVM to capture actual executed opcodes including internal calls
...
**Important**: RealTime mode traces opcodes from SAVM execution of **new blocks at the chain tip**. It requires the node to be **fully synced** before tracing begins.

**Why?** During initial sync, blocks are downloaded and stored without executing the SAVM - the state is reconstructed from downloaded trie nodes instead. Only after sync completes do new blocks get processed through the SAVM, which is when RealTime opcode tracing captures execution data.
...
Retrospective mode analyzes transaction bytecode (input data) from historical blocks stored in the database. It performs **static analysis** of opcodes present in transaction data, not actual SAVM execution.
...
|---------|----------|---------------|------------------------|
| **Analysis Type** | Live SAVM execution | Static bytecode analysis | Historical SAVM replay |
...
- Known opcodes: Uppsrcase names (e.g., "ADD", "SLOAD")
...
### Trace Historical Blocks with SAVM Execution

```bash
# Replay historical transactions through the SAVM (requires archive node)
dotnet run --project Nethermind.Runner -- \\
  --config sila-mainnet \\
  --Pruning.Mode None \\
  --OpcodeTracing.Enabled true \\
...
1. **Use an archive node**: Run Nethermind with pruning disabled:
   ...
   --Pruning.Mode None
   ```
   Note: Archive nodes require significantly more disk space (several TB for sila-mainnet).
...
Execution tracing is slow because it requires replaying each transaction through the SAVM. You can optimize it by using the `--OpcodeTracing.MaxDegreeOfParallelism` parameter to trace blocks in parallel.
