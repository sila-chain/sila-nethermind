...
public interface IOpcodeTracingConfig : IConfig
{
    ...
    /// </summary>
    [ConfigItem(Description = "Tracing mode: RealTime (trace during processing), Retrospective (static bytecode analysis from database), or RetrospectiveExecution (full SAVM replay from database).", DefaultValue = "\"RealTime\"")]
    string Mode { get; set; }
}
