...
public partial class Bls12381G2MsmPrecompile
{
    ...
    private Result<byte[]> Mul(ReadOnlyMemory<byte> inputData)
    {
        ...
        Result result = x.TryDecodeRaw(inputData[..Sip2537.LenG2].Span);
        ...
        bool scalarIsZero = !inputData[Sip2537.LenG2..].Span.ContainsAnyExcept((byte)0);

        if (scalarIsZero || x.IsInf())
            return Sip2537.G2Infinity;

        Span<byte> scalar = stackalloc byte[32];
        inputData.Span[Sip2537.LenG2..].CopyTo(scalar);
        scalar.Reverse();
    ...
    private Result<byte[]> Msm(ReadOnlyMemory<byte> inputData, int nItems)
    {
        ...
        for (int i = 0; i < nItems; i++)
        {
            ...
            ReadOnlySpan<byte> rawPoint = inputData[offset..(offset + Sip2537.LenG2)].Span;
...
#pragma warning disable CS0162 // Unreachable code detected
        if (Sip2537.DisableConcurrency)
        {
            ...
            {
                result = Sip2537.TryDecodeG2ToBuffer(inputData, pointBuffer.AsMemory(), scalarBuffer.AsMemory(), pointDestinations[i], i);
            }
        }
        else
        {
            ...
            {
                Result local = Sip2537.TryDecodeG2ToBuffer(inputData, pointMemory, scalarMemory, pointDestinations[index], index);
                if (!local)
                {
                    result = local;
                    state.Break();
