...
public partial class Bls12381G1MsmPrecompile
{
    ...
    private Result<byte[]> Mul(ReadOnlyMemory<byte> inputData)
    {
        ...
        Result result = x.TryDecodeRaw(inputData[..Sip2537.LenG1].Span);

        if (!result)
            return result.Error!;

        if (!(Sip2537.DisableSubgroupChecks || x.InGroup()))
        ...
        bool scalarIsZero = !inputData.Span[Sip2537.LenG1..].ContainsAnyExcept((byte)0);

        if (scalarIsZero || x.IsInf())
            return Sip2537.G1Infinity;

        Span<byte> scalar = stackalloc byte[32];
        inputData.Span[Sip2537.LenG1..].CopyTo(scalar);
        scalar.Reverse();
    ...
    private Result<byte[]> Msm(ReadOnlyMemory<byte> inputData, int nItems)
    {
        ...
        for (int i = 0; i < nItems; i++)
        {
            ...
            ReadOnlySpan<byte> rawPoint = inputData[offset..(offset + Sip2537.LenG1)].Span;
...
#pragma warning disable CS0162 // Unreachable code detected
        if (Sip2537.DisableConcurrency)
        {
            for (int i = 0; i < pointDestinations.Count && result; i++)
            {
                result = Sip2537.TryDecodeG1ToBuffer(inputData, rawPoints.AsMemory(), rawScalars.AsMemory(), pointDestinations[i], i);
            }
        }
        else
        {
            ...
            {
                Result local = Sip2537.TryDecodeG1ToBuffer(inputData, rawPointsMemory, rawScalarsMemory, pointDestinations[index], index);
                if (!local)
                {
                    result = local;
                    state.Break();
