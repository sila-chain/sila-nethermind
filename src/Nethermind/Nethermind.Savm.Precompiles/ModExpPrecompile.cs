...
/// <summary>
/// <see href="https://sips.sila.org/SIPS/sip-198" />
/// </summary>
public partial class ModExpPrecompile : IPrecompile<ModExpPrecompile>
{
    /// <summary>
    /// Maximum input size (in bytes) for the modular exponentiation operation under SIP-7823.
    /// This constant defines the upper limit for the size of the input data that can be processed.
    /// <see href="https://sips.sila.org/SIPS/sip-7823" />
    /// </summary>
    public const uint ModExpMaxInputSizeSip7823 = 1024;
    ...
    private static readonly ulong IterationCountMultiplierSip2565 = 8;
    private static readonly ulong IterationCountMultiplierSip7883 = 16;
    ...
    /// <summary>
    /// <see href="https://sips.sila.org/SIPS/sip-2565" />
    ...
    /// <returns>Gas cost of the MODEXP operation in the context of SIP2565</returns>
    public ulong DataGasCost(ReadOnlyMemory<byte> inputData, IReleaseSpec releaseSpec)
    {
        if (!releaseSpec.IsSip2565Enabled)
        {
#pragma warning disable 618
            return ModExpPrecompilePreSip2565.Instance.DataGasCost(inputData, releaseSpec);
#pragma warning restore 618
    ...
    private static ulong DataGasCostInternal(ReadOnlySpan<byte> inputData, IReleaseSpec releaseSpec)
    {
        ...
        ulong complexity = MultComplexity(baseLength, modulusLength, releaseSpec.IsSip7883Enabled);
        ...
        UInt256 iterationCount = CalculateIterationCount(expLength, exp, releaseSpec.IsSip7883Enabled);

        bool overflow = UInt256.MultiplyOverflow(complexity, iterationCount, out UInt256 result);

        if (!releaseSpec.IsSip7883Enabled)
        {
            result /= 3;
        }
        ...
        return result > ulong.MaxValue || overflow
            ? ulong.MaxValue
            : Math.Max(releaseSpec.IsSip7883Enabled ? GasCostOf.MinModExpSip7883 : GasCostOf.MinModExpSip2565, (ulong)result);
    }
    ...
    /// </summary>
    /// <returns></returns>
    private static ulong MultComplexity(uint baseLength, uint modulusLength, bool isSip7883Enabled)
    {
        ...
        // If SIP-7883 => small-case = 16, else 2*sq when max>32
        if (isSip7883Enabled)
        {
    ...
    /// <param name="isSip7883Enabled"></param>
    /// <returns></returns>
    private static UInt256 CalculateIterationCount(uint exponentLength, UInt256 exponent, bool isSip7883Enabled)
    {
        ...
        else
        {
            ...
            ulong multiplicationResult = (exponentLength - LengthSize) * (isSip7883Enabled ? IterationCountMultiplierSip7883 : IterationCountMultiplierSip2565);
    ...
    private static bool ExceedsMaxInputSize(IReleaseSpec releaseSpec, uint baseLength, uint expLength, uint modulusLength) => releaseSpec.IsSip7823Enabled
            ? (baseLength > ModExpMaxInputSizeSip7823 || expLength > ModExpMaxInputSizeSip7823 || modulusLength > ModExpMaxInputSizeSip7823)
