...
internal static class TestEnrBuilder
{
    ...
    private static void Sign(NodeRecord enr, PrivateKey privateKey) => new NodeRecordSigner(new SilaEcdsa(0), privateKey).Sign(enr);
}

internal sealed class TestEth2Entry() : EnrContentEntry<byte[]>([1, 2, 3, 4])
{
    public override string Key => EnrContentKey.Sil2;
