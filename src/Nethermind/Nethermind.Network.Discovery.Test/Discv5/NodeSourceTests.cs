...
public class NodeSourceTests
{
    ...
    {
        Node incompatibleNode = CreateNode(1, silForkHash: IncompatibleForkHash);
        Node compatibleNode = CreateNode(2, silForkHash: CompatibleForkHash);
        IKademlia<PublicKey, Node> kademlia = Substitute.For<IKademlia<PublicKey, Node>>();
    ...
    private static Node CreateNode(int index, int tcpPort = 30303, int udpPort = 30304, bool includeEth2 = false, uint? silForkHash = null)
    {
        ...
        NodeRecord enr = TestEnrBuilder.BuildSigned(
            ...
            configureExtras: enr =>
            {
                ...
                if (silForkHash is { } forkHash) enr.SetEntry(new SilEntry(new ForkId(forkHash, 0).HashBytes, 0));
            });
