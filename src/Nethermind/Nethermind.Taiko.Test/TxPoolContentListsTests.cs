...
public class TxPoolContentListsTests
{
        ...
        int maxTransactionsLists)
    {
        ...
        return result.Data!.Select(static list => list.TxList.OfType<SIP1559TransactionForRpc>().Select(static tx => (int)tx.Input![0]).ToArray()).ToArray();
    }
                ...
                [
                    txs.ToDictionary(
                        ...
                        static kv => kv.Value.Select(static txId =>
                            Build.A.Transaction.WithType(TxType.SIP1559).WithMaxFeePerGas(7).WithNonce(1).WithValue(1).WithGasPrice(20).WithData([(byte)txId]).SignedAndResolved().TestObject
                        ).ToArray()),
        ...
        ulong surgeMaxGasLimitRatio, int expectedBatchCount)
    {
        ...
        Transaction tx1 = Build.A.Transaction
            .WithType(TxType.SIP1559).WithMaxFeePerGas(100)
