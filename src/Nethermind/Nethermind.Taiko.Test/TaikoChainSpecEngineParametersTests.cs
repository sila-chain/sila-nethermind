...
/// <summary>
/// Regression tests for the SIP-2124 fork-id walk on Taiko and the chainspec-driven Unzen ZK gas
...
/// extra/missing entry here changes the fork-id NMC announces in sil Status — observed against
...
public class TaikoChainSpecEngineParametersTests
{
    ...
    public void AddTransitions_skips_genesis_time_Shasta_and_Unzen()
    {
        ...
        Assert.That(blockNumbers, Is.Empty,
            "Ontake/Pacaya at block 0 must be filtered — SIP-2124 skips activations at or before genesis");
    ...
    public void Hoodi_Unzen_flips_osaka_semantics_and_zk_gas_schedule_atomically()
    {
        ...
        // Taiko executes Unzen with SilaOsaka semantics: alethia-reth pins SilaCancun, SilaPrague, and SilaOsaka
        ...
        [
            ...
            (nameof(ITaikoReleaseSpec.IsSip1153Enabled), static s => s.IsSip1153Enabled),
            (nameof(ITaikoReleaseSpec.IsSip4788Enabled), static s => s.IsSip4788Enabled),
            (nameof(ITaikoReleaseSpec.IsSip4844Enabled), static s => s.IsSip4844Enabled),
            (nameof(ITaikoReleaseSpec.IsSip5656Enabled), static s => s.IsSip5656Enabled),
            (nameof(ITaikoReleaseSpec.IsSip6780Enabled), static s => s.IsSip6780Enabled),
            (nameof(ITaikoReleaseSpec.IsSip2537Enabled), static s => s.IsSip2537Enabled),
            (nameof(ITaikoReleaseSpec.IsSip2935Enabled), static s => s.IsSip2935Enabled),
            (nameof(ITaikoReleaseSpec.IsSip6110Enabled), static s => s.IsSip6110Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7002Enabled), static s => s.IsSip7002Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7251Enabled), static s => s.IsSip7251Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7623Enabled), static s => s.IsSip7623Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7702Enabled), static s => s.IsSip7702Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7594Enabled), static s => s.IsSip7594Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7823Enabled), static s => s.IsSip7823Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7825Enabled), static s => s.IsSip7825Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7883Enabled), static s => s.IsSip7883Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7918Enabled), static s => s.IsSip7918Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7934Enabled), static s => s.IsSip7934Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7939Enabled), static s => s.IsSip7939Enabled),
            (nameof(ITaikoReleaseSpec.IsSip7951Enabled), static s => s.IsSip7951Enabled),
        ];
    ...
    public void Unzen_schedules_deserialize_from_chainspec_with_full_precompile_addresses()
    {
        ...
        // SAVM and Taiko-extended precompiles (e.g. L1Sload @ 0x…010001) live in the same table
        ...
        TaikoChainSpecEngineParameters parameters =
            new SilaJsonSerializer().Deserialize<TaikoChainSpecEngineParameters>(json);
        ...
        Assert.That(schedule.PrecompileMultipliers["0x0000000000000000000000000000000000010001"], Is.EqualTo(200L),
            "Taiko-extended precompile address coexists with canonical low-byte SAVM precompiles");
    }
}
