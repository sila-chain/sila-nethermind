...
public class ZkGasMeterTests
{
    ...
    public void OpcodeMultipliers_Spot_Check_Spec_Values()
    {
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(ZkGasTestSchedules.OpcodeMultipliers.Span[0x1e], Is.EqualTo((ushort)14)); // clz (SIP-7939)
    ...
    public void PrecompileMultipliers_Spot_Check_Spec_Values()
    {
        using (Assert.EnterMultipleScope())
        {
            Assert.That(ZkGasTestSchedules.PrecompileMultiplier(0x05), Is.EqualTo((ushort)154)); // modexp — SIP-7883 6× gas-cost slope
            ...
            Assert.That(ZkGasTestSchedules.PrecompileMultiplier(0x0d), Is.EqualTo((ushort)230)); // bls12_g2add (SIP-2537)
            Assert.That(ZkGasTestSchedules.PrecompileMultiplier(0x10), Is.EqualTo((ushort)246)); // bls12_map_fp_to_g1
            Assert.That(ZkGasTestSchedules.PrecompileMultiplier(0x11), Is.EqualTo((ushort)208)); // bls12_map_fp2_to_g2 (last canonical SIP-2537 slot)
            ...
            Assert.That(ZkGasTestSchedules.PrecompileMultipliers.ContainsKey(Address.FromNumber(0x12)), Is.False,
                "0x12 sat on the pre-final SIP-2537 draft and is not in the canonical SilaOsaka set — meter charges fail-safe");
    ...
    {
        // SIP-7939 added CLZ at opcode 0x1e in SilaOsaka, which Unzen extends. The default Alethia
        // schedule charges it at multiplier 14.
        const byte clz = 0x1e;
        const ulong rawGas = 5; // CLZ base gas cost under SIP-7939
    ...
    {
        // RIP-7212 / p256verify lives at 0x100 and is active wherever Unzen extends SilaOsaka.
        // The default Alethia schedule lists it at multiplier 163.
        Address p256Verify = Address.FromNumber(0x100);
        const ulong gasUsed = 6_900; // SecP256r1Precompile.BaseGasCost under SIP-7951
