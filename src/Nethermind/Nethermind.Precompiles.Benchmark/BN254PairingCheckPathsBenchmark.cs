...
{
    // SIP-197 vector with e(P, Q) * e(-P, Q) == 1, so repeating the block leaves the product equal to one.
    ...
    [Benchmark]
    public Result<byte[]> CheckPairing() => Input.Precompile.Run(Input.Bytes, SilaCancun.Instance);
}
