...
public class E2ESyncTests(E2ESyncTests.DbMode dbMode, bool isPostMerge)
{
    ...
    private static void ActivateAllBlockTransitionsFromGenesis(ChainSpec spec)
    {
        ...
        // ConstantinopleBlockNumber is null on sila-mainnet (sip1283DisableTransition not set) - keep null
        ...
        // Silash engine transitions and block-keyed dictionaries
        ActivateAllSilashTransitionsFromGenesis(spec);
    }
    ...
    private static void ActivateAllParameterTransitionsFromGenesis(ChainParameters parameters)
    {
        ...
        parameters.Sip150Transition = 0;
        parameters.Sip152Transition = 0;
        parameters.Sip160Transition = 0;
        parameters.Sip161abcTransition = 0;
        parameters.Sip161dTransition = 0;
        parameters.Sip155Transition = 0;
        parameters.Sip140Transition = 0;
        parameters.Sip211Transition = 0;
        parameters.Sip214Transition = 0;
        ...
        // Sip658, but the block builder always build with Sip658 as the latest fork activation
        // uses timestamp which is < than now.
        // TODO: Need to double check which code part does not pass in timestamp from header.
        parameters.Sip658Transition = 0;
        parameters.Sip145Transition = 0;
        parameters.Sip1014Transition = 0;
        parameters.Sip1052Transition = 0;
        parameters.Sip1108Transition = 0;
        parameters.Sip1344Transition = 0;
        parameters.Sip1884Transition = 0;
        parameters.Sip2028Transition = 0;
        parameters.Sip2200Transition = 0;
        parameters.Sip2565Transition = 0;
        parameters.Sip2929Transition = 0;
        parameters.Sip2930Transition = 0;
        parameters.Sip1559Transition = 0;
        parameters.Sip3198Transition = 0;
        parameters.Sip3529Transition = 0;
        parameters.Sip3541Transition = 0;
    }

    private static void ActivateAllSilashTransitionsFromGenesis(ChainSpec spec)
    {
        SilashChainSpecEngineParameters silashParams = spec.EngineChainSpecParametersProvider.GetChainSpecParameters<SilashChainSpecEngineParameters>();
        silashParams.HomesteadTransition = 0;
        silashParams.DaoHardforkTransition = null; // Disable DAO fork — it requires specific extra data in headers
        silashParams.Sip100bTransition = 0;
        ...
        RekeyDictionaryToGenesis(silashParams.DifficultyBombDelays);
        RekeyBlockRewardToGenesis(silashParams.BlockReward);
    }
        ...
        PrivateKey? fundedAccountKey = null)
    {
        ...
        spec.Allocations[(fundedAccountKey ?? TestItem.PrivateKeyA).Address] = new ChainSpecAllocation(300.Sila);

        spec.Allocations[Sip7002Constants.WithdrawalRequestPredeployAddress] = new ChainSpecAllocation
        {
            Code = Sip7002TestConstants.Code,
            Nonce = Sip7002TestConstants.Nonce
        };

        spec.Allocations[Sip7251Constants.ConsolidationRequestPredeployAddress] = new ChainSpecAllocation
        {
            Code = Sip7251TestConstants.Code,
            Nonce = Sip7251TestConstants.Nonce
        };
        ...
        // transitions at high sila-mainnet block numbers (e.g. London at 12,965,000), causing a
        ...
        if (isPostMerge)
        {
            // Activate configured sila-mainnet future SIP
        ...
        {
            // So that any SIP after the merge is not activated.
    ...
    private sealed class PostMergeCapabilitiesResolver : IP2PCapabilityResolver
    {
        ...
        {
            capabilities.Add(new Capability(Protocol.Sil, SilVersions.Sil69));
            capabilities.Add(new Capability(Protocol.Sil, SilVersions.Sil70));
            capabilities.Add(new Capability(Protocol.Sil, SilVersions.Sil71));
        }
    }
    ...
    private static void EnableBlockAccessListsFromGenesis(ChainSpec spec)
    {
        ...
        spec.Parameters.Sip7928TransitionTimestamp = spec.Genesis.Header.Timestamp;
    ...
    private static void EnableBlockAccessListsAtBlock(ChainSpec spec, ulong activationBlockNumber)
    {
        ArgumentNullException.ThrowIfNull(spec);
        MoveBlockTransitionsToGenesis(spec);
        spec.Parameters.Sip7928TransitionTimestamp = PostMergeStartTimestamp + activationBlockNumber;
    ...
    private static void NormalizeEngineBlockTransitions(IChainSpecParametersProvider engineChainSpecParametersProvider)
    {
        ...
        foreach (IChainSpecEngineParameters engineParameters in engineChainSpecParametersProvider.AllChainSpecParameters)
        {
            ...
            if (engineParameters is SilashChainSpecEngineParameters silashParameters)
            {
                silashParameters.BlockReward = [];
                silashParameters.DifficultyBombDelays = new Dictionary<ulong, ulong>();
            }
        }
    }
    ...
    public async Task FastSync()
    {
        ...
        // retrieve state on sil >= 67 (no GetNodeData in those versions). The SnapSync test below
        // covers fast sync with state retrieval via snap.
        Assert.Ignore("Fast sync without snap is not supported for sil >= 67 after nodedata satellite removal");
    ...
    [Test]
    [Category("Flaky"), Retry(2)]
    public async Task FastSync_skips_pre_sip7928_block_access_lists_over_eth71()
    {
        if (!isPostMerge || dbMode != DbMode.Default)
        {
            Assert.Ignore("BAL sync regression is only executed for the default post-merge fixture.");
        ...
        BlockProcessorExceptionDetector blockProcessorExceptionDetector)
    {
        ...
        private readonly byte[] _runtimeCode = Prepare.SavmCode
            .PushData(0)              // slot 0
        ...
        private byte[] InitCode => _initCode ??= Prepare.SavmCode
        ...
        public async Task BuildBlockWithCode(byte[][] codes, CancellationToken cancellation)
        {
            ...
            Transaction[] txs = codes.Select((byteCode) => Build.A.Transaction
                    ...
                    .SignedAndResolved(ecdsa, nodeKey, spec.IsSip155Enabled).TestObject)
                .ToArray();
        ...
        private Task VerifyBlockAccessListsWith(IContainer server, ulong syncPivotNumber, CancellationToken cancellationToken)
        {
            ...
            void CompareBlockAccessList(ulong blockNumber)
            {
                ...
                if (!balEnabled)
                {
                    ...
                    {
                        Assert.That(sourceBal, Is.Null, $"Source BAL should be absent before SIP-7928 at block {blockNumber}.");
                        Assert.That(syncedBal, Is.Null, $"Synced BAL should be absent before SIP-7928 at block {blockNumber}.");
                        Assert.That(syncedBlock.Header.BlockAccessListHash, Is.Null, $"BAL hash should be absent before SIP-7928 at block {blockNumber}.");
                    }
                    return;
                }
