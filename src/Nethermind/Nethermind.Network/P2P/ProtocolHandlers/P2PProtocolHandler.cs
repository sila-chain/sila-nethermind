    ...
    : ProtocolHandlerBase(session, nodeStatsManager, serializer, backgroundTaskScheduler, logManager), IPingSender, IP2PProtocolHandler
{
    ...
    public override void HandleMessage(Packet msg)
    {
            ...
            case P2PMessageCode.Hello:
                {
                    ...
                    // which should be constant for the whole session. Some protocols (like Sil) are sending messages
            ...
            case P2PMessageCode.Disconnect:
                {
                    ...
                    SilDisconnectReason disconnectReason =
                        FastEnum.IsDefined((SilDisconnectReason)disconnectMessage.Reason)
                            ? (SilDisconnectReason)disconnectMessage.Reason
                            : SilDisconnectReason.Other;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        void TraceDisconnect(int reason, SilDisconnectReason disconnectReason)
            => Logger.Trace(!FastEnum.IsDefined((SilDisconnectReason)reason)
    ...
    private void Close(SilDisconnectReason silDisconnectReason)
    {
        Dispose();
        if (silDisconnectReason != SilDisconnectReason.TooManyPeers &&
            silDisconnectReason != SilDisconnectReason.Other &&
            silDisconnectReason != SilDisconnectReason.DisconnectRequested)
        {
            if (Logger.IsDebug) DebugReceivedDisconnect(silDisconnectReason);
        }
        else
        {
            if (Logger.IsTrace) TraceReceivedDisconnect(silDisconnectReason);
        }

        // Received disconnect message, triggering direct TCP disconnection
        Session.MarkDisconnected(silDisconnectReason.ToDisconnectReason(), DisconnectType.Remote, "message");

        [MethodImpl(MethodImplOptions.NoInlining)]
        void DebugReceivedDisconnect(SilDisconnectReason reason)
            => Logger.Debug($"{Session} received disconnect [{reason}]");

        [MethodImpl(MethodImplOptions.NoInlining)]
        void TraceReceivedDisconnect(SilDisconnectReason reason)
