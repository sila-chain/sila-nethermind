...
namespace Nethermind.Network.P2P.Subprotocols.Sil.V70;

/// <summary>
/// https://sips.sila.org/SIPS/sip-7975 - partial block receipt lists.
/// </summary>
public class Sil70ProtocolHandler : Sil69ProtocolHandler, IStaticProtocolInfo
{
    ...
    public override string Name => "sil70";

    public new static byte Version => SilVersions.Sil70;
    ...
    protected override bool HandleMessageCore(ZeroPacket message)
    {
        ...
        {
            case Sil70MessageCode.Receipts:
                ...
                return true;
            case Sil70MessageCode.GetReceipts:
    ...
    private ReceiptsResponse FulfillReceiptsRequest(GetReceiptsMessage70 getReceiptsMessage, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            for (int blockIndex = 0; blockIndex < hashes.Length; blockIndex++)
            {
                ...
                if (taken == 0)
                {
                    throw new SubprotocolException($"Single receipt exceeds hard sil/70 receipts response size limit ({HardOutgoingReceiptsMessageSizeLimit} bytes)");
                }
    ...
    private async Task<(IOwnedReadOnlyList<TxReceipt[]>, long)> SendGetReceiptsWithPaging(IOwnedReadOnlyList<Hash256> blockHashes, CancellationToken token)
    {
...

        {
            ...
            for (int i = 0; i < blockHashesSpan.Length; i++)
            {
                ...
                receiptRlpBehaviors.Add(spec.IsSip658Enabled ? RlpBehaviors.Sip658Receipts : RlpBehaviors.None);
                validateReceiptGasUpperBoundAgainstHeader.Add(!spec.IsSip8037Enabled);
                validateReceiptGasEqualToHeader.Add(!spec.IsSip7778Enabled && !spec.IsSip8037Enabled);
            }
        }
        ...
        try
        {
            while (blockIndex < blockHashes.Count)
            {
                ...
                using (response)
                {
                    ...
                    {
                        throw new SubprotocolException($"Received sil/70 receipts response above hard limit ({size} bytes > {HardOutgoingReceiptsMessageSizeLimit} bytes)");
                    }

                    if (response.TxReceipts.Count > blockHashes.Count - blockIndex)
                    {
                        throw new SubprotocolException("Received more receipts than requested in sil/70 response");
                    }
    ...
    private ulong GetReceiptSize(TxReceipt receipt, ref ulong lastBlockNumber, ref RlpBehaviors behaviors)
    {
        if (receipt.BlockNumber != lastBlockNumber)
        {
            ...
            behaviors = receiptSpec.IsSip658Enabled ? RlpBehaviors.Sip658Receipts : RlpBehaviors.None;
        }
