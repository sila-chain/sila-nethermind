...
{
    public class AuthSip8MessageSerializer(IMessagePad messagePad) : IZeroMessageSerializer<AuthSip8Message>
    {
        private readonly IMessagePad _messagePad = messagePad;

        public void Serialize(IByteBuffer byteBuffer, AuthSip8Message msg)
        {
        ...
        public static int GetLength(AuthSip8Message msg)
        {
        ...
        public AuthSip8Message Deserialize(IByteBuffer msgBytes) =>
            msgBytes.DeserializeRlp(Deserialize);

        private static AuthSip8Message Deserialize(ref RlpReader ctx)
        {
            AuthSip8Message authMessage = new();
            ctx.ReadSequenceLength();
            ReadOnlySpan<byte> sigAllBytes = ctx.DecodeByteArraySpan(RlpLimit.L65);
            Signature signature = new(sigAllBytes[..64], sigAllBytes[64]); // since Signature class is Sila style it expects V as the 65th byte, hence we use RecoveryID constructor
