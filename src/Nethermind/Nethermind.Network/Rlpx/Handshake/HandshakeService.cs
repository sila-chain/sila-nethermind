...
{
    /// <summary>
    ///     https://github.com/sila-chain/devp2p/blob/master/rlpx.md
    /// </summary>
    public class HandshakeService : IHandshakeService
    {
        ...
        public Packet Auth(PublicKey remoteNodeId, EncryptionHandshake handshake, bool preSip8Format = false)
        {
            ...
            {
                AuthSip8Message authMessage = new()
                {
                    Nonce = handshake.InitiatorNonce,
                    PublicKey = _privateKey.PublicKey,
                    Signature = _ecdsa.Sign(handshake.EphemeralPrivateKey, new ValueHash256(forSigning))
        ...
        public Packet Ack(EncryptionHandshake handshake, Packet auth)
        {
            ...
            bool preSip8Format = false;
            byte[] plainText = null;
            try
            {
                if (_logger.IsTrace) _logger.Trace($"Trying to decrypt an old version of {nameof(AuthMessage)}");
                (preSip8Format, plainText) = _eciesCipher.Decrypt(_privateKey, auth.Data);
            }
            catch (Exception ex)
            {
            ...
            {
                if (_logger.IsTrace) _logger.Trace($"Trying to decrypt version 4 of {nameof(AuthSip8Message)}");
                byte[] sizeData = auth.Data.Slice(0, 2);
                (_, plainText) = _eciesCipher.Decrypt(_privateKey, auth.Data.Slice(2), sizeData);
                authMessage = _messageSerializationService.Deserialize<AuthSip8Message>(plainText);
            }
            ...
            {
                if (_logger.IsTrace) _logger.Trace($"Building an {nameof(AckSip8Message)}");
                AckSip8Message ackMessage = new()
                {
                    EphemeralPublicKey = handshake.EphemeralPrivateKey.PublicKey,
                    Nonce = handshake.RecipientNonce
                };
        ...
        public void Agree(EncryptionHandshake handshake, Packet ack)
        {
            ...
            try
            {
                (preSip8Format, plainText) = _eciesCipher.Decrypt(_privateKey, ack.Data);
            }
            catch (Exception ex)
            {
            ...
            else
            {
                ...
                AckSip8Message ackSip8Message = _messageSerializationService.Deserialize<AckSip8Message>(plainText);
                if (_logger.IsTrace) _logger.Trace($"Received ACK v{ackSip8Message.Version}");

                handshake.RemoteEphemeralPublicKey = ackSip8Message.EphemeralPublicKey;
                handshake.RecipientNonce = ackSip8Message.Nonce;
            }
