...
namespace Nethermind.Mining.Test
{
    ...
    public class MinGasPriceTests
    {
        ...
        public void Test1559(long minimum, long maxFeePerGas, long maxPriorityFeePerGas, bool expectedResult)
        {
            ...
            specProvider.GetSpec(Arg.Any<ForkActivation>()).IsSip1559Enabled.Returns(true);
            specProvider.GetSpec(Arg.Any<ForkActivation>()).BaseFeeCalculator.Returns(new DefaultBaseFeeCalculator());

            specProvider.GetSpec(Arg.Any<ForkActivation>()).ForkBaseFee.Returns(Sip1559Constants.DefaultForkBaseFee);
            specProvider.GetSpec(Arg.Any<ForkActivation>()).BaseFeeMaxChangeDenominator.Returns(Sip1559Constants.DefaultBaseFeeMaxChangeDenominator);
            specProvider.GetSpec(Arg.Any<ForkActivation>()).ElasticityMultiplier.Returns(Sip1559Constants.DefaultElasticityMultiplier);
