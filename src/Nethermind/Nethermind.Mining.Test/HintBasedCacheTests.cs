...
{
    private class NullDataSet : ISilashDataSet
    {
        public void Dispose()
        {
        }
    ...
    public void Sync_hint_and_get()
    {
        ...
        Assert.That(hintBasedCache.Get(Silash.GetEpoch(200000UL)), Is.Not.Null);
    }
    ...
    public async Task Many_threads()
    {
        ...
        for (uint i = (uint)(range / Silash.EpochLength); i < (uint)((range + 120000) / Silash.EpochLength); i++)
        {
            Assert.That(hintBasedCache.Get(i), Is.Not.Null);
        }
    }
