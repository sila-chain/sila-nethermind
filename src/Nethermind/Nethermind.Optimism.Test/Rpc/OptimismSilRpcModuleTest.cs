...
public class OptimismEthRpcModuleTest
{
    ...
    public async Task Sequencer_send_transaction_with_signature_will_not_try_to_sign()
    {
        ...
        SilaEcdsa silaEcdsa = new(chainId: TestBlockchainIds.ChainId);
        ...
        Transaction tx = Build.A.Transaction
            .Signed(ecdsa: silaEcdsa, privateKey: TestItem.PrivateKeyA)
            .TestObject;
        string serialized = await rpcBlockchain.TestEthRpc("sil_sendRawTransaction", Rlp.Encode(item: tx, behaviors: RlpBehaviors.None).Bytes.ToHexString());
    ...
    public async Task GetTransactionByHash_ReturnsCorrectTransactionType()
    {
        ...
        string serialized = await rpcBlockchain.TestEthRpc("sil_getTransactionByHash", TestItem.KeccakA);
        string expected = $$"""
                         {
                            "jsonrpc":"2.0",
                            "result": {
    ...
    public async Task GetTransactionByBlockAndIndex_ReturnsCorrectTransactionType()
    {
        ...
        {
            // By block hash
            string serialized = await rpcBlockchain.TestEthRpc("sil_getTransactionByBlockHashAndIndex", block.Hash, "0x0");
            Assert.That(JToken.Parse(serialized), Is.EqualTo(JToken.Parse(expected)).Using(JToken.EqualityComparer));
        }
        {
            // By block number
            string serialized = await rpcBlockchain.TestEthRpc("sil_getTransactionByBlockNumberAndIndex", block.Number, "0x0");
            Assert.That(JToken.Parse(serialized), Is.EqualTo(JToken.Parse(expected)).Using(JToken.EqualityComparer));
        }
    }
        ...
        ReceiptOrder order, string? expectedDepositVersion)
    {
        ...
        string serialized = await rpcBlockchain.TestEthRpc("sil_getBlockByHash", block.Hash, true);
    ...
    public async Task GetBlockReceipts_ReturnsDefaultAndOptimismReceipts()
    {
        ...
        string serialized = await rpcBlockchain.TestEthRpc("sil_getBlockReceipts", new BlockParameter(block.Number));
        string expected = $$"""
                         {
                            "jsonrpc":"2.0",
                            "result": [
    ...
    public async Task GetTransactionReceipt_ReturnsDepositReceipt()
    {
        ...
        string serialized = await rpcBlockchain.TestEthRpc("sil_getTransactionReceipt", tx.Hash);
        string expected = $$"""
                         {
                            "jsonrpc":"2.0",
                            "result": {
...
internal static class TestRpcBlockchainExt
{
    public static TestRpcBlockchain.Builder<TestRpcBlockchain> WithOptimismEthRpcModule(
        ...
        @this.WithEthRpcModule(blockchain => new OptimismEthRpcModule(
            ...
            new SilSyncingInfo(blockchain.BlockTree, Substitute.For<ISyncPointers>(), new SyncConfig(),
