...
public class OptimismPayloadAttributesTests
{
    ...
    public void Compute_PayloadID_with_SIP1559Params((string HexStringSIP1559Params, string PayloadId) testCase)
    {
        ...
        OptimismPayloadAttributes payloadAttributes = new()
        {
            ...
            SIP1559Params = Bytes.FromHexString(testCase.HexStringSIP1559Params)
        };

        Assert.That(payloadAttributes.GetPayloadId(blockHeader), Is.EqualTo(testCase.PayloadId));
    }

    private static IEnumerable<(int? length, Valid isValid)> Validate_SIP1559Params_TestCases()
    {
    ...
    [Test]
    public void Validate_SIP1559Params(
        [ValueSource(nameof(Validate_SIP1559Params_TestCases))] (int? length, Valid isValid) testCase,
    ...
    )
    {
        OptimismPayloadAttributes payloadAttributes = new()
        {
            ...
            SIP1559Params = testCase.length is { } length ? new byte[length] : null,
