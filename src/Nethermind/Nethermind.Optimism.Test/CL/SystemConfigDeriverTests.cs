...
public class SystemConfigDeriverTests
{
    ...
    [Test]
    public void UpdateSystemConfigFromL1BLock_UpdatedSIP1559Params()
    {
        byte[] sip1559ParamsRaw = [0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8];
        UInt256 sip1559Params = new(sip1559ParamsRaw, isBigEndian: true);

        byte[] encodedSIP1559Params = AbiEncoder.Instance.Encode(AbiEncodingStyle.None, UInt256Signature, sip1559Params);
        byte[] encodedData = AbiEncoder.Instance.Encode(AbiEncodingStyle.None, BytesSignature, encodedSIP1559Params);

        ReceiptForRpc[] receipts = BuildReceipts(encodedData, SystemConfigUpdate.SIP1559Params);
        ...
        SystemConfig expectedConfig = new()
        {
            SIP1559Params = [.. new byte[24], 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8]
        };
