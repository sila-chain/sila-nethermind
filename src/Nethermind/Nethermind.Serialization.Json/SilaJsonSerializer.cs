...
namespace Nethermind.Serialization.Json
{
    ...
    public enum JsonTypeInfoResolverPriority
    {
        ...
        Facade = 20,
        /// <summary>Sil/debug/trace metadata for proofs, traces, and related payloads.</summary>
        SilRpc = 30,
    ...
    public sealed class SilaJsonSerializer : IJsonSerializer
    {
        // Must accommodate the deepest possible callTracer output: each NativeCallTracerCallFrame
        // contributes ~2 JSON levels (object + "calls" array), the SAVM allows up to MaxCallDepth=1024
        // (Yellow Paper / Nethermind.Savm.VirtualMachine.MaxCallDepth), plus a few levels of JSON-RPC
        ...
        public SilaJsonSerializer(IEnumerable<JsonConverter> converters, int maxDepth = DefaultMaxDepth)
        {
            _maxDepth = maxDepth;
        ...
        public SilaJsonSerializer(int maxDepth = DefaultMaxDepth)
        {
            _maxDepth = maxDepth;
            _instanceConverters = [];
            RefreshInstanceOptions();
        ...
        /// <summary>Options for RPC request parameter deserialization, enforcing SIP-1474 QUANTITY format when <see cref="StrictHexFormat"/> is <see langword="true"/>.</summary>
