...
public sealed class OptimismTxDecoder<T>(Func<T>? transactionFactory = null)
    : BaseSIP1559TxDecoder<T>(TxType.DepositTx, transactionFactory) where T : Transaction, new()
{
    protected override int GetSignatureLength(Signature? signature, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0) => 0;

    protected override void EncodeSignature<TWriter>(Signature? signature, ref TWriter writer, bool forSigning, bool isSip155Enabled = false, ulong chainId = 0)
    {
    }
