...
public class OptimismTransactionProcessor(
    ...
    ) : SilaTransactionProcessorBase(blobBaseFeeCalculator, specProvider, worldState, virtualMachine, codeInfoRepository, logManager)
{
        ...
        in UInt256 effectiveGasPrice, out UInt256 premiumPerGas, out UInt256 senderReservedGasPayment, out UInt256 blobBaseFee)
    {
        ...
        if (!tx.IsDeposit())
        {
            ...
            if (spec.IsSip1559Enabled && !tx.IsFree() && (overflows || balanceLeft < maxGasFee))
            {
    ...
    protected override GasConsumed Refund(Transaction tx, BlockHeader header, IReleaseSpec spec, ExecutionOptions opts,
        in TransactionSubstate substate, in SilaGasPolicy unspentGas, in UInt256 gasPrice, ulong codeInsertRefunds, in SilaGasPolicy floorGas, in SilaGasPolicy intrinsicGasStandard, long postIntrinsicStateReservoir, bool createdTargetAlive = false)
    {
