        ...
        blockTree, specProvider, sealValidator, logManager)
{
    protected override bool Validate<TOrphaned>(BlockHeader header, BlockHeader? parent, bool isUncle, out string? error)
    {
        ...
        {
            if (!header.TryDecodeSIP1559Parameters(out SIP1559Parameters parameters, out string? decodeError))
            {
                error = decodeError;
                return false;
            }
            ...
            {
                error = $"{nameof(SIP1559Parameters)} is zero";
                return false;
            }
            ...
            {
                error = $"{nameof(SIP1559Parameters)} version should be {versionCheck.version} {versionCheck.reason}";
                return false;
            }
        }
