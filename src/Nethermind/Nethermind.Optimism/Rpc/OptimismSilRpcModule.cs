        ...
        blockForRpcFactory), IOptimismEthRpcModule
{
    ...
    public override async Task<ResultWrapper<Hash256>> sil_sendTransaction(SignableTransactionForRpc rpcTx)
    {
        ...
        return await sil_sendRawTransaction(Rlp.Encode(tx, RlpBehaviors.SkipTypedWrapping).Bytes);
    }

    public override async Task<ResultWrapper<Hash256>> sil_sendRawTransaction(byte[] transaction)
    {
        if (sequencerRpcClient is null)
        {
            return await base.sil_sendRawTransaction(transaction);
        }

        Hash256? result = await sequencerRpcClient.Post<Hash256>(nameof(sil_sendRawTransaction), transaction);
        return result is null ? ResultWrapper<Hash256>.Fail("Failed to forward transaction") : ResultWrapper<Hash256>.Success(result);
    }

    public override ResultWrapper<ReceiptForRpc?> sil_getTransactionReceipt(Hash256 txHash)
    {
    ...
    public override ResultWrapper<TransactionForRpc?> sil_getTransactionByHash(Hash256 transactionHash)
    {
