        ...
        PayloadAttributes payloadAttributes, Block currentBestBlock, DateTimeOffset startDateTime, UInt256 currentBlockFees, SharedCancellationTokenSource cts)
    {
        if (payloadAttributes is OptimismPayloadAttributes optimismPayload)
        {
            ...
            if (spec.IsOpHoloceneEnabled)
            {
                ...
                if (!optimismPayload.TryDecodeSIP1559Parameters(out SIP1559Parameters sip1559Parameters, out string? error))
                {
                    throw new InvalidOperationException($"{nameof(BlockHeader)} was not properly validated: {error}");
                }

                if (sip1559Parameters.IsZero())
                {
                    sip1559Parameters = new SIP1559Parameters(sip1559Parameters.Version, (UInt32)spec.BaseFeeMaxChangeDenominator, (UInt32)spec.ElasticityMultiplier);
                }

                currentBestBlock.Header.ExtraData = new byte[sip1559Parameters.ByteLength];
                sip1559Parameters.WriteTo(currentBestBlock.Header.ExtraData);
