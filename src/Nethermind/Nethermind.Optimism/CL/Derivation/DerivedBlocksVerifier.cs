...
public class DerivedBlocksVerifier(ILogManager logManager) : IDerivedBlocksVerifier
{
    ...
    public bool ComparePayloadAttributes(OptimismPayloadAttributes expected, OptimismPayloadAttributes actual, ulong blockNumber)
    {
        ...
        if ((expected.SIP1559Params is null && actual.SIP1559Params is not null) ||
            (expected.SIP1559Params is not null && actual.SIP1559Params is null) ||
            (expected.SIP1559Params is not null && actual.SIP1559Params is not null &&
             !expected.SIP1559Params.SequenceEqual(actual.SIP1559Params)))
        {
            if (_logger.IsWarn) _logger.Warn($"Invalid Sip1559Params expected: {expected.SIP1559Params?.ToHexString()}, actual: {actual.SIP1559Params?.ToHexString()}");
            result = false;
        }
