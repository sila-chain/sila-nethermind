...
public sealed class BatchV1
{
                ...
                case TxType.Legacy:
                    {
                        ...
                        if (protectedBit)
                        {
                            v = SilaEcdsaExtensions.CalculateV(chainId, parityBit);
                        }
                        else
                        {
                ...
                case TxType.SIP1559:
                    {
                        ...
                        (tx.Value, tx.GasPrice, tx.DecodedMaxFeePerGas, tx.Data, tx.AccessList) =
                            DecodeSip1559Transaction(Txs.Data[(int)txIdx].Span);
                        break;
                    }
                default:
                    {
    ...
    private (UInt256 Value, UInt256 MaxPriorityFeePerGas, UInt256 MaxFeePerGas, byte[] Data, AccessList? AccessList) DecodeSip1559Transaction(ReadOnlySpan<byte> encoded)
    {
