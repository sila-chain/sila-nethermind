...
public class SilaBeaconApi : IBeaconApi
{
    ...
    public SilaBeaconApi(Uri beaconApiUri, IJsonSerializer jsonSerializer, ISilaEcdsa ecdsa, ILogManager logManager)
    {
        _client = new HttpClient { BaseAddress = beaconApiUri };
        _jsonSerializer = jsonSerializer;
        _ecdsa = ecdsa;
        _logger = logManager.GetClassLogger<SilaBeaconApi>();
    }

    public async Task<BlobSidecar[]> GetBlobSidecars(ulong slot, int indexFrom, int indexTo, CancellationToken cancellationToken)
    {
        string req =
            $"/sil/v1/beacon/blob_sidecars/{slot}?indices={string.Join(',', Enumerable.Range(indexFrom, indexTo - indexFrom + 1))}";
