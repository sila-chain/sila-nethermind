    ...
    ILogManager logManager) : IL2Api
{
    ...
    private PayloadAttributesRef PayloadAttributesFromBlockForRpc(BlockForRpc? block)
    {
        ...
        {
            NoTxPool = true,
            SIP1559Params = block.ExtraData.Length == 0 ? null : block.ExtraData[1..],
    ...
    {
        // TODO: Retry logic
        ResultWrapper<AccountProof> result = l2EthRpc.sil_getProof(accountAddress, storageKeys, new BlockParameter(blockNumber));
    ...
    {
        ResultWrapper<BlockForRpc>? result = l2EthRpc.sil_getBlockByNumber(blockParameter, true);
        while (result?.Result.ResultType != ResultType.Success && result?.ErrorCode != ErrorCodes.UnknownBlockError)
        {
            ...
            await Task.Delay(L2ApiRetryDelayMilliseconds);
            result = l2EthRpc.sil_getBlockByNumber(blockParameter, true);
        }
