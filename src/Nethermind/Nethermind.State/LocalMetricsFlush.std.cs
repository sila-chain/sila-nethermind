...
using SavmMetrics = Nethermind.Savm.Metrics;
...
/// <see cref="DbMetrics"/> / <see cref="SavmMetrics"/> counters and resets it.
/// </summary>
/// <remarks>
/// Lives in <c>Nethermind.State</c> because it is the only assembly that references both
/// <c>Nethermind.Db</c> and <c>Nethermind.Savm</c>. Called on the same thread that accumulated, so
...
internal static class LocalMetricsFlush
{
    internal static void Flush(this LocalMetrics m)
    {
        ...
        if (m.AccountWrites != 0) SavmMetrics.AddAccountWrites(m.AccountWrites);
        if (m.AccountDeleted != 0) SavmMetrics.AddAccountDeleted(m.AccountDeleted);
        if (m.StorageWrites != 0) SavmMetrics.AddStorageWrites(m.StorageWrites);
        if (m.CodeWrites != 0) SavmMetrics.AddCodeWrites(m.CodeWrites);
        if (m.CodeBytesWritten != 0) SavmMetrics.AddCodeBytesWritten(m.CodeBytesWritten);
