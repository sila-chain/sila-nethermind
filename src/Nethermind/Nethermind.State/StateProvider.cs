...
internal partial class StateProvider(ILogManager logManager, LocalMetrics metrics) : IJournal<int>
{
...
#if ZK_SAVM
    // Single-entry cache in front of _intraTxCache: the SAVM accesses the same
    ...
    public bool InsertCode(Address address, in ValueHash256 codeHash, ReadOnlyMemory<byte> code, IReleaseSpec spec, bool isGenesis = false)
    {
        ...
        else if (spec.IsSip158Enabled && !isGenesis)
        {
    ...
    private void SetNewBalance(Address address, in UInt256 balanceChange, IReleaseSpec releaseSpec, bool isSubtracting, out UInt256 oldBalance)
    {
        ...
        if (isZero)
        {
            ...
            if (releaseSpec.IsSip158Enabled && !isSubtracting)
            {
        ...
        for (int i = 0; i <= stepsBack; i++)
        {
            ...
            switch (change.ChangeType)
            {
                ...
                case ChangeType.Update:
                    {
                        if (releaseSpec.IsSip158Enabled && change.Account.IsEmpty && !isGenesis)
                        {
    ...
    {
        if (isZero && address == releaseSpec.Sip158IgnoredAccount) return;
