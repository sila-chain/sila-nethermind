...
{
    private const int BlobBytes = (int)Sip4844Constants.GasPerBlob;  // 131,072 — 4096 BLS field elements × 32 B
    private const int CommitmentBytes = 48;    // KZG commitment (BLS12-381 G1 compressed)
    private const int ProofBytes = 48;         // KZG proof

    // Heavy-block baseline: 250 × ~600 B RLP ≈ 150 KB of tx data — typical sila-mainnet block.
    ...
    public void GlobalSetup()
    {
        ...
        _jsonEncoded = JsonSerializer.SerializeToUtf8Bytes(_result, SilaJsonSerializer.JsonOptions);
    ...
    public byte[] SerializeJson() =>
        JsonSerializer.SerializeToUtf8Bytes(_result, SilaJsonSerializer.JsonOptions);
