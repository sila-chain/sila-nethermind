...
public class NewPayloadSerializationBenchmarks : IDisposable
{
    ...
    // Heavy-block baseline: 250 × 600 B ≈ 150 KB of tx data — typical sila-mainnet block.
    ...
    public void GlobalSetup()
    {
        ...
        _jsonPayloadOnly = JsonSerializer.SerializeToUtf8Bytes(payload, SilaJsonSerializer.JsonOptions);
    ...
    public byte[] SerializeJsonPayloadStatus() =>
        JsonSerializer.SerializeToUtf8Bytes(ValidResult.Data, SilaJsonSerializer.JsonOptions);
    ...
    private static ExecutionPayloadV3 BuildPayload(int blobs, int txs) => new()
    {
        ...
        BlobGasUsed = (ulong)blobs * Sip4844Constants.GasPerBlob,
    ...
    private static byte[][] BuildTransactions(int count, int sizeEach)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            txs[i][0] = 0x02; // SIP-1559 type byte
        }
        return txs;
    }
    ...
    private static Hash256[] BuildBlobVersionedHashes(int count)
    {
        ...
        Span<byte> bytes = stackalloc byte[Sip4844Constants.BytesPerBlobVersionedHash];
    ...
    private static byte[] EncodeJsonBody(ExecutionPayloadV3 payload, Hash256[] blobs, Hash256 parentRoot)
    {
        ...
        JsonSerializer.Serialize(w, payload, SilaJsonSerializer.JsonOptions);
        JsonSerializer.Serialize(w, blobs, SilaJsonSerializer.JsonOptions);
        JsonSerializer.Serialize(w, parentRoot, SilaJsonSerializer.JsonOptions);
        w.WriteEndArray();
        w.WriteEndObject();
        w.Flush();
