...
public class JsonRpcSocketsClientTests
{
    ...
    public async Task Socket_sink_writes_response_with_one_message_boundary(bool isBatch)
    {
        ...
        if (isBatch)
        {
            ...
            await fixture.Sink.WriteBatchItemAsync(new JsonRpcSuccessResponse { Id = 1, Result = "0x1" }, new RpcReport("sil_blockNumber", 1, true), CancellationToken.None);
            await fixture.Sink.WriteBatchItemAsync(new JsonRpcSuccessResponse { Id = 2, Result = "0x2" }, new RpcReport("sil_chainId", 1, true), CancellationToken.None);
            await fixture.Sink.EndBatchAsync(CancellationToken.None);
        }
        else
        {
            await fixture.Sink.WriteSingleAsync(new JsonRpcSuccessResponse { Id = 1, Result = "0x1" }, new RpcReport("sil_blockNumber", 1, true), CancellationToken.None);
        }
    ...
    public class UsingIpc
    {
        ...
        {
            string blockNumber = CreateJsonRequest(1, "sil_blockNumber");
            string chainId = CreateJsonRequest(2, "sil_chainId");
            ...
            string request1 = CreateJsonRequest(1, "sil_call", $"[\"{new string('x', 5000)}\"]");
            string request2 = CreateJsonRequest(2, "sil_call", $"[\"{new string('y', 5000)}\"]");
        ...
        private static TestCaseData LargeChunkedJsonMessageCase(int messageCount)
        {
            ...
            for (int i = 0; i < messageCount; i++)
            {
                ...
                expectedPayloads[i] = CreateJsonRequest(i, "sil_call", $"[\"{payload}\"]");
            }
        ...
        public async Task Overflow_after_buffer_shrink_preserves_data_order()
        {
            ...
            string msg2 = CreateJsonRequestWithoutVersion(2, "sil_call", $"[\"{large2}\"]");
            string msg3 = CreateJsonRequestWithoutVersion(3, "sil_call");
        ...
        {
            string json1 = CreateJsonRequest(1, "sil_blockNumber");
            string json2 = CreateJsonRequest(2, "sil_chainId");
    ...
    public class UsingWebSockets
    {
        ...
        public async Task Can_serialize_collection()
        {
            ...
            JsonRpcSuccessResponse[]? response = new SilaJsonSerializer().Deserialize<JsonRpcSuccessResponse[]>(stream);
