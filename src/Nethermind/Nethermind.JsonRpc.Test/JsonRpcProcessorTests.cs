...
public class JsonRpcProcessorTests
{
    ...
    public async Task Http_generated_method_names_use_cached_instances(
        [Values("engine_newPayloadV4", "engine_getBlobsV2", "sil_call", "sil_getBlockByNumber", "sil_chainId", "sil_unknown")] string methodName,
        [Values(false, true)] bool inBatch)
    {
        bool expectedCached = methodName != "sil_unknown";
        string? capturedMethod = null;
        IJsonRpcService service = CreateService(request =>
        {
            capturedMethod = request.Method;
    ...
    public async Task Sink_processor_entry_point_propagates_stop_requested_to_inline_batch_processing()
    {
        ...
        await ProcessAsync(processor,
            CreateBatchRequest(CreateRequest("1", "sil_getTransactionCount"), CreateRequest("2", "sil_blockNumber"), CreateRequest("3", "net_version")),
            CreateHttpContext(),
            sink);
    ...
    public async Task ProcessAsync_makes_the_request_context_current_for_the_handler(RpcEndpoint endpoint, bool expectedAuthenticated)
    {
        ...
        using CollectedJsonRpcResponses result = await ProcessAsync(processor, CreateRequest("1", "sil_blockNumber"), context);
    ...
    private static string CreateTransactionCountRequest(string idJson, string? paramsName = "params", string paramsJson = TransactionCountParamsJson) =>
        paramsName is null
            ? $$"""{"id":{{idJson}},"jsonrpc":"2.0","method":"sil_getTransactionCount"}"""
            : $$"""{"id":{{idJson}},"jsonrpc":"2.0","method":"sil_getTransactionCount","{{paramsName}}":{{paramsJson}}}""";
    ...
    {
        JsonRpcEnvelope envelope = ReadEnvelope("{\"id\":12345678901234567890,\"method\":\"sil_unknown\"}", out _);

        Assert.That(envelope.Id, Is.EqualTo(new JsonRpcId(decimal.Parse("12345678901234567890"))));
        Assert.That(envelope.Method, Is.EqualTo("sil_unknown"));
    ...
    {
        JsonRpcEnvelope envelope = ReadEnvelope("{\"id\":1e2,\"method\":\"sil_blockNumber\"}", out _);
    ...
    {
        Action read = () => ReadEnvelope("{\"id\":1.1,\"method\":\"sil_blockNumber\"}", out _);
    ...
    public async Task Request_recorder_captures_payload(RpcEndpoint endpoint)
    {
        ...
        string request = endpoint == RpcEndpoint.Http
            ? CreateRequest("1", "sil_blockNumber")
            : CreateRequest("1", "sil_blockNumber") + CreateRequest("2", "net_version");

        using CollectedJsonRpcResponses result = await ProcessAsync(processor, request, new JsonRpcContext(endpoint));

        Assert.That(records.Count(static record => record.Contains("\"method\":\"sil_blockNumber\"")), Is.EqualTo(1));
    ...
    public async Task Response_recorder_captures_responses(bool isBatch, int expectedRecordCount)
    {
        ...
        string request = isBatch
            ? CreateBatchRequest(CreateRequest("1", "sil_blockNumber"), CreateRequest("2", "net_version"))
            : CreateRequest("1", "sil_blockNumber");
        ...
        Assert.That(records.Any(static record => record.Contains("sil_blockNumber")), Is.True);
        if (isBatch)
        {
    ...
    public async Task Params_document_is_disposed_after_sink_write(bool isBatch)
    {
        ...
        JsonRpcProcessor processor = CreateProcessor(service);
        string request = isBatch
            ? CreateBatchRequest(CreateRequest("1", "sil_blockNumber", "[1]"), CreateRequest("2", "net_version", "[2]"))
            : CreateRequest("1", "sil_blockNumber", "[{\"a\":1}]");
    ...
    public async Task Response_disposables_run_after_sink_write()
    {
        ...
        await ProcessAsync(processor, CreateRequest("1", "sil_blockNumber"), CreateHttpContext(), sink);
    ...
    [TestCase(false)]
    [TestCase(true)]
    public async Task Can_process_uppsrcase_params(bool returnErrors)
    {
    ...
    public async Task Should_complete_pipe_reader_when_shutdown_requested()
    {
        ...
        Pipe pipe = new();
        await pipe.Writer.WriteAsync(Encoding.UTF8.GetBytes(CreateRequest("1", "sil_blockNumber")));
    ...
    [TestCase("sil_getTransactionCount", false, "sil_getTransactionCount", true, TestName = "Resolved method")]
    ...
    public async Task Input_depth_is_bounded_by_reader_default_max_depth(int paramNestingDepth, bool expectParseError)
    {
        ...
        Assert.That(capturedMethod, Is.EqualTo("sil_getTransactionCount"));
