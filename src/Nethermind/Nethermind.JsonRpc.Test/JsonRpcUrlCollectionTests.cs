...
{
    [SetUp]
    public void Initialize() => _enabledModules = [ModuleType.Sil, ModuleType.Web3, ModuleType.Net];
    ...
    public void Skips_additional_urls_with_port_conflicts()
    {
        JsonRpcConfig jsonRpcConfig = new()
        {
            ...
            [
                ...
                "https://127.0.0.1:1234|https;wss|sil;web3",
                "https://127.0.0.1:9876|https;wss|net;proof",
                "http://localhost:1234|http;ws|db;src20"
            ]
        };
        ...
        Assert.That(new Dictionary<int, JsonRpcUrl>()
        {
            ...
            { 1234, new JsonRpcUrl("https", "127.0.0.1", 1234, RpcEndpoint.Https | RpcEndpoint.Wss, false, ["sil", "web3"]) }
        }, Is.EqualTo(urlCollection));
    }
    ...
    public void Skips_additional_urls_when_invalid()
    {
        JsonRpcConfig jsonRpcConfig = new()
        {
            ...
            [
                string.Empty,
                "test",
                "http://localhost:1234|http|db;src20;web3"
            ]
        };
        ...
        {
            { 8545, new JsonRpcUrl("http", "127.0.0.1", 8545, RpcEndpoint.Http | RpcEndpoint.Ws, false, _enabledModules) },
            { 1234, new JsonRpcUrl("http", "localhost", 1234, RpcEndpoint.Http, false, ["db", "src20", "web3"]) }
        }, Is.EqualTo(urlCollection)); ;
    }
    ...
    public void EngineHost_and_EnginePort_specified()
    {
        JsonRpcConfig jsonRpcConfig = new()
        {
            ...
            AdditionalRpcUrls =
            [
                "http://127.0.0.1:8551|http|sil;web3;engine"
            ],
            EngineHost = "127.0.0.1",
            EnginePort = 8551,
            EngineEnabledModules = ["sil"]
        };
        ...
        {
            { 8545, new JsonRpcUrl("http", "127.0.0.1", 8545, RpcEndpoint.Http | RpcEndpoint.Ws, false, _enabledModules) },
            { 8551, new JsonRpcUrl("http", "127.0.0.1", 8551, RpcEndpoint.Http | RpcEndpoint.Ws, true, [ModuleType.Sil, ModuleType.Engine]) },
        }, Is.EqualTo(urlCollection)); ;
    }
    ...
    public void Health_host_patterns_only_include_ports_with_health_module()
    {
        ...
        {
            Enabled = true,
            EnabledModules = [ModuleType.Sil, ModuleType.Web3, ModuleType.Net, ModuleType.Health],
            AdditionalRpcUrls =
            [
                "http://127.0.0.1:1234|http|sil;web3",
                "http://127.0.0.1:5678|http|sil;health"
            ]
        };
    ...
    public void Skips_AdditionalUrl_with_engine_module_enabled_when_EngineUrl_specified()
    {
        ...
        {
            { 8545, new JsonRpcUrl("http", "127.0.0.1", 8545, RpcEndpoint.Http, false, _enabledModules) },
            { 8552, new JsonRpcUrl("http", "127.0.0.1", 8552, RpcEndpoint.Http, true, [ModuleType.Sil, ModuleType.Engine]) },
            { 1234, new JsonRpcUrl("http", "127.0.0.1", 1234, RpcEndpoint.Http, false, [ModuleType.Sil, ModuleType.Web3])}
        }, Is.EqualTo(urlCollection));
    }
}
