...
public class TransactionForRpcDeserializationTests
{
    ...
    public static IEnumerable TxJsonTestCases
    {
        get
        {
            ...
            yield return Make(TxType.Legacy, """{"nonce":"0x0","to":null,"gasPrice":"0x0","gas":"0x0","input":null}""");
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","input":null}""");
            yield return Make(TxType.SIP1559, """{}""");
            yield return Make(TxType.SIP1559, """{"type":null}""");
            yield return Make(TxType.SIP1559, """{"additionalField":""}""");
            yield return Make(TxType.SIP1559, """{"MaxFeePerBlobGas":"0x0"}""");
            ...
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","to":null,"value":"0x0","accessList":[],"maxPriorityFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","to":null,"value":"0x0","accessList":null,"maxPriorityFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","to":null,"value":"0x0","maxPriorityFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","to":null,"value":"0x0","maxFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"value":"0x0","maxPriorityFee":"0x0", "maxFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"maxPriorityFeePerGas":"0x0", "maxFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"maxFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"maxPriorityFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"MaxPriorityFeePerGas":"0x0"}""");
            yield return Make(TxType.SIP1559, """{"nonce":"0x0","to":null,"value":"0x0","maxPriorityFeePerGas":"0x0", "maxFeePerGas":"0x0","maxFeePerBlobGas":"0x0"}""");
            ...
            yield return Make(TxType.SIP1559, """{"type":"0x2"}""");
    ...
    public static IEnumerable SpecAwareResolutionCases
    {
        get
        {
            ...
            // Defaulted type on pre-Berlin (no SIP-2930) → Legacy
            ...
            // Defaulted type on post-Berlin → keeps SIP1559
            yield return Make(TxType.SIP1559, """{}""", Berlin.Instance);
            yield return Make(TxType.SIP1559, """{}""", London.Instance);

            // Explicit type is preserved regardless of spec
            yield return Make(TxType.SIP1559, """{"type":"0x2"}""", Istanbul.Instance);
            ...
            yield return Make(TxType.SIP1559, """{"maxFeePerGas":"0x0"}""", Istanbul.Instance);
            ...
            // No spec (null) → keeps defaulted SIP1559
            yield return Make(TxType.SIP1559, """{}""", null);
        }
    }
    ...
    public static IEnumerable DefaultedTypeResolutionCases
    {
        get
        {
            ...
            yield return new TestCaseData(London.Instance, true)
                .SetName("Post-London spec resolves to SIP1559 with AccessList")
                .Returns(TxType.SIP1559);
        }
    }
    ...
    public void Test_BlobTransaction_WithTooManyBlobHashes_ReturnsBlobGasLimitError_WhenUserInputValidated()
    {
        ...
        Result<Transaction> result = rpc.ToTransaction(validateUserInput: true, spec: SilaCancun.Instance);

        Assert.That(result.IsError, Is.True);
        Assert.That(
            result.Error,
            Is.EqualTo(BlockErrorMessages.BlobGasUsedAboveBlockLimit(SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock, 7, 7 * Sip4844Constants.GasPerBlob)));
    }
}
