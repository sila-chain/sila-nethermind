...
public class RpcModuleProviderTests
{
    ...
    private static RpcModuleProvider CreateProvider(IJsonRpcConfig? config = null, IFileSystem? fileSystem = null, IReadOnlyList<RpcModuleInfo>? rpcModules = null) =>
        new(fileSystem ?? Substitute.For<IFileSystem>(), config ?? new JsonRpcConfig(), new SilaJsonSerializer(), rpcModules ?? [], LimboLogs.Instance);
    ...
    [TestCase("sil_.*", ModuleResolution.Unknown)]
    ...
    [TestCase("sil_call", ModuleType.Sil)]
    [TestCase("sil_getBlockByNumber", ModuleType.Sil)]
    [TestCase("sil_chainId", ModuleType.Sil)]
    ...
    {
        JsonTypeInfo<StorageValuesResult> firstGeneratedLookup = RpcPayloadTypeInfo<StorageValuesResult>.Get(SilaJsonSerializer.JsonOptions);
        JsonTypeInfo<StorageValuesResult> secondGeneratedLookup = RpcPayloadTypeInfo<StorageValuesResult>.Get(SilaJsonSerializer.JsonOptions);
        Assert.That(RpcPayloadTypeInfo<FallbackPayload>.Get(SilaJsonSerializer.JsonOptions), Is.Not.Null);
    ...
    {
        List<string> missing = [];
        AddMissingAssemblyPayloadTypes(typeof(ISilRpcModule).Assembly, missing);
    ...
    {
        JsonRpcConfig config = new() { EnabledModules = [ModuleType.Engine, ModuleType.Sil] };
        _moduleProvider = CreateProvider(config, _fileSystem);
    ...
    {
        public ResultWrapper<HexBytes> sil_call() => ResultWrapper<HexBytes>.Success(default);

        public ResultWrapper<string> sil_getBlockByNumber() => ResultWrapper<string>.Success(string.Empty);

        public ResultWrapper<string> sil_chainId() => ResultWrapper<string>.Success(string.Empty);
    }
