...
namespace Nethermind.JsonRpc.Test.Modules
{
    ...
    public class GasPriceOracleTests
    {
        ...
        public void AddValidTxAndReturnCount_GivenSip1559Txs_EffectiveGasPriceProperlyCalculated(bool sip1559Enabled, ulong[] expected)
        {
            Transaction[] sip1559TxGroup = {
                Build.A.Transaction.WithMaxFeePerGas(27).WithMaxPriorityFeePerGas(25).WithType(TxType.SIP1559).TestObject,
                Build.A.Transaction.WithMaxFeePerGas(27).WithMaxPriorityFeePerGas(26).WithType(TxType.SIP1559).TestObject,
                Build.A.Transaction.WithMaxFeePerGas(27).WithMaxPriorityFeePerGas(27).WithType(TxType.SIP1559).TestObject
            };
            Block sip1559Block = Build.A.Block.Genesis.WithTransactions(sip1559TxGroup).WithBaseFeePerGas(1).TestObject;
            IBlockFinder blockFinder = Substitute.For<IBlockFinder>();
            blockFinder.FindBlock(0UL).Returns(sip1559Block);
            GasPriceOracle gasPriceOracle = new(blockFinder, GetSpecProviderWithSip1559EnabledAs(sip1559Enabled), LimboLogs.Instance);
        ...
        [TestCase(false, new ulong[] { 25, 26, 27 })]
        public void AddValidTxAndReturnCount_GivenNonSip1559Txs_EffectiveGasPriceProperlyCalculated(bool sip1559Enabled, ulong[] expected)
        {
            Transaction[] nonSip1559TxGroup = {
            ...
            Block nonSip1559Block = Build.A.Block.Genesis.WithTransactions(nonSip1559TxGroup).WithBaseFeePerGas(1).TestObject;
            IBlockFinder blockFinder = Substitute.For<IBlockFinder>();
            blockFinder.FindBlock(0UL).Returns(nonSip1559Block);
            GasPriceOracle gasPriceOracle = new(blockFinder, GetSpecProviderWithSip1559EnabledAs(sip1559Enabled), LimboLogs.Instance);
        ...
        public static ISpecProvider GetSpecProviderWithSip1559EnabledAs(bool isSip1559) =>
            new CustomSpecProvider(((ForkActivation)0, isSip1559 ? London.Instance : Berlin.Instance));
        ...
        [Test]
        public void AddValidTxAndReturnCount_Sip1559NotEnabled_EffectiveGasPricesShouldBeMoreThanIgnoreUnder()
        {
            Transaction[] transactions =
            {
                Build.A.Transaction.WithGasPrice(1).TestObject, //1
                Build.A.Transaction.WithGasPrice(2).WithType(TxType.SIP1559).TestObject, //2
                Build.A.Transaction.WithGasPrice(3).WithType(TxType.SIP1559).TestObject, //3
            };
            Block testBlock = Build.A.Block.Genesis.WithBaseFeePerGas(1).WithTransactions(transactions).TestObject;
            IBlockFinder blockFinder = Substitute.For<IBlockFinder>();
            blockFinder.FindBlock(0UL).Returns(testBlock);
            GasPriceOracle gasPriceOracle = new(blockFinder, GetSpecProviderWithSip1559EnabledAs(false), LimboLogs.Instance);
        ...
        [Test]
        public void AddValidTxAndReturnCount_Sip1559Enabled_EffectiveGasPricesShouldBeMoreThanIgnoreUnder()
        {
            Transaction[] transactions =
            {
                Build.A.Transaction.WithMaxFeePerGas(3).WithGasPrice(1).TestObject, //Min(1, 1+1) => 1
                Build.A.Transaction.WithMaxFeePerGas(4).WithGasPrice(2).WithType(TxType.SIP1559).TestObject, //Min(4, 2 + 1) => 3
                Build.A.Transaction.WithMaxFeePerGas(5).WithGasPrice(3).WithType(TxType.SIP1559).TestObject, //Min(5, 3 + 1) => 4
            };
            Block testBlock = Build.A.Block.Genesis.WithBaseFeePerGas(1).WithTransactions(transactions).TestObject;
            IBlockFinder blockFinder = Substitute.For<IBlockFinder>();
            blockFinder.FindBlock(0UL).Returns(testBlock);
            GasPriceOracle gasPriceOracle = new(blockFinder, GetSpecProviderWithSip1559EnabledAs(true), LimboLogs.Instance);
