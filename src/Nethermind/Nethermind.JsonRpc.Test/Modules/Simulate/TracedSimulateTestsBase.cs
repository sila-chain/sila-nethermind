...
public abstract class TracedSimulateTestsBase<TTrace>
{
    ...
    public async Task Test_simulate_serialization()
    {
        ...
        SimulatePayload<TransactionForRpc> payload = SilSimulateTestsBlocksAndTransactions.CreateSerializationPayload(chain);
    ...
    public async Task Test_simulate_eth_moved()
    {
        ...
        Transaction txMainnetAtoB = SilSimulateTestsBlocksAndTransactions.GetTransferTxData(nonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1, type: TxType.Legacy);

        SimulatePayload<TransactionForRpc> payload = SilSimulateTestsBlocksAndTransactions.CreateEthMovedPayload(chain, nonceA);
    ...
    public async Task Test_simulate_transactions_forced_fail()
    {
        ...
        Transaction txMainnetAtoB =
            SilSimulateTestsBlocksAndTransactions.GetTransferTxData(nonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1, type: TxType.Legacy);

        SimulatePayload<TransactionForRpc> payload = SilSimulateTestsBlocksAndTransactions.CreateTransactionsForcedFail(chain, nonceA);
