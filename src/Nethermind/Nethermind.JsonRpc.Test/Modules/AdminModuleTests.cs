...
public class AdminModuleTests
{
    ...
    private SilaJsonSerializer _serializer = null!;
    ...
    public void Setup()
    {
        ...
        _serializer = new SilaJsonSerializer();
    }
    ...
    public async Task AdminPeers_WithSingleStaticPeer_ReturnsOnePeerInfo()
    {
        ...
        List<PeerInfo> peerInfoList = ((JsonElement)response.Result!).Deserialize<List<PeerInfo>>(SilaJsonSerializer.JsonOptions)!;
    ...
    public async Task AdminNodeInfo_OnDefaultModule_ReturnsExpectedNodeAndProtocolFields()
    {
        ...
        JsonRpcSuccessResponse response = _serializer.Deserialize<JsonRpcSuccessResponse>(serialized);
        NodeInfo nodeInfo = ((JsonElement)response.Result!).Deserialize<NodeInfo>(SilaJsonSerializer.JsonOptions)!;
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(nodeInfo.Protocols, Has.Count.EqualTo(1), "only the sil protocol is registered in this test setup");
            Assert.That(nodeInfo.Protocols["sil"].Difficulty, Is.EqualTo(_blockTree.Head?.TotalDifficulty ?? 0), "difficulty mirrors the head total difficulty");
            Assert.That(nodeInfo.Protocols["sil"].HeadHash, Is.EqualTo(_blockTree.HeadHash), "head hash mirrors the block tree head");
            Assert.That(nodeInfo.Protocols["sil"].GenesisHash, Is.EqualTo(_blockTree.GenesisHash), "genesis hash mirrors the block tree genesis");
            Assert.That(nodeInfo.Protocols["sil"].NetworkId, Is.EqualTo(_blockTree.NetworkId), "network id mirrors the block tree network id");
            Assert.That(nodeInfo.Protocols["sil"].ChainId, Is.EqualTo(_blockTree.ChainId), "chain id mirrors the block tree chain id");
        }
    }
    ...
    public async Task AdminAddTrustedPeer_WithValidEnode_AddsAsTrustedPeerAndReturnsTrue(bool persistent, bool expectedUpdateFile)
    {
        ...
        JsonRpcSuccessResponse response = _serializer.Deserialize<JsonRpcSuccessResponse>(serialized);
        bool result = ((JsonElement)response.Result!).Deserialize<bool>(SilaJsonSerializer.JsonOptions);
    ...
    {
        Capability[] capabilities = [new Capability("sil", 68), new Capability("snap", 1)];
    ...
    {
        Capability[] capabilities = [new Capability("sil", 67), new Capability("sil", 68), new Capability("snap", 1)];
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(peerInfo.Caps, Is.EqualTo(capabilities), "all advertised sil versions plus snap must be reported");
            Assert.That(peerInfo.Protocols.Keys, Is.SupersetOf(new[] { "sil", "snap" }), "both sil and snap protocols are present in the capabilities");
        }
    }
    ...
    public void AdminPeers_WithEthOnlyPeer_DoesNotIncludeSnapProtocol(int silVersion, string clientId)
    {
        Capability[] capabilities = [new Capability("sil", silVersion)];
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(peerInfo.Caps, Is.EqualTo(capabilities), "only the advertised sil capability must surface");
            Assert.That(peerInfo.Protocols, Does.ContainKey("sil"), "the sil protocol entry must be present");
