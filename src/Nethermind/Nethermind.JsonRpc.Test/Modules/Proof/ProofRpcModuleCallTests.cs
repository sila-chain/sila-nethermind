...
public class ProofRpcModuleCallTests
{
    ...
    public async Task Proof_call_returns_witness_with_executed_block_header_as_last_entry()
    {
        ...
        using ResultWrapper<CallResultWithProof> wrapper = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc { To = TestItem.AddressB },
    ...
    public async Task Proof_call_against_genesis_is_rejected()
    {
        ...
        using ResultWrapper<CallResultWithProof> result = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc { To = TestItem.AddressA },
    ...
    public async Task Proof_call_captures_account_and_one_storage_slot()
    {
        ...
        using ResultWrapper<CallResultWithProof> wrapper = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc
            {
                To = contractAddress,
                Gas = 200_000,
            },
    ...
    public async Task Proof_call_revert_surfaces_error_and_witness(byte[] runtimeCode, string expectedMessage, string expectedDataPrefix)
    {
        ...
        ulong blockNumber = blockchain.BlockTree.Head!.Number;
        using ResultWrapper<CallResultWithProof> wrapper = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc { To = contractAddress, Gas = 200_000 },
            new BlockParameter(blockNumber));
        CallResultWithProof result = wrapper.Data!;

        // Revert surfaces no top-level Result; the payload lives in error.data (hex), mirroring sil_call.
    ...
    /// (matching <c>sil_call</c>); without it, BuyGas rejects with MaxFeePerGasBelowBaseFee.
    /// </summary>
    [Test]
    public async Task Proof_call_legacy_tx_with_no_gas_price_on_post_london_chain_zeroes_base_fee()
    {
        ...
        Assert.That(wrapper.Data!.Error, Is.Null,
            "legacy call with no GasPrice must zero BaseFeePerGas (mirroring sil_call), not surface MaxFeePerGasBelowBaseFee");
    ...
    public async Task Proof_call_with_two_accounts_captures_storage_trie_for_each()
    {
        ...
        using ResultWrapper<CallResultWithProof> wrapper = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc { To = contract, Gas = 200_000 },
            new BlockParameter(blockNumber));
        CallResultWithProof result = wrapper.Data!;
    ...
    public async Task Proof_call_witness_round_trips_through_stateless_reconstruction_for_balance_opcode()
    {
        ...
        ulong blockNumber = blockchain.BlockTree.Head!.Number;
        using ResultWrapper<CallResultWithProof> wrapper = blockchain.ProofRpcModule.proof_call(
            new Facade.Sil.RpcTransaction.LegacyTransactionForRpc { To = caller, Gas = 200_000 },
            new BlockParameter(blockNumber));
    ...
    {
        ulong nonce = blockchain.ReadOnlyState.GetNonce(TestItem.AddressA);
        byte[] initCode = Prepare.SavmCode.ForInitOf(runtimeCode).Done;
        Address contractAddress = ContractAddress.From(TestItem.PrivateKeyA.Address, nonce);
    ...
    /// empty payload, matching <c>sil_call</c>. Consumers branch on the presence of <c>data</c>;
    /// silently dropping it for empty payloads would diverge from <c>sil_call</c>'s wire format.
    /// </summary>
    [Test]
    public async Task Proof_call_revert_with_empty_payload_includes_data_0x()
    {
        ...
        Assert.That(result.Error, Is.Not.Null);
        Assert.That(result.Error!.Code, Is.EqualTo(ErrorCodes.ExecutionReverted));
        Assert.That(result.Error.Data, Is.Not.Null, "empty REVERT payload must still set data — matches sil_call");
    ...
    /// <c>nonce</c> must succeed against the current state nonce, matching <c>sil_call</c>'s
    /// "ignore caller-supplied nonce" behavior. Without this fix, the SAVM's pre-VM validation
    ...
    public async Task Proof_call_with_from_but_no_nonce_resolves_nonce_from_state()
    {
        ...
        // If the nonce fix regressed, the SAVM would reject the call pre-VM with
