...
public partial class DebugRpcModuleTests
{
    ...
    {
        Transactions = [new LegacyTransactionForRpc { To = SilRpcSimulateTestsBase.GasProbeContractAddress, Gas = gas }],
        StateOverrides = new Dictionary<Address, AccountOverride>
        {
            [SilRpcSimulateTestsBase.GasProbeContractAddress] = new()
            {
    ...
    private static async Task<Context> CreateContext()
    {
        ...
        try
        {
            await ctx.Blockchain.AddFunds(TestItem.AddressD, 100.Sila);
        }
        catch
        {
            ctx.Dispose();
    ...
    {
        using Context ctx = await CreateContext();
        TransactionBundle bundle = CreateBundle(CreateTransaction(value: 200.Sila));
    ...
    public async Task Debug_traceCallMany_with_state_override()
    {
        ...
        {
            [TestItem.AddressD] = new() { Balance = 100.Sila }
        };
