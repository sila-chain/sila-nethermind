...
{
    [Test]
    public async Task Sil_estimateGas_web3_should_return_insufficient_balance_error()
    {
        ...
        string serialized =
            await ctx.Test.TestEthRpc("sil_estimateGas", transaction);
        Assert.That(
    ...
    [Test]
    public async Task Sil_estimateGas_web3_sample_not_enough_gas_system_account()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_estimateGas_web3_sample_not_enough_gas_other_account()
    {
        using Context ctx = await Context.Create();
        AssertAccountDoesNotExist(ctx, TestAccount);
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_estimateGas_web3_above_block_gas_limit()
    {
        using Context ctx = await Context.Create();
        AssertAccountDoesNotExist(ctx, TestAccount);
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    private static IEnumerable<TestCaseData> CreateAccessListGasCases()
    {
        ...
        yield return new TestCaseData(false, 2, Sip7981Spec).SetName("SIP-7981: noOpt, 2");
        yield return new TestCaseData(true, 2, Sip7981Spec).SetName("SIP-7981: opt, 2");
        yield return new TestCaseData(true, 17, Sip7981Spec).SetName("SIP-7981: opt, 17");
    }

    [TestCaseSource(nameof(CreateAccessListGasCases))]
    public async Task Sil_create_access_list_calculates_proper_gas(bool optimize, long loads, IReleaseSpec spec)
    {
        ...
        string serializedCreateAccessList = await test.TestEthRpc("sil_createAccessList",
        ...
        string serializedEstimateGas =
            await test.TestEthRpc("sil_estimateGas", transaction, "0x0");
    ...
    public async Task Sil_estimate_gas_with_accessList(bool senderAccessList, long gasPriceWithoutAccessList,
        long gasPriceWithAccessList)
    {
        ...
        string serialized = await test.TestEthRpc("sil_estimateGas", transaction, "0x0");
        Assert.That(
        ...
        serialized = await test.TestEthRpc("sil_estimateGas", transaction, "0x0");
        Assert.That(
            serialized, Is.EqualTo($"{{\"jsonrpc\":\"2.0\",\"result\":\"{gasPriceWithAccessList.ToHexString(true)}\",\"id\":67}}"));
    }

    [Test]
    public async Task Sil_estimate_gas_is_lower_with_optimized_access_list()
    {
        TestRpcBlockchain test = await TestRpcBlockchain.ForTest(SealEngineType.NethDev)
            .Build(new TestSpecProvider(Berlin.Instance));
    ...
    public async Task Estimate_gas_without_gas_pricing()
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_estimateGas", transaction);
    ...
    [Test]
    public async Task should_estimate_transaction_with_deployed_code_when_sip3607_enabled()
    {
        OverridableReleaseSpec releaseSpec = new(London.Instance) { Sip1559TransitionBlock = 1, IsSip3607Enabled = true };
        ...
        LegacyTransactionForRpc transaction = new(
            tx,
            new(tx.ChainId ?? BlockchainIds.SilaMainnet))
        {
        ...
        string serialized =
            await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest");
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"result\":\"0x5208\",\"id\":67}"));
    }
    ...
    [TestCase(
        ...
        """{"jsonrpc":"2.0","result":"0x5208","id":67}""" // SIL transfer (intrinsic transaction cost)
    )]
    [TestCase(
    ...
    public async Task Estimate_gas_with_state_override(string name, string transactionJson, string stateOverrideJson, string expectedResult)
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride);
    ...
    public async Task Estimate_gas_with_state_override_does_not_affect_other_calls(string name, string transactionJson, string stateOverrideJson)
    {
        ...
        string resultOverrideBefore = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride);

        string resultNoOverride = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest");

        string resultOverrideAfter = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride);
    ...
    public async Task Estimate_gas_uses_block_gas_limit_when_not_specified()
    {
        ...
        string blockNumberResponse = await ctx.Test.TestEthRpc("sil_blockNumber");
        string blockNumber = JToken.Parse(blockNumberResponse).Value<string>("result")!;
        string blockResponse = await ctx.Test.TestEthRpc("sil_getBlockByNumber", blockNumber, false);
    ...
    [Test]
    public async Task Sil_estimateGas_ignores_invalid_nonce()
    {
        using Context ctx = await Context.Create();
        byte[] code = Prepare.SavmCode
         .Op(Instruction.STOP)
        ...
        Transaction tx = Build.A.Transaction
            ...
            .TestObject;
        SIP1559TransactionForRpc transaction = new(tx, new(tx.ChainId ?? BlockchainIds.SilaMainnet));
    ...
    [Test]
    public async Task Sil_estimateGas_simple_transfer()
    {
        using Context ctx = await Context.Create();
        byte[] code = [];
        Transaction tx = Build.A.Transaction
    ...
    [Test]
    public async Task Sil_estimateGas_succeeds_when_gas_price_set_but_balance_below_block_gas_limit_times_gas_price()
    {
        ...
        // because the SIP-1559 pre-check used tx.GasLimit (= blockGasLimit) instead of the actual estimated gas.
        // blockGasLimit(4M) × gasPrice(50Gwei) = 0.2 SIL > balance(0.1 SIL).
        // Actual gas needed ≈ 0x53b8 ≈ 21432 → cost = 21432 × 50Gwei ≪ 0.1 SIL.
        using Context ctx = await Context.CreateWithLondonEnabled();
    ...
    [Test]
    public async Task Sil_estimateGas_returns_execution_reverted_when_gas_price_set_and_contract_reverts()
    {
        // Regression for: balance < explicit_gas × gasPrice, but the SAVM should still run and surface the revert.
        // Before fix: TransactionProcessor rejected with "insufficient MaxFeePerGas for sender balance" before SAVM ran.
        // explicit_gas(0xE234=57908) × gasPrice(50Gwei) ≈ 0.0029 SIL > balance(0.002 SIL).
        ...
        object transaction = JsonSerializer.Deserialize<object>(
            ...
            JsonSerializerOptions.Default)!;
        // balance = 0.002 SIL (below gas × gasPrice = 0.0029 SIL but above intrinsicGas × gasPrice)
    ...
    private static IEnumerable<TestCaseData> EstimateGasLowerBoundFollowUpCases()
    {
        yield return new TestCaseData(
            ...
            .SetName("Sil_estimateGas_lower_bound_probe_follow_up_revert_wins");
        ...
        yield return new TestCaseData(
            ...
            .SetName("Sil_estimateGas_lower_bound_probe_follow_up_success_wins");
        ...
        yield return new TestCaseData(
            ...
            .SetName("Sil_estimateGas_lower_bound_probe_follow_up_allowance_wins");
    }

    [TestCaseSource(nameof(EstimateGasLowerBoundFollowUpCases))]
    public async Task Sil_estimateGas_lower_bound_probe_follow_up_cases(string transactionJson, string stateOverrideJson, string expectedJson)
    {
    ...
    private static readonly OverridableReleaseSpec Sip7976Spec = new(SilaPrague.Instance) { IsSip7976Enabled = true };
    private static readonly OverridableReleaseSpec Sip7981Spec = new(SilaAmsterdam.Instance) { IsSip7976Enabled = true, IsSip7981Enabled = true };

    private static IEnumerable<TestCaseData> EstimateGasFloorCostCases()
    {
        // SIP-7976: 100 zero bytes → floor = 21000 + 100 * 4 * 16 = 27400
        ulong sip7976Floor100 = GasCostOf.Transaction + 100UL * Sip7976Spec.GasCosts.TxDataNonZeroMultiplier * Sip7976Spec.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(Sip7976Spec, new byte[100], 100_000UL, null,
                $"{{\"jsonrpc\":\"2.0\",\"result\":\"{sip7976Floor100.ToHexString(true)}\",\"id\":67}}")
            .SetName("SIP-7976: data heavy tx returns floor cost");

        // SIP-7623: 100 zero bytes → floor = 21000 + 100 * 10 = 22000
        ulong sip7623Floor100 = GasCostOf.Transaction + 100UL * SilaPrague.Instance.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(SilaPrague.Instance, new byte[100], 100_000UL, null,
                $"{{\"jsonrpc\":\"2.0\",\"result\":\"{sip7623Floor100.ToHexString(true)}\",\"id\":67}}")
            .SetName("SIP-7623: data heavy tx returns lower floor");

        // SIP-7976: gas at standard but below floor → "gas below floor data cost"
        ...
        ulong sip7976Floor1Byte = GasCostOf.Transaction + 1UL * Sip7976Spec.GasCosts.TxDataNonZeroMultiplier * Sip7976Spec.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(Sip7976Spec, new byte[] { 0 }, atStandard, null,
                $"{{\"jsonrpc\":\"2.0\",\"error\":{{\"code\":-32000,\"message\":\"failed with {atStandard} gas: gas below floor data cost: have {atStandard}, want {sip7976Floor1Byte}\"}},\"id\":67}}")
            .SetName("SIP-7976: gas at standard but below floor returns floor error");

        // SIP-7976: mixed calldata (0x00001122 = 2 zero + 2 nonzero bytes)
        ulong sip7976Floor4 = GasCostOf.Transaction + 4UL * Sip7976Spec.GasCosts.TxDataNonZeroMultiplier * Sip7976Spec.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(Sip7976Spec, new byte[] { 0x00, 0x00, 0x11, 0x22 }, 100_000UL, null,
                $"{{\"jsonrpc\":\"2.0\",\"result\":\"{sip7976Floor4.ToHexString(true)}\",\"id\":67}}")
            .SetName("SIP-7976: mixed calldata returns floor");

        // SIP-7981: access list with 1 address, no calldata — standard wins.
        ulong sip7981Standard = GasCostOf.Transaction + Sip8038Constants.AccessListAddressCost
            + 80UL * Sip7981Spec.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(Sip7981Spec, Array.Empty<byte>(), 100_000UL,
                new AccessList.Builder().AddAddress(Address.Zero).Build(),
                $"{{\"jsonrpc\":\"2.0\",\"result\":\"{sip7981Standard.ToHexString(true)}\",\"id\":67}}")
            .SetName("SIP-7981: standard wins with access list");

        // Modest calldata: standard still exceeds the floor (floor-wins covered by data-heavy cases).
        ulong sip7981StandardWithCalldata = GasCostOf.Transaction + Sip8038Constants.AccessListAddressCost
            + 100UL * GasCostOf.TxDataZero
            + 80UL * Sip7981Spec.GasCosts.TotalCostFloorPerToken;
        yield return new TestCaseData(Sip7981Spec, new byte[100], 100_000UL,
                new AccessList.Builder().AddAddress(Address.Zero).Build(),
                $"{{\"jsonrpc\":\"2.0\",\"result\":\"{sip7981StandardWithCalldata.ToHexString(true)}\",\"id\":67}}")
            .SetName("SIP-7981: standard wins with calldata and access list");
    }

    [TestCaseSource(nameof(EstimateGasFloorCostCases))]
    public async Task Sil_estimateGas_floor_cost(IReleaseSpec spec, byte[] data, ulong gasLimit, AccessList? accessList, string expectedJson)
    {
    ...
    public async Task Estimate_gas_baseFeePerGas_override_allows_tx_with_gasPrice_below_real_baseFee()
    {
        ...
        object? stateOverride = JsonSerializer.Deserialize<object>(
            "{\"" + sender + "\":{\"balance\":\"0xde0b6b3a7640000\"}}"); // 1 SIL

        string withoutOverride = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride);
        ...
        string withOverride = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride, blockOverride);
    ...
    public async Task Estimate_gas_block_override_gasLimit_bounds_estimation()
    {
        ...
        object? stateOverride = JsonSerializer.Deserialize<object>(
            "{\"" + TestItem.AddressA + "\":{\"balance\":\"0xde0b6b3a7640000\"}}"); // 1 SIL
        object? blockOverride = JsonSerializer.Deserialize<object>("""{"gasLimit":"0xC350"}"""); // 50000

        string serialized = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride, blockOverride);
    ...
    public async Task Sil_estimateGas_blob_transaction(string name, string transactionJson, string stateOverrideJson, string expectedResult)
    {
        ...
        string serialized = await test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride);

        Assert.That(JToken.Parse(serialized), Is.EqualTo(JToken.Parse(expectedResult)).Using(JToken.EqualityComparer));
    }

    [Test]
    public async Task Sil_estimateGas_blob_transaction_rejected_when_blobBaseFee_block_override_exceeds_maxFeePerBlobGas()
    {
        ...
        string serialized = await test.TestEthRpc("sil_estimateGas", transaction, "latest", stateOverride, blockOverride);
    ...
    public async Task Sil_estimateGas_setCode_invalid_transaction_returns_error(string txJson, string expectedMessage)
    {
        TestSpecProvider specProvider = new(SilaPrague.Instance);
        using Context ctx = await Context.Create(specProvider);
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_estimateGas", transaction, "latest");
    ...
    [Test]
    public async Task Sil_estimateGas_setCode_missing_yParity_returns_error()
    {
        TestSpecProvider specProvider = new(SilaPrague.Instance);
        using Context ctx = await Context.Create(specProvider);
