...
public partial class SilRpcModuleTests
{
    ...
    private static SilCapabilities GetCaps(
        ...
        IHistoryPruner? historyPruner = null)
    {
        ...
        return new SilCapabilitiesProvider(
            blockTree,
            boundary,
    ...
    [Test]
    public void Sil_capabilities_no_head_disables_all_resources()
    {
        IReadOnlyBlockTree blockTree = Substitute.For<IReadOnlyBlockTree>();
        blockTree.Head.Returns((Block?)null);

        SilCapabilities caps = new SilCapabilitiesProvider(
            blockTree,
    ...
    public void Sil_capabilities_returns_expected_availability_for(CapabilitiesScenario s)
    {
        SilCapabilities caps = GetCaps(s.RetentionWindow, s.HeadNumber, s.OldestStateBlock, s.SyncConfig,
    ...
    /// <summary>
    /// JSON schema mirroring sila/execution-apis#755 — guards against naming-policy regressions
    /// (e.g. accidental return to <c>blockNumber</c>/<c>blockHash</c>) and missing required fields.
    /// </summary>
    private const string SilCapabilitiesSchema = """
        {
    ...
    [Test]
    public async Task Sil_capabilities_json_matches_spec_schema()
    {
        SilCapabilities caps = GetCaps(
            retentionWindow: 64,
            headNumber: 1000,
            syncConfig: new SyncConfig { DownloadReceiptsInFastSync = true, PivotNumber = 0 });

        string json = new SilaJsonSerializer().Serialize(caps);
        JsonSchema schema = await JsonSchema.FromJsonAsync(SilCapabilitiesSchema);
