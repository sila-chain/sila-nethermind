...
public partial class SilRpcModuleTests
{
    ...
    [TestCase(TxType.SIP1559, "maxFeePerGas", FeeFieldsMissingMessage, TestName = "Sip1559MaxFeePerGasMissing")]
    [TestCase(TxType.SIP1559, "maxPriorityFeePerGas", FeeFieldsMissingMessage, TestName = "Sip1559MaxPriorityFeePerGasMissing")]
    ...
    [TestCase(TxType.SIP1559, "nonce", "nonce not specified", TestName = "Sip1559NonceMissing")]
    ...
    {
        SIP1559TransactionForRpc rpcTx = (SIP1559TransactionForRpc)BuildTx(TxType.SIP1559);
        rpcTx.MaxFeePerGas = (UInt256)50_000_000_000_000UL; // 50000 gwei * 30400 gas = 1.52 SIL > 1 SIL cap
    ...
    [TestCase(false, typeof(SIP1559TransactionForRpc), TestName = "WithoutExplicitType_PromotedToSip1559")]
    ...
    public async Task SignTransaction_LegacyShapeJson_RespectsExplicitTypePinning(bool withExplicitType, Type expectedEchoType)
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_signTransaction", param);
        ...
        Assert.That(response.Result!.Tx, Is.TypeOf(expectedEchoType),
            "no-type input must auto-promote to SIP-1559; explicit type must be preserved");
    }
    ...
    [TestCase(TxType.SIP1559, typeof(SIP1559TransactionForRpc), TestName = "Sip1559")]
    ...
    public async Task SignTransaction_WhenValid_RawRoundTripsAndTxEcho(TxType type, Type expectedEchoType)
    {
        ...
        Address recovered = new SilaEcdsa(decoded.ChainId ?? 1).RecoverAddress(decoded)!;
    ...
    private async Task<string> SignTransaction(TransactionForRpc rpcTx)
    {
        using Context ctx = await Context.Create();
        ctx.Test.RpcConfig.EnableEthSignTransaction = true;
        return await ctx.Test.TestEthRpc("sil_signTransaction", rpcTx);
    }
    ...
    private async Task<ParsedSignTransactionResult> SignTransactionForResult(TransactionForRpc rpcTx)
    {
        using Context ctx = await Context.Create();
        ctx.Test.RpcConfig.EnableEthSignTransaction = true;
        string serialized = await ctx.Test.TestEthRpc("sil_signTransaction", rpcTx);
        JsonRpcResponse<ParsedSignTransactionResult> response = ctx.Test.JsonSerializer.Deserialize<JsonRpcResponse<ParsedSignTransactionResult>>(serialized)!;
        Assert.That(response.Result, Is.Not.Null, "precondition: signing must succeed for valid input");
    ...
    private static TransactionForRpc BuildTx(TxType type, string? omitField = null, string? fromOverride = null)
    {
        ...
        return type switch
        {
            TxType.SIP1559 => new SIP1559TransactionForRpc
            {
                From = from,
                To = to,
                Value = value,
