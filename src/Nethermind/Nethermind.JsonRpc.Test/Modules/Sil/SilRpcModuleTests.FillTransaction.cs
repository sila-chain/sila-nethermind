...
public partial class SilRpcModuleTests
{
    ...
    {
        yield return new TestCaseData((TransactionForRpc)new SIP1559TransactionForRpc
        {
    ...
    public async Task FillTransaction_WhenFieldsMissing_FillsNonceGasFeesAndChainId(TransactionForRpc rpcTx)
    {
        ...
        if (filled is SIP1559TransactionForRpc sip1559)
        {
            Assert.That(sip1559.MaxPriorityFeePerGas, Is.Not.Null, "maxPriorityFeePerGas must be filled from the gas-price oracle");
            Assert.That(sip1559.MaxFeePerGas, Is.Not.Null, "maxFeePerGas must be filled so the tx can be submitted as-is");
        }
        else
        {
    ...
    {
        SIP1559TransactionForRpc rpcTx = new()
        {
            From = TestItem.AddressC,
            To = TestItem.AddressB,
            Value = 1,
        ...
        SIP1559TransactionForRpc filled = (SIP1559TransactionForRpc)await FillTransactionForResult(rpcTx);
    ...
    private static IEnumerable<TestCaseData> InvalidInputCases()
    {
        yield return new TestCaseData(
            (TransactionForRpc)new SIP1559TransactionForRpc { From = null, To = TestItem.AddressB, Value = 1 },
            "from address not specified").SetName("FromMissing");

        yield return new TestCaseData(
            (TransactionForRpc)new SIP1559TransactionForRpc
            {
                From = TestItem.AddressC,
                To = TestItem.AddressB,
                Value = 1,
        ...
        yield return new TestCaseData(
            (TransactionForRpc)new SIP1559TransactionForRpc { From = TestItem.AddressC, To = null, Value = 1 },
    ...
    public async Task FillTransaction_WithUnfillableInput_ReturnsInvalidInput(TransactionForRpc rpcTx, string? expectedMessage)
    {
        ...
        string response = await ctx.Test.TestEthRpc("sil_fillTransaction", rpcTx);
    ...
    {
        // Seven blobs is over the SilaCancun per-tx limit; this must be rejected before any KZG work runs.
    ...
    {
        using Context ctx = await Context.CreateWithCancunEnabled();
        string serialized = await ctx.Test.TestEthRpc("sil_fillTransaction", rpcTx);
