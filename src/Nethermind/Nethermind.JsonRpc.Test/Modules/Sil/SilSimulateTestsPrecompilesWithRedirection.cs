...
public class SilSimulateTestsPrecompilesWithRedirection
{
    [Test]
    ...
    public async Task Test_eth_simulate_ecr_moved()
    {
        ...
        byte[] transactionData = SilRpcSimulateTestsBase.GetTxData(chain, TestItem.PrivateKeyA);

        Hash256 headHash = chain.BlockFinder.Head!.Hash!;
        Address contractAddress = await SilRpcSimulateTestsBase.DeployECRecoverContract(chain, TestItem.PrivateKeyB, SilSimulateTestsSimplePrecompiles.ECRecoverCallerContractBytecode);

        SilRpcSimulateTestsBase.ECRecoverCall(chain, TestItem.AddressB, transactionData, contractAddress);
