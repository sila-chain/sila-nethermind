...
namespace Nethermind.JsonRpc.Test.Modules.Sil.Simulate;

public class SilSimulateTestsBlocksAndTransactions
{
    public static SimulatePayload<TransactionForRpc> CreateSerializationPayload(TestRpcBlockchain chain)
    {
        ...
        Transaction txToFail = GetTransferTxData(nonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 10_000_000);
        ulong nextNonceA = ++nonceA;
        Transaction tx = GetTransferTxData(nextNonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 4_000_000);
        ...
        return new()
        {
            ...
            [
                new()
                {
                    ...
                    {
                        { TestItem.AddressA, new AccountOverride { Balance = 2100.Sila } }
                    }
                }
            ],
            TraceTransfers = true,
    ...
    {
        Transaction txAtoB1 = GetTransferTxData(nonceA + 1, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1);
        Transaction txAtoB2 = GetTransferTxData(nonceA + 2, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1);
        Transaction txAtoB3 = GetTransferTxData(nonceA + 3, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1);
        Transaction txAtoB4 = GetTransferTxData(nonceA + 4, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1);
    ...
    // Helper to convert Transaction to RPC format suitable for input (clears GasPrice for SIP-1559+ to avoid ambiguity)
    private static TransactionForRpc ToRpcForInput(Transaction tx)
    {
        TransactionForRpc rpc = TransactionForRpc.FromTransaction(tx);
        if (rpc is SIP1559TransactionForRpc sip1559Rpc)
            sip1559Rpc.GasPrice = null;
        return rpc;
    }
    ...
    public static SimulatePayload<TransactionForRpc> CreateTransactionsForcedFail(TestRpcBlockchain chain, ulong nonceA)
    {
        //shall be Ok
        Transaction txAtoB1 =
            GetTransferTxData(nonceA + 1, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1);

        //shall fail
        Transaction txAtoB2 =
            GetTransferTxData(nonceA + 2, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, UInt256.MaxValue);
    ...
    public static Transaction GetTransferTxData(ulong nonce, ISilaEcdsa silaEcdsa, PrivateKey from, Address to, UInt256 amount, TxType type = TxType.SIP1559)
    {
        Transaction tx = new()
        {
            Type = type,
            ...
            To = to,
            GasPrice = 20.GWei,
            DecodedMaxFeePerGas = type >= TxType.SIP1559 ? 20_000_000_000UL : 0UL
        };

        silaEcdsa.Sign(from, tx);
    ...
    public async Task Test_eth_simulateV1_empty_blockStateCalls_returns_error()
    {
        ...
        ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> result =
            chain.SilRpcModule.sil_simulateV1(payload, BlockParameter.Latest);
    ...
    public async Task Test_eth_simulateV1_block_time_override_crossing_a_fork_uses_the_post_fork_spec()
    {
        ...
        CustomSpecProvider specProvider = new(
            ((ForkActivation)0, SilaParis.Instance),
            (ForkActivation.TimestampOnly(shanghaiTimestamp), SilaShanghai.Instance));
    ...
    public async Task Test_eth_simulate_eth_moved()
    {
        ...
        ulong nonceA = chain.ReadOnlyState.GetNonce(TestItem.AddressA);
        Transaction txMainnetAtoB = GetTransferTxData(nonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1, type: TxType.Legacy);
    ...
    public async Task Test_eth_simulate_transactions_forced_fail()
    {
        ...
        Transaction txMainnetAtoB =
            GetTransferTxData(nonceA, chain.SilaEcdsa, TestItem.PrivateKeyA, TestItem.AddressB, 1, type: TxType.Legacy);
    ...
    public async Task Test_eth_simulate_respects_gas_cap(ulong gasCap, ulong? requestGas, bool expectCapped)
    {
        ...
        ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> result = chain.SilRpcModule.sil_simulateV1(
    ...
    [Test]
    public async Task TestTransferLogsAddress([Values] bool sip7708)
    {
        ...
        TestRpcBlockchain chain = await SilRpcSimulateTestsBase.CreateChain(spec);
        spec.IsSip7708Enabled = sip7708;
        Console.WriteLine("current test: simulateTransferOverBlockStateCalls");
        ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> result = chain.SilRpcModule.sil_simulateV1(payload!, BlockParameter.Latest);
        Log[] logs = result.Data.First().Calls.First().Logs.ToArray();
        Assert.That(logs.Length, Is.EqualTo(1));
        Assert.That(logs.First().Address == (sip7708 ? TransferLog.Sender : TransferLog.Src20Sender));
    }
    ...
    public async Task TestSerializationEthSimulate()
    {
        SimulatePayload<TransactionForRpc> payload = CreateTransferLogsAddressPayload();
        TestRpcBlockchain chain = await SilRpcSimulateTestsBase.CreateChain();
        JsonRpcResponse response = await RpcTest.TestRequest(chain.SilRpcModule, "sil_simulateV1", payload!, "latest");
        ...
        SimulatePayload<TransactionForRpc> payload = new()
        {
            ...
            [
                new()
                {
                    StateOverrides = new Dictionary<Address, AccountOverride>
                    {
                        { TestItem.AddressA, new AccountOverride { Balance = 100.Sila } },
    ...
    public async Task sil_simulateV1_MovePrecompileToAddress_invalid_override_returns_error(string payloadJson, int expectedErrorCode, string expectedMessage)
    {
        SilaJsonSerializer serializer = new();
    ...
    private static Task<TestRpcBlockchain> CreatePostMergeChain()
    {
        ...
        TestSpecProvider specProvider = new(SilaCancun.Instance);
    ...
    public async Task sil_simulateV1_prevrandao_opcode_returns_expected_value(string? overrideHex)
    {
            ...
            [
                new()
                {
                    ...
                    StateOverrides = new Dictionary<Address, AccountOverride>
                    {
                        ...
                        { TestItem.AddressA, new AccountOverride { Balance = 1.Sila } }
                    },
                    Calls =
                    [
                        new LegacyTransactionForRpc
    ...
    // Regression test for https://github.com/sila-chain/sila-nethermind/issues/8480
    // Verifies that blockOverrides.time is respected by the SAVM TIMESTAMP opcode in sil_simulateV1
    [TestCase(false)]
    [TestCase(true)]
    ...
    private static SimulatePayload<TransactionForRpc> InsufficientFundsPayload(bool validation) => new()
    {
        BlockStateCalls =
        [
            new() { Calls = [new LegacyTransactionForRpc { From = TestItem.AddressA, To = TestItem.AddressB, Value = 1_000_000.Sila }] }
        ],
        Validation = validation
    };
    ...
    private static SimulatePayload<TransactionForRpc> NoncePayload(ulong accountNonce, ulong txNonce) => new()
    {
        ...
        [
            new()
            {
                StateOverrides = new Dictionary<Address, AccountOverride>
                {
                    { TestItem.AddressA, new AccountOverride { Balance = 1.Sila, Nonce = accountNonce } }
                },
                Calls =
                [
                    new LegacyTransactionForRpc
    ...
    /// <summary>
    /// Regression test covering the execution-apis spec error codes/messages sil_simulateV1 must
    ...
    public async Task sil_simulateV1_returns_spec_error(
        ...
        string? expectedMessage)
    {
        ...
        ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> result =
            chain.SilRpcModule.sil_simulateV1(payloadFactory(), BlockParameter.Latest);
    ...
    /// <summary>
    /// Regression test for the Hive <c>silSimulate-simple-send-from-contract-with-validation</c> case:
    /// sil_simulateV1 must allow a state-overridden contract address as the <c>from</c> sender even
    /// when <c>validation:true</c>. SIP-3607 must not be enforced inside simulate.
    /// </summary>
    [Test]
    public async Task sil_simulateV1_contract_sender_with_state_override_succeeds_when_validation_enabled()
    {
        OverridableReleaseSpec spec = new(London.Instance) { IsSip3607Enabled = true };
    ...
    public async Task sil_simulateV1_blob_tx_rejected_when_max_fee_per_blob_gas_below_block_override(bool validation)
    {
        ...
        TestRpcBlockchain chain = await SilRpcSimulateTestsBase.CreateChain(SilaCancun.Instance);
