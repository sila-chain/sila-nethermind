...
/// <summary>
/// Regression tests for SIP-7610 collision detection in sil_simulateV1.
/// SIP-7610: Revert CREATE/CREATE2 if the target address already has non-empty storage.
/// </summary>
[TestFixture]
public class SilSimulateSip7610Tests
{
    ...
    /// <summary>
    /// SIP-7610: sil_simulateV1 must detect storage collision from a prior simulated block.
    ///
    ...
    /// </summary>
    [Test]
    public async Task Simulate_SIP7610_detects_storage_collision_from_prior_simulated_block()
    {
        TestRpcBlockchain chain = await SilRpcSimulateTestsBase.CreateChain(SilaOsaka.Instance);
        ...
        ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> step1 =
            chain.SilRpcModule.sil_simulateV1(block1Only, BlockParameter.Latest);
        ...
        Assert.That(returnedZeroAddress, Is.True,
            $"SIP-7610 storage collision not detected: block 2 CREATE2 returned {returnedAddress} " +
