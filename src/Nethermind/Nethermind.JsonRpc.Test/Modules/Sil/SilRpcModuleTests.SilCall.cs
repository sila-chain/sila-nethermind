...
{
    [Test]
    public async Task Sil_call_web3_sample()
    {
        using Context ctx = await Context.Create();
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
            $"{{\"data\": \"{BalanceOfCallData}\", \"to\": \"{BatTokenAddress}\"}}");
        string serialized =
            await ctx.Test.TestEthRpc("sil_call", transaction, "0x0");
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"result\":\"0x\",\"id\":67}"));
    }

    [Test]
    public async Task Sil_call_web3_sample_not_enough_gas_system_account()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_call_web3_should_return_insufficient_balance_error()
    {
        using Context ctx = await Context.Create();
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction);
    ...
    [Test]
    public async Task Sil_call_web3_sample_not_enough_gas_other_account()
    {
        using Context ctx = await Context.Create();
        AssertAccountDoesNotExist(ctx, TestAccount);
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_call_no_sender()
    {
        using Context ctx = await Context.Create();
        LegacyTransactionForRpc transaction = new(new Transaction(), new(BlockchainIds.SilaMainnet))
        {
        ...
        string serialized =
            await ctx.Test.TestEthRpc("sil_call", transaction, "latest");
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"result\":\"0x\",\"id\":67}"));
    }

    [Test]
    public async Task Sil_call_no_recipient_should_work_as_init()
    {
        using Context ctx = await Context.Create();
        LegacyTransactionForRpc transaction = new(new Transaction(), new(BlockchainIds.SilaMainnet))
        {
    ...
    [Test]
    public async Task should_not_reject_transactions_with_deployed_code_when_sip3607_enabled()
    {
        ...
        LegacyTransactionForRpc transaction = new(tx, new(tx.ChainId ?? BlockchainIds.SilaMainnet))
        {
            To = TestItem.AddressB,
            GasPrice = 0
        };
    ...
    [Test]
    public async Task Sil_call_sila_recipient()
    {
        using Context ctx = await Context.Create();
        string serialized = await ctx.Test.TestEthRpc("sil_call",
            "{\"data\":\"0x12\",\"from\":\"0x7301cfa0e1756b71869e93d4e4dca5c7d0eb0aa6\",\"to\":\"sila\"}",
            "latest");
        Assert.That(serialized.StartsWith("{\"jsonrpc\":\"2.0\",\"error\""), Is.True);
    }

    [Test]
    public async Task Sil_call_ok()
    {
        using Context ctx = await Context.Create();
        LegacyTransactionForRpc transaction = new(new Transaction(), new(BlockchainIds.SilaMainnet))
        {
    ...
    [Test]
    public async Task Sil_call_with_blockhash_ok()
    {
        using Context ctx = await Context.Create();
        LegacyTransactionForRpc transaction = new(new Transaction(), new(BlockchainIds.SilaMainnet))
        {
        ...
        string serialized =
            await ctx.Test.TestEthRpc("sil_call", transaction, "{\"blockHash\":\"0xf0b3f69cbd4e1e8d9b0ef02ff5d1384d18e19d251a4052f5f90bab190c5e8937\"}");
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32000,\"message\":\"header not found\"},\"id\":67}"));
    }

    [Test]
    public async Task Sil_call_create_tx_with_empty_data()
    {
        using Context ctx = await Context.Create();
        LegacyTransactionForRpc transaction = new(new Transaction(), new(BlockchainIds.SilaMainnet))
        {
    ...
    [Test]
    public async Task Sil_call_missing_state_after_fast_sync()
    {
    ...
    [Test]
    public async Task Sil_call_with_accessList()
    {
        ...
        string serialized = await test.TestEthRpc("sil_call", transaction, "0x0");
        Assert.That(serialized, Is.EqualTo("{\"jsonrpc\":\"2.0\",\"result\":\"0x010203\",\"id\":67}"));
    }

    [Test]
    public async Task Sil_call_without_gas_pricing()
    {
        using Context ctx = await Context.Create();
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_call_keeps_explicit_zero_gas_limit()
    {
        using Context ctx = await Context.Create();
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_call_with_gas_pricing()
    {
        using Context ctx = await Context.Create();
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_call_without_gas_pricing_after_1559_legacy()
    {
    ...
    [Test]
    public async Task Sil_call_without_gas_pricing_after_1559_new_type_of_transaction()
    {
        using Context ctx = await Context.CreateWithLondonEnabled();
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
    ...
    [Test]
    public async Task Sil_call_with_base_fee_opcode_should_return_0()
    {
        using Context ctx = await Context.CreateWithLondonEnabled();
    ...
    [Test]
    public async Task Sil_call_with_base_fee_opcode_without_from_address_should_return_0()
    {
        using Context ctx = await Context.CreateWithLondonEnabled();
    ...
    [Test]
    public async Task Sil_call_with_coinbase_opcode_should_return_block_override_fee_recipient()
    {
        using Context ctx = await Context.Create();
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction, "latest", null, blockOverride);
    ...
    [Test]
    public async Task Sil_call_with_value_transfer_without_from_address_should_throw()
    {
        using Context ctx = await Context.CreateWithLondonEnabled();

        string dataStr = BaseFeeReturnCode.ToHexString(true);
        TransactionForRpc transaction = ctx.Test.JsonSerializer.Deserialize<TransactionForRpc>(
            $"{{\"type\": \"0x2\", \"value\":\"{1.Sila}\", \"data\": \"{dataStr}\"}}");
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction);
        JToken parsed = JToken.Parse(serialized);
        Assert.That(parsed["error"]!["code"]!.Value<int>(), Is.EqualTo(-32000));
        Assert.That(parsed["error"]!["message"]!.Value<string>(), Does.Contain(TxErrorMessages.InsufficientFundsForGas));
    ...
    [Test]
    public async Task Sil_call_with_revert()
    {
        using Context ctx = await Context.CreateWithLondonEnabled();
    ...
    [Test]
    public async Task Sil_call_with_custom_error_revert_puts_bytes_only_in_data()
    {
        ...
        // See: https://github.com/sila-chain/sila-nethermind/issues/11095
        using Context ctx = await Context.CreateWithLondonEnabled();
    ...
    [Test]
    public async Task Sil_call_with_revert_sentinel_string_as_message_still_appends_decoded_message()
    {
    ...
    public async Task Sil_call_with_state_override(string name, string transactionJson, string stateOverrideJson, string expectedResult)
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction, "latest", stateOverride);
    ...
    public async Task Sil_call_with_state_override_does_not_affect_other_calls(string name, string transactionJson, string stateOverrideJson)
    {
        object? transaction = JsonSerializer.Deserialize<object>(transactionJson);
        object? stateOverride = JsonSerializer.Deserialize<object>(stateOverrideJson);
        ...
        string resultOverrideBefore = await ctx.Test.TestEthRpc("sil_call", transaction, "latest", stateOverride);

        string resultNoOverride = await ctx.Test.TestEthRpc("sil_call", transaction, "latest");

        string resultOverrideAfter = await ctx.Test.TestEthRpc("sil_call", transaction, "latest", stateOverride);
    ...
    [Test]
    public async Task Sil_call_uses_gas_cap_when_not_specified()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_call_uses_specified_gas_limit()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_call_cannot_exceed_gas_cap()
    {
        using Context ctx = await Context.Create();
    ...
    /// <summary>
    /// Verifies gas cap is enforced by measuring actual gas available to the SAVM.
    /// Gas cap enforcement is in the shared <c>TxExecutor.Execute()</c> base class,
    /// so this also covers <c>sil_estimateGas</c> and <c>sil_createAccessList</c>.
    /// </summary>
    [Test]
    public async Task Sil_call_gas_available_is_capped_by_gas_cap()
    {
        using Context ctx = await Context.Create();
        ulong gasCap = 50_000;
    ...
    /// </summary>
    [Test]
    public async Task Sil_call_without_gas_defaults_to_gas_cap_not_block_gas_limit()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_call_ignores_invalid_nonce()
    {
        using Context ctx = await Context.Create();
        byte[] code = Prepare.SavmCode
         .Op(Instruction.STOP)
    ...
    [Test]
    public async Task Sil_call_contract_creation()
    {
        using Context ctx = await Context.Create();
        byte[] code = Prepare.SavmCode
            .Op(Instruction.STOP)
    ...
    [TestCase(null)]
    [TestCase(new byte[0])]
    public async Task Sil_call_to_is_null_and_not_contract_creation(byte[]? data)
    {
        using Context ctx = await Context.Create();
        Transaction tx = Build.A.Transaction
            .WithGasLimit(100000)
    ...
    [Test]
    public async Task Sil_call_gas_price_in_sip1559_tx()
    {
        using Context ctx = await Context.Create();
        Transaction tx = Build.A.Transaction
            .WithGasLimit(100000)
    ...
    [TestCase(true)]
    [TestCase(false)]
    public async Task Sil_call_no_blobs_in_blob_tx(bool isNull)
    {
        using Context ctx = await Context.Create();
        Transaction tx = Build.A.Transaction
            .WithGasLimit(100000)
            ...
            .SignedAndResolved(TestItem.PrivateKeyA)
            .TestObject;
        BlobTransactionForRpc transaction = new(tx, new(tx.ChainId ?? BlockchainIds.SilaMainnet));
        transaction.GasPrice = null;
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction);
    ...
    [Test]
    public async Task Sil_call_maxFeePerBlobGas_is_zero()
    {
        using Context ctx = await Context.Create();
    ...
    [Test]
    public async Task Sil_call_missing_to_in_blob_tx()
    {
        using Context ctx = await Context.Create();
        byte[] code = Prepare.SavmCode
            .Op(Instruction.STOP)
    ...
    [Test]
    public async Task Sil_call_maxFeePerGas_smaller_then_maxPriorityFeePerGas()
    {
        using Context ctx = await Context.Create();
        Transaction tx = Build.A.Transaction
            .WithGasLimit(100000)
    ...
    public async Task Sil_call_invalid_blob_versioned_hash(byte[]? hash, string expectedError)
    {
        using Context ctx = await Context.Create();
        Transaction tx = Build.A.Transaction
            .WithGasLimit(100000)
    ...
    [Test]
    public async Task Sil_call_rejects_blob_transaction_with_too_many_blob_hashes()
    {
        using Context ctx = await Context.Create(new SingleReleaseSpecProvider(SilaCancun.Instance, BlockchainIds.SilaMainnet, BlockchainIds.SilaMainnet));
    ...
    [Test]
    public async Task Sil_call_bubbles_up_precompile_errors()
    {
        using Context ctx = await Context.Create(new SingleReleaseSpecProvider(SilaOsaka.Instance, BlockchainIds.SilaMainnet, BlockchainIds.SilaMainnet));
        Transaction tx = Build.A.Transaction
    ...
    [TestCase("""{"input":"0x1ab"}""", TestName = "SIP1559 tx odd-length input")]
    [TestCase("""{"data":"0x1ab","maxFeePerGas":"0x1"}""", TestName = "SIP1559 tx odd-length data")]
    public async Task Sil_call_odd_length_input_returns_invalid_params(string txJson)
    {
        using Context ctx = await Context.Create();
        JsonElement txParam = JsonDocument.Parse(txJson).RootElement;
        string serialized = await ctx.Test.TestEthRpc("sil_call", txParam, "latest");
        Assert.That(JToken.Parse(serialized)["error"]!["code"]!.Value<int>(), Is.EqualTo(-32602));
    }

    [Test]
    public async Task Sil_call_non_existent_block_returns_not_found()
    {
        using Context ctx = await Context.Create();
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction, "0xFFFFFFFF");
    ...
    public async Task Sil_call_with_block_override(string name, string txJson, string stateOverrideJson, string blockOverrideJson, string expectedResult)
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction, "latest", stateOverride, blockOverride);

        Assert.That(JToken.Parse(serialized)["result"]!.Value<string>(), Is.EqualTo(expectedResult));
    }

    [Test]
    public async Task Sil_call_feeless_with_positive_blockOverride_baseFeePerGas_succeeds()
    {
    ...
    public async Task Sil_call_blobBaseFeePerGas_override_test(
        ...
        string? expectedResult, string? expectedError)
    {
        ...
        string serialized = await test.TestEthRpc("sil_call", transaction, "latest", stateOverride, blockOverride);
        JToken parsed = JToken.Parse(serialized);

        if (expectedResult is not null)
        {
            Assert.That(parsed["error"], Is.Null, "opcode must be valid under SilaCancun");
    ...
    [Test]
    public async Task Sil_call_blob_tx_rejected_when_maxFeePerBlobGas_below_block_blobBaseFee()
    {
        ...
        string serialized = await test.TestEthRpc("sil_call", tx, "latest", stateOverride, null);
        Assert.That(
            JToken.Parse(serialized)["error"]!["message"]!.Value<string>(),
    ...
    public async Task Sil_call_setCode_invalid_transaction_returns_error(string txJson, string expectedMessage)
    {
        ...
        string serialized = await ctx.Test.TestEthRpc("sil_call", transaction, "latest");
    ...
    [Test]
    public async Task Sil_call_setCode_missing_yParity_returns_error()
    {
        TestSpecProvider specProvider = new(SilaPrague.Instance);
        using Context ctx = await Context.Create(specProvider);
    ...
    /// that is SIP-158 empty.
    /// </summary>
    [Test]
    public async Task Sil_call_state_override_with_storage_blocks_create2_via_sip7610()
    {
        using Context ctx = await Context.Create(new TestSpecProvider(SilaOsaka.Instance));
        (object stateOverride, object transaction) = BuildSip7610Fixture();
    ...
    public async Task Sil_call_zero_balance_reports_full_required_balance(
        string transactionJson, string? blockOverrideJson, string expectedDetailSubstring)
    {
        using Context ctx = await Context.Create(new TestSpecProvider(SilaCancun.Instance));
    ...
    private static IEnumerable<TestCaseData> ZeroBalanceWantCases()
    {
        ...
        { TestName = "Sip1559: want includes gasLimit*maxFeePerGas" };
    ...
    // SIP-1474: QUANTITY fields in BlockOverrides must not have leading zero digits.
    ...
    public async Task Sil_call_block_override_leading_zero_quantity_is_rejected(string blockOverrideJson)
    {
        using Context ctx = await Context.Create();
    ...
    public async Task Sil_call_block_override_valid_quantity_is_accepted(string blockOverrideJson)
    {
        using Context ctx = await Context.Create();
        object transaction = JsonSerializer.Deserialize<object>("""{"to":"0xc200000000000000000000000000000000000000"}""")!;
        object blockOverride = JsonSerializer.Deserialize<object>(blockOverrideJson)!;
