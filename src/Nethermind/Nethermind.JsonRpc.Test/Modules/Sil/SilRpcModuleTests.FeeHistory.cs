...
public partial class SilRpcModuleTests
{
    ...
    public async Task Sil_feeHistory(long blockCount, string blockParameter, string expected)
    {
        using Context ctx = await Context.CreateWithLondonEnabled();
        string serialized = await ctx.Test.TestEthRpc("sil_feeHistory", blockCount.ToString(), blockParameter, "[0,10.5,20,60,90]");
        Assert.That(serialized, Is.EqualTo(expected));
    }

    [TestCaseSource(nameof(FeeHistoryBlobTestCases))]
    public (UInt256[], double[]) Sil_feeHistory_ShouldReturnCorrectBlobValues(ulong?[] excessBlobGas, ulong?[] blobGasUsed)
    {
        ...
        ISpecProvider specProvider = new TestSingleReleaseSpecProvider(SilaCancun.Instance);
        ...
        get
        {
            ...
            yield return new TestCaseData(
                new ulong?[] { 1,
                    2,
                    0,
                    SilaCancun.Instance.GasCosts.TargetBlobGasPerBlock,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock * 4 },
                new ulong?[] { 0,
                    Sip4844Constants.GasPerBlob * 2,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock,
                    SilaCancun.Instance.GasCosts.MaxBlobGasPerBlock })
