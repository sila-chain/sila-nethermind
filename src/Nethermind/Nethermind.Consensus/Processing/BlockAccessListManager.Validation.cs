...
///     SIP-8037 2D inclusion check on the cumulative gas, then merges the per-tx BAL slice
///     and calls <see cref="ValidateBlockAccessList"/>.
///   * <see cref="CheckPerTxInclusion"/> is the SIP-8037 worst-case 2D inclusion check.
...
public partial class BlockAccessListManager
{
    public void IncrementalValidation(Block block, GasValidationResultSlot[] gasResults, BlockReceiptsTracer[] receiptsTracers, BlockValidationTransactionsExecutor.ITransactionProcessedEventHandler? transactionProcessedEventHandler, CancellationToken token)
    {
        ...
        // SIP-8037: 2D gas accounting — block gasUsed = max(sum_regular, sum_state)
        _blockExecutionContext.Value.Header.GasUsed = SilaGasPolicy.CombineBlockGas(totalRegularGas, totalStateGas);

        static void CheckGasUsed(int index, Block block, ulong totalRegularGas, ulong totalStateGas)
        {
            // SIP-8037: block gasUsed = max(sum_regular, sum_state)
            ulong effectiveGas = SilaGasPolicy.CombineBlockGas(totalRegularGas, totalStateGas);
    ...
    // SIP-8037 worst-case 2D inclusion check. Only fires when SIP-8037 is active; legacy and
    // pre-SIP-8037 blocks rely solely on the post-execution running max(R,S) check.
    ...
    {
        if (!spec.IsSip8037Enabled) return;
        ...
        if (outcome != Sip8037BlockGasInclusionCheck.Outcome.Ok)
        {
            throw new InvalidBlockException(block,
                $"Block gas limit exceeded: tx {index} fails SIP-8037 inclusion check ({outcome}); " +
    ...
    private static bool IsSystemContract(Address address)
        => address == Sip7002Constants.WithdrawalRequestPredeployAddress
        || address == Sip7251Constants.ConsolidationRequestPredeployAddress
        || address == Sip8282Constants.BuilderDepositRequestPredeployAddress
        || address == Sip8282Constants.BuilderExitRequestPredeployAddress;
    ...
    {
        if (validateStorageReads && surplusReads > 0ul && _gasRemaining < surplusReads * Sip7928Constants.ItemCost)
        {
