    ...
    : IBlockProcessor
{
        ...
        CancellationToken token)
    {
        ...
        // the spec has Sip158Enabled=false, so we end up persisting empty accounts created while processing withdrawals.
        CommitState(spec);
    ...
    // Timing sinks — forward elapsed ticks into the appropriate SAVM metric counters.
    ...
    {
        public static void AddTicks(long ticks) => Savm.Metrics.IncrementCommitTime(ticks);
    ...
    private readonly struct StorageMerkleTimeSink : IMetricSink
    {
        ...
        {
            Savm.Metrics.IncrementStateHashTime(ticks);
            Savm.Metrics.IncrementStorageMerkleTime(ticks);
        }
        public static bool IsEnabled => ExecutionMetricsFlag.IsActive;
    }
    ...
    private readonly struct StateRootTimeSink : IMetricSink
    {
        ...
        {
            Savm.Metrics.IncrementStateHashTime(ticks);
            Savm.Metrics.IncrementStateRootTime(ticks);
        }
        public static bool IsEnabled => ExecutionMetricsFlag.IsActive;
    }
    ...
    {
        public static void AddTicks(long ticks) => Savm.Metrics.IncrementReceiptsRootTime(ticks);
        public static bool IsEnabled => ExecutionMetricsFlag.IsActive;
    }

    private readonly struct BloomsTimeSink : IMetricSink
    {
        public static void AddTicks(long ticks) => Savm.Metrics.IncrementBloomsTime(ticks);
        public static bool IsEnabled => ExecutionMetricsFlag.IsActive;
    }
    ...
    {
        if (_logger.IsTrace) _logger.Trace($"  {(BigInteger)reward.Value / (BigInteger)Unit.Sila:N3}{Unit.SilSymbol} for account at {reward.Address}");
