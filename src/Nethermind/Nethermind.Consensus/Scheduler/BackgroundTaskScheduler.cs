...
public class BackgroundTaskScheduler : IBackgroundTaskScheduler, IAsyncDisposable
{
            ...
            while (await _taskQueue.Reader.WaitToReadAsync(_mainCancellationTokenSource.Token))
            {
                ...
                try
                {
                    ...
                    while (_taskQueue.Reader.TryRead(out IActivity activity))
                    {
                        ...
                        Savm.Metrics.IncrementTotalBackgroundTasksExecuted();
                    }
                }
    ...
    public bool TryScheduleTask<TReq>(TReq request, Func<TReq, CancellationToken, Task> fulfillFunc, TimeSpan? timeout = null, string? source = null)
    {
        ...
        Savm.Metrics.IncrementTotalBackgroundTasksQueued();
        ...
        Savm.Metrics.IncrementTotalBackgroundTasksDropped();
        ...
        if (_logger.IsWarn && now - lastLog > 10_000 && Interlocked.CompareExchange(ref _lastDropLogTicks, now, lastLog) == lastLog)
        {
            _logger.Warn(
                ...
                $"Totals: queued={Savm.Metrics.TotalBackgroundTasksQueued}, executed={Savm.Metrics.TotalBackgroundTasksExecuted}, " +
                $"dropped={Savm.Metrics.TotalBackgroundTasksDropped}");
        }
        Interlocked.Decrement(ref _queueCount);
        request.TryDispose();
        activity.Return();
    ...
    private void UpdateQueueCount() => Savm.Metrics.NumberOfBackgroundTasksScheduled = Volatile.Read(ref _queueCount);
