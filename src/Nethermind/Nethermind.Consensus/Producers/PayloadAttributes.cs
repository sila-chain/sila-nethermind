...
public class PayloadAttributes
{
    ...
    private static bool IsSupportedFcuForkCombination(int fcuVersion, int payloadVersion) =>
        ...
        {
            // Exception: FCUv2 also accepts SilaParis (V1) attributes for backward compatibility.
    ...
    /// </summary>
    /// <returns>
    /// <see cref="PayloadAttributesValidationResult.UnsupportedFork"/> — FCU version doesn't support this fork (post-SilaParis only);
...
public static class PayloadAttributesExtensions
{
    ...
    public static int ExpectedPayloadAttributesVersion(this IReleaseSpec spec) =>
        spec switch
        {
            { IsSip7843Enabled: true } => PayloadAttributesVersions.V4,
            { IsSip4844Enabled: true } => PayloadAttributesVersions.V3,
