    ...
    ILogManager logManager)
{
    ...
    private BlockProcessor GetProcessor()
    {
        ...
        SilaTransactionProcessor txProcessor = CreateTransactionProcessor(WorldState, blockhashProvider);
        BlockAccessListManager blockAccessListManager = new(
            ...
            codeInfoRepositoryFactory: static state => new CacheCodeInfoRepository(state, new SilaPrecompileProvider(), NoopCodeCache.Instance),
    ...
    private SilaTransactionProcessor CreateTransactionProcessor(IWorldState state, IBlockhashProvider blockhashProvider)
        => new(
            BlobBaseFeeCalculator.Instance,
            specProvider,
            state,
            new SilaVirtualMachine(blockhashProvider, specProvider, logManager),
            new CacheCodeInfoRepository(state, new SilaPrecompileProvider(), StaticCodeCache.Instance),
            logManager
        );
}
