...
/// <param name="Output">SAVM return data — non-null on success (possibly empty for void returns) and on revert (revert payload); null when the call produced no return data (out-of-gas, invalid jump, input error).</param>
...
/// <param name="ExecutionReverted">True iff the SAVM exited via REVERT; mirrors <c>CallOutput.ExecutionReverted</c>.</param>
...
/// Used by proof_call to capture state accessed during a single sil_call.
/// </summary>
public interface ISingleCallWitnessCollector
{
    ...
    /// <param name="cancellationToken">Aborts the SAVM run if it outlives the caller's deadline (e.g. the RPC timeout).</param>
    /// <returns>The call's return data, sil_call-style success/error flags, and the execution witness containing all state data accessed during the call.</returns>
    ...
    ITransactionProcessor transactionProcessor) : ISingleCallWitnessCollector
{
    public SingleCallWitnessResult ExecuteCallAndCollectWitness(BlockHeader blockHeader, Transaction transaction, CancellationToken cancellationToken = default)
    {
        ...
        // `nonce` fails pre-VM validation (e.g. TransactionNonceTooHigh) before the SAVM runs, and
        // diverges from sil_call which performs the same ignore-nonce step.
        ...
        // the SAVM throws OperationCanceledException, surfaced by JsonRpcService as a Timeout error.
        ...
        return new SingleCallWitnessResult(
            ...
            ExecutionReverted: txResult.SavmExceptionType == SavmExceptionType.Revert,
