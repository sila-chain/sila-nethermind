...
public sealed class TxValidator : ITxValidator
{
    ...
    public TxValidator(ulong chainId)
    {
        ...
        RegisterValidator(TxType.AccessList, new CompositeTxValidator([
            new ReleaseSpecTxValidator(static spec => spec.IsSip2930Enabled),
            NonceCapTxValidator.Instance,
            IntrinsicGasTxValidator.Instance,
        ...
        RegisterValidator(TxType.SIP1559, new CompositeTxValidator([
            new ReleaseSpecTxValidator(static spec => spec.IsSip1559Enabled),
            NonceCapTxValidator.Instance,
            IntrinsicGasTxValidator.Instance,
            SignatureTxValidator.Instance,
            expectedChainIdTxValidator,
        ...
        RegisterValidator(TxType.Blob, new CompositeTxValidator([
            new ReleaseSpecTxValidator(static spec => spec.IsSip4844Enabled),
            NonceCapTxValidator.Instance,
            IntrinsicGasTxValidator.Instance,
            SignatureTxValidator.Instance,
            expectedChainIdTxValidator,
        ...
        RegisterValidator(TxType.SetCode, new CompositeTxValidator([
            new ReleaseSpecTxValidator(static spec => spec.IsSip7702Enabled),
            NonceCapTxValidator.Instance,
            IntrinsicGasTxValidator.Instance,
            SignatureTxValidator.Instance,
            expectedChainIdTxValidator,
    ...
    /// as we cannot generalize correctness of the transaction without knowing the SIPs implemented
...
public sealed class IntrinsicGasTxValidator : ITxValidator
{
    ...
    public ValidationResult IsWellFormed(Transaction transaction, IReleaseSpec releaseSpec, ulong blockGasLimit)
    {
        IntrinsicGas<SilaGasPolicy> intrinsicGas = SilaGasPolicy.CalculateIntrinsicGas(transaction, releaseSpec, blockGasLimit);
        if (releaseSpec.IsSip8037Enabled && intrinsicGas.ExceedsCap(Sip7825Constants.DefaultTxGasLimitCap, out ulong regular, out ulong floor))
        {
            return TxErrorMessages.TxIntrinsicGasExceedsCap(regular, floor, Sip7825Constants.DefaultTxGasLimitCap);
        }

        // Implicit conversion combines Standard + StateReservoir (SIP-8037) before max'ing with FloorGas.
        SilaIntrinsicGas combined = intrinsicGas;
...
public sealed class BlobFieldsTxValidator : ITxValidator
{
    ...
    private static ValidationResult ValidateBlobFields(Transaction transaction, IReleaseSpec spec)
    {
        ...
        for (int i = 0; i < blobCount; i++)
        {
            switch (transaction.BlobVersionedHashes[i])
            {
                ...
                case { Length: not Sip4844Constants.BytesPerBlobVersionedHash }: return TxErrorMessages.InvalidBlobVersionedHashSize;
                case { Length: Sip4844Constants.BytesPerBlobVersionedHash } when transaction.BlobVersionedHashes[i][0] != KzgPolynomialCommitments.KzgBlobHashVersionV1: return TxErrorMessages.InvalidBlobVersionedHashVersion;
            }
        }
    ...
    {
        if (txBlobCount < Sip4844Constants.MinBlobsPerTransaction)
        {
...
public abstract class BaseSignatureTxValidator : ITxValidator
{
    ...
    public ValidationResult IsWellFormed(Transaction transaction, IReleaseSpec releaseSpec)
    {
        ...
        UInt256 sMax = releaseSpec.IsSip2Enabled ? SecP256k1Curve.HalfNPlusOne : SecP256k1Curve.N;
...
public sealed class LegacySignatureTxValidator(ulong chainId) : BaseSignatureTxValidator
{
    protected override ValidationResult ValidateChainId(Transaction transaction, IReleaseSpec releaseSpec)
    {
        ...
        return releaseSpec.IsSip155Enabled && (v == chainId * 2 + 35ul || v == chainId * 2 + 36ul)
            ? ValidationResult.Success
