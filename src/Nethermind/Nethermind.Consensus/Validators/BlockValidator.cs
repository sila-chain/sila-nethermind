    ...
    : IBlockValidator
{
    ...
    private readonly SilaEcdsa _ecdsa = new(specProvider.ChainId);
    ...
    /// <summary>
    /// Suggested block validation runs basic checks that can be executed before going through the expensive SAVM processing.
    /// </summary>
    /// <param name="block">A block to validate</param>
    ...
    private bool ValidateBlock<TOrphaned>(Block block, BlockHeader? parent, out string? errorMessage, bool validateHashes = true) where TOrphaned : struct, IFlag
    {
        ...
        return ValidateBlockSize(block, spec, ref errorMessage) &&
               ...
               ValidateSip4844Fields(block, spec, ref errorMessage) &&
    ...
    {
        if (spec.IsSip7934Enabled)
        {
            int encodedSize = block.EncodedSize ?? _blockDecoder.GetLength(block, RlpBehaviors.None);
            if (encodedSize > spec.Sip7934MaxRlpBlockSize)
            {
                if (_logger.IsDebug) _logger.Debug($"{Invalid(block)} RLP encoded size of {encodedSize} bytes exceeds the max limit of {spec.Sip7934MaxRlpBlockSize} bytes");
                errorMessage = BlockErrorMessages.ExceededBlockSizeLimit(spec.Sip7934MaxRlpBlockSize);
                return false;
            }
        }
    ...
    /// </summary>
    /// <param name="processedBlock">This should be the block processing result (after going through the SAVM processing)</param>
    ...
    {
        // SIP-7928: enforce the BAL item gas-limit floor against the BAL produced during
    ...
    protected virtual bool ValidateWithdrawals(Block block, IReleaseSpec spec, bool validateHashes, ref string? error)
    {
        if (validateHashes)
        {
            if (spec.WithdrawalsEnabled && block.Withdrawals is null)
            {
                ...
                if (_logger.IsWarn) _logger.Warn($"Withdrawals cannot be null in block {block.Hash} when SIP-4895 activated.");
                return false;
            }
            ...
            if (!spec.WithdrawalsEnabled && block.Withdrawals is not null)
            {
                ...
                if (_logger.IsWarn) _logger.Warn($"Withdrawals must be null in block {block.Hash} when SIP-4895 not activated.");
                return false;
            }
    ...
    protected virtual bool ValidateTransactions(Block block, IReleaseSpec spec, ref string? errorMessage)
    {
        ...
        for (int txIndex = 0; txIndex < transactions.Length; txIndex++)
        {
            ...
            // Recover the sender if a preprocessor hasn't yet: the SIP-2780 self-transfer discount
            // makes the intrinsic-gas validation below sender-dependent.
            if (spec.IsSip2780Enabled && transaction.SenderAddress is null && transaction.Signature is not null)
    ...
    protected virtual bool ValidateSip4844Fields(Block block, IReleaseSpec spec, ref string? error)
    {
    ...
    public virtual bool ValidateBlockLevelAccessList(Block block, IReleaseSpec spec, ref string? error)
    {
        ...
        // even when SIP-7928 is active. We only validate the body if it's present.
        ...
        if (!spec.BlockLevelAccessListsEnabled && block.BlockAccessList is not null)
        {
            ...
            if (_logger.IsWarn) _logger.Warn($"Block level access list must be null in block {block.Hash} when SIP-7928 not activated.");
    ...
    private bool ValidateBlockLevelAccessListSize(Block block, ref string? error)
    {
        ...
        ulong maxBalItems = block.Header.GasLimit / Sip7928Constants.ItemCost;
    ...
    // SIP-7928: BlockAccessIndex valid range is [0, txCount + 1]
