    ...
    SimulatePayload<TransactionWithSourceDetails>>(blockchainBridge, blockFinder, rpcConfig)
{
        ...
        foreach (SimulateBlockResult<TTrace> item in results.Items)
        {
            if (item is SimulateBlockResult<SimulateCallResult> result)
            {
                foreach (SimulateCallResult? call in result.Calls)
                {
                    ...
                    {
                        SavmExceptionType exception = call.Error.SavmException;
                        call.Error.Code = MapSavmExceptionType(exception);
                        if (exception != SavmExceptionType.Revert)
                        {
                            call.Error.Message = call.Error.SavmException.GetSavmExceptionDescription();
    ...
    private static int MapSimulateErrorCode(TransactionResult txResult)
    {
        ...
        return MapSavmExceptionType(txResult.SavmExceptionType);
    }

    /// <summary>
    /// Returns the spec-mandated error message for well-known sil_simulateV1 error types,
    ...
    private static int MapSavmExceptionType(SavmExceptionType type) => type switch
    {
        SavmExceptionType.Revert => ErrorCodes.ExecutionReverted,
        _ => ErrorCodes.VMError
    };
}

/// <summary>
/// Canonical sil_simulateV1 error message strings shared between the executor and tests.
/// </summary>
