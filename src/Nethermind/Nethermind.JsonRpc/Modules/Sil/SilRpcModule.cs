...
public partial class SilRpcModule(
    ...
    IBlockForRpcFactory blockForRpcFactory) : ISilRpcModule
{
    ...
    protected readonly ILogger _logger = logManager.GetClassLogger<SilRpcModule>();
    ...
    private readonly ISilCapabilitiesProvider _capabilitiesProvider = capabilitiesProvider ?? throw new ArgumentNullException(nameof(capabilitiesProvider));
    protected readonly ISilSyncingInfo _ethSyncingInfo = silSyncingInfo ?? throw new ArgumentNullException(nameof(silSyncingInfo));
    ...
    readonly JsonSerializerOptions UnchangedDictionaryKeyOptions = new(SilaJsonSerializer.JsonOptionsIndented) { DictionaryKeyPolicy = null };

    public ResultWrapper<string> sil_protocolVersion()
    {
        int highestVersion = _protocolsManager.GetHighestProtocolVersion(Protocol.Sil);
        return ResultWrapper<string>.Success(highestVersion.ToHexString());
    }

    public ResultWrapper<SyncingResult> sil_syncing() => ResultWrapper<SyncingResult>.Success(_ethSyncingInfo.GetFullInfo());

    public ResultWrapper<byte[]> sil_snapshot() => ResultWrapper<byte[]>.Fail("sil_snapshot not supported");

    public ResultWrapper<Address> sil_coinbase() => ResultWrapper<Address>.Success(Address.Zero);

    public async Task<ResultWrapper<UInt256?>> sil_gasPrice() => ResultWrapper<UInt256?>.Success(await _gasPriceOracle.GetGasPriceEstimate());

    public ResultWrapper<UInt256?> sil_blobBaseFee()
    {
    ...
    public ResultWrapper<UInt256?> sil_baseFee()
    {
        BlockHeader? head = _blockFinder.Head?.Header;
        if (head is null)
        {
        ...
        ISip1559Spec specFor1559 = _specProvider.GetSpecFor1559(head.Number + 1);
        if (!specFor1559.IsSip1559Enabled)
        {
            return ResultWrapper<UInt256?>.Success(null);
        }
    ...
    public ResultWrapper<UInt256?> sil_maxPriorityFeePerGas()
    {
    ...
    public ResultWrapper<FeeHistoryResults> sil_feeHistory(ulong blockCount, BlockParameter newestBlock, double[] rewardPercentiles) => _feeHistoryOracle.GetFeeHistory(blockCount, newestBlock, rewardPercentiles);

    public ResultWrapper<IEnumerable<Address>> sil_accounts()
    {
        try
        {
    ...
    public Task<ResultWrapper<ulong?>> sil_blockNumber()
    {
        ulong number = _blockchainBridge.HeadBlock?.Number ?? 0;
        return Task.FromResult(ResultWrapper<ulong?>.Success(number));
    }

    public Task<ResultWrapper<UInt256?>> sil_getBalance(Address address, BlockParameter? blockParameter = null)
    {
    ...
    public ResultWrapper<byte[]> sil_getStorageAt(Address address, StorageIndex positionIndex,
    ...
    public ResultWrapper<StorageValuesResult> sil_getStorageValues(
    ...
    public virtual Task<ResultWrapper<UInt256>> sil_getTransactionCount(Address address, BlockParameter? blockParameter)
    {
    ...
    public ResultWrapper<UInt256?> sil_getBlockTransactionCountByHash(Hash256 blockHash)
    {
    ...
    public ResultWrapper<UInt256?> sil_getBlockTransactionCountByNumber(BlockParameter blockParameter)
    {
    ...
    public ResultWrapper<UInt256?> sil_getUncleCountByBlockHash(Hash256 blockHash)
    {
        SearchResult<Block> searchResult = _blockFinder.SearchForBlock(new BlockParameter(blockHash));
        return searchResult.IsError
            ? ResultWrapper<UInt256?>.Success(null)
    ...
    public ResultWrapper<UInt256?> sil_getUncleCountByBlockNumber(BlockParameter? blockParameter)
    {
        SearchResult<Block> searchResult = _blockFinder.SearchForBlock(blockParameter);
        return searchResult.IsError
            ? ResultWrapper<UInt256?>.Success(null)
    ...
    public ResultWrapper<byte[]> sil_getCode(Address address, BlockParameter? blockParameter = null)
    {
        SearchResult<BlockHeader> searchResult = _blockFinder.SearchForHeader(blockParameter);
        if (searchResult.IsError)
        {
    ...
    public ResultWrapper<Signature> sil_sign(Address addressData, byte[] message)
    {
        ...
        if (_logger.IsTrace) _logger.Trace($"sil_sign request {addressData}, {message}, result: {sig}");
        return ResultWrapper<Signature>.Success(sig);
    }

    public virtual Task<ResultWrapper<Hash256>> sil_sendTransaction(SignableTransactionForRpc rpcTx)
    {
    ...
    public virtual async Task<ResultWrapper<Hash256>> sil_sendRawTransaction(byte[] transaction)
    {
        try
        {
    ...
    public virtual ResultWrapper<SignTransactionResult> sil_signTransaction(SignableTransactionForRpc rpcTx)
    {
        if (!_rpcConfig.EnableEthSignTransaction)
            return ResultWrapper<SignTransactionResult>.Fail("sil_signTransaction is disabled", ErrorCodes.MethodNotFound);
        ...
        if (_logger.IsInfo) _logger.Info($"sil_signTransaction signed tx {tx.Hash} from {tx.SenderAddress}");
    ...
    private ResultWrapper<SignTransactionResult>? CheckTxFeeCap(Transaction tx)
    {
        ...
        UInt256 perGas = tx.Type >= TxType.SIP1559 ? tx.MaxFeePerGas : tx.GasPrice;
        UInt256 capWei = cap;
        ...
        decimal capEth = capWei.ToDecimal(null) / (decimal)Unit.Sila;
        string message = overflow
            ? string.Create(CultureInfo.InvariantCulture, $"tx fee (overflow sila) exceeds the configured cap ({capEth:F2} sila)")
            : string.Create(CultureInfo.InvariantCulture, $"tx fee ({totalFee.ToDecimal(null) / (decimal)Unit.Sila:F2} sila) exceeds the configured cap ({capEth:F2} sila)");
        return ResultWrapper<SignTransactionResult>.Fail(message, ErrorCodes.InvalidInput);
    }

    public virtual async Task<ResultWrapper<FillTransactionResult>> sil_fillTransaction(SignableTransactionForRpc rpcTx)
    {
        BlockHeader? head = _blockFinder.Head?.Header;
        if (head is null)
        ...
        {
            ResultWrapper<UInt256?> gasEstimate = sil_estimateGas(rpcTx, BlockParameter.Latest);
    ...
    public async Task<ResultWrapper<ReceiptForRpc?>> sil_sendRawTransactionSync(byte[] transaction, ulong? timeoutMs = null)
    {
        int waitMs = ResolveSyncTimeoutMs(timeoutMs);
        using CancellationTokenSource cts = new(waitMs);

        // Submit via the virtual sil_sendRawTransaction so subclass overrides
        // propagate without needing a separate sync override.
        ResultWrapper<Hash256> sendResult = await sil_sendRawTransaction(transaction);
        ...
        while (true)
        {
            ...
            ResultWrapper<ReceiptForRpc?> receiptResult = sil_getTransactionReceipt(hash);
    ...
    public virtual ResultWrapper<HexBytes> sil_call(SignableTransactionForRpc transactionCall, BlockParameter? blockParameter = null, Dictionary<Address, AccountOverride>? stateOverride = null, BlockOverride? blockOverride = null) =>
    ...
    public ResultWrapper<IReadOnlyList<SimulateBlockResult<SimulateCallResult>>> sil_simulateV1(SimulatePayload<TransactionForRpc> payload, BlockParameter? blockParameter = null) =>
    ...
    public virtual ResultWrapper<UInt256?> sil_estimateGas(SignableTransactionForRpc transactionCall, BlockParameter? blockParameter, Dictionary<Address, AccountOverride>? stateOverride = null, BlockOverride? blockOverride = null) =>
        new EstimateGasTxExecutor(_blockchainBridge, _blockFinder, _rpcConfig, _specProvider)
            .ExecuteTx(transactionCall, blockParameter, stateOverride, blockOverride);

    public virtual ResultWrapper<AccessListResultForRpc?> sil_createAccessList(SignableTransactionForRpc transactionCall, BlockParameter? blockParameter = null, Dictionary<Address, AccountOverride>? stateOverride = null, bool optimize = true) =>
    ...
    public ResultWrapper<BlockForRpc> sil_getBlockByHash(Hash256 blockHash, bool returnFullTransactionObjects) => GetBlock(new BlockParameter(blockHash), returnFullTransactionObjects);

    public ResultWrapper<BlockForRpc> sil_getBlockByNumber(BlockParameter blockParameter,
    ...
    public ResultWrapper<BlockHeaderForRpc?> sil_getHeaderByHash(Hash256 blockHash)
        => GetHeader(new BlockParameter(blockHash));

    public ResultWrapper<BlockHeaderForRpc?> sil_getHeaderByNumber(BlockParameter blockParameter)
    ...
    public virtual ResultWrapper<TransactionForRpc?> sil_getTransactionByHash(Hash256 transactionHash)
    {
        ...
        TransactionForRpc transactionModel = TransactionForRpc.FromTransaction(
            ...
            extraData: extraData);
        if (_logger.IsTrace) _logger.Trace($"sil_getTransactionByHash request {transactionHash}, result: {transactionModel.Hash}");
    ...
    public virtual ResultWrapper<TransactionForRpc[]> sil_pendingTransactions()
    {
        ...
        if (_logger.IsTrace) _logger.Trace($"sil_pendingTransactions request, result: {transactionsModels.Length}");
        return ResultWrapper<TransactionForRpc[]>.Success(transactionsModels);
    }

    public ResultWrapper<TransactionForRpc> sil_getTransactionByBlockHashAndIndex(Hash256 blockHash, UInt256 positionIndex)
    {
        ResultWrapper<TransactionForRpc> result = GetTransactionByBlockAndIndex(new BlockParameter(blockHash), positionIndex);
        if (_logger.IsTrace && result.Result.ResultType == ResultType.Success) _logger.Trace($"sil_getTransactionByBlockHashAndIndex request {blockHash}, index: {positionIndex}, result: {result.Data?.Hash}");
        return result;
    }

    public ResultWrapper<TransactionForRpc> sil_getTransactionByBlockNumberAndIndex(BlockParameter blockParameter, UInt256 positionIndex)
    {
        ResultWrapper<TransactionForRpc> result = GetTransactionByBlockAndIndex(blockParameter, positionIndex);
        if (_logger.IsTrace && result.Result.ResultType == ResultType.Success) _logger.Trace($"sil_getTransactionByBlockNumberAndIndex request {blockParameter}, index: {positionIndex}, result: {result.Data?.Hash}");
        return result;
    }

    public ResultWrapper<ArrayPoolList<byte>?> sil_getRawTransactionByBlockHashAndIndex(Hash256 blockHash, UInt256 positionIndex)
    {
        ResultWrapper<ArrayPoolList<byte>?> result = GetRawTransactionByBlockAndIndex(new BlockParameter(blockHash), positionIndex);
        if (_logger.IsTrace && result.Result.ResultType == ResultType.Success) _logger.Trace($"sil_getRawTransactionByBlockHashAndIndex request {blockHash}, index: {positionIndex}, result length: {result.Data?.Count ?? 0}");
        return result;
    }

    public ResultWrapper<ArrayPoolList<byte>?> sil_getRawTransactionByBlockNumberAndIndex(BlockParameter blockParameter, UInt256 positionIndex)
    {
        ResultWrapper<ArrayPoolList<byte>?> result = GetRawTransactionByBlockAndIndex(blockParameter, positionIndex);
        if (_logger.IsTrace && result.Result.ResultType == ResultType.Success) _logger.Trace($"sil_getRawTransactionByBlockNumberAndIndex request {blockParameter}, index: {positionIndex}, result length: {result.Data?.Count ?? 0}");
        return result;
    }
    ...
    public ResultWrapper<BlockForRpc?> sil_getUncleByBlockHashAndIndex(Hash256 blockHash, UInt256 positionIndex) => GetUncle(new BlockParameter(blockHash), positionIndex);

    public ResultWrapper<BlockForRpc?> sil_getUncleByBlockNumberAndIndex(BlockParameter blockParameter,
    ...
    public ResultWrapper<UInt256?> sil_newFilter(Filter filter)
    {
    ...
    public ResultWrapper<UInt256?> sil_newBlockFilter()
    {
        int filterId = _blockchainBridge.NewBlockFilter();
        return ResultWrapper<UInt256?>.Success((UInt256)filterId);
    }

    public ResultWrapper<UInt256?> sil_newPendingTransactionFilter()
    {
        int filterId = _blockchainBridge.NewPendingTransactionFilter();
        return ResultWrapper<UInt256?>.Success((UInt256)filterId);
    }

    public ResultWrapper<bool?> sil_uninstallFilter(UInt256 filterId)
    {
    ...
    public ResultWrapper<IEnumerable<object>> sil_getFilterChanges(UInt256 filterId)
    {
    ...
    public ResultWrapper<IEnumerable<FilterLog>> sil_getFilterLogs(UInt256 filterId)
    {
    ...
    public ResultWrapper<IEnumerable<FilterLog>> sil_getLogs(Filter filter)
    {
        BlockParameter fromBlock = filter.FromBlock!;
        BlockParameter toBlock = filter.ToBlock!;
    ...
    // https://github.com/sila-chain/SIPs/issues/1186
    public ResultWrapper<AccountProof> sil_getProof(Address accountAddress, StorageKeys storageKeys, BlockParameter? blockParameter)
    {
    ...
    public ResultWrapper<ulong> sil_chainId()
    {
    ...
    public ResultWrapper<AccountForRpc?> sil_getAccount(Address accountAddress, BlockParameter? blockParameter)
    {
        SearchResult<BlockHeader> searchResult = _blockFinder.SearchForHeader(blockParameter);
        if (searchResult.IsError)
        {
    ...
    public ResultWrapper<AccountInfoForRpc?> sil_getAccountInfo(Address accountAddress, BlockParameter? blockParameter)
    {
        SearchResult<BlockHeader> searchResult = _blockFinder.SearchForHeader(blockParameter);
        if (searchResult.IsError)
        {
    ...
    public virtual ResultWrapper<ReceiptForRpc?> sil_getTransactionReceipt(Hash256 txHash)
    {
        ...
        if (_logger.IsTrace) _logger.Trace($"sil_getTransactionReceipt request {txHash}, result: {txHash}");
        return ResultWrapper<ReceiptForRpc>.Success(new(txHash, receipt, blockTimestamp, gasInfo.Value, logIndexStart));
    }

    public virtual ResultWrapper<ReceiptForRpc[]?> sil_getBlockReceipts(BlockParameter blockParameter)
    {
    ...
    public ResultWrapper<JsonNode> sil_config()
    {
        ...
        static ForkConfig? GetForkConfig(Fork? fork, ISpecProvider specProvider)
        {
            ...
            return new ForkConfig
            {
                ...
                BlobSchedule = spec.IsSip4844Enabled ? new BlobScheduleSettingsForRpc
                {
    ...
    public ResultWrapper<ReadOnlyBlockAccessList?> sil_getBlockAccessListByHash(Hash256 blockHash)
        => GetBlockAccessList(blockHash, null);

    public ResultWrapper<ReadOnlyBlockAccessList?> sil_getBlockAccessListByNumber(ulong blockNumber)
    ...
    public ResultWrapper<SilCapabilities> sil_capabilities() =>
        ResultWrapper<SilCapabilities>.Success(_capabilitiesProvider.GetCapabilities());
