    ...
    ILogger logger) : StreamingResultBase(timeoutCts, logger), IBatchAwareStreamableResultWithStatus, IEnumerable<FilterLog>
{
    ...
    private const string TimeoutLogMessage = "sil_getLogs streaming timed out mid-response; client receives a partial result with the JSON envelope closed.";
    private const string CancellationLogMessage = "sil_getLogs streaming cancelled mid-response; client receives a partial result with the JSON envelope closed.";
    ...
    private async ValueTask<StreamableResultStatus> WriteLogsAsync(PipeWriter writer, long? maxResponseBodySize, CancellationToken cancellationToken)
    {
        ...
        StreamableResultStatus? TryBufferNextLog(IEnumerator<FilterLog> enumerator, ArrayBufferWriter<byte> itemBuffer)
        {
            try
            {
                ...
                JsonSerializer.Serialize(itemWriter, enumerator.Current, SilaJsonSerializer.JsonOptions);
                return null;
            }
            ...
            {
                if (Logger.IsWarn) Logger.Warn($"sil_getLogs streaming failed mid-response: {ex}");
