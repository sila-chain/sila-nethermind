    ...
    IJsonRpcConfig jsonRpcConfig)
{
    ...
    /// <c>sil_call</c>); in-VM failures (revert, out-of-gas, ...) are surfaced in the payload alongside
    ...
    public ResultWrapper<CallResultWithProof> Execute(TransactionForRpc callRequest, BlockParameter? blockParameter)
    {
        ...
        // Default gas to the block gas limit so callers can omit it (mirrors sil_call).
        ...
        // — matches sil_call / trace_call / debug_traceCall.
        ...
        // header so the SAVM sees a consistent block context (IsPostMerge, base fee, blob fees).
        ...
        // Bound the SAVM run by the RPC timeout so a heavy call can't pin a thread past the deadline; the
        // SAVM then throws OperationCanceledException, which JsonRpcService maps to a Timeout error. A
