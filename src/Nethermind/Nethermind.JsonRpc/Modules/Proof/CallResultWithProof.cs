...
/// </summary>
/// <remarks>
/// On clean success, <see cref="Result"/> holds the SAVM return data and <see cref="Error"/> is null.
...
/// error rather than this payload — matching <c>sil_call</c> exactly.
/// <para>
/// The deliberate divergence from <c>sil_call</c>: in-VM failures (revert, OOG, ...) succeed at the
/// JSON-RPC layer here and carry the same error object in the payload, alongside the witness, so a
/// verifier can independently re-prove the failure. <c>sil_call</c> would fail those at the envelope
...
public class CallResultWithProof : IDisposable
{
    ...
    /// <c>sil_call</c>), because there is no witness to attach to a transaction that never executed.
    ...
    public static ResultWrapper<CallResultWithProof> FromWitnessResult(SingleCallWitnessResult result, ulong txGasLimit = 0)
    {
        if (result.InputError)
        {
            ...
            // is wrapped the same way sil_call wraps it ("err: <inner> (supplied gas <gas>)") so a
            // caller that already handles sil_call's envelope can reuse the parser.
            result.Witness.Dispose();
            string message = result.Error is null
                ? "execution failed"
                : ErrorWrapper.SilCall(result.Error, txGasLimit);
    ...
    /// so it matches <c>sil_call</c> byte-for-byte. The raw payload is hex-encoded in <c>data</c> —
    /// empty payloads render as <c>"0x"</c> (matching <c>sil_call</c>), not omitted, so consumers
