...
public interface IJsonRpcConfig : IConfig
{
    ...
    [ConfigItem(
        Description = """
            ...
            Caps heavy methods promoted to sharable — `sil_call`, `sil_estimateGas`,
            `sil_createAccessList` — preventing unbounded concurrency from exhausting memory.
            Light sharable methods (e.g. `sil_blockNumber`, `sil_getBalance`) complete in <1 ms and
    ...
    [ConfigItem(
        Description = """
            An array of JSON-RPC namespaces to enable. For instance, `[debug,sil]`.
            ...
            """,
        DefaultValue = "[Sil,Subscribe,Trace,TxPool,Web3,Proof,Net,Parity,Health,Rpc]")]
    string[] EnabledModules { get; set; }

    [ConfigItem(
        Description = "An array of additional JSON-RPC URLs to listen at with protocol and JSON-RPC namespace list. For instance, `[http://localhost:8546|http;ws|sil;web3]`.",
        DefaultValue = "[]")]
    string[] AdditionalRpcUrls { get; set; }

    [ConfigItem(Description = "The maximum gas limit for `sil_call` and `sil_estimateGas`.", DefaultValue = "100000000")]
    ...
    [ConfigItem(
        Description = "The max number of logs per response for the `sil_getLogs` JSON-RPC method. `0` to lift the limit.",
        DefaultValue = "20000")]
    public int MaxLogsPerResponse { get; set; }

    [ConfigItem(
        Description = "Whether to stream `debug_trace*` and `trace_*` responses as the SAVM executes (lower TTFB and bounded memory). For `debug_trace*` can be overridden per-call via `GethTraceOptions.StreamMode`.",
        DefaultValue = "true")]
    public bool EnableTracingStreamMode { get; set; }

    [ConfigItem(
        Description = "Whether to stream `sil_getLogs` and `sil_getFilterLogs` responses as logs are found. When enabled, unauthenticated responses stop at `MaxLogsPerResponse` or `MaxLogsResponseBodySize` instead of buffering the full result and returning a limit error.",
        DefaultValue = "false")]
    public bool EnableLogsStreamMode { get; set; }

    [ConfigItem(
        Description = "The max response body size, in bytes, for streamed `sil_getLogs` and `sil_getFilterLogs` JSON-RPC responses. Ignored unless `EnableLogsStreamMode` is enabled. `null` to use `MaxBatchResponseBodySize`.",
        DefaultValue = "null")]
    ...
    [ConfigItem(
        Description = """
            ...
            concurrency cap on the override-path env pool used by sharable `sil_call` /
            `sil_estimateGas` / `sil_createAccessList` when called with state or blob-base-fee
            overrides: calls beyond this cap fail with a `LimitExceeded` JSON-RPC error. Defaults
            to the number of logical processors.
            """)]
    int? SilModuleConcurrentInstances { get; set; }
    ...
    [ConfigItem(
        ...
        DefaultValue = "[engine_newPayloadV1,engine_newPayloadV2,engine_newPayloadV3,engine_forkchoiceUpdatedV1,engine_forkchoiceUpdatedV2,flashbots_validateBuilderSubmissionV3,sil_signTransaction]")]
    ...
    [ConfigItem(
        ...
        DefaultValue = "[Net,Sil,Subscribe,Web3]")]
    ...
    [ConfigItem(Description = "The max block count limit for the `sil_simulate` JSON-RPC method.", DefaultValue = "256")]
    long? MaxSimulateBlocksCap { get; set; }

    [ConfigItem(Description = "The error margin used in the `sil_estimateGas` JSON-RPC method, in basis points.", DefaultValue = "150")]
    int EstimateErrorMargin { get; set; }

    [ConfigItem(Description = "Maximum total tx fee (gasPrice * gasLimit, in wei) the node will sign in sil_signTransaction. 0 disables the cap. Default 1 SIL.", DefaultValue = "1000000000000000000")]
    ulong RpcTxFeeCap { get; set; }

    [ConfigItem(Description = "Whether to enable sil_signTransaction. Disabled by default; enable only on nodes that explicitly manage unlocked accounts.", DefaultValue = "false")]
    ...
    [ConfigItem(Description = "The sil_filters timeout, in milliseconds.", DefaultValue = "900000")]
    ...
    [ConfigItem(Description = "Default server-side wait, in milliseconds, for sil_sendRawTransactionSync when the caller omits the timeout argument.", DefaultValue = "20000")]
    int RpcTxSyncDefaultTimeoutMs { get; set; }

    [ConfigItem(Description = "Maximum server-side wait, in milliseconds, that sil_sendRawTransactionSync will accept; client-supplied timeouts above this are clamped down.", DefaultValue = "60000")]
