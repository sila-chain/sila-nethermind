...
public sealed class JsonRpcProcessor : IJsonRpcProcessor
{
    ...
    private static string SerializeForDiagnostics(in JsonRpcResult.Entry response)
    {
        ...
        JsonRpcResponseWriter.Write(writer, responseToSerialize, SilaJsonSerializer.JsonOptionsIndented);
        using JsonDocument document = JsonDocument.Parse(writer.WrittenMemory);
        return JsonSerializer.Serialize(new DiagnosticJsonRpcResult(document.RootElement, response.Report), SilaJsonSerializer.JsonOptionsIndented);
    }

    private static string SerializeResponseForDiagnostics(JsonRpcResponse response)
    {
        ArrayBufferWriter<byte> writer = new();
        JsonRpcResponseWriter.Write(writer, response, SilaJsonSerializer.JsonOptionsIndented);
