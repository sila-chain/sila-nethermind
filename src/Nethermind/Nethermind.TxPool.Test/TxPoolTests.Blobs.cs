...
namespace Nethermind.TxPool.Test
{
    ...
    public partial class TxPoolTests
    {
        ...
        [TestCase(TxType.SIP1559, 0, 5, 100)]
        [TestCase(TxType.Blob, 5, 0, 100)]
        [TestCase(TxType.SIP1559, 10, 0, 10)]
        [TestCase(TxType.Blob, 0, 15, 15)]
        [TestCase(TxType.SIP1559, 20, 25, 20)]
        ...
        public void should_reject_tx_with_FeeTooLow_even_if_is_blob_type([Values(true, false)] bool isBlob, [Values(true, false)] bool persistentStorageEnabled)
        {
            ...
            for (int i = 0; i < poolSize; i++)
            {
                Transaction tx = Build.A.Transaction
                    .WithNonce(i)
                    .WithType(isBlob ? TxType.Blob : TxType.SIP1559)
                    .WithShardBlobTxTypeAndFieldsIfBlobTx()
                    .WithMaxFeePerGas(1.GWei + (UInt256)(100 - i))
                    .WithMaxPriorityFeePerGas(1.GWei + (UInt256)(100 - i))
                    .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).TestObject;
        ...
        public void should_dump_GasBottleneck_of_blob_tx_to_zero_if_MaxFeePerBlobGas_is_lower_than_current([Values(true, false)] bool isBlob, [Values(true, false)] bool isPersistentStorage)
        {
            ...
            {
                Assert.That(_txPool.TryGetPendingTransaction(tx.Hash!, out Transaction sip1559Tx), Is.True);
                Assert.That(sip1559Tx, Is.EqualTo(tx).UsingTransactionComparer());
                Assert.That(sip1559Tx.GasBottleneck, Is.EqualTo(1.GWei));
            }
        }
        ...
        public void should_allow_to_replace_blob_tx_by_the_one_with_network_wrapper_in_higher_version()
        {
            ...
            Transaction firstTx = Build.A.Transaction
                .WithShardBlobTxTypeAndFields(spec: new ReleaseSpec() { IsSip7594Enabled = false })
                .WithNonce(0)
                .WithMaxFeePerGas(1.GWei)
                .WithMaxPriorityFeePerGas(1.GWei)
                .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).TestObject;
            ...
            Transaction secondTx = Build.A.Transaction
                .WithShardBlobTxTypeAndFields(spec: new ReleaseSpec() { IsSip7594Enabled = true })
                .WithNonce(0)
            ...
            // switch to SilaOsaka
            releaseSpec = new OverridableReleaseSpec(SilaOsaka.Instance);
            specProvider.GetCurrentHeadSpec().Returns(releaseSpec);
        ...
        public void RecoverAddress_should_work_correctly()
        {
            ...
            Assert.That(_silaEcdsa.RecoverAddress(tx), Is.EqualTo(tx.SenderAddress));
        }
            ...
            Parallel.ForEach(testPrivateKeys, privateKey =>
            {
                for (int i = 0; i < txsPerSender; i++)
                {
                    Transaction tx = Build.A.Transaction
                        ...
                        .SignedAndResolved(_silaEcdsa, privateKey).TestObject;
        ...
        [Test]
        public void should_add_blob_tx_in_sip7594_form_and_return_when_requested([Values(true, false)] bool isPersistentStorage, [Values(true, false)] bool hasTxCellProofs, [Values(true, false)] bool isOsakaActivated)
        {
            // tx is valid if:
            ...
            Transaction blobTxAdded = Build.A.Transaction
                .WithShardBlobTxTypeAndFields(spec: new ReleaseSpec() { IsSip7594Enabled = hasTxCellProofs })
                .WithMaxFeePerGas(1.GWei)
                .WithMaxPriorityFeePerGas(1.GWei)
                .WithNonce(0)
                .SignedAndResolved(_silaEcdsa, TestItem.PrivateKeyA).TestObject;
        ...
        public async Task<int> should_evict_based_on_proof_version_and_fork(BlobsSupportMode poolMode, TestAction[] testActions)
        {
            ...
            (ChainSpecBasedSpecProvider provider, _) = TestSpecHelper.LoadChainSpec(new ChainSpecJson
            {
                ...
                {
                    Sip4844TransitionTimestamp = head.Timestamp,
                    Sip7002TransitionTimestamp = head.Timestamp,
                    Sip7594TransitionTimestamp = head.Timestamp + 1,
                }
            });
        ...
        public static IEnumerable BlobScheduleActivationsTestCaseSource
        {
            get
            {
                ...
                {
                    yield return MakeTestCase("V0 should be evicted in SilaOsaka", 0, mode, TestAction.AddV0, TestAction.Fork);
                    yield return MakeTestCase("Take only V0 ones before SilaOsaka", 2, mode, TestAction.AddV0, TestAction.AddV0, TestAction.AddV1);
                    ...
                    yield return MakeTestCase("Ignore V1 before SilaOsaka, no gaps", 0, mode, TestAction.AddV1);
                }
            }
        }
        ...
        {
            const ulong regularMaxBlobCount = Sip7594Constants.MaxBlobsPerTx - 1;
