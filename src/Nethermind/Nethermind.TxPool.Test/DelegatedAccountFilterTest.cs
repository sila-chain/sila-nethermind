...
{
    private static readonly SilaEcdsa Ecdsa = new(0);
    ...
    private static TestReadOnlyStateProvider CreateDelegatedStateProvider()
    {
        ...
        byte[] code = [.. Sip7702Constants.DelegationHeader, .. TestItem.PrivateKeyA.Address.Bytes];
    ...
    public void Accept_SenderIsNotDelegated_ReturnsAccepted()
    {
        ...
        DelegatedAccountFilter filter = CreateFilter(SilaPrague.Instance, standardPool, blobPool);
    ...
    public void Accept_SenderIsDelegatedWithNoTransactionsInPool_ReturnsAccepted()
    {
        ...
        (TxDistinctSortedPool standardPool, TxDistinctSortedPool blobPool) = CreatePools();
        DelegatedAccountFilter filter = CreateFilter(SilaPrague.Instance, standardPool, blobPool, stateProvider);
        Transaction transaction = Build.A.Transaction.SignedAndResolved(Ecdsa, TestItem.PrivateKeyA).TestObject;
    ...
    private static readonly object[] Sip7702ActivationCases =
    {
        new object[] { true, AcceptTxResult.NotCurrentNonceForDelegation },
        new object[] { false, AcceptTxResult.Accepted },
    };

    [TestCaseSource(nameof(Sip7702ActivationCases))]
    public void Accept_Sip7702IsNotActivated_ReturnsExpected(bool isActive, AcceptTxResult expected)
    {
        IReleaseSpec spec = isActive ? SilaPrague.Instance : SilaCancun.Instance;
        (TxDistinctSortedPool standardPool, TxDistinctSortedPool blobPool) = CreatePools();
    ...
    public void Accept_SenderHasPendingDelegation_OnlyAcceptsIfNonceIsExactMatch(ulong nonce, AcceptTxResult expected)
    {
        ...
        DelegatedAccountFilter filter = CreateFilter(SilaPrague.Instance, standardPool, blobPool, delegationCache: pendingDelegations);
