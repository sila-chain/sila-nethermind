...
/// <summary>
///     Code adapted from silaJ (https://github.com/sila-chain/silaj)
/// </summary>
public class EciesCipher(ICryptoRandom cryptoRandom) : IEciesCipher
{
    ...
    {
        if (cipherText[0] != 4) // if not a compressed public key then probably we need to use SIP8
        {
            return (false, null);
        }
    ...
    public byte[] Encrypt(PublicKey recipientPublicKey, byte[] plainText, byte[] macData)
    {
        ...
        SilaIesEngine iesEngine = MakeIesEngine(true, recipientPublicKey, ephemeralPrivateKey, iv);
    ...
    {
        SilaIesEngine iesEngine = MakeIesEngine(false, ephemeralPublicKey, privateKey, iv);
    ...
    private static SilaIesEngine MakeIesEngine(bool isEncrypt, PublicKey publicKey, PrivateKey privateKey, byte[] iv)
    {
        IBlockCipher aesFastEngine = AesUtilities.CreateEngine();

        SilaIesEngine iesEngine = new(
