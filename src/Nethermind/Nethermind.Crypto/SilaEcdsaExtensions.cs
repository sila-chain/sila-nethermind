...
public static class SilaEcdsaExtensions
{
    ...
    public static AuthorizationTuple Sign(this ISilaEcdsa ecdsa, PrivateKey signer, ulong chainId, Address codeAddress, ulong nonce)
    {
    ...
    public static void Sign(this ISilaEcdsa ecdsa, PrivateKey privateKey, Transaction tx, bool isSip155Enabled = true)
    {
        ...
        KeccakRlpWriter writer = new();
        _txDecoder.EncodeTx(ref writer, tx, RlpBehaviors.SkipTypedWrapping, true, isSip155Enabled, ecdsa.ChainId);
        ValueHash256 hash = writer.GetValueHash();
        tx.Signature = ecdsa.Sign(privateKey, in hash);

        if (tx.Type == TxType.Legacy && isSip155Enabled)
        {
    ...
    /// <param name="tx"></param>
    /// <returns></returns>
    public static bool Verify(this ISilaEcdsa ecdsa, Address sender, Transaction tx)
    {
    ...
    /// <param name="useSignatureChainId">Whether to use the chain id encoded in a legacy SIP-155 signature.</param>
    /// <returns>The recovered address, or <see langword="null"/> when recovery fails.</returns>
    public static Address? RecoverAddress(this ISilaEcdsa ecdsa, Transaction tx, bool useSignatureChainId = false)
    {
    ...
    public static PublicKey? RecoverPublicKey(this ISilaEcdsa ecdsa, Transaction tx, bool useSignatureChainId = false)
    {
        Signature signature = tx.Signature
    ...
    private static ValueHash256 CalculateSignatureHash(ISilaEcdsa ecdsa, Transaction tx, Signature signature, bool useSignatureChainId)
    {
        useSignatureChainId &= signature.ChainId.HasValue;

        // feels like it is the same check twice
        bool applySip155 = useSignatureChainId
        ...
        KeccakRlpWriter writer = new();
        _txDecoder.EncodeTx(ref writer, tx, RlpBehaviors.SkipTypedWrapping, true, applySip155, chainId);
    ...
    public static Address? RecoverAddress(this ISilaEcdsa ecdsa, AuthorizationTuple tuple)
    {
        KeccakRlpWriter writer = new();
