...
public interface IBlobProofsBuilder
{
    ...
    byte[][] ComputeHashes(ShardBlobNetworkWrapper wrapper)
    {
        ...
        {
            hashes[i] = new byte[Sip4844Constants.BytesPerBlobVersionedHash];
...
public interface IBlobProofsVerifier
{
    ...
    public bool ValidateHashes(ShardBlobNetworkWrapper blobs, ReadOnlySpan<byte[]> blobVersionedHashes)
    {
        ...
        Span<byte> hash = stackalloc byte[Sip4844Constants.BytesPerBlobVersionedHash];

        for (int i = 0; i < blobVersionedHashes.Length; i++)
        {
            if (blobVersionedHashes[i].Length != Sip4844Constants.BytesPerBlobVersionedHash || blobVersionedHashes[i][0] != KzgPolynomialCommitments.KzgBlobHashVersionV1)
            {
                return false;
            }
        }
