...
public class ChainHeadSpecProviderTests
{
    ...
    public void Repeat_call_with_same_head_resolves_spec_once_and_returns_it()
    {
        ...
        (ISpecProvider specProvider, IBlockFinder blockFinder) = SetupForSingleHeader(header, SilaCancun.Instance);
        ChainHeadSpecProvider provider = new(specProvider, blockFinder);

        for (int i = 0; i < 100; i++)
        {
            Assert.That(provider.GetCurrentHeadSpec(), Is.SameAs(SilaCancun.Instance));
        }
    ...
    public void New_head_invalidates_cache_and_re_resolves_spec()
    {
        ...
        specProvider.GetSpec(first).Returns(SilaCancun.Instance);
        specProvider.GetSpec(second).Returns(SilaPrague.Instance);
        ...
        Assert.That(provider.GetCurrentHeadSpec(), Is.SameAs(SilaCancun.Instance));
        Assert.That(provider.GetCurrentHeadSpec(), Is.SameAs(SilaPrague.Instance));
    }
    ...
    {
        ISpecProvider specProvider = Substitute.For<ISpecProvider>();
        specProvider.GetSpec((ForkActivation)0L).Returns(SilaCancun.Instance);
    ...
    public async Task Concurrent_callers_during_head_rotation_never_observe_torn_pair()
    {
        ...
        IReleaseSpec[] specs = { SilaCancun.Instance, SilaPrague.Instance, SilaOsaka.Instance };
