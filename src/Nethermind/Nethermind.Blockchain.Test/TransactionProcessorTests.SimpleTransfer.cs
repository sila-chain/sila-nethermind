...
public partial class TransactionProcessorTests
{
    ...
    {
        IReleaseSpec spec = SilaPrague.Instance;
        Address recipient = testCase.CreateRecipient(_stateProvider, spec);
        _stateProvider.Commit(spec);
        _stateProvider.CommitTree(0);

        (CountingVirtualMachine virtualMachine, SilaTransactionProcessor transactionProcessor) = CreateProcessor(_specProvider);
    ...
    public void Simple_transfer_fast_path_reports_sip7708_log_only_for_non_zero_transfer_to_different_account(
        ...
        bool expectTransferLog)
    {
        ...
        (CountingVirtualMachine virtualMachine, SilaTransactionProcessor transactionProcessor) = CreateProcessor(specProvider);
    ...
    // The SAVM path must charge NEW_ACCOUNT exactly once, mirroring ExecuteSimpleTransfer.
    [Test]
    public void Sip8037_savm_path_value_transfer_to_dead_recipient_charges_new_account_state_gas()
    {
        ...
        _stateProvider.Commit(SilaAmsterdam.Instance);
        _stateProvider.CommitTree(0);
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(spec.IsSip8037Enabled, Is.True);
            Assert.That(liveResult.TransactionExecuted, Is.True);
            Assert.That(deadResult.TransactionExecuted, Is.True);
            Assert.That(virtualMachine.ExecuteTransactionCalls, Is.EqualTo(2)); // both took the SAVM path
    ...
    [Test]
    public void Sip8037_value_transfer_to_dead_precompile_charges_new_account_state_gas()
    {
    ...
    private Transaction BuildSetCodeTransfer(Address recipient, UInt256 value, PrivateKey sender, PrivateKey authority, ulong nonce) =>
        Build.A.Transaction
            ...
            .WithNonce(nonce)
            .WithAuthorizationCode(_silaEcdsa.Sign(authority, _specProvider.ChainId, Address.Zero, 0))
            .SignedAndResolved(_silaEcdsa, sender, sip155Enabled)
            .TestObject;

    private (CountingVirtualMachine Vm, SilaTransactionProcessor Processor) CreateProcessor(ISpecProvider specProvider)
    {
        ...
        SilaTransactionProcessor processor = new(
    ...
    private Transaction BuildSimpleTransfer(Address recipient, UInt256 value, bool withAuthorizationList, ulong gasLimit = 100_000)
    {
        ...
        if (withAuthorizationList)
        {
            builder
                .WithType(TxType.SetCode)
                .WithAuthorizationCode(_silaEcdsa.Sign(TestItem.PrivateKeyB, _specProvider.ChainId, Address.Zero, 0));
        }
