...
public class BlockValidatorTests
{
    ...
    public void Accepts_valid_block()
    {
        ...
        Block block = Build.A.Block.WithParent(header).WithEncodedSize(Sip7934Constants.DefaultMaxRlpBlockSize).TestObject;
    ...
    private static IEnumerable<TestCaseData> BadSuggestedBlocks()
    {
        ...
        yield return new TestCaseData(
            Build.A.Block.WithBlobGasUsed(131072)
                ...
                .TestObject,
            parent,
            new CustomSpecProvider(((ForkActivation)0, SilaCancun.Instance)),
            "max fee per blob gas less than block blob gas fee")
        { TestName = "InsufficientMaxFeePerBlobGas" };

        yield return new TestCaseData(
            Build.A.Block.WithParent(parent).WithEncodedSize(Sip7934Constants.DefaultMaxRlpBlockSize + 1).TestObject,
            parent,
            new CustomSpecProvider(((ForkActivation)0, SilaOsaka.Instance)),
        ...
        yield return new TestCaseData(
            ...
            parent,
            new CustomSpecProvider(((ForkActivation)0, SilaAmsterdam.Instance)),
    ...
    // WithPrecompileChanges yields 25 BAL items; the cap is itemCount <= gasLimit / Sip7928Constants.ItemCost.
    ...
    public void ValidateSuggestedBlock_enforces_bal_index_bounds(uint index, bool expectedValid)
    {
        ...
        Block suggestedBlock = Build.A.Block
            ...
            .WithTransactions(2, SilaAmsterdam.Instance)
            .WithBlobGasUsed(0)
            .WithWithdrawals([])
            .WithBal(bal)
            .TestObject;
    ...
    public void ValidateProcessedBlock_enforces_bal_item_gas_limit_boundary_for_rlp_imported_blocks(ulong gasLimit, bool expectedValid)
    {
        ...
        contribution.AddStorageChange(Sip2935Constants.BlockHashHistoryAddress, 0, 0, new UInt256(parent.Hash!.BytesToArray(), isBigEndian: true));
        UInt256 sip4788Slot1 = 12u % Sip4788Constants.RingBufferSize;
        UInt256 sip4788Slot2 = (12u % Sip4788Constants.RingBufferSize) + Sip4788Constants.RingBufferSize;
        contribution.AddStorageChange(Sip4788Constants.BeaconRootsAddress, sip4788Slot1, 0, 12);
        contribution.AddStorageRead(Sip4788Constants.BeaconRootsAddress, sip4788Slot2);
        for (UInt256 i = 0; i < 4; i++)
        {
            contribution.AddStorageRead(Sip7002Constants.WithdrawalRequestPredeployAddress, i);
            contribution.AddStorageRead(Sip7251Constants.ConsolidationRequestPredeployAddress, i);
        }
    ...
    // SIP-7928 BlockAccessIndex must be in [0, txCount + 1]: 0 = pre-execution,
    ...
    private static BlockValidator AmsterdamSut(ITxValidator? tx = null) =>
        new(tx ?? new TxValidator(TestBlockchainIds.ChainId), Always.Valid, Always.Valid,
            new CustomSpecProvider(((ForkActivation)0, SilaAmsterdam.Instance)), LimboLogs.Instance);
