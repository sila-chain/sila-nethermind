...
public class WithdrawalValidatorTests
{
    ...
    private static IEnumerable<TestCaseData> WithdrawalValidationCases()
    {
        ...
        yield return new TestCaseData(SilaShanghai.Instance, twoWithdrawals, TestItem.KeccakD)
        {
            TestName = "Withdrawals_with_incorrect_withdrawals_root_are_invalid",
            ExpectedResult = false
        };
        yield return new TestCaseData(SilaShanghai.Instance, empty, new WithdrawalTrie(empty).RootHash)
        {
            TestName = "Empty_withdrawals_are_valid_post_shanghai",
            ExpectedResult = true
        };
        yield return new TestCaseData(SilaShanghai.Instance, twoWithdrawals, new WithdrawalTrie(twoWithdrawals).RootHash)
        {
