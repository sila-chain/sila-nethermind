...
public class TxValidatorTests
{
    ...
    private static byte CalculateV() => (byte)SilaEcdsaExtensions.CalculateV(TestBlockchainIds.ChainId);
    ...
    [TestCase(true)]
    [TestCase(false)]
    public void Before_sip_155_has_to_have_valid_chain_id_unless_overridden(bool validateChainId)
    {
        byte[] sigData = new byte[65];
        sigData[31] = 1; // correct r
        sigData[63] = 1; // correct s
        ...
        releaseSpec.IsSip155Enabled.Returns(false);
    ...
    public bool Before_sip_2930_has_to_be_legacy_tx(TxType txType, bool sip2930)
    {
        byte[] sigData = new byte[65];
        sigData[31] = 1; // correct r
        sigData[63] = 1; // correct s
        ...
        TxValidator txValidator = new(TestBlockchainIds.ChainId);
        return txValidator.IsWellFormed(tx, sip2930 ? Berlin.Instance : MuirGlacier.Instance);
    }
    ...
    [TestCase(TxType.SIP1559, true, false, ExpectedResult = false)]
    [TestCase(TxType.SIP1559, true, true, ExpectedResult = true)]
    public bool Before_sip_1559_has_to_be_legacy_or_access_list_tx(TxType txType, bool sip2930, bool sip1559)
    {
        byte[] sigData = new byte[65];
        sigData[31] = 1; // correct r
        sigData[63] = 1; // correct s
        ...
        Transaction tx = Build.A.Transaction
            .WithType(txType)
            .WithChainId(TestBlockchainIds.ChainId)
            .WithMaxPriorityFeePerGas(txType == TxType.SIP1559 ? 10.GWei : 5.GWei)
            .WithMaxFeePerGas(txType == TxType.SIP1559 ? 10.GWei : 5.GWei)
            .WithAccessList(txType is TxType.AccessList or TxType.SIP1559
                ? AccessList.Empty
                : null)
            .WithSignature(signature).TestObject;
        ...
        TxValidator txValidator = new(TestBlockchainIds.ChainId);
        IReleaseSpec releaseSpec = new ReleaseSpec() { IsSip2930Enabled = sip2930, IsSip1559Enabled = sip1559 };
    ...
    [TestCase(TxType.SIP1559, ExpectedResult = false)]
    ...
    [TestCase(false, -1, true)]
    public void Transaction_with_init_code_above_max_value_is_rejected_when_sip3860Enabled(bool sip3860Enabled,
        int dataSizeAboveInitCode, bool expectedResult)
    {
        IReleaseSpec releaseSpec = sip3860Enabled ? SilaShanghai.Instance : GrayGlacier.Instance;
    ...
    //leading zeros in AccessList - expected to pass (real sila-mainnet tx)
    ...
    public bool Incorrect_transactions_are_rejected(string rlp)
    {
        try
        {
            ...
            TxValidator txValidator = new(BlockchainIds.SilaMainnet);
            return txValidator.IsWellFormed(tx, London.Instance);
        }
    ...
    public void ShardBlobTransactions_should_have_destination_set()
    {
        ...
        Assert.That(txValidator.IsWellFormed(txWithoutTo, SilaCancun.Instance).AsBool(), Is.False);
        Assert.That(txValidator.IsWellFormed(txWithTo, SilaCancun.Instance).AsBool());
    }

    [MaxTime(Timeout.MaxTestTime)]
    [TestCase(TxType.SIP1559, false, ExpectedResult = true)]
    [TestCase(TxType.SIP1559, true, ExpectedResult = false)]
    ...
    public bool MaxFeePerBlobGas_should_be_set_for_blob_tx_only(TxType txType, bool isMaxFeePerBlobGasSet)
    {
        TransactionBuilder<Transaction> txBuilder = Build.A.Transaction
            ...
            .WithBlobVersionedHashes(txType == TxType.Blob ? Sip4844Constants.MinBlobsPerTransaction : null)
            .WithChainId(TestBlockchainIds.ChainId)
            .SignedAndResolved();
        ...
        TxValidator txValidator = new(TestBlockchainIds.ChainId);
        return txValidator.IsWellFormed(tx, SilaCancun.Instance);
    }
    ...
    public void IsWellFormed_NotBlobTxWithBlobField_ReturnFalse(Func<TransactionBuilder<Transaction>, TransactionBuilder<Transaction>> addField)
    {
        ...
        Assert.That(txValidator.IsWellFormed(tx, SilaCancun.Instance).AsBool(), Is.False);
    }
    ...
    public void IsWellFormed_CreateTxInSetCode_ReturnsFalse()
    {
        ...
        Assert.Multiple(() =>
        {
            ValidationResult validationResult = txValidator.IsWellFormed(tx, SilaPrague.Instance);
    ...
    public void IsWellFormed_AuthorizationListTxInPragueSpec_ReturnsTrue()
    {
        ...
        Assert.That(txValidator.IsWellFormed(tx, SilaPrague.Instance).AsBool, Is.True);
    }
    ...
    public void IsWellFormed_EmptyOrNullAuthorizationList_ReturnsFalse(bool useEmpty)
    {
        ...
        Assert.That(txValidator.IsWellFormed(tx, SilaPrague.Instance).AsBool, Is.False);
    }
    ...
    public void IsWellFormed_NonSetCodeTxHasAuthorizationList_ReturnsFalse(TxType type)
    {
        ...
        Assert.That(txValidator.IsWellFormed(tx, SilaPrague.Instance).Error, Is.EqualTo(TxErrorMessages.NotAllowedAuthorizationList));
    }

    [Test]
    public void IsWellFormed_TransactionWithGasLimitExceedingSip7825Cap_ReturnsFalse()
    {
        Transaction tx = Build.A.Transaction
        ...
        IReleaseSpec releaseSpec = new ReleaseSpec() { IsSip7825Enabled = true };
        ValidationResult result = txValidator.IsWellFormed(tx, releaseSpec);

        using (Assert.EnterMultipleScope())
        {
            Assert.That(result.AsBool, Is.False);
            Assert.That(result.Error, Is.EqualTo(TxErrorMessages.TxGasLimitCapExceeded(tx.GasLimit, Sip7825Constants.DefaultTxGasLimitCap)));
        }
    }

    [Test]
    public void IsWellFormed_Sip8037FloorGasExceedingRegularCap_ReturnsFalse()
    {
        ...
        TxValidator txValidator = new(TestBlockchainIds.ChainId);
        ValidationResult result = txValidator.IsWellFormed(tx, SilaAmsterdam.Instance);
    ...
    private static ValidationResult IsWellFormed_Nonce_Limit(ulong nonce, TxType txType)
    {
        ...
        TxValidator txValidator = new(TestBlockchainIds.ChainId);
        IReleaseSpec releaseSpec = SilaOsaka.Instance;
        return txValidator.IsWellFormed(tx, releaseSpec);
    }
    ...
    private static IEnumerable<TestCaseData> ShardBlobTxIncorrectTransactions
    {
        get
        {
            ...
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject().SignedAndResolved().TestObject)
            {
                TestName = "A correct shard blob tx",
                ExpectedResult = true
            };

            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject(0)
                .SignedAndResolved().TestObject)
            {
                TestName = "BlobVersionedHashes are empty",
                ExpectedResult = false
            };
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject(Sip4844Constants.MinBlobsPerTransaction - 1)
                .SignedAndResolved().TestObject)
            {
                TestName = "Not enough BlobVersionedHashes",
                ExpectedResult = false
            };
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject(Sip4844Constants.MinBlobsPerTransaction)
                .SignedAndResolved().TestObject)
            {
            ...
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject((int)SilaCancun.Instance.MaxBlobCount - 1)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaCancun less than maximum BlobVersionedHashes",
                ExpectedResult = true
            };
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject((int)SilaCancun.Instance.MaxBlobCount)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaCancun maximum BlobVersionedHashes",
                ExpectedResult = true
            };
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject((int)SilaCancun.Instance.MaxBlobCount + 1)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaCancun too many BlobVersionedHashes",
                ExpectedResult = false
            };

            yield return new TestCaseData(SilaPrague.Instance, MakeTestObject((int)SilaPrague.Instance.MaxBlobCount - 1)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaPrague less than maximum BlobVersionedHashes",
                ExpectedResult = true
            };
            yield return new TestCaseData(SilaPrague.Instance, MakeTestObject((int)SilaPrague.Instance.MaxBlobCount)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaPrague maximum BlobVersionedHashes",
                ExpectedResult = true
            };
            yield return new TestCaseData(SilaPrague.Instance, MakeTestObject((int)SilaPrague.Instance.MaxBlobCount + 1)
                .SignedAndResolved().TestObject)
            {
                TestName = "SilaPrague too many BlobVersionedHashes",
                ExpectedResult = false
            };

            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject()
            ...
            yield return new TestCaseData(SilaCancun.Instance, MakeTestObject(2)
