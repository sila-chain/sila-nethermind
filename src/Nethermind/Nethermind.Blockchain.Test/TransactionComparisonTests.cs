...
public class TransactionComparisonTests
{
    ...
    [MaxTime(Timeout.MaxTestTime)]
    // head block number before sip 1559 transition
    [TestCase(10, 10, 0, 0ul, 0)]
    [TestCase(15, 10, 10, 1ul, -1)]
    [TestCase(2, 3, 20, 0ul, 1)]
    // head block number after sip 1559 transition
    ...
    {
        ulong sip1559Transition = 5;
        TestingContext context = new TestingContext(true, sip1559Transition)
    ...
    {
        ulong sip1559Transition = 5;
        TestingContext context = new(true, sip1559Transition);
        IComparer<Transaction> comparer = context.GetProducerComparer(new BlockPreparationContext(0, 0));
        AssertLegacyTransactions(comparer, gasPriceX, gasPriceY, expectedResult);
    }
    ...
    public void GasPriceComparer_for_sip1559_transactions(int feeCapX, int gasPremiumX, int feeCapY, int gasPremiumY, int headBaseFee, ulong headBlockNumber, int expectedResult)
    {
        ulong sip1559Transition = 5;
        TestingContext context = new TestingContext(true, sip1559Transition)
            .WithHeadBaseFeeNumber((UInt256)headBaseFee)
    ...
    {
        ulong sip1559Transition = ulong.MaxValue;
        TestingContext context = new TestingContext(false, sip1559Transition)
    ...
    [TestCase(10, 5, 12, 4, 4, 3ul, -1)]
    [TestCase(10, 5, 12, 4, 10, 3ul, -1)]
    public void ProducerGasPriceComparer_for_sip1559_transactions_1559(int feeCapX, int gasPremiumX, int feeCapY, int gasPremiumY, int headBaseFee, ulong headBlockNumber, int expectedResult)
    {
        ulong sip1559Transition = 5;
        TestingContext context = new(true, sip1559Transition);
    ...
    private void Assert1559Transactions(IComparer<Transaction> comparer, int feeCapX, int gasPremiumX, int feeCapY, int gasPremiumY, int expectedResult)
    {
        Transaction x = Build.A.Transaction.WithSenderAddress(TestItem.AddressA)
            ...
            .WithType(TxType.SIP1559).TestObject;
        Transaction y = Build.A.Transaction.WithSenderAddress(TestItem.AddressA)
    ...
    private class TestingContext
    {
        ...
        public TestingContext(bool isSip1559Enabled = false, ulong sip1559TransitionBlock = 0)
        {
            ReleaseSpec releaseSpec = new();
            ReleaseSpec sip1559ReleaseSpec = new() { IsSip1559Enabled = isSip1559Enabled, Sip1559TransitionBlock = sip1559TransitionBlock };
            ISpecProvider specProvider = Substitute.For<ISpecProvider>();
            specProvider.GetSpec(Arg.Is<ForkActivation>(x => x.BlockNumber >= sip1559TransitionBlock)).Returns(sip1559ReleaseSpec);
            specProvider.GetSpec(Arg.Is<ForkActivation>(x => x.BlockNumber < sip1559TransitionBlock)).Returns(releaseSpec);
