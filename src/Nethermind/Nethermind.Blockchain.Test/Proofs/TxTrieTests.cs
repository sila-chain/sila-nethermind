...
public class TxTrieTests(bool useSip2718)
{
    private readonly IReleaseSpec _releaseSpec = useSip2718 ? Berlin.Instance : MuirGlacier.Instance;
    ...
    public void Encoded_and_decoded_transaction_paths_have_same_root()
    {
        ...
        [
            ...
            Build.A.Transaction.WithNonce(2).WithType(useSip2718 ? TxType.SIP1559 : TxType.Legacy).Signed().TestObject,
            Build.A.Transaction.WithNonce(3).WithType(useSip2718 ? TxType.AccessList : TxType.Legacy).Signed().TestObject,
        ];
