...
/// <summary>
/// Integration tests exsrcising SIP-8037 per-tx 2D block-gas accounting
...
public class Sip8037BlockGasIntegrationTests
{
    ...
    private static (BlockAccessListManager, Block) BuildAmsterdamBlock(ulong blockGasLimit, params Transaction[] txs)
    {
        ...
        balManager.SetBlockExecutionContext(new BlockExecutionContext(block.Header, SilaAmsterdam.Instance));
    ...
    [TestCase(200_000ul, true, TestName = "Sip8037_boundary_state_exact_fit_accepts")]
    [TestCase(200_001ul, false, TestName = "Sip8037_boundary_state_exceeded_by_one_rejects")]
    public void Sip8037_boundary_state(ulong blockStateGasUsed, bool accepts)
    {
    ...
    /// </summary>
    [Test]
    public void Sip8037_creation_tx_regular_check_actual_usage_modest_accepts()
    {
    ...
    /// </summary>
    [Test]
    public void Sip8037_single_tx_state_check_exceeds_block_limit_rejects()
    {
        ...
        Assert.Throws<InvalidBlockException>(() =>
            mgr.IncrementalValidation(block, results, new BlockReceiptsTracer[1], null, CancellationToken.None),
            "SIP-8037 requires rejection at inclusion when tx.gas > state_gas_available (full reservation, no intrinsic subtraction)");
    }
    ...
    /// </summary>
    [Test]
    public void Sip8037_creation_tx_state_check_exceeded_rejects()
    {
        ...
        Assert.Throws<InvalidBlockException>(() =>
            mgr.IncrementalValidation(block, results, new BlockReceiptsTracer[2], null, CancellationToken.None),
            "SIP-8037 requires rejection on state dimension at inclusion");
    }

    /// <summary>
    /// SIP-7825 cap: even when (tx.gas - intrinsic.state) is huge, regular worst-case is
    ...
    /// </summary>
    [Test]
    public void Sip8037_sip7825_cap_with_modest_actual_gas_accepts()
    {
    ...
    [Test]
    public void Sequential_executor_applies_sip8037_inclusion_check_before_execution()
    {
        IWorldState stateProvider = TestWorldStateFactory.CreateForTest();
        TestSingleReleaseSpecProvider specProvider = new(SilaAmsterdam.Instance);
        ...
        ulong blockGasLimit = Sip7825Constants.DefaultTxGasLimitCap + 100ul;
        ...
        BlockProcessor.ParallelBlockValidationTransactionsExecutor executor = new(
            ...
            balManager,
            LimboLogs.Instance);
        BlockExecutionContext executionContext = new(block.Header, SilaAmsterdam.Instance);
        balManager.PrepareForProcessing(block, SilaAmsterdam.Instance, ProcessingOptions.None);
        ...
        Assert.That(ex!.Message, Does.Contain("SIP-8037 inclusion check"));
    }
}
