...
public class FileLocalDataSourceTests
{
    ...
    public void correctly_reads_existing_file()
    {
        ...
        // var x = new SilaJsonSerializer().Serialize(new string []{"A", "B", "C"});
        using FileLocalDataSource<string[]> fileLocalDataSource = new(tempFile.Path, new SilaJsonSerializer(), new RealFileSystem(), LimboLogs.Instance);
    ...
    public async Task correctly_updates_from_existing_file()
    {
        ...
        int interval = 30;
        using FileLocalDataSource<string[]> fileLocalDataSource = new(tempFile.Path, new SilaJsonSerializer(), new RealFileSystem(), LimboLogs.Instance, interval);
        int changedRaised = 0;
    ...
    {
        using TempPath tempFile = TempPath.GetTempFile();
        using FileLocalDataSource<string[]> fileLocalDataSource = new(tempFile.Path, new SilaJsonSerializer(), new RealFileSystem(), LimboLogs.Instance, 10);
        int changedRaised = 0;
        SemaphoreSlim handle = new(0);
        fileLocalDataSource.Changed += (sender, args) =>
        {
    ...
    public async Task loads_default_when_deleted_file()
    {
        using TempPath tempFile = TempPath.GetTempFile();
        await File.WriteAllTextAsync(tempFile.Path, GenerateStringJson("A"));
        using FileLocalDataSource<string[]> fileLocalDataSource = new(tempFile.Path, new SilaJsonSerializer(), new RealFileSystem(), LimboLogs.Instance, 50);
        int changedRaised = 0;
        SemaphoreSlim handle = new(0);
        fileLocalDataSource.Changed += (sender, args) =>
        {
