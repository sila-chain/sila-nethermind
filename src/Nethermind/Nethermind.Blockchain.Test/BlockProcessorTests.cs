...
public class BlockProcessorTests
{
    ...
    public void Parallel_validation_parent_reader_scope_is_per_worker_and_disposed_on_return()
    {
        ...
        {
            stateProvider.Commit(SilaAmsterdam.Instance, isGenesis: true);
        ...
        using BlockAccessListManager balManager = new(
            stateProvider,
            new TestSingleReleaseSpecProvider(SilaAmsterdam.Instance),
            Substitute.For<IBlockhashProvider>(),
            LimboLogs.Instance,
            new BlocksConfig { ParallelExecution = true },
            new WithdrawalProcessorFactory(LimboLogs.Instance),
            static worldState => new SilaCodeInfoRepository(worldState),
        ...
        PrepareSetup(balManager, block, SilaAmsterdam.Instance);
    ...
    public void Parallel_validation_parent_reader_uses_parent_root_captured_before_pre_block_changes()
    {
        ...
        balManager.PrepareForProcessing(block, SilaAmsterdam.Instance, ProcessingOptions.None);

        stateProvider.CreateAccount(TestItem.AddressB, 1);
        stateProvider.Commit(SilaAmsterdam.Instance, commitRoots: false);

        balManager.SetBlockExecutionContext(new(block.Header, SilaAmsterdam.Instance));
        ...
        Action<IWorldState> assertState)
    {
        ...
        {
            BlockAccessListManager.ApplyStateChanges(bal, stateProvider, SilaAmsterdam.Instance, shouldComputeStateRoot: false);
            assertState(stateProvider);
        }
    }
        ...
        IBlockCachePreWarmer? preWarmer = null)
    {
        ...
        BlockAccessListManager balManager = new(stateProvider, HoodiSpecProvider.Instance, Substitute.For<IBlockhashProvider>(), LimboLogs.Instance, new BlocksConfig(), new WithdrawalProcessorFactory(LimboLogs.Instance), static worldState => new SilaCodeInfoRepository(worldState));
    ...
    public async Task Process_long_running_branch(int blocksAmount)
    {
        ...
        await testRpc.AddFunds(address, 1.Sila);
    ...
    {
        IReleaseSpec spec = SilaOsaka.Instance;
    ...
    {
        // One extra storage read in suggested BAL costs Sip7928Constants.ItemCost (2000) gas
        IWorldState stateProvider = TestWorldStateFactory.CreateForTest();
    ...
    [TestCase(1)]
    [TestCase(2)]
    public void PrepareForProcessing_keeps_parallel_bal_execution_for_validated_sip8037_blocks(int txCount) =>
    ...
    public void PrepareForProcessing_disables_parallel_bal_execution_when_state_provider_is_not_scoped()
    {
        ...
        Block block = Build.A.Block
            .WithNumber(1)
            .WithTransactions(1, SilaAmsterdam.Instance)
            .WithBlockAccessList(new ReadOnlyBlockAccessList())
            .TestObject;
    ...
    [Test]
    public void IncrementalValidation_rejects_sip8037_tx_at_inclusion_when_worst_case_exceeds_remaining_budget()
    {
        ...
        Assert.That(exception!.Message, Does.Contain("SIP-8037 inclusion check"));
        Assert.That(exception.Message, Does.Contain("RegularDimensionExceeded"));
    }

    [Test]
    public void IncrementalValidation_rejects_sip8037_tx_when_actual_cumulative_gas_exceeds_limit()
    {
        BlockAccessListManager balManager = CreateAmsterdamBalManager();
        Transaction firstTx = Build.A.Transaction
            .WithHash(TestItem.KeccakA)
    ...
    [Test]
    public void IncrementalValidation_sip8037_block_gas_check_uses_actual_state_dimension()
    {
        // CREATE tx GasLimit sized to pass the SIP-8037 worst-case inclusion check
    ...
    public void IncrementalValidation_surfaces_worker_exception_before_gas_accounting()
    {
        ...
        // Tx GasLimit must pass the SIP-8037 inclusion check (worst-case <= block budget)
    ...
    private static void AssertParallelBalExecutionEnabled(BlockAccessListManager balManager, int txCount)
    {
        Block block = Build.A.Block
            .WithNumber(1)
            .WithTransactions(txCount, SilaAmsterdam.Instance)
            .WithBlockAccessList(new ReadOnlyBlockAccessList())
            .TestObject;
