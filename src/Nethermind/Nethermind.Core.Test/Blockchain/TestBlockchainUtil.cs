...
)
{
    ...
    public async Task<Block> AddBlock(BlockHeader parentToBuildOn, AddBlockFlags flags, CancellationToken cancellationToken, params Transaction[] transactions)
    {
        ...
        {
            mainProcessingContext.Value.BlockchainProcessor.Process(block!, ProcessingOptions.SilaMerge | ProcessingOptions.ForceProcessing, NullBlockTracer.Instance, cancellationToken);
        }
