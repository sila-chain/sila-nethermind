...
public class TestBlockchain : IDisposable
{
    ...
    public ISilaEcdsa SilaEcdsa => _fromContainer.SilaEcdsa;
    ...
    public static readonly UInt256 InitialValue = 1000.Sila;
    ...
    )
    {
        ...
        public ISilaEcdsa SilaEcdsa => silaEcdsa.Value;
    ...
    private static ISpecProvider WrapSpecProvider(ISpecProvider specProvider)
    {
        ...
        return unwrapped is TestSpecProvider { AllowTestChainOverride: false }
            ? specProvider
            : new OverridableSpecProvider(specProvider, static s => new OverridableReleaseSpec(s) { IsSip3607Enabled = false });
    }
    ...
    ) : IGenesisBuilder
    {
        public Block Build()
        {
            // Sip4788 precompile state account
            if (specProvider.GenesisSpec.IsBeaconBlockRootAvailable)
            {
                state.CreateAccount(specProvider.GenesisSpec.Sip4788ContractAddress!, 1);
                state.InsertCode(Sip4788Constants.BeaconRootsAddress, Sip4788TestConstants.CodeHash, Sip4788TestConstants.Code, specProvider.GenesisSpec);
            }

            // Sip2935
            if (specProvider.GenesisSpec.IsSip2935Enabled)
            {
                state.CreateAccount(specProvider.GenesisSpec.Sip2935ContractAddress!, 1);
                state.InsertCode(Sip2935Constants.BlockHashHistoryAddress, Sip2935TestConstants.CodeHash, Sip2935TestConstants.Code, specProvider.GenesisSpec);
            }
            ...
            {
                state.CreateAccount(Sip7002Constants.WithdrawalRequestPredeployAddress, 0, Sip7002TestConstants.Nonce);
                state.InsertCode(Sip7002Constants.WithdrawalRequestPredeployAddress, Sip7002TestConstants.CodeHash, Sip7002TestConstants.Code, specProvider.GenesisSpec);
            }

            if (finalSpec?.ConsolidationRequestsEnabled is true)
            {
                state.CreateAccount(Sip7251Constants.ConsolidationRequestPredeployAddress, 0, Sip7251TestConstants.Nonce);
                state.InsertCode(Sip7251Constants.ConsolidationRequestPredeployAddress, Sip7251TestConstants.CodeHash, Sip7251TestConstants.Code, specProvider.GenesisSpec);
            }

            if (finalSpec?.BuilderRequestsEnabled is true)
            {
                state.CreateAccount(Sip8282Constants.BuilderDepositRequestPredeployAddress, 0, Sip8282TestConstants.BuilderDeposit.Nonce);
                state.InsertCode(Sip8282Constants.BuilderDepositRequestPredeployAddress, Sip8282TestConstants.BuilderDeposit.CodeHash, Sip8282TestConstants.BuilderDeposit.Code, specProvider.GenesisSpec);

                state.CreateAccount(Sip8282Constants.BuilderExitRequestPredeployAddress, 0, Sip8282TestConstants.BuilderExit.Nonce);
                state.InsertCode(Sip8282Constants.BuilderExitRequestPredeployAddress, Sip8282TestConstants.BuilderExit.CodeHash, Sip8282TestConstants.BuilderExit.Code, specProvider.GenesisSpec);
            }

            BlockBuilder genesisBlockBuilder = Builders.Build.A.Block.Genesis;

            if (specProvider.GenesisSpec.IsSip4844Enabled)
            {
            ...
            if (specProvider.GenesisSpec.IsSip7843Enabled)
            {
    ...
    private TransactionBuilder<Transaction> CreateTransactionBuilder()
    {
        ...
        if (head is not null)
        {
            ...
            if (headReleaseSpec.IsSip1559Enabled && headReleaseSpec.Sip1559TransitionBlock <= head.Number)
            {
    ...
    /// <summary>
    /// Creates a simple transfer transaction with value defined by <paramref name="sila"/>
    /// from a rich account to <paramref name="address"/>
    /// </summary>
    /// <param name="address">Address to add funds to</param>
    /// <param name="sila">Value of sila to add to the account</param>
    /// <returns></returns>
    public async Task AddFunds(Address address, UInt256 sila) =>
        await AddBlockMayMissTx(GetFundsTransaction(address, sila));

    public async Task AddFunds(params (Address address, UInt256 sila)[] funds) =>
        await AddBlock(funds.Select((f, i) => GetFundsTransaction(f.address, f.sila, (uint)i)).ToArray());

    public async Task AddFundsAfterLondon(params (Address address, UInt256 sila)[] funds) =>
        await AddBlock(funds.Select((f, i) => GetFunds1559Transaction(f.address, f.sila, (uint)i)).ToArray());

    private Transaction GetFundsTransaction(Address address, UInt256 sila, uint index = 0)
    {
    ...
    private Transaction GetFunds1559Transaction(Address address, UInt256 sila, uint index = 0)
    {
        ulong nonce = StateReader.GetNonce(BlockTree.Head!.Header, TestItem.AddressA);
        Transaction tx = Builders.Build.A.Transaction
            .SignedAndResolved(TestItem.PrivateKeyA)
