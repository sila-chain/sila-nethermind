...
public class BlockHeaderTests
{
    ...
    [Test]
    public void Sip_1559_CalculateBaseFee_should_returns_zero_when_sip1559_not_enabled()
    {
        IReleaseSpec releaseSpec = ReleaseSpecSubstitute.Create();
        releaseSpec.IsSip1559Enabled.Returns(false);
    ...
    public void Sip_1559_CalculateBaseFee(long gasTarget, long baseFee, long expectedBaseFee, ulong gasUsed, long? minimalBaseFee = null)
    {
        IReleaseSpec releaseSpec = ReleaseSpecSubstitute.Create();
        releaseSpec.BaseFeeCalculator.Returns(new DefaultBaseFeeCalculator());
        releaseSpec.IsSip1559Enabled.Returns(true);
        releaseSpec.Sip1559BaseFeeMinValue.Returns((UInt256?)minimalBaseFee);
        releaseSpec.ForkBaseFee.Returns(Sip1559Constants.DefaultForkBaseFee);
        releaseSpec.BaseFeeMaxChangeDenominator.Returns(Sip1559Constants.DefaultBaseFeeMaxChangeDenominator);
        releaseSpec.ElasticityMultiplier.Returns(Sip1559Constants.DefaultElasticityMultiplier);

        BlockHeader blockHeader = Build.A.BlockHeader.TestObject;
        blockHeader.Number = 2001;
        blockHeader.GasLimit = (ulong)gasTarget * Sip1559Constants.DefaultElasticityMultiplier;
    ...
    [TestCaseSource(nameof(Sip1559BaseFeeTestSource))]
    public void Sip_1559_CalculateBaseFee_shared_test_cases((BaseFeeTestCases Info, string Description) testCase)
    {
        IReleaseSpec releaseSpec = ReleaseSpecSubstitute.Create();
        releaseSpec.IsSip1559Enabled.Returns(true);
        releaseSpec.ForkBaseFee.Returns(Sip1559Constants.DefaultForkBaseFee);
        ...
        BlockHeader blockHeader = Build.A.BlockHeader.TestObject;
        blockHeader.Number = 2001;
        blockHeader.GasLimit = testCase.Info.ParentTargetGasUsed * Sip1559Constants.DefaultElasticityMultiplier;
    ...
    private static IEnumerable<(BaseFeeTestCases, string)> Sip1559BaseFeeTestSource()
    {
