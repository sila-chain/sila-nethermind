...
namespace Nethermind.Core.Test
{
    ...
    public class EcdsaTests
    {
        ...
        {
            SilaEcdsa silaEcdsa = new(BlockchainIds.Olympic);

            ValueHash256 message = ValueKeccak.Compute("Test message");
            PrivateKey privateKey = Build.A.PrivateKey.TestObject;
            Signature signature = silaEcdsa.Sign(privateKey, in message);
            Assert.That(silaEcdsa.RecoverAddress(signature, in message), Is.EqualTo(privateKey.Address));
        }

        [Test]
        public void Decompress()
        {
            _ = new SilaEcdsa(BlockchainIds.Olympic);
            PrivateKey privateKey = Build.A.PrivateKey.TestObject;
            CompressedPublicKey compressedPublicKey = privateKey.CompressedPublicKey;
            PublicKey expected = privateKey.PublicKey;
            PublicKey actual = SilaEcdsa.Decompress(compressedPublicKey);
