...
public class BlockAccessListDecoderTests
{
    ...
    // 0x80 is an empty byte string, not an SIP-7928 BAL list. The public Decode<T>
    ...
    // 0xc1 0xc0 = outer list of 1 containing an empty inner list. SIP-7928 requires each
    ...
    {
        byte[] encoded = EncodeSlotChangesWithEmptyStorageChangeEntries(Sip7928Constants.MaxTxs + 1);

        Assert.That(
            () => Rlp.Decode<ReadOnlySlotChanges>(encoded, RlpBehaviors.None),
            Throws.TypeOf<RlpLimitException>()
                .With.Message.Contains($"over limit {Sip7928Constants.MaxTxs}"));
    }
    ...
    {
        // SIP-7928 widened BlockAccessIndex to uint32 (commit 645099785a). This test catches
    ...
    public void Decoding_account_changes_with_unsorted_storage_changes_throws()
    {
        ...
        // Each SlotChanges must have at least one StorageChange (per SIP-7928 / geth bal-devnet-4
