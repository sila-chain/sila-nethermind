...
namespace Nethermind.Core.Test.Crypto
{
    ...
    public class SilaEcdsaTests
    {
        ...
        {
            SilaEcdsa ecdsa = new(BlockchainIds.SilaSepolia);
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void Signature_test_sepolia(bool sip155)
        {
            SilaEcdsa ecdsa = new(BlockchainIds.SilaSepolia);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.TestObject;
            ecdsa.Sign(key, tx, sip155);
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void Signature_test_sepolia_1559(bool sip155)
        {
            SilaEcdsa ecdsa = new(BlockchainIds.SilaSepolia);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.WithType(TxType.SIP1559).TestObject;
            ecdsa.Sign(key, tx, sip155);
            Address? address = ecdsa.RecoverAddress(tx);
            Assert.That(address, Is.EqualTo(key.Address));
        }
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void Signature_test_olympic(bool isSip155Enabled)
        {
            SilaEcdsa ecdsa = new(BlockchainIds.SilaMainnet);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.TestObject;
            ecdsa.Sign(key, tx, isSip155Enabled);
            Address? address = ecdsa.RecoverAddress(tx);
            Assert.That(address, Is.EqualTo(key.Address));
        }
        ...
        [TestCase(TxType.SIP1559, true)]
        public void RecoverPublicKey_transaction_recovers_signer_public_key(TxType txType, bool isSip155Enabled)
        {
            SilaEcdsa ecdsa = new(BlockchainIds.SilaSepolia);
            PrivateKey key = Build.A.PrivateKey.TestObject;
        ...
        public void Sign_generic_network()
        {
            ...
            SilaEcdsa ecdsa = new(BlockchainIds.GenericNonRealNetwork);
            PrivateKey key = Build.A.PrivateKey.TestObject;
            Transaction tx = Build.A.Transaction.TestObject;
        ...
        public void RecoverAddress_repeat_recovery_uses_sender_cache_for_typed_tx_only(TxType txType, bool servedFromCache)
        {
            ...
            SilaEcdsaExtensions.ClearSenderCache();
        ...
        public void RecoverAddress_AuthorizationTupleOfDifferentSize_RecoversAddressCorrectly()
        {
            ...
            AuthorizationTuple authorizationTuple = new SilaEcdsa(BlockchainIds.GenericNonRealNetwork)
                .Sign(signer,
