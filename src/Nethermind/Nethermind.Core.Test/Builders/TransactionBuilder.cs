...
namespace Nethermind.Core.Test.Builders
{
    public class TransactionBuilder<T> : BuilderBase<T> where T : Transaction, new()
    {
        ...
        public TransactionBuilder<T> Signed(ISilaEcdsa ecdsa, PrivateKey privateKey, bool isSip155Enabled = true)
        {
            ecdsa.Sign(privateKey, TestObjectInternal, isSip155Enabled);
            return this;
        }

        public TransactionBuilder<T> Signed(PrivateKey? privateKey = null, bool isSip155Enabled = true)
        {
            privateKey ??= TestItem.IgnoredPrivateKey;
            SilaEcdsa ecdsa = new(TestObjectInternal.ChainId ?? TestBlockchainIds.ChainId);

            return Signed(ecdsa, privateKey, isSip155Enabled);
        }

        // TODO: auto create ecdsa here
        public TransactionBuilder<T> SignedAndResolved(ISilaEcdsa ecdsa, PrivateKey privateKey, bool isSip155Enabled = true)
        {
        ...
        public TransactionBuilder<T> SignedAndResolved(PrivateKey? privateKey = null, bool isSip155Enabled = true)
        {
            privateKey ??= TestItem.IgnoredPrivateKey;
            SilaEcdsa ecdsa = new(TestObjectInternal.ChainId ?? TestBlockchainIds.ChainId);
            ecdsa.Sign(privateKey, TestObjectInternal, isSip155Enabled);
