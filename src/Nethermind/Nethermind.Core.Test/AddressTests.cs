...
#pragma warning restore CS1718
    ...
    public static IEnumerable PointEvaluationPrecompileTestCases
    {
        get
        {
            yield return new TestCaseData(SilaShanghai.Instance) { ExpectedResult = false, TestName = nameof(SilaShanghai) };
            yield return new TestCaseData(SilaCancun.Instance) { ExpectedResult = true, TestName = nameof(SilaCancun) };
        }
    }
}
