...
public partial class EngineModuleTests
{
    ...
    {
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaCancun.Instance);
    ...
    {
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaShanghai.Instance);
        IEngineRpcModule rpcModule = chain.EngineRpcModule;
        ExecutionPayload executionPayload = CreateBlockRequest(
            chain, CreateParentBlockRequestOnHead(chain.BlockTree), TestItem.AddressD, withdrawals: [],
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, _, _) = await BuildAndGetPayloadV3Result(SilaShanghai.Instance);
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, _, _) = await BuildAndGetPayloadV3Result(SilaCancun.Instance);
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, _, _) = await BuildAndGetPayloadV3Result(SilaCancun.Instance, blobTxCount, oneBlobPerTx: oneBlobPerTx);
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, Transaction[] transactions, _) = await BuildAndGetPayloadV3Result(SilaCancun.Instance, 1);
    ...
    {
        (IEngineRpcModule prevRpcModule, string? payloadId, Transaction[] transactions, _) = await BuildAndGetPayloadV3Result(SilaCancun.Instance, 1);
    ...
    {
        (JsonRpcService jsonRpcService, JsonRpcContext context, SilaJsonSerializer serializer, ExecutionPayloadV3 executionPayload)
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, _, _) = await BuildAndGetPayloadV3Result(SilaCancun.Instance, blobTxCount);
        ExecutionPayloadV3 payload = (await rpcModule.engine_getPayloadV3(Bytes.FromHexString(payloadId!))).Data!.ExecutionPayload;
    ...
    {
        (JsonRpcService jsonRpcService, JsonRpcContext context, SilaJsonSerializer _, ExecutionPayloadV3 _)
            = await PreparePayloadRequestEnv();
    ...
    private async Task<(JsonRpcService jsonRpcService, JsonRpcContext context, SilaJsonSerializer serializer, ExecutionPayloadV3 correctExecutionPayload)>
            PreparePayloadRequestEnv()
    {
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaCancun.Instance);
        IEngineRpcModule rpcModule = chain.EngineRpcModule;
        JsonRpcConfig jsonRpcConfig = new() { EnabledModules = new[] { ModuleType.Engine } };
        RpcModuleProvider moduleProvider = new(new RealFileSystem(), jsonRpcConfig, new SilaJsonSerializer(), LimboLogs.Instance);
    ...
    public async Task ForkChoiceUpdated_should_return_invalid_params_but_change_latest_block()
    {
        (IEngineRpcModule rpcModule, string? payloadId, Transaction[] transactions, MergeTestBlockchain chain) =
            await BuildAndGetPayloadV3Result(SilaCancun.Instance, 0);
        ExecutionPayloadV3 payload = (await rpcModule.engine_getPayloadV3(Bytes.FromHexString(payloadId!))).Data!.ExecutionPayload;
    ...
    public async Task ForkChoiceUpdated_should_return_valid_for_previous_blocks_without_state_synced()
    {
        ...
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaCancun.Instance, mergeConfig: new MergeConfig()
        {
    ...
    {
        using MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaCancun.Instance, mergeConfig: new MergeConfig()
        {
            NewPayloadBlockProcessingTimeout = 1000
        });
        IEngineRpcModule rpcModule = chain.EngineRpcModule;
    ...
    public async Task GetBlobsV1_should_return_requested_blobs([Values(1, 2, 3, 4, 5, 6)] int numberOfBlobs)
    {
        ...
        Transaction blobTx = Build.A.Transaction
            ...
            .SignedAndResolved(chain.SilaEcdsa, TestItem.PrivateKeyA).TestObject;
    ...
    {
        byte[] blob = RandomBytes((int)Sip4844Constants.GasPerBlob);
        byte[]?[] blobs = [blob];
        ...
        Assert.That(writer.WrittenText, Is.EqualTo(JsonSerializer.Serialize(response, SilaJsonSerializer.JsonOptions)));
    }
    ...
    {
        // fork A
        MergeTestBlockchain chainA = await CreateBlockchain(releaseSpec: SilaCancun.Instance, mergeConfig: new MergeConfig()
        {
            NewPayloadBlockProcessingTimeout = 1000
        });
        ...
        // main fork B
        MergeTestBlockchain chainB = await CreateBlockchain(releaseSpec: SilaCancun.Instance, mergeConfig: new MergeConfig()
        {
            NewPayloadBlockProcessingTimeout = 1000
        });
        ...
        // syncing chain
        MergeTestBlockchain chainC = await CreateBlockchain(releaseSpec: SilaCancun.Instance, mergeConfig: new MergeConfig()
        {
            NewPayloadBlockProcessingTimeout = 1000
        });
    ...
    public static IEnumerable<TestCaseData> ForkchoiceUpdatedV3DeclinedTestCaseSource
    {
        get
        {
            yield return new TestCaseData(SilaShanghai.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV2), true)
            {
                TestName = "ForkchoiceUpdatedV2 To Request SilaShanghai Payload, Zero Beacon Root",
                ExpectedResult = MergeErrorCodes.InvalidPayloadAttributes,
            };
            yield return new TestCaseData(SilaCancun.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV2), true)
            {
                TestName = "ForkchoiceUpdatedV2 To Request SilaCancun Payload, Zero Beacon Root",
                ExpectedResult = MergeErrorCodes.InvalidPayloadAttributes,
            };
            yield return new TestCaseData(SilaCancun.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV2), false)
            {
                TestName = "ForkchoiceUpdatedV2 To Request SilaCancun Payload, Null Beacon Root",
                ExpectedResult = MergeErrorCodes.UnsupportedFork,
            };

            yield return new TestCaseData(SilaShanghai.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV3), false)
            {
                TestName = "ForkchoiceUpdatedV3 To Request SilaShanghai Payload, Null Beacon Root",
                ExpectedResult = MergeErrorCodes.InvalidPayloadAttributes,
            };
            yield return new TestCaseData(SilaShanghai.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV3), true)
            {
                TestName = "ForkchoiceUpdatedV3 To Request SilaShanghai Payload, Zero Beacon Root",
                ExpectedResult = MergeErrorCodes.UnsupportedFork,
            };
            yield return new TestCaseData(SilaCancun.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV3), false)
            {
                TestName = "ForkchoiceUpdatedV3 To Request SilaCancun Payload, Null Beacon Root",
                ExpectedResult = MergeErrorCodes.InvalidPayloadAttributes,
            };
        }
    }
    ...
    public static IEnumerable<TestCaseData> ForkchoiceUpdatedV3AcceptedTestCaseSource
    {
        get
        {
            yield return new TestCaseData(SilaShanghai.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV2), false)
            {
                TestName = "ForkchoiceUpdatedV2 To Request SilaShanghai Payload, Null Beacon Root",
                ExpectedResult = ErrorCodes.None,
            };

            yield return new TestCaseData(SilaCancun.Instance, nameof(IEngineRpcModule.engine_forkchoiceUpdatedV3), true)
            {
                TestName = "ForkchoiceUpdatedV3 To Request SilaCancun Payload, Zero Beacon Root",
                ExpectedResult = ErrorCodes.None,
            };
        }
    }
