...
{
    private static TxType[] TxTypes() => [TxType.AccessList, TxType.SIP1559, TxType.Blob];
    ...
    public void TryGetTransactions_decodes_many_txs_in_order()
    {
        ...
        for (int i = 0; i < count; i++) rlps[i] = EncodeTx(TxType.SIP1559, nonce: (ulong)i);
    ...
    private static byte[] EncodeTx(TxType txType, ulong nonce = 0)
    {
        ...
        builder = txType switch
        {
            ...
            TxType.SIP1559 => builder.WithMaxFeePerGas(50).WithMaxPriorityFeePerGas(10),
