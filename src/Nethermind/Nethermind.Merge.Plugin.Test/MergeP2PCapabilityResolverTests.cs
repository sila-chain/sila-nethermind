...
public class MergeP2PCapabilityResolverTests
{
    ...
    public void Resolve_advertises_post_merge_eth_capabilities_once_post_merge(bool transitionFinished, bool hasReachedTerminalBlock, bool expected)
    {
        ...
        Assert.That(capabilities.Contains(new Capability(Protocol.Sil, 69)), Is.EqualTo(expected));
        Assert.That(capabilities.Contains(new Capability(Protocol.Sil, 70)), Is.EqualTo(expected));
        Assert.That(capabilities.Contains(new Capability(Protocol.Sil, 71)), Is.EqualTo(expected));
    }
