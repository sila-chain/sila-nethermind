...
public partial class EngineModuleTests
{
    ...
    {
        (IEngineRpcModule rpcModule, string? payloadId, _, _) = await BuildAndGetPayloadV3Result(SilaOsaka.Instance, blobTxCount, oneBlobPerTx: oneBlobPerTx);
    ...
    {
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaOsaka.Instance);
    ...
    {
        MergeTestBlockchain chain = await CreateBlockchain(releaseSpec: SilaOsaka.Instance, mergeConfig: new MergeConfig()
        {
    ...
    public async Task GetBlobsV2_should_return_requested_blobs([Values(1, 2, 3, 4, 5, 6)] int numberOfBlobs)
    {
        ...
        Transaction blobTx = Build.A.Transaction
            .WithShardBlobTxTypeAndFields(numberOfBlobs, spec: SilaOsaka.Instance)
