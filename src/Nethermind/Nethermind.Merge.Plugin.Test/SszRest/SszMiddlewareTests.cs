...
public class SszMiddlewareTests
{
    ...
    public async Task GetPayloadBodiesByHash_marks_out_of_fork_blocks_unavailable()
    {
        ...
        _specProvider.GetSpec(Arg.Is<ForkActivation>(fa => fa.Timestamp == 1_000UL)).Returns(SilaShanghai.Instance);
        _specProvider.GetSpec(Arg.Is<ForkActivation>(fa => fa.Timestamp == 2_000UL)).Returns(SilaCancun.Instance);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(decoded.Entries![0].Available, Is.True, "SilaShanghai block at /shanghai/bodies must stay available");
            Assert.That(decoded.Entries[1].Available, Is.False, "SilaCancun block at /shanghai/bodies must surface as unavailable");
        }
    }
    ...
    {
        // Two distinct spec objects for SilaShanghai and SilaCancun, identified purely by reference
    ...
    {
        // SilaParis is a supported fork but predates getPayloadBodies (introduced in SilaShanghai), so the
    ...
    public async Task NewPayloadWithWitness_on_pre_Amsterdam_fork_returns_400()
    {
        ...
        Assert.That(ctx.Response.StatusCode, Is.EqualTo(StatusCodes.Status400BadRequest),
            "the witness endpoint is gated to SilaAmsterdam+; a recognized pre-SIP-7928 fork is 400 unsupported-fork");
