...
public class SszCodecTests
{
    ...
    public void EncodeBuiltPayloadParis_fields_land_at_spec_defined_offsets()
    {
        ...
        Assert.That(decodedBaseFee, Is.EqualTo(ep.BaseFeePerGas), "baseFeePerGas must be encoded at byte offset 440 of the inner payload per the Sila consensus spec");
        ...
        Assert.That(payload.Slice(472, 32).ToArray(), Is.EqualTo(ep.BlockHash!.Bytes.ToArray()), "block_hash must be encoded at byte offset 472 of the inner payload per the Sila consensus spec");
    }
    ...
    // Shared assertion for the SilaPrague+ BuiltPayload fixed section: identical for V4/V5/V6 since
