...
public partial class EngineModuleTests
{
        ...
        string stateRoot, string payloadId)
    {
        using MergeTestBlockchain chain =
            await CreateBlockchain(SilaPrague.Instance, new MergeConfig { TerminalTotalDifficulty = "0" });
    ...
    public async Task NewPayloadV4_reject_payload_with_bad_authorization_list_rlp()
    {
        ...
        using MergeTestBlockchain chain = await CreateBlockchain(SilaPrague.Instance, null, null, executionRequestsProcessorMock);
        IEngineRpcModule rpc = chain.EngineRpcModule;
        ...
        Transaction invalidSetCodeTx = Build.A.Transaction
          ...
          .WithAuthorizationCode(new JsonRpc.Test.Modules.Sil.SilRpcModuleTests.AllowNullAuthorizationTuple(0, null, 0, new Signature(new byte[65])))
    ...
    {
        using MergeTestBlockchain chain = await CreateBlockchain(SilaPrague.Instance);
