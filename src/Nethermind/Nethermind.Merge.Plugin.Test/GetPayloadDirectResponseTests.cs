...
public class GetPayloadDirectResponseTests
{
        ...
        ulong? slotNumber)
    {
        ...
        byte[] expected = JsonSerializer.SerializeToUtf8Bytes(plain, plain.GetType(), SilaJsonSerializer.JsonOptions);
    ...
    public async Task Payload_bodies_raw_block_rlp_response_matches_dto_json_and_ssz(int version, bool blockAccessList, int withdrawalCount)
    {
        ...
        [
            ...
            CreateTransaction(TxType.SIP1559),
    ...
    public async Task Json_rpc_envelope_cancellation_does_not_complete_partial_direct_response()
    {
        ...
        try
        {
            Func<Task> act = async () => await JsonRpcResponseWriter.WriteAsync(writer, response, SilaJsonSerializer.JsonOptions, cancellationTokenSource.Token);
    ...
    public static IEnumerable<TestCaseData> TransactionCases()
    {
        ...
        yield return new TestCaseData(CreateTransaction(TxType.SIP1559)).SetName("SIP1559");
    ...
    private static Transaction CreateTransaction(TxType type)
    {
        ...
        if (type == TxType.Blob)
        {
            builder.WithShardBlobTxTypeAndFields(blobCount: 1, spec: SilaOsaka.Instance);
        }
    ...
    private static async Task<byte[]> WriteResponseAsync(JsonRpcResponse response)
    {
        using MemoryStream stream = new();
        PipeWriter writer = PipeWriter.Create(stream, new StreamPipeWriterOptions(leaveOpen: true));
        await JsonRpcResponseWriter.WriteAsync(writer, response, SilaJsonSerializer.JsonOptions, CancellationToken.None);
        await writer.FlushAsync();
        await writer.CompleteAsync();
        return stream.ToArray();
    }
