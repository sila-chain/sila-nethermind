...
using Nethermind.Runner.Sila.Modules;
...
public class MergePluginTests
{
    ...
    public void AddTypeInfoResolver_updates_existing_serializer_instances()
    {
        ...
        SilaJsonSerializer.AddTypeInfoResolver(new ThrowingProbeResolver());
    ...
    public void AddTypeInfoResolver_orders_resolvers_by_priority_then_registration_order()
    {
        ...
        SilaJsonSerializer.AddTypeInfoResolver(new RecordingProbeResolver("external", calls));
        SilaJsonSerializer.AddTypeInfoResolver(new RecordingProbeResolver("json-rpc-response", calls), JsonTypeInfoResolverPriority.JsonRpcResponse);
        SilaJsonSerializer.AddTypeInfoResolver(new RecordingProbeResolver("engine-api", calls), JsonTypeInfoResolverPriority.EngineApi);
        SilaJsonSerializer.AddTypeInfoResolver(new RecordingProbeResolver("facade-first", calls), JsonTypeInfoResolverPriority.Facade);
        SilaJsonSerializer.AddTypeInfoResolver(new RecordingProbeResolver("facade-second", calls), JsonTypeInfoResolverPriority.Facade);

        _ = SilaJsonSerializer.JsonOptions.GetTypeInfo(typeof(PriorityProbe));
    ...
    public async Task InitThrowsWhenNoEngineApiUrlsConfigured(bool jsonRpcEnabled, bool configuredViaAdditionalUrls)
    {
        ...
        if (configuredViaAdditionalUrls)
        {
            jsonRpcConfig = new JsonRpcConfig()
            {
                ...
                AdditionalRpcUrls = ["http://localhost:8550|http;ws|net;sil;subscribe;web3;client|no-auth"]
            };
        }
        else
        {
    ...
    public async Task InitDisableJsonRpcUrlWithNoEngineUrl()
    {
        ...
        {
            Enabled = false,
            EnabledModules = ["sil", "subscribe"],
            AdditionalRpcUrls =
            [
                "http://localhost:8550|http;ws|net;sil;subscribe;web3;client|no-auth",
                "http://localhost:8551|http;ws|net;sil;subscribe;web3;engine;client"
            ]
        };
        ...
        Assert.That(jsonRpcConfig.AdditionalRpcUrls, Is.EqualTo(new[] { "http://localhost:8551|http;ws|net;sil;subscribe;web3;engine;client" }));
    }
}
