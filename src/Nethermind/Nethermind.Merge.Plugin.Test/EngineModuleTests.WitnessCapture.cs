...
public partial class EngineModuleTests
{
    ...
    public async Task Witness_includes_bytecode_and_storage_proof_for_a_called_contract()
    {
        ...
        // Deploy from AddressB (a funded EOA) so the deployer has no code (SIP-3607 friendly).
        ...
        Transaction deploy = Build.A.Transaction
            ...
            .WithMaxFeePerGas(20.GWei).WithMaxPriorityFeePerGas(1.GWei).WithType(TxType.SIP1559)
            .SignedAndResolved(chain.SilaEcdsa, TestItem.PrivateKeyB).TestObject;
    ...
    /// <summary>
    /// Builds one SilaAmsterdam block on the current head (including <paramref name="txs"/>), submits it via
    ...
    private static async Task<Witness> ProduceWitnessedBlock(MergeTestBlockchain chain, params Transaction[] txs)
    {
        ...
        Assert.That(witness, Is.Not.Null, "a VALID SilaAmsterdam block must produce a witness");
        return witness!;
    }

    /// <summary>Builds, imports, and forkchoice-canonicalizes one SilaAmsterdam block (advancing the head).</summary>
