...
public partial class EngineModuleTests
{
    ...
    public virtual Task NewPayloadV5_accepts_valid_BAL(string? blockHash, string? receiptsRoot, string? stateRoot, bool sip8037Enabled, bool useEnginePipeline) =>
        !sip8037Enabled && !useEnginePipeline
            ? NewPayloadV5_via_manual_block(blockHash, receiptsRoot, stateRoot)
            : NewPayloadV5_via_engine_built(blockHash, receiptsRoot, stateRoot, sip8037Enabled, useSerializedRpc: !useEnginePipeline);
    ...
    {
        using MergeTestBlockchain chain = await CreateBlockchain(SilaAmsterdam.NoSip8037Instance);
        IEngineRpcModule rpc = chain.EngineRpcModule;
    ...
    public virtual Task NewPayloadV5_rejects_invalid_BAL_early(
        ...
        bool sip8037Enabled, bool useEnginePipeline, BalErrorKind errorKind)
    {
        bool useManualBlock = !sip8037Enabled && !useEnginePipeline;
        ...
        return useManualBlock
            ...
            : NewPayloadV5_via_engine_built(blockHash, receiptsRoot, stateRoot, sip8037Enabled, expectedError, errorKind, useSerializedRpc: !useEnginePipeline);
    }
    ...
    public virtual async Task GetPayloadV6_builds_block_with_BAL(string? customWithdrawalContractAddress)
    {
        ...
        Transaction tx = Build.A.Transaction
            ...
            .SignedAndResolved(chain.SilaEcdsa, TestItem.PrivateKeyA)
            .TestObject;
    ...
    /// <summary>
    /// Tests BAL validation with a manually constructed block via RPC serialization (no SIP-8037).
    /// </summary>
        ...
        string? customWithdrawalContractAddress = null)
    {
        ...
        ReadOnlyBlockAccessList CreateBlockAccessList()
        {
            AccountChangesBuilder newContractAccount = Build.An.AccountChanges
                ...
                .WithCodeChanges([new(2, Sip2935TestConstants.Code)]);
    ...
    private async Task NewPayloadV5_via_engine_built(
        ...
        bool sip8037Enabled = true,
        string? expectedError = null,
        BalErrorKind errorKind = BalErrorKind.None,
        bool useSerializedRpc = false)
    {
        IReleaseSpec spec = sip8037Enabled ? SilaAmsterdam.Instance : SilaAmsterdam.NoSip8037Instance;
    ...
    private static (Transaction tx, Transaction tx2, Transaction tx3, Withdrawal withdrawal) BuildTestTransactionsAndWithdrawal(ulong gasPrice, ulong gasLimit)
    {
        ...
        Transaction tx2 = Build.A.Transaction
            ...
            .WithGasPrice(gasPrice)
            .WithGasLimit(gasLimit)
            .WithCode(Sip2935TestConstants.InitCode)
            .SignedAndResolved(TestItem.PrivateKeyA)
            .TestObject;
