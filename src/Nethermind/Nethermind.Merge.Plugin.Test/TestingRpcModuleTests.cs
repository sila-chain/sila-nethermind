...
public class TestingRpcModuleTests
{
    ...
    public async Task Sets_excess_blob_gas_and_withdrawals_root()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(payloadResult.ExecutionPayload.ExcessBlobGas, Is.EqualTo(BlobGasCalculator.CalculateExcessBlobGas(parentHeader, SilaOsaka.Instance)));
    ...
    {
        yield return new TestCaseData(SilaOsaka.Instance, false, false)
            .SetName("Returns_fork_specific_payload_for_fusaka");
        yield return new TestCaseData(SilaAmsterdam.Instance, true, true)
        ...
        IBlockTree? blockTree = null)
    {
        ...
        specProvider.GetSpec(Arg.Any<ForkActivation>()).Returns(spec ?? SilaOsaka.Instance);
