...
/// with sila-mainnet-like block scenarios under <see cref="SilaOsaka"/> rules.
///
...
/// - Sip1559_200, AccessList_50, ContractDeploy_10
/// - ContractCall_200, MixedBlock (100 legacy + 60 SIP-1559 + 30 AL + 10 calls)
/// </summary>
...
public class BlockProcessingBenchmark
{
    ...
    private Block _sip1559_200Block = null!;
    ...
    public void GlobalSetup()
    {
        ...
        _sip1559_200Block = BuildBlock(BuildSip1559Transfers(200, 0));
        ...
        // MixedBlock: 100 legacy + 60 SIP-1559 + 30 access-list + 10 contract calls
        Transaction[] mixedTxs = new Transaction[200];
        ulong nonce = 0;
        BuildLegacyTransfers(100, nonce).CopyTo(mixedTxs, 0);
        nonce += 100;
        BuildSip1559Transfers(60, nonce).CopyTo(mixedTxs, 100);
        nonce += 60;
        ...
        // with TestSpecProvider(SilaOsaka.Instance) and in-memory databases.
        ...
        {
            stateProvider.CreateAccount(_sender, 1_000_000.Sila);

            stateProvider.CreateAccount(TestItem.AddressB, UInt256.Zero);
            stateProvider.InsertCode(TestItem.AddressB, ContractCode, Spec);

            stateProvider.CreateAccount(Sip7002Constants.WithdrawalRequestPredeployAddress, UInt256.Zero);
            stateProvider.InsertCode(Sip7002Constants.WithdrawalRequestPredeployAddress, StopCode, Spec);
            stateProvider.CreateAccount(Sip7251Constants.ConsolidationRequestPredeployAddress, UInt256.Zero);
            stateProvider.InsertCode(Sip7251Constants.ConsolidationRequestPredeployAddress, StopCode, Spec);
    ...
    public Block[] Sip1559_200()
    {
        Block[] result = null!;
        for (int i = 0; i < N_SMALL; i++)
            result = _branchProcessor.Process(_parentHeader, [_sip1559_200Block],
                ProcessingOptions.NoValidation, NullBlockTracer.Instance);
        return result;
    }
    ...
    private Transaction[] BuildSip1559Transfers(int count, ulong startNonce)
    {
        Transaction[] txs = new Transaction[count];
        for (int i = 0; i < count; i++)
        {
