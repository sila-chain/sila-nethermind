...
/// <see cref="SilaTransactionProcessor"/>. Uses <c>CallAndRestore</c> so
/// state is rolled back after each call, allowing 1 000 iterations per invocation.
///
/// Transaction types covered:
/// - Legacy SIL transfer (simple, data, zero-data, large-data)
/// - SIP-2930 access-list transaction (type 1)
/// - SIP-1559 type-2 transaction
/// - Contract deployment via CREATE (no recipient)
/// - Contract call into deployed code
///
/// All benchmarks run under <see cref="SilaOsaka"/> rules (latest fork).
...
public class TxProcessingBenchmark
{
    ...
    private static readonly IReleaseSpec Spec = SilaOsaka.Instance;
    private static readonly ISpecProvider SpecProvider = new TestSpecProvider(SilaOsaka.Instance);

    private static readonly byte[] ContractCode = Prepare.SavmCode
        .PushData(0x01)
        .Op(Instruction.STOP)
        .Done;
    ...
    private Transaction _simpleTx = null!;  // 21 000 gas SIL transfer
    ...
    private Transaction _accessListTx = null!;  // SIP-2930: access-list tx
    private Transaction _sip1559Tx = null!;  // SIP-1559: type-2 tx
    ...
    public void GlobalSetup()
    {
        ...
        _stateProvider.CreateAccount(TestItem.AddressA, 10_000.Sila);
        ...
        SilaCodeInfoRepository codeInfo = new(_stateProvider);
        SilaVirtualMachine vm = new(
            new TestBlockhashProvider(),
            SpecProvider,
            LimboLogs.Instance);
        _processor = new SilaTransactionProcessor(
        ...
        _sip1559Tx = Build.A.Transaction
        ...
        _processor.CallAndRestore(_sip1559Tx, NullTxTracer.Instance);
    ...
    /// <summary>Simple SIL transfer — 21 000 intrinsic gas, baseline.</summary>
    ...
    /// <summary>SIP-2930 access-list transaction (type 1, Berlin+).</summary>
    [Benchmark(OperationsPerInvoke = N)]
    ...
    /// <summary>SIP-1559 type-2 transaction with max-fee / priority-fee (London+).</summary>
    [Benchmark(OperationsPerInvoke = N)]
    public TransactionResult Sip1559Tx()
    {
        TransactionResult result = default;
        for (int i = 0; i < N; i++)
            result = _processor.CallAndRestore(_sip1559Tx, NullTxTracer.Instance);
        return result;
    }

    // ── SAVM-executing transactions ─────────────────────────────────────────

    /// <summary>Call into a deployed contract (SAVM execution path).</summary>
    [Benchmark(OperationsPerInvoke = N)]
