...
/// <summary>
/// Benchmarks that execute real SAVM instruction handlers via function pointer dispatch,
...
/// Run: dotnet run -c Release --filter "*SavmOpcodesBenchmark*"
/// </summary>
[Config(typeof(SavmOpcodesBenchmarkConfig))]
public unsafe class SavmOpcodesBenchmark
{
    private const int InnerCount = 8192;
    private const int KeccakWordSize = SavmStack.WordSize;
    private const int DynamicStorageKeyCount = InnerCount * 8;
    private const int DynamicCallTargetCount = InnerCount;

    private delegate*<VirtualMachine<SilaGasPolicy>, ref SavmStack, ref SilaGasPolicy, ref int, SavmExceptionType>[] _opcodes = null!;
    private BenchmarkVm _vm = null!;
    private byte[] _stackBuffer = null!;
    private int _stackOffset;
    private int _stackLength;
    private SilaGasPolicy _gas;
    private ExecutionEnvironment _env = null!;
    private VmState<SilaGasPolicy> _vmState = null!;
    ...
    private SilaCodeInfoRepository _codeInfoRepository = null!;
    ...
    public void Setup()
    {
        ...
        _gas = SilaGasPolicy.FromULong(ulong.MaxValue);
        ...
        _vmState = VmState<SilaGasPolicy>.RentTopLevel(
            SilaGasPolicy.FromULong(ulong.MaxValue),
        ...
        _opcodes = SavmInstructions.GenerateOpCodes<SilaGasPolicy, OffFlag>(spec);
    ...
    public SavmExceptionType ExecuteOpcode()
    {
    ...
    private SavmExceptionType ExecuteOpcodeWithStackWalk()
    {
        SavmStack stack = new(_stackDepth, NullTxTracer.Instance, ref MemoryMarshal.GetReference(GetAlignedStackSpan()), default);
        SavmExceptionType result = SavmExceptionType.None;
    ...
    private SavmExceptionType ExecuteOpcodeWithPerRunRefresh()
    {
        SavmStack stack = new(_stackDepth, NullTxTracer.Instance, ref MemoryMarshal.GetReference(GetAlignedStackSpan()), default);
        SavmExceptionType result = SavmExceptionType.None;
    ...
    private SavmExceptionType ExecuteOpcodeWithIndependentBinaryInputs()
    {
        SavmStack stack = new(_stackDepth, NullTxTracer.Instance, ref MemoryMarshal.GetReference(GetAlignedStackSpan()), default);
        SavmExceptionType result = SavmExceptionType.None;
        int remaining = InnerCount;
    ...
    public void SetupStackForOpcode()
    {
        ...
        if (RequiresIndependentBinaryInputs(Opcode))
        {
            _runsPerBatch = Math.Min(InnerCount, SavmStack.MaxStackSize / 2);
    ...
    {
        SavmOpcodesBenchmark probe = new() { Opcode = opcode };
        try
        {
            probe.Setup();
    ...
    private static int CalculateRunsPerBatch(Instruction opcode, int depth, int requestedRuns)
    {
        ...
        {
            int maxHead = SavmStack.MaxStackSize - 1;
    ...
    {
        int effectiveRuns = Math.Clamp(runs, 1, SavmStack.MaxStackSize - 1);
    ...
    {
        int effectiveRuns = Math.Clamp(runs, 1, (SavmStack.MaxStackSize - 1) / 2);
    ...
    {
        int availableSlots = _stackLength / SavmStack.WordSize;
    ...
    {
        if (_vm.ReturnData is VmState<SilaGasPolicy> nestedFrame)
        {
            nestedFrame.Dispose();
            _vm.ReturnData = null!;
        }
    ...
    {
        int effectiveRuns = Math.Clamp(runs, 1, SavmStack.MaxStackSize / 2);
    ...
    public static (byte[] Buffer, int Offset, int Size) CreateStackBuffer()
    {
        ...
        if (slots < SavmStack.MaxStackSize)
        {
            slots = SavmStack.MaxStackSize;
        }

        int stackSize = slots * SavmStack.WordSize;
        byte[] buffer = GC.AllocateUninitializedArray<byte>(stackSize + SavmStack.WordSize, pinned: true);
        int offset = GetAlignmentOffset(buffer, SavmStack.WordSize);
    ...
    private class BenchmarkVm(IBlockhashProvider bhp, ISpecProvider sp, ILogManager lm)
        : VirtualMachine<SilaGasPolicy>(bhp, sp, lm)
    {
        private static readonly FieldInfo CodeInfoRepositoryField =
            typeof(VirtualMachine<SilaGasPolicy>)
                .GetField("_codeInfoRepository", BindingFlags.Instance | BindingFlags.NonPublic)!;

        public void SetVmState(VmState<SilaGasPolicy> state) => VmState = state;
    ...
    public class SavmOpcodesBenchmarkConfig : ManualConfig
    {
        public SavmOpcodesBenchmarkConfig()
        {
            ...
            AddColumnProvider(new SavmOpcodeGasColumnProvider());
        }
    }
}
