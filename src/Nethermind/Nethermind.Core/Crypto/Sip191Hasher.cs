...
{
    public static class Sip191Hasher
    {
        // SIP-191 version byte (0x19) followed by the personal_sign domain separator.
        private static readonly byte[] HeaderBytes = [0x19, .. "Sila Signed Message:\n"u8];
        private static ReadOnlySpan<byte> Header => HeaderBytes;

        // SIP-191: keccak256("\x19Sila Signed Message:\n" || ascii(byteLength) || messageBytes).
