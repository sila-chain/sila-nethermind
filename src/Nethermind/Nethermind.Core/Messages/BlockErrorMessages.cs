...
public static class BlockErrorMessages
{
    ...
    public static string InsufficientMaxFeePerBlobGas(Address? address, UInt256? blobGasFeeCap, UInt256 blobBaseFee) =>
        $"max fee per blob gas less than block blob gas fee: address {address?.ToString(withSip55Checksum: true)}, blobGasFeeCap: {blobGasFeeCap}, blobBaseFee: {blobBaseFee}";
    ...
    public const string MissingRequests =
        "MissingRequests: Requests cannot be null in block when SIP-6110 or SIP-7002 are activated.";

    public const string RequestsNotEnabled =
        "RequestsNotEnabled: Requests must be null in block when SIP-6110 and SIP-7002 are not activated.";
    ...
    public static string BlockAccessListGasLimitExceeded(ulong balItems, ulong maxBalItems) =>
        $"BlockAccessListGasLimitExceeded: BAL has {balItems} items, exceeds limit of {maxBalItems} (block_gas_limit / {Sip7928Constants.ItemCost}).";
