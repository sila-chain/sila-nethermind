...
namespace Nethermind.Core
{
    public static class TransactionExtensions
    {
        extension(Transaction tx)
        {
            ...
            public UInt256 CalculateTransactionPotentialCost(bool sip1559Enabled, in UInt256 baseFee)
            {
                if (sip1559Enabled)
                {
                    ...
                    {
                        true => UInt256.Zero,
                        _ => tx.CalculateEffectiveGasPrice(sip1559Enabled, baseFee)
                    };
            ...
            public UInt256 CalculateEffectiveGasPrice(bool sip1559Enabled, in UInt256 baseFee) =>
                !sip1559Enabled ? tx.MaxPriorityFeePerGas
            ...
            public UInt256 CalculateMaxPriorityFeePerGas(bool sip1559Enabled, in UInt256 baseFee) =>
                sip1559Enabled ? UInt256.Min(tx.MaxPriorityFeePerGas, tx.MaxFeePerGas > baseFee ? tx.MaxFeePerGas - baseFee : 0) : tx.MaxPriorityFeePerGas;

            public bool IsAboveInitCode(IReleaseSpec spec) =>
                tx.IsContractCreation && spec.IsSip3860Enabled && tx.DataLength > spec.MaxInitCodeSize;

            public ulong GetBlobGas() => (ulong)tx.GetBlobCount() * Sip4844Constants.GasPerBlob;
