...
#if ZK_SAVM
// RISC-V lacks CRC32C; route hashing to the guest substitute (see ZkSavmBitOperations).
using BitOperations = Nethermind.Core.Extensions.ZkSavmBitOperations;
#endif
...
namespace Nethermind.Core.Extensions
{
    public static class SpanExtensions
    {
        ...
        extension(in ReadOnlySpan<byte> span)
        {
            ...
            public string ToHexString(bool withZeroX, bool noLeadingZeros, bool withSip55Checksum) =>
                ToHexViaLookup(span, withZeroX, noLeadingZeros, withSip55Checksum);
        }
        ...
        [DebuggerStepThrough]
        private static unsafe string ToHexViaLookup(ReadOnlySpan<byte> bytes, bool withZeroX, bool skipLeadingZeros, bool withSip55Checksum)
        {
            if (withSip55Checksum)
            {
                return ToHexStringWithSip55Checksum(bytes, withZeroX, skipLeadingZeros);
            }
        ...
        private static string ToHexStringWithSip55Checksum(ReadOnlySpan<byte> bytes, bool withZeroX, bool skipLeadingZeros)
        {
            int leadingZeros = skipLeadingZeros ? bytes.CountLeadingNibbleZeros() : 0;
            int length = bytes.Length * 2 + (withZeroX ? 2 : 0) - leadingZeros;
            if (skipLeadingZeros && length == (withZeroX ? 2 : 0))
            ...
            try
            {
                bytes.OutputBytesToCharHexWithSip55Checksum(chars, withZeroX, leadingZeros);
