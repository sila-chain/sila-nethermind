...
public class BlockAccessListAtIndex : IJournal<int>, IResettable
{
    ...
    public void AddBalanceChange(Address address, UInt256 before, UInt256 after)
    {
        ...
        // include touched-but-unchanged accounts (e.g. SIP-1559 zero-tip coinbase credits) in
            ...
            switch (change.Type)
            {
                ...
                case ChangeType.StorageChange:
                    ...
                    else
                    {
                        ...
                        // SLOAD at the SAVM level), so mark it as a read to preserve that the
    ...
    private readonly struct ChangeValue
    {
        ...
        public ChangeValue(in SavmWord storage) => _data = Unsafe.As<SavmWord, UInt256>(ref Unsafe.AsRef(in storage));

        public UInt256 Balance => _data;

        public ulong Nonce => _data.u0;

        public SavmWord Storage => Unsafe.As<UInt256, SavmWord>(ref Unsafe.AsRef(in _data));
    }
}
