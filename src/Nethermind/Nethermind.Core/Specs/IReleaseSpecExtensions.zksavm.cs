...
{
    private static IReleaseSpec GetNoSip158Spec(IReleaseSpec spec) => new NoSip158Spec(spec);
    private static IReleaseSpec GetNoSip3607Spec(IReleaseSpec spec) => new NoSip3607Spec(spec);
    ...
    // Each `spec.IsSipXxxEnabled` is an IReleaseSpec interface dispatch, and the getters below are read
    ...
    {
        _clearEmptyAccountWhenTouched = spec.IsSip158Enabled;
        _useHotAndColdStorage = spec.IsSip2929Enabled;
        _chargeForTopLevelCreate = spec.IsSip2Enabled;
        _failOnOutOfGasCodeDeposit = spec.IsSip2Enabled;
        _useShanghaiDDosProtection = spec.IsSip150Enabled;
        _useConstantinopleNetGasMetering = spec.IsSip1283Enabled;
        _useIstanbulNetGasMetering = spec.IsSip2200Enabled;
        _flagsSpec = spec;
    }
