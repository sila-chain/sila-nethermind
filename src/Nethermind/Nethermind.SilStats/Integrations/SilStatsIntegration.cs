...
using SilStatsTransaction = Nethermind.SilStats.Messages.Models.Transaction;

namespace Nethermind.SilStats.Integrations
{
    public class SilStatsIntegration(
        string name,
        string node,
        int port,
        string network,
        ...
        string secret,
        ISilStatsClient silStatsClient,
        ...
        ILogManager logManager) : ISilStatsIntegration
    {
        ...
        private readonly ISilStatsClient _ethStatsClient = silStatsClient;
        ...
        private readonly ILogger _logger = logManager.GetClassLogger<SilStatsIntegration>();
        private readonly IGasPriceOracle _gasPriceOracle = gasPriceOracle;
        private readonly ISilSyncingInfo _ethSyncingInfo = silSyncingInfo;
        ...
        private void Run(Timer timer)
        {
            ...
            {
                _connected = false;
                if (_logger.IsInfo) _logger.Info($"SIL Stats disconnected, reason: {reason}");
            });
        ...
        private void TimerOnElapsed(object? sender, ElapsedEventArgs e)
        {
            if (!_connected)
                return;
            if (_logger.IsDebug) _logger.Debug("SIL Stats sending 'stats' message...");
            _ = SendStatsAsync();
            _ = SendNodePingAsync();
        ...
        private void BlockTreeOnNewHeadBlock(object? sender, BlockEventArgs e)
        {
            ...
            if (block is null)
            {
                _logger.Error($"{nameof(SilStatsIntegration)} received null as the new head block.");
                return;
            }

            if (_logger.IsDebug) _logger.Debug("SIL Stats sending 'block', 'pending' messages...");
        ...
        private async Task ReconnectHelloAsync()
        {
            try
            {
                if (_logger.IsInfo) _logger.Info("SIL Stats reconnected, sending 'hello' message...");
                await SendHelloAsync();
                _connected = true;
            }
            catch (Exception e)
            {
                if (_logger.IsWarn) _logger.Warn($"SIL Stats hello failed after reconnect: {e}");
            }
        }

        private Task SendHistoryAsync(SilStatsHistoryRequest request)
        {
            NormalizeHistoryRange(request, out ulong min, out ulong max);

            List<SilStatsBlock> history = new((int)(max - min + 1));
            ...
            if (_logger.IsDebug) _logger.Debug($"SIL Stats sending 'history' message for range {min}-{max}.");
        ...
        private Task SendNodePingAsync()
        {
            ...
            if (_logger.IsDebug) _logger.Debug("SIL Stats sending 'node-ping' message...");
        ...
        {
            if (!SilStatsMessageParser.TryParse(message, out SilStatsIncomingMessage incomingMessage))
            {
                return;
            }
            ...
            try
            {
                ...
                {
                    SilStatsIncomingMessageType.History when incomingMessage.HistoryRequest is not null =>
                        SendHistoryAsync(incomingMessage.HistoryRequest.Value),
                    SilStatsIncomingMessageType.NodePing =>
                        SendNodePongAsync(incomingMessage.NodeTiming?.ClientTime),
                    SilStatsIncomingMessageType.NodePong =>
            ...
            {
                if (_logger.IsWarn) _logger.Warn($"Failed to handle SIL Stats message '{incomingMessage.EventTypeName}': {e}");
            }
        }
        ...
        private Task SendNodePongAsync(long? clientTime)
        {
            ...
            if (_logger.IsDebug) _logger.Debug("SIL Stats sending 'node-pong' message...");
            return _sender.SendAsync(_websocketClient, new NodePongMessage(clientTime, serverTime), "node-pong");
        }

        private Task SendLatencyFromNodePongAsync(SilStatsNodeTiming? nodeTiming)
        {
            ...
            if (now < clientTime)
            {
                if (_logger.IsDebug) _logger.Debug($"Ignoring SIL Stats 'node-pong' latency with future clientTime {clientTime}.");
                return Task.CompletedTask;
            }

            long latency = now - clientTime;

            if (_logger.IsDebug) _logger.Debug($"SIL Stats sending 'latency' message after 'node-pong': {latency} ms.");
        ...
        {
            if (_logger.IsDebug) _logger.Debug($"Ignoring unsupported SIL Stats message '{eventType}'.");
            return Task.CompletedTask;
        }

        internal static void NormalizeHistoryRange(SilStatsHistoryRequest request, out ulong min, out ulong max)
        {
        ...
        private static SilStatsBlock CreateBlockModel(CoreBlock block)
        {
            List<SilStatsTransaction> transactions = new(block.Transactions.Length);
            foreach (CoreTransaction transaction in block.Transactions)
            {
                transactions.Add(new SilStatsTransaction((transaction.Hash ?? Keccak.Zero).ToString()));
            }
        ...
        private async Task SendStatsAsync()
        {
            ...
            {
                // SilStats doesn't work with UInt256, long should be enough
                if (_logger.IsTrace) _logger.Trace($"Gas price beyond the sil stats expected scope {gasPrice}");
