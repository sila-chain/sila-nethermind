...
public class BlockCachePreWarmerTests
{
    ...
    public void Setup()
    {
        _container = new ContainerBuilder()
            .AddModule(new TestNethermindModule(SilaOsaka.Instance))
            .Build();
        ...
        {
            worldState.CreateAccount(TestItem.AddressA, 1_000_000.Sila);
            worldState.CreateAccount(TestItem.AddressB, 1_000_000.Sila);
            ...
            worldState.Commit(SilaOsaka.Instance);
    ...
    public async Task Dispose_WhenCalled_DisposesRetainedEnvsInPool()
    {
        ...
        await RunPreWarmCaches(preWarmer, BuildTwoSenderBlock(), BuildParentHeader(), SilaOsaka.Instance);
    ...
    public async Task PreWarmCaches_WithBal_PopulatesStateCacheForBalAddresses()
    {
        ...
        await RunPreWarmCaches(preWarmer, block, BuildParentHeader(), SilaAmsterdam.Instance);
    ...
    /// even when a BAL is present and SIP-7928 is enabled.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// Verifies that the BAL path is not triggered for pre-SilaAmsterdam specs even when a
    ...
    public async Task PreWarmCaches_WithBalButPreAmsterdam_UsesSpeculativePath()
    {
        ...
        // Use SilaOsaka which does NOT have SIP-7928 — BAL path should not trigger
        await RunPreWarmCaches(preWarmer, block, BuildParentHeader(), SilaOsaka.Instance);
    ...
    /// <summary>Prewarming warms a transaction's declared SIP-2930 access-list slots for the main thread.</summary>
    [Test]
    ...
    /// flag from the caller, so speculative SAVM work is attributed to the _other* metric counters.
    /// </summary>
    [Test]
    public async Task PreWarmCaches_DoesNotFlowIsBlockProcessingThread_IntoTask()
    {
        ...
        try
        {
            prewarmTask = flagWarmer.PreWarmCaches(BuildTwoSenderBlock(), BuildParentHeader(), SilaOsaka.Instance);
            Assert.That(
    ...
    public async Task PreWarmCaches_GateLogic(bool parallelExecution, bool hasBal, bool batchRead, bool expectWarmed)
    {
        ...
        // Use SilaAmsterdam (SIP-7928) when testing BALs, SilaOsaka otherwise
        IReleaseSpec spec = hasBal ? SilaAmsterdam.Instance : SilaOsaka.Instance;
    ...
    public async Task ParentReaderEnvPolicy_SharesBalWarmupCachesAndPopulatesMisses()
    {
        ...
        Assert.That(scope.WorldState.GetBalance(TestItem.AddressB), Is.EqualTo(1_000_000.Sila));
        ...
        Assert.That(populatedAccount!.Balance, Is.EqualTo(1_000_000.Sila));
    ...
    public async Task PreWarmCaches_HandoffSentinelSurvival(bool speculative, bool sameParent, bool expectSurvives)
    {
        ...
        if (speculative) await RunSpeculativePreWarm(preWarmer, head, SilaOsaka.Instance);
        ...
        await RunPreWarmCaches(preWarmer, next, head, SilaOsaka.Instance);
    ...
    public async Task PreWarmCaches_HandoffMarker_IsConsumedOnce()
    {
        ...
        BlockHeader head = BuildParentHeader();
        await RunSpeculativePreWarm(preWarmer, head, SilaOsaka.Instance);

        await RunPreWarmCaches(preWarmer, BuildChildBlock(head), head, SilaOsaka.Instance);
    ...
    public async Task StartSpeculativePreWarm_CachesCommittedBaseState_NotSpeculativeWrites()
    {
        ...
        await RunSpeculativePreWarm(preWarmer, BuildParentHeader(), SilaOsaka.Instance);
        ...
        Assert.That(cachedA!.Balance, Is.EqualTo(1_000_000.Sila),
    ...
    public async Task PreWarmCaches_SkipStarted_SkipsTransactionsMainThreadHasStarted()
    {
        ...
        using (ManualResetEventSlim gate = new(initialState: false))
        {
            ...
            {
                Task task = preWarmer.PreWarmCaches(block, BuildParentHeader(), SilaOsaka.Instance);
        ...
        : IPooledObjectPolicy<IReadOnlyTxProcessorSource>
    {
        ...
        private sealed class CountingScope(IReadOnlyTxProcessingScope inner, Action onWarmup) : IReadOnlyTxProcessingScope
        {
            ...
            public Nethermind.Savm.TransactionProcessing.ITransactionProcessor TransactionProcessor => _processor;
            public IWorldState WorldState => inner.WorldState;
            public void Dispose() => inner.Dispose();
        }

        private sealed class CountingTxProcessor(Nethermind.Savm.TransactionProcessing.ITransactionProcessor inner, Action onWarmup)
            : Nethermind.Savm.TransactionProcessing.ITransactionProcessor
        {
            public Nethermind.Savm.TransactionProcessing.TransactionResult Process(Transaction transaction, Nethermind.Savm.Tracing.ITxTracer txTracer, Nethermind.Savm.TransactionProcessing.ExecutionOptions options)
            {
                if ((options & Nethermind.Savm.TransactionProcessing.ExecutionOptions.Warmup) != 0) onWarmup();
            ...
            public void SetBlockExecutionContext(in Nethermind.Savm.BlockExecutionContext blockExecutionContext) => inner.SetBlockExecutionContext(in blockExecutionContext);
        }
    }
}
