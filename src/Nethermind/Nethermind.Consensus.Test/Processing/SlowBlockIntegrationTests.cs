...
/// <summary>
/// Integration tests verifying that SAVM metrics flow correctly into slow block JSON logs.
...
public class SlowBlockIntegrationTests
{
    ...
    public void SLOAD_tracks_storage_reads()
    {
        ...
        byte[] code = Prepare.SavmCode.Op(Instruction.PUSH0).Op(Instruction.SLOAD).Op(Instruction.POP).Op(Instruction.STOP).Done;
        ...
        Assert.That(log.Savm.Sload, Is.GreaterThan(0));
    }
    ...
    public void SSTORE_tracks_storage_writes_and_deletions()
    {
        ...
        Assert.That(log.Savm.Sstore, Is.GreaterThanOrEqualTo(2));
    }
    ...
    public void Contract_CALL_tracks_code_reads()
    {
        ...
        _harness.DeployCode(callee, Prepare.SavmCode.Op(Instruction.STOP).Done);
        _harness.DeployCode(caller, Prepare.SavmCode.Call(callee, 50_000).Op(Instruction.STOP).Done);
        ...
        Assert.That(log.Savm.Calls, Is.GreaterThan(0));
    }

    [Test]
    public void SIP7702_delegation_set_tracked()
    {
        PrivateKey sender = TestItem.PrivateKeyA;
        ...
        Assert.That(setLog.StateWrites.Sip7702DelegationsSet, Is.EqualTo(1));
    }

    [Test]
    public void SIP7702_delegation_clear_tracked()
    {
        PrivateKey sender = TestItem.PrivateKeyA;
        PrivateKey signer = TestItem.PrivateKeyB;
        Address codeSource = TestItem.AddressC;
        ...
        Assert.That(clearLog.StateWrites.Sip7702DelegationsCleared, Is.EqualTo(1));
    }
