...
namespace Nethermind.Consensus.Test
{
    public class TargetAdjustedGasLimitCalculatorTests
    {
        ...
        [Test]
        public void Is_bump_on_1559_sip_block()
        {
            const int londonBlock = 5;
            const long parentGasLimit = 1000000000000000000;
            OverridableReleaseSpec spec = new(London.Instance) { Sip1559TransitionBlock = londonBlock };

            Assert.That(
                Calc(spec, parentGasLimit, blockNumber: londonBlock),
                Is.EqualTo(parentGasLimit * Sip1559Constants.DefaultElasticityMultiplier));
        }
        ...
        public void Is_calculating_correct_gasLimit(long parentGasLimit, long configTarget, long expected)
            => Assert.That(Calc(SilaPrague.Instance, parentGasLimit, configTarget: configTarget), Is.EqualTo(expected));
        ...
        public void Override_takes_precedence_over_config_target(long parentGasLimit, long configTarget, long overrideTarget, long expected)
            => Assert.That(Calc(SilaPrague.Instance, parentGasLimit, configTarget, overrideTarget), Is.EqualTo(expected));
        ...
        public void Null_override_falls_back_to_config_target()
        {
            ...
            Assert.That(
                Calc(SilaPrague.Instance, parentGasLimit, configTarget, overrideTarget: null),
                Is.EqualTo(Calc(SilaPrague.Instance, parentGasLimit, configTarget)));
        }
