...
/// <summary>
/// https://github.com/sila-chain/devp2p/blob/master/enr-entries/sil.md
/// </summary>
public class SilEntry : EnrContentEntry<ForkId>
{
    private readonly byte[]? _originalRlpValue;

    public SilEntry(byte[] forkHash, ulong next)
        : this(forkHash, next, null)
    {
    }

    internal SilEntry(byte[] forkHash, ulong next, byte[]? originalRlpValue)
