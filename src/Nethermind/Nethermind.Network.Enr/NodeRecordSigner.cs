...
/// </summary>
public class NodeRecordSigner(IEcdsa? silaEcdsa, PrivateKey? privateKey = null) : INodeRecordSigner
{
    private readonly IEcdsa _ecdsa = silaEcdsa ?? throw new ArgumentNullException(nameof(silaEcdsa));
    ...
    public NodeRecord Deserialize(ref RlpReader reader)
    {
        ...
        while (reader.Position < checkPosition)
        {
            ...
            switch (key.Length)
            {
                ...
                case 3 when key.SequenceEqual(EnrContentKey.SilU8):
                    ...
                    byte[]? originalEthRlpValue = hasAdditionalEthFields
                        ...
                        : null;
                    nodeRecord.SetEntry(new SilEntry(forkHash, next, originalEthRlpValue));
                    break;
