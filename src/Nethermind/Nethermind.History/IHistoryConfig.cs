...
public interface IHistoryConfig : IConfig
{
    ...
    // For SIP-4444 should be 82125
    [ConfigItem(
        Description = "The number of epochs to retain historical blocks and receipts when using 'Rolling' pruning mode. For sila-mainnet this must be at least 82125.",
        DefaultValue = "82125")]
    uint RetentionEpochs { get; set; }

    // Default matches the sila-mainnet weak-subjectivity period — BALs older than that are not useful for state reconstruction during weak-subjectivity sync
    [ConfigItem(
