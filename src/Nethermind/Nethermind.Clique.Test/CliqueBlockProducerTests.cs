...
public class CliqueBlockProducerTests
{
    private class On : IDisposable
    {
        ...
        private readonly SilaEcdsa _silaEcdsa = new(BlockchainIds.SilaSepolia);
        ...
        public On CreateNode(PrivateKey privateKey, bool withGenesisAlreadyProcessed = false)
        {
            ...
            IContainer container = new ContainerBuilder()
                ...
                .AddSingleton<ISigner>(new Signer(BlockchainIds.SilaSepolia, privateKey, LimboLogs.Instance))
            ...
            {
                stateProvider.CreateAccount(TestItem.PrivateKeyD.Address, 100.Sila);
                if (finalSpec.WithdrawalsEnabled)
                {
                    ...
                    stateProvider.InsertCode(Sip7002Constants.WithdrawalRequestPredeployAddress, Sip7002TestConstants.CodeHash, Sip7002TestConstants.Code, testnetSpecProvider.GenesisSpec);
                }
                ...
                if (finalSpec.ConsolidationRequestsEnabled)
                {
                    ...
                    stateProvider.InsertCode(Sip7251Constants.ConsolidationRequestPredeployAddress, Sip7251TestConstants.CodeHash, Sip7251TestConstants.Code, testnetSpecProvider.GenesisSpec);
                }
        ...
        public On AddPendingTransaction(PrivateKey nodeKey)
        {
            ...
            _silaEcdsa.Sign(TestItem.PrivateKeyD, transaction, true);
        ...
        public On AddAllBadTransactions(PrivateKey nodeKey)
        {
            ...
            transaction = new Transaction();
            transaction.Value = 1000000000.Sila;
            transaction.To = TestItem.AddressC;
            transaction.GasLimit = 30000;
            transaction.GasPrice = 20.GWei;
            transaction.Nonce = _currentNonce;
