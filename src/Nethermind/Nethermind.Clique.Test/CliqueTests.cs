...
public class CliqueTests
{
    ...
    public void Setup_chain()
    {
        ...
        _ecdsa = new SilaEcdsa(BlockchainIds.SilaSepolia);
        _snapshotManager = new SnapshotManager(config, db, _blockTree, _ecdsa, LimboLogs.Instance);
        _clique = new CliqueSealer(new Signer(BlockchainIds.SilaSepolia, key, LimboLogs.Instance), config, _snapshotManager, LimboLogs.Instance);
