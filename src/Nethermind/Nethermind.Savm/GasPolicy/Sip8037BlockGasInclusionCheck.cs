...
// SIP-8037 per-tx 2D block-gas inclusion check.
...
public static class Sip8037BlockGasInclusionCheck
{
        ...
        ulong txGas)
    {
        ...
        // SIP-8037: reserve the full gas limit in each dimension (no intrinsic subtraction). Only the
        // regular dimension is bounded by the SIP-7825 per-tx cap; state work can exceed it via the reservoir.
        ulong worstCaseRegular = Math.Min(Sip7825Constants.DefaultTxGasLimitCap, txGas);
    ...
    // tx_regular_gas = tx_gas_used_before_refund - max(0, tx_state_gas) (SIP-8037's SIP-7778 form;
