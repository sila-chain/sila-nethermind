...
public interface IGasPolicy<TSelf> where TSelf : struct, IGasPolicy<TSelf>
{
    ...
    /// <summary>SIP-8037 pre-refund spent gas: <c>txGasLimit - gas_left - state reservoir</c>.</summary>
    /// <remarks>
    ...
    // SIP-8037 state-cost accessors. Pre-SIP-8037 policies return the constant fallback.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    // SIP-8037 state-accounting accessors. Pre-SIP-8037 policies return 0.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static virtual bool ConsumeCreateGas<TSip8037, TOpCreate>(ref TSelf gas, IReleaseSpec spec, ulong initCodeWords)
        where TSip8037 : struct, IFlag
        where TOpCreate : struct, SavmInstructions.IOpCreate
    {
        ulong baseCost = spec.IsSip8038Enabled ? Sip8038Constants.CreateAccess
            : TSip8037.IsActive ? GasCostOf.CreateRegular
            : GasCostOf.Create;
        ulong initCodeWordCost = spec.IsSip3860Enabled ? GasCostOf.InitCodeWord * initCodeWords : 0;
        ulong create2HashCost = typeof(TOpCreate) == typeof(SavmInstructions.OpCreate2) ? GasCostOf.Sha3Word * initCodeWords : 0;
    ...
    static abstract bool UpdateMemoryCost(ref TSelf gas, in UInt256 position, in UInt256 length, ref SavmPooledMemory memory);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static virtual bool UpdateMemoryCost(ref TSelf gas, in UInt256 position, ulong length, ref SavmPooledMemory memory)
    {
    ...
    // Pre-SIP-8037 fallback: state gas folded into regular gas.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    static abstract bool ConsumeStorageWrite<TSip8037, TIsSlotCreation>(ref TSelf gas, IReleaseSpec spec)
        where TSip8037 : struct, IFlag
        where TIsSlotCreation : struct, IFlag;

    // Pre-SIP-8037 fallback: refund into regular gas.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    // SIP-8037 top-level halt: snap state-gas back to (R0, intrinsicStateUsed, 0); the
    ...
    // SIP-7702 code-insert refund regular-gas portion. Pre-SIP-8037: (NewAccount - PerAuthBaseCost) each.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    // SIP-8037: replenishes tx state reservoir before exec (intrinsic state gas already charged).
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    static abstract bool ConsumeCallValueTransferSip2780(ref TSelf gas);
    static abstract bool ConsumeNewAccountCreation<TSip8037>(ref TSelf gas) where TSip8037 : struct, IFlag;
    ...
    where TGasPolicy : struct, IGasPolicy<TGasPolicy>
{
    ...
    /// <summary>
    /// SIP-8037: rejects a transaction whose intrinsic regular or floor gas exceeds <paramref name="cap"/>.
    /// </summary>
