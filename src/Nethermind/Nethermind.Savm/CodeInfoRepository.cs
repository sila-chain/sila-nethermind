...
public class CodeInfoRepository : ICodeInfoRepository
{
        ...
        out byte[] authorizedBuffer)
    {
        ...
        {
            authorizedBuffer = new byte[Sip7702Constants.DelegationHeader.Length + Address.Size];
            Sip7702Constants.DelegationHeader.CopyTo(authorizedBuffer);
            codeSource.Bytes.CopyTo(authorizedBuffer.AsSpan(Sip7702Constants.DelegationHeader.Length));
        ...
        if (result)
        {
            if (codeSource != Address.Zero)
            {
                Metrics.IncrementSip7702DelegationsSet();
            }
            else
            {
                Metrics.IncrementSip7702DelegationsCleared();
            }
        }
