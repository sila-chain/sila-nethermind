...
public static partial class SavmInstructions
{
    ...
    /// <summary>
    /// Executes a shift operation on the SAVM stack using the specified <typeparamref name="TOpShift"/>.
    ...
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionShift<TGasPolicy, TOpShift, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
    ...
    internal static SavmExceptionType ShiftCore<TOpShift, TTracingInst>(ref SavmStack stack)
        where TOpShift : struct, IOpShift
        where TTracingInst : struct, IFlag
    {
    ...
    /// <param name="stack">The SAVM stack used for operands and result storage.</param>
    /// <param name="gas">The gas state which is updated by the operation's cost.</param>
    /// <param name="programCounter">Reference to the program counter (unused in this operation).</param>
    /// <returns>
    /// <see cref="SavmExceptionType.None"/> if successful; otherwise, <see cref="SavmExceptionType.StackUnderflow"/>
    /// if insufficient stack elements are available.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionSar<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
