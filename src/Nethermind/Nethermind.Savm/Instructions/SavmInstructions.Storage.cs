...
/// <summary>
/// Implements various SAVM instruction handlers for transient storage, memory, and persistent storage operations.
/// </summary>
public static partial class SavmInstructions
{
    ...
    /// <param name="stack">The SAVM stack.</param>
    /// <param name="gas">The gas state, updated by the operation's cost.</param>
    /// <param name="programCounter">The program counter.</param>
    /// <returns>An <see cref="SavmExceptionType"/> indicating the result of the operation.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionTLoad<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        SavmExceptionType pushResult = stack.PushBytes<TTracingInst>(value);
    ...
    /// <param name="gas">The gas state, updated by the operation's cost.</param>
    /// <param name="programCounter">The program counter.</param>
    /// <returns>An <see cref="SavmExceptionType"/> indicating success or failure.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionTStore<TGasPolicy>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
    {
    ...
    /// <param name="programCounter">The program counter.</param>
    /// <returns>An <see cref="SavmExceptionType"/> result.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionMStore<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
    ...
    /// <returns>An <see cref="SavmExceptionType"/> result.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionMStore8<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        TGasPolicy.Consume<VeryLowGasCost>(ref gas);
    ...
    /// <returns>An <see cref="SavmExceptionType"/> result.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionMLoad<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        TGasPolicy.Consume<VeryLowGasCost>(ref gas);
        ...
        if (TTracingInst.IsActive)
        {
            vm.TxTracer.ReportMemoryChange(result, MemoryMarshal.CreateReadOnlySpan(ref wordBytes, SavmPooledMemory.WordSize));
        }
    ...
    /// <returns>An <see cref="SavmExceptionType"/> result.</returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionMCopy<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        ...
        ulong words = SavmCalculations.Div32Ceiling(c, out bool outOfGas);
    ...
    /// <param name="gas">The gas state, updated by the operation's cost.</param>
    /// <param name="programCounter">The program counter.</param>
    /// <returns>An <see cref="SavmExceptionType"/> indicating the outcome.</returns>
    [SkipLocalsInit]
    internal static SavmExceptionType InstructionSStoreUnmetered<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
    ...
    /// <returns>An <see cref="SavmExceptionType"/> indicating the outcome.</returns>
    [SkipLocalsInit]
    internal static SavmExceptionType InstructionSStoreMetered<TGasPolicy, TTracingInst, TUseNetGasStipendFix, TSip8037>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
        ...
        where TSip8037 : struct, IFlag
    {
        ...
        else
        {
            ...
            if (currentSameAsOriginal)
            {
                if (currentIsZero)
                {
                    bool ssetOutOfGas = !TGasPolicy.ConsumeStorageWrite<TSip8037, OnFlag>(ref gas, spec);
                    if (ssetOutOfGas) goto OutOfGas;
                }
                else
                {
                    if (!TGasPolicy.ConsumeStorageWrite<TSip8037, OffFlag>(ref gas, spec))
                        goto OutOfGas;
            ...
            else
            {
                ...
                {
                    // SIP-8038: restoring a slot's original value refunds the first-change STORAGE_WRITE;
                    // a freshly-created slot (original 0) additionally refunds its state gas in-frame.
                    long refundFromReversal = spec.IsSip8038Enabled
                        ? (long)Sip8038Constants.StorageWrite
                        : (long)gasCosts.RefundFromReversal(originalIsZero);

                    if (TSip8037.IsActive && originalIsZero)
                    {
                        vm.CreditStateGasRefund(ref gas, TGasPolicy.GetStorageSetStateCost());
                        if (!spec.IsSip8038Enabled)
    ...
    /// <returns>An <see cref="SavmExceptionType"/> indicating the result of the operation.</returns>
    [SkipLocalsInit]
    internal static SavmExceptionType InstructionSLoad<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        IReleaseSpec spec = vm.Spec;
    ...
    /// </summary>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionCallDataLoad<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        TGasPolicy.Consume<VeryLowGasCost>(ref gas);
