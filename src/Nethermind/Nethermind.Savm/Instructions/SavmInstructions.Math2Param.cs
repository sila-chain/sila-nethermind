...
public static partial class SavmInstructions
{
    ...
    /// otherwise, <see cref="SavmExceptionType.StackUnderflow"/> if insufficient stack elements are available.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionMath2Param<TGasPolicy, TOpMath, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
    ...
    internal static SavmExceptionType Math2ParamCore<TOpMath, TTracingInst>(ref SavmStack stack)
        where TOpMath : struct, IOpMath2Param
        where TTracingInst : struct, IFlag
    {
    ...
    /// If the divisor is zero, returns zero per SAVM semantics.
    /// </summary>
    ...
    /// <returns>
    /// <see cref="SavmExceptionType.None"/> on success; or <see cref="SavmExceptionType.StackUnderflow"/> if not enough items on stack.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionExp<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm, ref SavmStack stack, ref TGasPolicy gas, ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
