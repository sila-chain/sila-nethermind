...
public static partial class SavmInstructions
{
    ...
    /// <param name="stack">The SAVM stack for retrieving call parameters and pushing results.</param>
    ...
    /// <returns>
    /// An <see cref="SavmExceptionType"/> value indicating success or the type of error encountered.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionCall<TGasPolicy, TOpCall, TTracingInst, TSip8037, TSip7708>(VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ...
        where TSip7708 : struct, IFlag
    {
        ...
        if (hasValueTransfer)
        {
            // SIP-2780 charges a flat value-move cost with no state read: the spec performs the
            // static gas check before any target access, so an OOG here must not touch the BAL.
            bool valueOutOfGas = spec.IsSip2780Enabled
                ? !TGasPolicy.ConsumeCallValueTransferSip2780(ref gas)
        ...
        // SIP-8038 charges a value transfer to a dead recipient the NEW_ACCOUNT state cost, separate
        // from the flat CALL_VALUE above; standalone SIP-2780 adds nothing extra here.
        bool chargesNewAccount = spec.IsSip8038Enabled
            ? hasValueTransfer && state.IsDeadAccount(target)
            : !spec.IsSip2780Enabled && (spec.ClearEmptyAccountWhenTouched switch
            {
                false => !state.AccountExists(target),
                true => hasValueTransfer && state.IsDeadAccount(target),
            });

        bool newAccountOutOfGas = chargesNewAccount && !TGasPolicy.ConsumeNewAccountCreation<TSip8037>(ref gas);

        if (newAccountOutOfGas) goto OutOfGas;

        // SIP-7702: load delegated code after cold-access charge above.
        if (delegated is not null)
        {
            // SIP-7928: decorator fast-path skips world-state reads; record explicitly.
            state.AddAccountRead(delegated);

            // SIP-7702: precompile MUST NOT execute via delegation; the decorator would route to the precompile CodeInfo.
            codeInfo = spec.IsPrecompile(delegated)
                ? CodeInfo.Empty
                : vm.CodeInfoRepository.GetCachedCodeInfoNoDelegation(delegated, spec);
        }

        // SIP-150: forward the requested gas to the child frame, capped at 63/64 of remaining.
            ...
            (hasValueTransfer && state.GetBalance(env.ExecutingAccount) < callValue))
        {
            ...
            SavmExceptionType pushResult = stack.PushZero<TTracingInst>();
            ...
            if (TTracingInst.IsActive)
            {
                ...
                vm.TxTracer.ReportOperationError(SavmExceptionType.NotEnoughBalance);
            }

            // Refund the remaining gas to the caller.
            TGasPolicy.UpdateGasUp(ref gas, gasLimitUl);
            // SIP-8037: no account is created when the call cannot proceed (depth exceeded or
        ...
        if (codeInfo.IsEmpty && !TTracingInst.IsActive && !vm.TxTracer.IsTracingActions)
        {
            ...
            SavmExceptionType pushResult = stack.PushBytes<TTracingInst>(StatusCode.SuccessBytes.Span);
            if (pushResult != SavmExceptionType.None) return pushResult;
            TGasPolicy.UpdateGasUp(ref gas, gasLimitUl);
            ...
            if (!isSelfCall)
            {
                if (hasValueTransfer)
                {
                    ...
                    vm.AddTransferLog<TSip7708>(caller, target, in callValue);
                }
        ...
        if (TOpCall.ExecutionType == ExecutionType.STATICCALL && codeInfo.IsPrecompile &&
            TryInlineStaticPrecompileCall<TGasPolicy, TTracingInst>(
                ...
                codeInfo.Precompile!, target, codeSource, gasLimitUl, out SavmExceptionType inlineResult))
        {
            return inlineResult;
        }
...
#endif
        static SavmExceptionType CreateFullCallFrame(
    ...
    /// </summary>
    /// <remarks>
    /// The implementation is build-specific: mainline runs the precompile directly, while the zkSAVM guest
    ...
    /// <param name="stack">The SAVM stack from which the offset and length are popped.</param>
    /// <param name="gas">The gas which is updated by the operation's cost.</param>
    /// <param name="programCounter">Reference to the program counter (unused in this operation).</param>
    /// <returns>
    /// <see cref="SavmExceptionType.None"/> on success; otherwise, an error such as <see cref="SavmExceptionType.StackUnderflow"/>,
    /// <see cref="SavmExceptionType.OutOfGas"/>, or <see cref="SavmExceptionType.BadInstruction"/>.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionReturn<TGasPolicy>(VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
