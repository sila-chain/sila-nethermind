...
public static partial class SavmInstructions
{
    ...
    private static SavmExceptionType DataCopy<TGasPolicy, TTracingInst>(
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        ulong words = SavmCalculations.Div32Ceiling(in result, out bool outOfGas);
    ...
    /// </summary>
    public static SavmExceptionType InstructionCodeCopy<TGasPolicy, TTracingInst>(
        VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
    ...
    /// </summary>
    public static SavmExceptionType InstructionCallDataCopy<TGasPolicy, TTracingInst>(
        VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ref int programCounter)
    ...
    /// </summary>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionReturnDataCopy<TGasPolicy, TTracingInst>(
        VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ref int programCounter)
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        ulong words = SavmCalculations.Div32Ceiling(in size, out bool outOfGas);
        TGasPolicy.ConsumeDataCopyGas(ref gas, vm.Spec, isExternalCode: false, words);
        if (outOfGas) goto OutOfGas;
    ...
    StackUnderflow:
        return SavmExceptionType.StackUnderflow;
    AccessViolation:
        return SavmExceptionType.AccessViolation;
    }
    ...
    /// <param name="stack">The SAVM stack for operand retrieval and memory copy operations.</param>
    ...
    /// <returns>
    /// <see cref="SavmExceptionType.None"/> on success, or an appropriate error code on failure.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionExtCodeCopy<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        ...
        // SIP-8038 charges an extra warm access for the second DB read EXTCODECOPY performs.
        if (spec.IsSip8038Enabled && !TGasPolicy.UpdateGas(ref gas, Sip8038Constants.WarmAccess))
            goto OutOfGas;
    ...
    /// <param name="stack">The SAVM stack from which the account address is popped and where the code size is pushed.</param>
    /// <param name="gas">The gas which is updated by the operation's cost.</param>
    /// <param name="programCounter">Reference to the program counter, which may be adjusted during optimization.</param>
    /// <returns>
    /// <see cref="SavmExceptionType.None"/> on success, or an appropriate error code if an error occurs.
    /// </returns>
    [SkipLocalsInit]
    public static SavmExceptionType InstructionExtCodeSize<TGasPolicy, TTracingInst>(VirtualMachine<TGasPolicy> vm,
        ref SavmStack stack,
        ref TGasPolicy gas,
        ref int programCounter)
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
        where TTracingInst : struct, IFlag
    {
        ...
        // SIP-8038 charges an extra warm access for the second DB read EXTCODESIZE performs.
        if (spec.IsSip8038Enabled && !TGasPolicy.UpdateGas(ref gas, Sip8038Constants.WarmAccess))
            goto OutOfGas;
