...
{
    /// <summary>Whether account code may be overridden (e.g. <c>sil_call</c> state overrides), disabling the simple-transfer fast path.</summary>
    ...
    {
        if (Sip7702Constants.IsDelegatedCode(code))
        {
            address = new Address(code[Sip7702Constants.DelegationHeader.Length..]);
            return true;
        }
...
public static class CodeInfoRepositoryExtensions
{
    ...
    /// <summary>
    /// Returns the <see cref="CodeInfo"/> at <paramref name="codeSource"/> without resolving any SIP-7702 delegation.
    /// </summary>
