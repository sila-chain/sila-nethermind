...
/// (writes, code-read diagnostics, SIP-7702 delegation set/clear, block timing breakdown).
/// </summary>
/// <remarks>
...
public class Metrics
{
    ...
    [CounterMetric]
    [Description("Number of SAVM exceptions thrown by contracts.")]
    public static long SavmExceptions { get; set; }
    ...
    [CounterMetric]
    [Description("Number of SIP-7702 delegations set during execution.")]
    public static long Sip7702DelegationsSet => _mainSip7702DelegationsSet.Value + _otherSip7702DelegationsSet.Value;
    private static CacheLinePaddedLong _mainSip7702DelegationsSet;
    private static CacheLinePaddedLong _otherSip7702DelegationsSet;
    internal static long MainThreadSip7702DelegationsSet => _mainSip7702DelegationsSet.Value;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static void IncrementSip7702DelegationsSet()
    {
        if (!ExecutionMetricsFlag.IsActive) return;
        Interlocked.Increment(ref IsBlockProcessingThread ? ref _mainSip7702DelegationsSet.Value : ref _otherSip7702DelegationsSet.Value);
    }

    [CounterMetric]
    [Description("Number of SIP-7702 delegations cleared during execution.")]
    public static long Sip7702DelegationsCleared => _mainSip7702DelegationsCleared.Value + _otherSip7702DelegationsCleared.Value;
    private static CacheLinePaddedLong _mainSip7702DelegationsCleared;
    private static CacheLinePaddedLong _otherSip7702DelegationsCleared;
    internal static long MainThreadSip7702DelegationsCleared => _mainSip7702DelegationsCleared.Value;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static void IncrementSip7702DelegationsCleared()
    {
        if (!ExecutionMetricsFlag.IsActive) return;
        Interlocked.Increment(ref IsBlockProcessingThread ? ref _mainSip7702DelegationsCleared.Value : ref _otherSip7702DelegationsCleared.Value);
    }
    ...
    /// <remarks>
    /// Prices >= <see cref="ulong.MaxValue"/> wei/gas (~18.4 SIL) are not meaningful and are skipped,
    ...
    /// system-only block). Skips zero base fee (pre-SIP-1559, genesis) - "0.000" is less useful than blank.
    /// </summary>
