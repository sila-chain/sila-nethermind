...
namespace Nethermind.Savm.TransactionProcessing
{
    ...
    /// <summary>
    /// Non-generic TransactionProcessor for backward compatibility with SilaGasPolicy.
    /// </summary>
    public sealed class SilaTransactionProcessor(
        ITransactionProcessor.IBlobBaseFeeCalculator blobBaseFeeCalculator,
        ISpecProvider? specProvider,
        IWorldState? worldState,
    ...
    public abstract class TransactionProcessorBase
    {
        ...
        private protected static void DestroyAccount(IWorldState worldState, Address toBeDestroyed, in UInt256 balance, bool commit, bool removeSelfdestructBurn)
        {
            ...
            // SIP-8246: preserve any remaining balance as a fresh nonce-0, code-less account;
            // an empty account stays deleted via SIP-161.
        ...
        where TGasPolicy : struct, IGasPolicy<TGasPolicy>
    {
            ...
            bool parallel = false)
        {
            ...
            Ecdsa = new SilaEcdsa(specProvider.ChainId);
        ...
        private TransactionResult Execute(Transaction tx, ITxTracer tracer, ExecutionOptions opts, BlockHeader header, IReleaseSpec spec, in IntrinsicGas<TGasPolicy> intrinsicGas)
        {
            ...
            bool commit = opts.HasFlag(ExecutionOptions.Commit) || (!opts.HasFlag(ExecutionOptions.SkipValidation) && !spec.IsSip658Enabled);

            TransactionResult result;
            if (!(result = ValidateStatic(tx, header, spec, opts, in intrinsicGas))) return result;

            UInt256 effectiveGasPrice = CalculateEffectiveGasPrice(tx, spec.IsSip1559Enabled, header.BaseFeePerGas, out UInt256 opcodeGasPrice);
            ...
            return useSimpleTransferFastPath
                ...
                : ExecuteSavmTransaction(tx, header, spec, tracer, opts, restore, commit, deleteCallerAccount, in intrinsicGas, gasAvailable, in opcodeGasPrice, in premiumPerGas, in senderReservedGasPayment, in blobBaseFee, preloadedCodeInfo, preloadedDelegationAddress);
        }
        ...
        private TransactionResult ExecuteSavmTransaction(
            Transaction tx,
            BlockHeader header,
            IReleaseSpec spec,
            ITxTracer tracer,
            ...
            Address? preloadedDelegationAddress)
        {
            ...
            // Top-level CREATE tx; the opcode-level CREATE/CREATE2 path bumps this counter from SavmInstructions.Create.
            ...
            if (spec.IsSip7702Enabled && tx.HasAuthorizationList)
            {
            ...
            // SIP-8037 top-frame charges. At most one applies: a delegated recipient has code and is
            // therefore never a dead account, hence the if/else-if.
            bool recipientIsDelegated = spec.IsSip7702Enabled && tx.To is not null
                && _codeInfoRepository.TryGetDelegation(tx.To, spec, out _);

            // The flag defers the halt to ExecuteSavmCall so the value transfer rolls back and the
            ...
            if (spec.IsSip8037Enabled && !tx.IsContractCreation && !tx.ValueRef.IsZero
            ...
            // SIP-8037: the delegation target costs one flat cold account access (its sole charge —
            // it is only pre-warmed for the frame). Read post-authorization, so same-tx installs count.
            else if (spec.IsSip8037Enabled && !tx.IsContractCreation && recipientIsDelegated)
            {
                ulong delegationCold = spec.IsSip8038Enabled ? Sip8038Constants.ColdAccountAccess : GasCostOf.ColdAccountAccess;
                topFrameOutOfGas = !TGasPolicy.UpdateGas(ref gasAvailable, delegationCold);
            }

            int statusCode = !tracer.IsTracingInstructions ?
                ExecuteSavmCall<OffFlag>(tx, header, spec, tracer, opts, delegationRefunds, intrinsicGas, accessTracker, gasAvailable, env, topFrameOutOfGas, out TransactionSubstate substate, out GasConsumed spentGas) :
                ExecuteSavmCall<OnFlag>(tx, header, spec, tracer, opts, delegationRefunds, intrinsicGas, accessTracker, gasAvailable, env, topFrameOutOfGas, out substate, out spentGas);

            UpdateHeaderGasUsedAndPayFees(tx, header, spec, tracer, opts, in substate, in spentGas, premiumPerGas, blobBaseFee, statusCode);

            // SIP-8037+SIP-7708: process destroy list after PayFees so burn logs include
            // the priority fee in the destroyed account's balance.
            if (spec.IsSip8037Enabled && spec.IsSip7708Enabled && statusCode == StatusCode.Success)
            {
                JournalSet<Address>? destroyList = substate.DestroyList;
                if (destroyList is not null)
                {
                    int count = destroyList.Count;
                    bool removeSelfdestructBurn = spec.IsSip8246Enabled;
                ...
                static void FinalizeDestroyedAccount(IWorldState worldState, in TransactionSubstate substate, Address toBeDestroyed, bool commit, bool removeSelfdestructBurn)
                {
                    ...
                    // Burn (not SelfDestruct) log; SIP-8246 removes the burn and its log entirely.
            ...
            in UInt256 blobBaseFee)
        {
            ...
            // SIP-8037: a value transfer materialising a new (dead) recipient — including an empty
            // precompile — pays NEW_ACCOUNT state gas; if uncovered, no value moves and all gas is forfeit.
            bool newAccountOutOfGas = false;
            if (spec.IsSip8037Enabled && hasValueTransfer && !senderIsRecipient
            ...
            if (spec.IsSip7708Enabled && hasValueTransfer && !senderIsRecipient && !newAccountOutOfGas)
            {
        ...
        private void WarmUpTxAccesses(Transaction tx, IReleaseSpec spec, in StackAccessTracker accessTracker, Address recipient)
        {
            ...
            if (spec.UseTxAccessLists)
                accessTracker.WarmUp(tx.AccessList); // sip-2930
            ...
            int statusCode)
        {
            ...
            {
                WorldState.Commit(spec, tracer.IsTracingState ? tracer : NullStateTracer.Instance, commitRoots: !spec.IsSip658Enabled);
            }
            else
            {
            ...
            return substate.SavmExceptionType != SavmExceptionType.None
                ? TransactionResult.SavmException(substate.SavmExceptionType, substate.SubstateError)
        ...
        {
            if (spec.IsSip8037Enabled && (delegationRefunds > 0 || delegationAuthBaseRefunds > 0))
            {
        ...
        {
            if (!spec.IsSip8037Enabled || !tx.HasAuthorizationList)
            {
                return TransactionResult.Ok;
            }
        ...
        {
            Debug.Assert(spec.IsSip7702Enabled && tx.HasAuthorizationList);
            ...
            [NotNullWhen(false)] out string? error)
        {
            ...
            if (authorizationTuple.Authority is null
                ...
                //V minus the offset can only be 1 or 0 since sip-155 does not apply to Setcode signatures
            ...
            in IntrinsicGas<TGasPolicy> intrinsicGas)
        {
            ...
            if (spec.IsSip8037Enabled && intrinsicGas.ExceedsCap(Sip7825Constants.DefaultTxGasLimitCap, out ulong regular, out ulong floor))
            {
                TraceLogInvalidTx(tx, $"TX_INTRINSIC_GAS_EXCEEDS_CAP regular={regular} floor={floor} > {Sip7825Constants.DefaultTxGasLimitCap}");
                return TransactionResult.ErrorType.GasLimitBelowIntrinsicGas.WithDetail(
                    TxErrorMessages.TxIntrinsicGasExceedsCap(regular, floor, Sip7825Constants.DefaultTxGasLimitCap));
            }
            ...
            ulong minGasRequired = spec.IsSip8037Enabled
        ...
        protected virtual TransactionResult ValidateGas(Transaction tx, BlockHeader header, IReleaseSpec spec, in TGasPolicy intrinsicGas, ulong minGasRequired, bool validate)
        {
            ...
            if (validate)
            {
                if (spec.IsSip8037Enabled)
                {
                    ...
                    // Per-block SIP-8037 inclusion depends on cumulative regular/state gas,
                ...
                // Admission must use the same basis as block accounting (header.GasUsed): pre-refund under SIP-7778, post-refund otherwise.
        ...
        protected virtual bool RecoverSenderIfNeeded(Transaction tx, IReleaseSpec spec, ExecutionOptions opts, in UInt256 effectiveGasPrice)
        {
            ...
            {
                bool commit = opts.HasFlag(ExecutionOptions.Commit) || !spec.IsSip658Enabled;
                bool restore = opts.HasFlag(ExecutionOptions.Restore);
        ...
        protected virtual UInt256 CalculateEffectiveGasPrice(Transaction tx, bool sip1559Enabled, in UInt256 baseFee, out UInt256 opcodeGasPrice)
        {
            opcodeGasPrice = tx.CalculateEffectiveGasPrice(sip1559Enabled, in baseFee);
                ...
                in UInt256 effectiveGasPrice, out UInt256 premiumPerGas, out UInt256 senderReservedGasPayment, out UInt256 blobBaseFee)
        {
            ...
            if (validate && !TryCalculatePremiumPerGas(tx, header.BaseFeePerGas, out premiumPerGas))
            {
                ...
                string errorDetail = $"max fee per gas less than block base fee: address {tx.SenderAddress?.ToString(withSip55Checksum: true) ?? "unknown"}, maxFeePerGas: {tx.MaxFeePerGas}, baseFee: {header.BaseFeePerGas}";
            ...
            // balanceCheck = gasLimit * MaxFeePerGas (SIP-1559) or mgval — the maximum the sender must hold.
            UInt256 balanceCheck;
            if (spec.IsSip1559Enabled && !tx.IsFree())
            {
        ...
        private static TransactionResult RequiredBalanceExceeds256Bits(Transaction tx) =>
            TransactionResult.ErrorType.InsufficientMaxFeePerGasForSenderBalance.WithDetail(
                $"{TxErrorMessages.InsufficientFundsForGas}: address {tx.SenderAddress?.ToString(withSip55Checksum: true)} required balance exceeds 256 bits");

        private static TransactionResult InsufficientFundsForGas(Transaction tx, UInt256 senderBalance, UInt256 want) =>
            TransactionResult.ErrorType.InsufficientMaxFeePerGasForSenderBalance.WithDetail(
                $"{TxErrorMessages.InsufficientFundsForGas}: address {tx.SenderAddress?.ToString(withSip55Checksum: true)} have {senderBalance} want {want}");
        ...
        protected virtual TransactionResult IncrementNonce(Transaction tx, BlockHeader header, IReleaseSpec spec, ITxTracer tracer, ExecutionOptions opts)
        {
            ...
            if (validate && tx.Nonce != nonce)
            {
                ...
                string sender = tx.SenderAddress?.ToString(withSip55Checksum: true) ?? "unknown";
            ...
            out ExecutionEnvironment env)
        {
            ...
            else
            {
                ...
                //We assume sip-7702 must be active if it is a delegation
        ...
        private int ExecuteSavmCall<TTracingInst>(
            Transaction tx,
            BlockHeader header,
            IReleaseSpec spec,
            ITxTracer tracer,
            ...
            where TTracingInst : struct, IFlag
        {
            ...
            // SIP-7702 + SIP-8037: capture the tx-start state reservoir after authorization refunds.
            ...
            // SIP-8037: a successful create to a pre-existing (alive) account refunds the intrinsic
            ...
            if (topFrameOutOfGas)
            {
                ...
                gasConsumed = CompleteSip8037Halt(tx, spec, opts, ref gasAvailable, VirtualMachine.TxExecutionContext.GasPrice, in oogIntrinsicGasStandard, floorGasLong, postIntrinsicStateReservoir);
                goto Complete;
            }
            ...
            using (VmState<TGasPolicy> state = VmState<TGasPolicy>.RentTopLevel(gasAvailable, executionType, env, in accessedItems, in snapshot))
            {
                ...
                else
                {
                    ...
                    // SIP-8037: defer destroy list processing to after PayFees so that
                    // burn logs include the priority fee in the balance.
                    bool deferFinalization = spec.IsSip7708Enabled && spec.IsSip8037Enabled;
                    JournalSet<Address>? destroyList = substate.DestroyList;
                    if (!deferFinalization && destroyList?.Count > 0)
                    {
                        // Same derivation as Execute: !commit = build-up round spanning the block.
                        bool commit = opts.HasFlag(ExecutionOptions.Commit) || (!opts.HasFlag(ExecutionOptions.SkipValidation) && !spec.IsSip658Enabled);
                        bool sip7708Enabled = spec.IsSip7708Enabled;
                        bool removeSelfdestructBurn = spec.IsSip8246Enabled;
                        bool tracingRefunds = tracer.IsTracingRefunds;
                        foreach (Address toBeDestroyed in destroyList)
                        {
                            ...
                            UInt256 balance = sip7708Enabled || removeSelfdestructBurn ? WorldState.GetBalance(toBeDestroyed) : default;

                            // SIP-7708 logs the burn; suppressed once SIP-8246 stops burning.
                            if (sip7708Enabled && !removeSelfdestructBurn && !balance.IsZero)
                            {
        ...
        FailContractCreate:
            ...
            if (spec.IsSip8037Enabled)
            {
                // Use postIntrinsicStateReservoir captured before SAVM execution so any
                // SIP-7702 auth refund applied via Apply8037DelegationRefunds is preserved
                // (otherwise the sender pays AccountCreationCost per refunded auth even
                // though no account was created).
                gasConsumed = CompleteSip8037Halt(tx, spec, opts, ref gasAvailable, VirtualMachine.TxExecutionContext.GasPrice, in intrinsicGasStandard, floorGasLong, postIntrinsicStateReservoir);
            }
            else
            {
        ...
        // Common SIP-8037 halt-prepare-then-restore sequence shared by FailContractCreate
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private GasConsumed CompleteSip8037Halt(
            Transaction tx,
            IReleaseSpec spec,
            ExecutionOptions opts,
            ref TGasPolicy gas,
            ...
            ulong floorGas = 0)
        {
            ...
            Debug.Assert(blockStateGas >= 0, $"SIP-8037 fail-path invariant violated: negative block state gas ({blockStateGas}).");
            ulong blockGas = Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas, (ulong)blockStateGas);

            return RefundFailedSip8037Gas(tx, spec, opts, in gasPrice, spentGas, blockGas, blockStateGas);
        }
            ...
            ulong floorGas)
        {
            ...
            return RefundFailedSip8037Gas(tx, spec, opts, in gasPrice, spentGas, spentGas, blockStateGas);
        }
            ...
            ulong codeInsertRegularRefund = 0)
        {
            ...
            Debug.Assert(stateReservoir >= 0 && (ulong)stateReservoir <= tx.GasLimit,
                $"SIP-8037 halt-path invariant violated: reservoir ({stateReservoir}) exceeds gasLimit ({tx.GasLimit}).");
            ...
            // The regular gas refund (e.g. SIP-7702 ACCOUNT_WRITE) survives a halt: the spec adds it to
            ...
            Debug.Assert(tx.IsSystem() || (ulong)effectiveStateGas <= preRefundGas,
                $"SIP-8037 halt-path invariant violated: state gas ({effectiveStateGas}) exceeds pre-refund gas ({preRefundGas}).");
            ...
            return RefundFailedSip8037Gas(tx, spec, opts, in gasPrice, spentGas, blockGas, effectiveStateGas);
        }

        private GasConsumed RefundFailedSip8037Gas(
            Transaction tx,
            IReleaseSpec spec,
            ExecutionOptions opts,
            in UInt256 gasPrice,
        ...
        protected virtual void PayFees(Transaction tx, BlockHeader header, IReleaseSpec spec, ITxTracer tracer, in TransactionSubstate substate, ulong spentGas, in UInt256 premiumPerGas, in UInt256 blobBaseFee, int statusCode)
        {
            ...
            // SIP-8037: always pay coinbase — deferred finalization will burn the balance
            bool gasBeneficiaryNotDestroyed = !substate.DestroyListContains(header.GasBeneficiary);
            if (statusCode == StatusCode.Failure || gasBeneficiaryNotDestroyed || spec.IsSip8037Enabled)
            {
                WorldState.AddToBalanceAndCreateIfNotExists(header.GasBeneficiary!, fees, spec);
            }

            UInt256 sip1559Fees = !tx.IsFree() ? header.BaseFeePerGas * spentGas : UInt256.Zero;
            UInt256 collectedFees = spec.IsSip1559Enabled ? sip1559Fees : UInt256.Zero;

            if (tx.SupportsBlobs && spec.IsSip4844FeeCollectorEnabled)
            {
            ...
            {
                tracer.ReportFees(fees, sip1559Fees + blobBaseFee);
            }
        }
            ...
            in TransactionSubstate substate, in TGasPolicy unspentGas, in UInt256 gasPrice, ulong codeInsertRefunds, in TGasPolicy floorGas, in TGasPolicy intrinsicGasStandard, long postIntrinsicStateReservoir, bool createdTargetAlive = false)
        {
            ...
            long stateGasFloor = TGasPolicy.GetStateReservoir(in intrinsicGasStandard);
            // SIP-8037: refund the top-level create's NEW_ACCOUNT state gas on revert or when the target
            // already existed; exceptional halts refund it via CompleteSip8037Halt instead.
            if (spec.IsSip8037Enabled && (substate.ShouldRevert || (!substate.IsError && createdTargetAlive)))
            {
                long refundedTopLevelCreateStateGas = CalculateTopLevelCreateIntrinsicStateRefund(tx, in intrinsicGasStandard);
                if (refundedTopLevelCreateStateGas > 0)
                {
            ...
            if (substate.IsError && spec.IsSip8037Enabled)
            {
                // Use postIntrinsicStateReservoir captured before SAVM execution so any
                // SIP-7702 auth refund applied via Apply8037DelegationRefunds is preserved
                // through the halt-reset.
                return CompleteSip8037Halt(tx, spec, opts, ref gasAfterExecution, in gasPrice, in intrinsicGasStandard, floorGasLong, postIntrinsicStateReservoir, codeInsertRegularRefund);
            }
        ...
        {
            long intrinsicStateGas = TGasPolicy.GetStateReservoir(in intrinsicGasStandard);
            return Math.Max(0, txGasLimit - intrinsicStateGas - (long)Sip7825Constants.DefaultTxGasLimitCap);
        }
            ...
            ulong codeInsertRegularRefund)
        {
            ...
            long quotient = spec.IsSip3529Enabled ? (long)RefundHelper.MaxRefundQuotientSIP3529 : (long)RefundHelper.MaxRefundQuotient;
        ...
        {
            if (!spec.IsSip8037Enabled)
                return (spec.IsSip7778Enabled ? Math.Max(preRefundGas, floorGas) : 0, 0);

            long blockStateGas = TGasPolicy.GetStateGasUsed(in gasAfterExecution);
            Debug.Assert(blockStateGas >= 0, $"SIP-8037 invariant violated: negative block state gas ({blockStateGas}).");
            ulong blockGas = Sip8037BlockGasInclusionCheck.CalculateBlockRegularGas(preRefundGas, (ulong)blockStateGas);
        ...
        // Devirtualised wrapper over Address.CompareTo (sealed -> already devirt'd inside) so the SIP-7708
    ...
    /// <summary>
    /// Non-generic TransactionProcessorBase for backward compatibility with SilaGasPolicy.
    /// </summary>
    public abstract class SilaTransactionProcessorBase(
        ITransactionProcessor.IBlobBaseFeeCalculator? blobBaseFeeCalculator,
        ISpecProvider? specProvider,
        IWorldState? worldState,
        IVirtualMachine? virtualMachine,
        ...
        ILogManager? logManager,
        bool parallel = false)
        : TransactionProcessorBase<SilaGasPolicy>(blobBaseFeeCalculator, specProvider, worldState, virtualMachine, codeInfoRepository, logManager, parallel);

    public readonly struct TransactionResult : IEquatable<TransactionResult>
    {
        private TransactionResult(ErrorType error = ErrorType.None, SavmExceptionType savmException = SavmExceptionType.None, string errorDescription = "")
        {
            Error = error;
            SavmExceptionType = savmException;
            ErrorDescription = errorDescription;
        }

        public ErrorType Error { get; }
        public bool TransactionExecuted => Error is ErrorType.None;
        public SavmExceptionType SavmExceptionType { get; }
        ...
        public static TransactionResult SavmException(SavmExceptionType savmExceptionType, string? description = null) =>
            new(savmException: savmExceptionType, errorDescription: description ?? "");
        ...
        public static readonly TransactionResult TransactionSizeOverMaxInitCodeSize = new(ErrorType.TransactionSizeOverMaxInitCodeSize, errorDescription: "SIP-3860 - transaction size over max init code size");
    ...
    public static class TransactionResultExtensions
    {
        ...
        /// tracer-supplied error. Used by <c>sil_call</c> and <c>proof_call</c> so both report the same
        ...
        {
            { TransactionExecuted: true } when txResult.SavmExceptionType is not (SavmExceptionType.None or SavmExceptionType.Revert)
                => txResult.ErrorDescription is { Length: > 0 } d ? d : txResult.SavmExceptionType.GetSavmExceptionDescription(),
