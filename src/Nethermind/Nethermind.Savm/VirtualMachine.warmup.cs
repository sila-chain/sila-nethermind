...
{
    public static void WarmUpSavmInstructions(IWorldState state, ICodeInfoRepository codeInfoRepository)
    {
        ...
        state.CreateAccount(addressOne, 1000.Sila);
        state.Commit(spec);
    ...
    private static void RunTransactions(TransactionProcessor<TGasPolicy> processor, IWorldState state, IReleaseSpec spec)
    {
        ...
        state.CreateAccountIfNotExists(recipient, 100.Sila);
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[] opcodes = vm.GenerateOpCodes<TTracingInst>(spec);
        ITxTracer txTracer = new FeesTracer();
        vm._txTracer = txTracer;
        vmState.InitializeStacks(txTracer, vmState.Env.CodeInfo.CodeSpan, out SavmStack stack);
