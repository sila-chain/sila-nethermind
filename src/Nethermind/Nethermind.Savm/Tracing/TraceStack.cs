...
public readonly struct TraceStack(ReadOnlyMemory<byte> stack)
{
    ...
    {
        get => _stack.Slice(SavmStack.WordSize * index, SavmStack.WordSize);
    }

    public int Count => _stack.Length / SavmStack.WordSize;
    ...
    /// Returns an empty array for an empty stack. The SAVM reuses its internal buffer across opcodes, so a copy is required.</summary>
