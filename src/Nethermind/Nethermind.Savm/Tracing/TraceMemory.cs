...
public readonly struct TraceMemory(ulong size, ReadOnlyMemory<byte> memory)
{
    ...
    {
        string[] memory = new string[(int)Size / SavmPooledMemory.WordSize + (Size % SavmPooledMemory.WordSize == 0 ? 0 : 1)];
        int traceLocation = 0;

        int i = 0;
        while ((ulong)traceLocation < Size)
        {
            int sizeAvailable = Math.Min(SavmPooledMemory.WordSize, _memory.Length - traceLocation);
            if (sizeAvailable > 0)
            {
            ...
            traceLocation += SavmPooledMemory.WordSize;
            i++;
        }
    ...
    /// Returns an empty array for zero-size memory. The SAVM reuses its internal buffer across opcodes, so a copy is required.</summary>
    public byte[] ToRawWordBytes()
    {
        if (Size == 0) return Array.Empty<byte>();
        int wordCount = (int)((Size + SavmPooledMemory.WordSize - 1) / SavmPooledMemory.WordSize);
        byte[] raw = new byte[wordCount * SavmPooledMemory.WordSize];
    ...
    public BigInteger GetUint(int offset) =>
        offset < 0 || (ulong)(offset + SavmPooledMemory.WordSize) > Size
            ? throw new ArgumentOutOfRangeException(nameof(offset), $"tracer accessed out of bound memory: available {Size}, offset {offset}, size {SavmPooledMemory.WordSize}")
            : new BigInteger(Slice(offset, SavmPooledMemory.WordSize), true, true);
}
