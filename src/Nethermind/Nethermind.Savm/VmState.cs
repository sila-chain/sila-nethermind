    ...
    where TGasPolicy : struct, IGasPolicy<TGasPolicy>
{
...
#if ZK_SAVM
        ZkSavmQueue<VmState<TGasPolicy>>
#else
    ...
    /// <summary>
    /// SIP-8037: the parent <c>*CALL</c> charged NEW_ACCOUNT state gas up-front for this (dead)
    ...
    private SavmPooledMemory _memory;
...
#if ZK_SAVM
        // Guest only: the SAVM memory buffer lives on the per-tx scratch arena (reclaimed at reset), so a
    ...
    public ref SavmPooledMemory Memory => ref _memory;
    ...
    public void InitializeStacks(ReadOnlySpan<byte> codeSpan, out SavmStack stack)
    {
    ...
    public void InitializeStacks(ITxTracer txTracer, ReadOnlySpan<byte> codeSpan, out SavmStack stack)
    {
        ObjectDisposedException.ThrowIf(_isDisposed, this);
        byte[] dataStack = DataStack;
        if (dataStack is null)
    ...
    public Memory<byte> MemoryStacks(int count)
    {
        ...
        return AsAligned32Memory(dataStack, size: count * SavmStack.WordSize);
    }
