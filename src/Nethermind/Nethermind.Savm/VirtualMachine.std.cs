...
public unsafe partial class VirtualMachine<TGasPolicy> where TGasPolicy : struct, IGasPolicy<TGasPolicy>
{
    ...
    // next fork). std-only: the zkSAVM guest runs a single fork and caches in plain statics (see .zksavm).
    private sealed unsafe class OpcodeTable
    {
        public delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[]? NoTrace;
        public delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[]? Traced;
    }

    // Weak keys: transient spec wrappers (e.g. the per-block WithoutSip3607 decorator in
    // sil_simulateV1) must not be retained forever by this process-wide cache.
    ...
    private partial void PrepareOpcodes<TTracingInst>(IReleaseSpec spec) where TTracingInst : struct, IFlag
    {
        ...
        if (!TTracingInst.IsActive)
        {
            ...
            if (_txCount < 500_000 && Interlocked.Increment(ref _txCount) % 10_000 == 0)
            {
                if (_logger.IsDebug)
                {
                    _logger.Debug("Refreshing SAVM instruction cache");
                }
    ...
    protected virtual delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[] GenerateOpCodes<TTracingInst>(IReleaseSpec spec) where TTracingInst : struct, IFlag =>
