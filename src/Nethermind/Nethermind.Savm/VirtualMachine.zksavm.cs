...
{
    private delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[] _opcodeMethods;

    // Cache the dispatch tables in plain per-TGasPolicy statics: the guest executes a single fork, and
    // ConditionalWeakTable (used by the std build) relies on GC dependent-handles the zkSAVM guest can't map.
    private static delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[]? _opcodesNoTrace;
    private static delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[]? _opcodesTraced;
    ...
    protected delegate*<VirtualMachine<TGasPolicy>, ref SavmStack, ref TGasPolicy, ref int, SavmExceptionType>[] GenerateOpCodes<TTracingInst>(IReleaseSpec spec) where TTracingInst : struct, IFlag =>
        SavmInstructions.GenerateOpCodes<TGasPolicy, TTracingInst>(spec);
    ...
    internal SavmExceptionType InlinePrecompileCall<TTracingInst>(
        ...
        where TTracingInst : struct, IFlag
    {
        ...
        if (failure is not null)
        {
            ...
            // SIP-8037: the failed call did not create its (dead) recipient; refund NEW_ACCOUNT.
        ...
        else
        {
            ...
            // SIP-8037: the reverted call did not create its (dead) recipient; refund NEW_ACCOUNT.
            if (child.NewAccountCharged)
                CreditStateGasRefund(ref parent.Gas, TGasPolicy.GetNewAccountStateCost());
        }

        ReturnDataBuffer = callResult.Output;
        SavmExceptionType push = stack.PushBytes<TTracingInst>(
            (reverted ? StatusCode.FailureBytes : StatusCode.SuccessBytes).Span);

        if (push == SavmExceptionType.None && outputLength > 0 && callResult.Output.Length > 0)
        {
            ...
            {
                push = SavmExceptionType.OutOfGas;
            }
            else
            {
        ...
        else
        {
            ...
            // OutOfGas by the output-copy memory expansion above. Per SIP-2929 the warm/cold access
