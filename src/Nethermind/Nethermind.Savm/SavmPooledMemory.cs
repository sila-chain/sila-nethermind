...
public struct SavmPooledMemory
{
    ...
    public bool TrySaveWord(in UInt256 location, Span<byte> word)
    {
        ...
        SavmWord word1 = Unsafe.As<byte, SavmWord>(ref MemoryMarshal.GetReference(word));
    ...
    internal void StoreWordAfterGas(in UInt256 location, ReadOnlySpan<byte> word)
    {
        Debug.Assert(location.IsUint64);
        int offset = TruncateToInt32(location.u0);
        SavmWord value = Unsafe.As<byte, SavmWord>(ref MemoryMarshal.GetReference(word));
    ...
    private static void ThrowArgumentOutOfRangeException()
    {
        ...
        throw new ArgumentOutOfRangeException("SavmWord size must be 32 bytes");
    }
}
