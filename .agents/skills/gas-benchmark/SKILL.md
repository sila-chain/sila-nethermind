...
allowed-tools:
  ...
  - Bash(gh release *)
  - Bash(gh api repos/NethermindSil/*)
...
1. **Show available releases** and ask the user to pick one:
   ```
   gh api repos/NethermindSil/gas-benchmarks/releases?per_page=15 \
...
3. **Ask for the network**: "Which network? (`perf-devnet-3`, `jochemnet`, `sila-mainnet`)"
...
| `--network` | `perf-devnet-3` | Network name (perf-devnet-3, jochemnet, sila-mainnet) |
...
| `--analyze-run` | (none) | Skip Phases 0-3; go straight to Phase 4 analysis on an existing run. Value is a run ID or URL (e.g., `--analyze-run 25725558942` or `--analyze-run https://github.com/NethermindSil/gas-benchmarks/actions/runs/25725558942`) |
...
2. Fetch run metadata: `gh run view <run-id> --repo NethermindSil/gas-benchmarks --json status,conclusion,jobs`
...
```
gh release view <tag> --repo NethermindSil/gas-benchmarks --json assets --jq '.assets[].name'
```
...
1. The release body contains a `**Branch:**` field that records which gas-benchmarks branch generated the test data. Parse it:
   ```
   gh release view <tag> --repo NethermindSil/gas-benchmarks --json body --jq '.body' \
...
2. If the release notes don't contain a branch field, fall back to listing branches:
   ```
   gh api repos/NethermindSil/gas-benchmarks/branches?per_page=100 \
...
3. Verify the workflow exists on the chosen branch:
   ```
   gh api repos/NethermindSil/gas-benchmarks/contents/.github/workflows/repricing-nethermind.yml?ref=<branch> --jq '.name' 2>/dev/null
   ```
...
```
gh api repos/NethermindSil/gas-benchmarks/contents/.github/workflows/repricing-nethermind.yml?ref=<branch> \
  --jq '.content' | base64 -d
```
...
- `sila-mainnet` → (no genesis_file flag)
...
MSYS_NO_PATHCONV=1 gh workflow run repricing-nethermind.yml \
  --repo NethermindSil/gas-benchmarks \
...
1. Find the run ID using the timestamp captured before triggering (same approach as Phase 1 step 5):
   ```
   gh run list --repo NethermindSil/gas-benchmarks --workflow=repricing-nethermind.yml \
     --limit 5 --json databaseId,createdAt \
     --jq '[.[] | select(.createdAt > "<BEFORE>")] | first | .databaseId'
   ```
2. Poll: `gh run view <run-id> --repo NethermindSil/gas-benchmarks --json status,conclusion` every 30 seconds.
...
Fetch job logs: `gh run view --job=<job-id> --repo NethermindSil/gas-benchmarks --log`
...
gh run download <run-id> --repo NethermindSil/gas-benchmarks \
...
gh run download <pr-run-id> --repo NethermindSil/gas-benchmarks \
  -n "results-1-nethermind-<cleaned-test-path>" -D /tmp/gb-pr
gh run download <base-run-id> --repo NethermindSil/gas-benchmarks \
...
```
gh release download <tag> --repo NethermindSil/gas-benchmarks \
...
```
gh api repos/NethermindSil/gas-benchmarks/actions/runs/<run-id>/artifacts \
...
5. **Interpretation guide:**
   ...
   - `Nethermind.Savm.VirtualMachine` functions indicate SAVM execution overhead
