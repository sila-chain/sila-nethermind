---
name: review
description: Deep code review for an Sila execution client. Checks consensus correctness, security, robustness, performance, DI patterns, breaking changes, and observability. Use when asked to "review", "check this PR", "look for bugs", "audit", or "review my changes".
allowed-tools:
  [
    Bash(git diff*),
    Bash(git merge-base*),
...
Nethermind is an Sila execution client built on .NET. Consensus correctness is non-negotiable — a wrong opcode, a missed fork condition, or an Engine API violation means invalid blocks on a live network.
...
**Prioritization:** If more than 20 production files survived filtering, review critical paths first (SAVM, Engine API, State, Trie, Specs, Blockchain), then the rest.
...
| Sila Foundation hive tests (consensus, RPC, Engine API) | `hive-tests.yml`, `hive-consensus-tests.yml`       |
...
- A new consensus rule, SIP, or opcode change with no corresponding test — wrong SAVM behaviour cannot be caught without one
...
- A non-obvious consensus rule or algorithm applied with no reference to the SIP or Yellow Paper section — link it so future reviewers can verify
...
### Consensus & SAVM correctness (CRITICAL)

Wrong SAVM behaviour produces invalid blocks, causing chain desync and validators building on an invalid chain — leading to missed attestations and potential safety failures.

- SAVM opcode with incorrect gas cost, wrong stack effect, or wrong memory expansion rule
- Fork/SIP activation condition checked by block number when it should use timestamp (SilaShanghai and all later forks activate by timestamp), or applied at the wrong boundary
...
- Blob transaction (SIP-4844) handling with incorrect blob gas accounting, wrong blob base fee calculation, missing KZG commitment verification, or per-fork blob limits not enforced (limits come from the active fork's blob schedule)
...
- System-level withdrawal processing (SIP-4895) applied out of order, to the wrong address, or with wrong amounts — withdrawals bypass the gas system and any error corrupts the state root
- SIP-1559 base fee computed incorrectly from parent gas used vs target — wrong base fee breaks transaction validity and fee burning for every subsequent block
- Incorrect RLP encoding or decoding of any Sila data structure
- `UInt256` arithmetic that could overflow or truncate (Sila arithmetic is exact 256-bit)
- BLS12-381 or secp256k1 operations with wrong input format or missing validation
- SIP-2930 access list gas accounting applied incorrectly — warm vs cold storage access costs (SIP-2929) must account for pre-warmed slots declared in the access list
- Keccak-256 computed using `System.Security.Cryptography.SHA3_256` instead of Nethermind's `ValueKeccak` / `KeccakHash` — NIST SHA3 and Sila's Keccak-256 use different padding and produce different output on every input

### Key code locations

- SAVM instruction handlers: `src/Nethermind/Nethermind.Savm/Instructions/SavmInstructions.*.cs` (dispatch in `VirtualMachine.cs`)
...
- P2P message handlers that process externally-supplied sizes or counts without an upper bound — Sila's devp2p has no built-in rate limiting, so unbounded `GetBlockHeaders` ranges or oversized transaction batches are a DoS vector

### Performance

Nethermind is the fastest Sila execution client. Guard regressions in hot paths.
...
- Chain-specific logic added to `Nethermind.Core`, `Nethermind.Savm`, or `Nethermind.Blockchain` instead of the appropriate chain plugin (`Nethermind.Optimism`, `Nethermind.Merge.Plugin`, etc.) — poisons the shared abstraction
...
2. **Impact** — one sentence: why it matters in an Sila client context _(omit if obvious)_
