...
| `NetworkModule` | devp2p message serializers and protocol handlers (sil, snap), RLPx host | New protocol version or message type |
| `DiscoveryModule` | Peer discovery, `INodeSource` | Discovery protocol changes |
| `RpcModules` | JSON-RPC modules (`Sil`, `Debug`, `Admin`, etc.) | New RPC method module |
...
`IWorldState` handles the SAVM→State interface. Previously it also handled storage concerns, but that was extracted into `IWorldStateScopeProvider`, leaving snapshot and journaling logic in `IWorldState`.
...
- Each environment is created by creating an Autofac child scope, modifying that child scope according to the use case, passing in a world state, and wrapping them in an env implementation (e.g. `IReadOnlyTxProcessorSource`, `IOverridableEnv`). It's usually useful to put the scope initialization in a factory implementation. For example:
  ...
  - `OverridableEnvFactory` (`Nethermind.State/OverridableEnv`) — adds use-case-specific wiring in the child scope: `AddDecorator<ICodeInfoRepository, OverridableCodeInfoRepository>()` so sil_call/sil_simulate can apply state, code, and spec overrides, wrapped in an `IOverridableEnv` that applies and resets the overrides around each scope.
...
3. Ensure it resolves via the runner tests — `SilaRunnerTests` (`Nethermind.Runner.Test`) builds the full production container for every shipped config, so a mis-registered dependency fails there.
...
`TestNethermindModule` combines both in one module with a `TestSpecProvider` default — the preferred shorthand, and what `Nethermind.Savm.Benchmark` uses (`new TestNethermindModule(SilaOsaka.Instance)`).
